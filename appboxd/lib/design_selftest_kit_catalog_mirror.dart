...
// `skills/appbox-designer/references/kit-catalog.md` is supposed to mirror
...
import 'package:appboxd/design_selftest.dart' show CheckOutcome;
import 'package:path/path.dart' as p;

/// Every `dir` in `config/kit-registry.json` must appear in
/// `skills/appbox-designer/references/kit-catalog.md` as `` `dir` ``.
///
/// [skill] is the designer skill dir the selftest runner already resolves
/// (`<repo>/skills/appbox-designer` by default — see `_defaultSkillDir` in
...
/// [skill] by walking up two segments (`skills/appbox-designer` -> repo).
...
Future<CheckOutcome> kitCatalogMirrorCheck(String art, String skill, String src) async {
  ...
  // real `<repo>/skills/appbox-designer` layout and breaks the moment the skill
