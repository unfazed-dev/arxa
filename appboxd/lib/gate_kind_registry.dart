...
// skills/appbox-scaffolder/scripts/validate-registry.py (Q7 + Q12).
//
// The registry under skills/appbox-scaffolder/kind-resolution.registry.json
// maps designer widget kinds to kit targets. It is checked against three
// ground truths:
//   1. VOCABULARY — skills/appbox-designer/starter-partials/widgets/_<kind>.tsx
...
KindRegistryReport validateKindRegistry({
  ...
  for (final kind in kinds.keys.toList()..sort()) {
    ...
    final refs = <String>[
      ...
      ...(((v['presentation'] as Map<String, dynamic>?) ?? {})
          .values
          .whereType<String>()
          .where((x) => x.startsWith('AppBoxKit'))),
  ...
  if (shapePkg == null || shapePkg.isEmpty) {
    ...
    if (!shapeFile.existsSync()) {
      ...
      if (!src.contains('class AppBoxKitInspectAttrs')) {
        bad.add('$shapePkg does not declare class AppBoxKitInspectAttrs');
      }
      ...
      final barrel =
          File(p.join(kitRoot, 'core', 'lib', 'appbox_kit_core.dart'));
      if (barrel.existsSync() &&
          !barrel.readAsStringSync().contains(p.basename(rel))) {
        bad.add(
            'inspectAttrs shape is not exported from appbox_kit_core.dart — importers cannot reach the single home');
      }
    }
  }
...
GateResult kindRegistryGate(GateContext ctx) {
  ...
  final report = validateKindRegistry(
    registryPath: p.join(
        root, 'skills', 'appbox-scaffolder', 'kind-resolution.registry.json'),
    partialsDir: p.join(
        root, 'skills', 'appbox-designer', 'starter-partials', 'widgets'),
  ...
  if (report.violations.contains('kind-resolution.registry.json not found')) {
    return GateResult(
      ...
      exitCode: envExit,
      summary:
          'kind_registry: no kind-resolution.registry.json under skills/appbox-scaffolder — not applicable',
    );
  }
