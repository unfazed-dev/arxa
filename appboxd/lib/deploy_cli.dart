// deploy_cli — CLI entry for `appbox deploy`.
//
...
//   appbox deploy --self-test                                   # assert shapes
//   appbox deploy doctor [--config <f>]                         # preflight (NOT a gate)
//   appbox deploy deploy --target T --version V --account A \
//                       --approval <tok> [--ledger <f>]         # run a target
//
// The ledger path resolves as --ledger > $APPBOX_DEPLOY_LEDGER >
// <repo-root>/pipeline/state/deploy-ledger.json (the Python's default, derived
// from the repo root — never a literal — R3). Note: `appbox gate deploy` is the
// strict gate that asserts the deploy triple; this command runs the mechanics.

import 'dart:convert';
import 'dart:io';

import 'package:appboxd/deploy.dart';

const String _usage = '''
Usage: appbox deploy <subcommand> [options]
...
Note: `appbox gate deploy` remains the strict deploy gate; this command is the
deploy runtime the gate wraps.''';

/// Entry point for `appbox deploy`. Returns the process exit code (0/1/2).
Future<int> deployMain(List<String> args) async {
  ...
  switch (cmd) {
    ...
    default:
      stderr.writeln("appbox deploy: unknown subcommand '$cmd'");
      stderr.writeln(_usage);
      return 2;
  }
}
...
Future<int> _runDeploy(List<String> args) async {
  ...
  for (final required in ['target', 'version', 'account', 'approval']) {
    if (o[required] == null) {
      stderr.writeln('appbox deploy deploy: --$required required');
      stderr.writeln(_usage);
      return 2;
    }
  }
...
/// Resolve the ledger path: --ledger > $APPBOX_DEPLOY_LEDGER > repo-root
...
String _resolveLedger(String? flag) {
  ...
  final env = Platform.environment['APPBOX_DEPLOY_LEDGER'];
  if (env != null && env.isNotEmpty) return env;
  final root = _findRepoRoot() ?? Directory.current.path;
  return '$root/pipeline/state/deploy-ledger.json';
}

/// Walk up for config/appbox.config.json — the repo-root convention shared with
/// bin/appbox.dart (kept private there; duplicated here because deploy_cli is a
...
Map<String, String>? _parse(List<String> args) {
  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (a.startsWith('--')) {
      ...
      if (i + 1 >= args.length) {
        stderr.writeln("appbox deploy: missing value for --$name");
        stderr.writeln(_usage);
        return null;
      }
      out[name] = args[++i];
    } else {
      stderr.writeln("appbox deploy: unexpected argument '$a'");
      stderr.writeln(_usage);
      return null;
    }
  }
