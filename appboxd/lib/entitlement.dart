/// Offline entitlement verification for appbox
...
/// offline by appboxd against a pinned public key, machine-bound).
///
/// ## Token format
///
/// The entitlement is a compact JWS (`header.payload.signature`, base64url)
/// signed with Ed25519 (`{"alg":"EdDSA","typ":"JWT"}`), cached at
/// `~/.appbox/entitlement.jwt`. Claims (the exact contract the `/activate`
...
class Entitlement {
  ...
  /// Default cache location: `~/.appbox/entitlement.jwt`.
  static String? defaultPath() {
    final home =
        Platform.environment['HOME'] ?? Platform.environment['USERPROFILE'];
    return home == null ? null : '$home/.appbox/entitlement.jwt';
  }

  /// The appbox entitlement PUBLIC key (Ed25519, 32 bytes).
  ///
  ...
  return (
    ...
    failLines: [
      ...
      'Scaffold is the paid boundary (D17/D18): the automated design→Flutter '
          ...
          'forever.',
      'Inspect the cached entitlement with `appbox entitlement status` '
          '(~/.appbox/entitlement.jwt), then re-run.',
    ],
  );
}
