...
/// `appbox design stamp <artifactDir>` — stamp every .tsx template under the
...
CmdResult stampMain(List<String> args) {
  if (args.isEmpty) {
    return CmdResult(2, stderrLines: [
      'appbox design stamp: missing <artifactDir> '
  ...
  if (!dir.existsSync()) {
    return CmdResult(2,
        stderrLines: ['appbox design stamp: no such dir: ${args.first}']);
  }
  final files = dir
  ...
  if (files.isEmpty) {
    return CmdResult(2,
        stderrLines: ['appbox design stamp: no .tsx under ${args.first}']);
  }
  final out = <String>[];
