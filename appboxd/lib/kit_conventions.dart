// kit_conventions.dart — enforce the appbox_kit structural contract
// (Dart port of appbox_kit/tools/conventions.sh).
//
...
const _allowedDeps = <String, Set<String>>{
  'data': {'appbox_kit_core'},
  'ui_library': {'appbox_kit_core', 'appbox_kit_motion'},
  'showcase_app': {'*'},
};

final _nameLine = RegExp(r'^name:\s*(\S+)');
// Indented (2-space) appbox_kit_* dependency entries only — excludes the
// kit's own column-0 `name:` line.
final _kitDep = RegExp(r'^  (appbox_kit_[a-z_]+)', multiLine: true);
...
KitConventionResult checkKitConventions(String kitRoot) {
  ...
  for (final entry in entries) {
    ...
    // Dependency topology — indented appbox_kit_* deps only. '*' allows all;
