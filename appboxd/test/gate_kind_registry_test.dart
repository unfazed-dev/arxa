...
// skills/appbox-scaffolder/scripts/validate-registry.py; this suite pins the
        ...
        '''
{
  ...
  "kinds": {
    "button": { "widget": "AppBoxKitButton" },
    "card": { "widget": "AppBoxKitCard", "escape": { "reason": "r", "owner": "o", "expires": "2027-01-01" } }
  },
  "resolution": { "order": ["widget", "variants", "composedFrom", "presentation"] },
  "designVocabulary": { "inspectAttrs": "package:appbox_kit_core/common/inspect_attrs.dart" },
...
class AppBoxKitButton {}
class AppBoxKitButtonDense {}
class AppBoxKitCard {}
''');
...
''');
    File(p.join(coreLib, 'appbox_kit_core.dart'))
    ...
    final (r, pa, kl, kr) = buildFixture(registryJson: '''
{
  ...
  "kinds": {
    "button": { "widget": "AppBoxKitButton" },
    "button": { "widget": "AppBoxKitButtonDense" },
    "card": { "widget": "AppBoxKitCard" }
  },
  "resolution": { "order": ["widget", "variants", "composedFrom", "presentation"] },
  "designVocabulary": { "inspectAttrs": "package:appbox_kit_core/common/inspect_attrs.dart" },
...
''');
  ...
  test('ORPHAN ENTRY — registry maps a kind with no authored partial', () {
    ...
    File(r).writeAsStringSync(src.replaceFirst('"card"',
        '"ghost": { "widget": "AppBoxKitCard" }, "card"'));
    final report = run(r, pa, kl, kr);
  ...
  test('invented target — widget names a class kit does not declare', () {
    final (r, pa, kl, kr) = buildFixture();
    final src = File(r).readAsStringSync();
    File(r).writeAsStringSync(src.replaceAll('AppBoxKitCard', 'AppBoxKitNope'));
    final report = run(r, pa, kl, kr);
    expect(report.violations,
        anyElement(contains("'AppBoxKitNope' is not a class")));
  });
  ...
  test('variants/companions/composedFrom refs are target-checked too', () {
    ...
    File(r).writeAsStringSync(src.replaceFirst(
        '"button": { "widget": "AppBoxKitButton" }',
        '"button": { "widget": "AppBoxKitButton", "variants": { "dense": "AppBoxKitPhantom" } }'));
    final report = run(r, pa, kl, kr);
    expect(report.violations,
        anyElement(contains("'AppBoxKitPhantom' is not a class")));
  });
  ...
  test('inspect shape must declare the class, the triple and the barrel export',
    ...
    final report = run(r, pa, kl, kr);
    expect(report.violations,
        anyElement(contains('does not declare class AppBoxKitInspectAttrs')));
    expect(report.violations, anyElement(contains("triple slot 'screenId'")));
    File(p.join(kr, 'core', 'lib', 'appbox_kit_core.dart'))
        .writeAsStringSync("export 'other.dart'");
    final report2 = run(r, pa, kl, kr);
    expect(report2.violations,
        anyElement(contains('not exported from appbox_kit_core.dart')));
  });
  ...
  test('gate wrapper — GateResult wiring, sarif and exit codes', () {
    final (r, pa, kl, kr) = buildFixture();
    Directory(p.join(tmp.path, 'skills', 'appbox-scaffolder'))
        .createSync(recursive: true);
    File(p.join(tmp.path, 'skills', 'appbox-scaffolder',
            'kind-resolution.registry.json'))
        .writeAsStringSync(File(r).readAsStringSync());
    final widgetsDir = p.join(
        tmp.path, 'skills', 'appbox-designer', 'starter-partials', 'widgets');
