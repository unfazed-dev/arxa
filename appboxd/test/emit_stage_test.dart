...
//   - extension points preserved; generated overwritten on appbox replay
...
void main() {
  ...
  test('greenfield new-app writes generated + extension, stamps, emits report.json', () {
    ...
    // appbox stamp carries the blueprint metadata
    final stamp =
        jsonDecode(File('$t/.appbox/manifest.json').readAsStringSync()) as Map<String, dynamic>;
  ...
  test('idempotent: a second unchanged run writes nothing', () {
    ...
    final stampBefore = File('$t/.appbox/manifest.json').readAsStringSync();
    ...
    // the first run stamped the target, so the second run detects it as appbox-built
    // (ADR-0002 #3 — .appbox/manifest.json present) and still replays freely.
    expect((report['isAppbox'] as Map)['isAppbox'], true);
    expect(report['replay'], true);
    expect(report['stamped'], true);

    // file bytes unchanged by the no-op replay
    expect(File('$t/lib/main.dart').readAsStringSync(), mainBefore);
    expect(File('$t/.appbox/manifest.json').readAsStringSync(), stampBefore);
  });

  test('extension points preserved; generated overwritten on appbox replay', () {
    final bp = plantBlueprint(tmp);
    final t = newTarget(tmp, 'replay');
    // make it an existing appbox-built target (stamped + has a pubspec)
    Directory('$t/.appbox').createSync(recursive: true);
    File('$t/.appbox/manifest.json').writeAsStringSync(jsonEncode({'factoryVersion': 'old'}));
  ...
  test('arbitrary existing target: dry-run blocks generated, then apply writes it', () {
    ...
    expect(File('$t/.appbox/manifest.json').existsSync(), isFalse);
