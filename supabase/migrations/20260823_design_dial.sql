-- The Design Dial's central store (locked amendment 2026-08-23:
-- 'the feedback dial becomes the Design Dial', decisions 6/7/9/11).
--
...
create table if not exists design_dial_pins (
  id          uuid primary key,
...
create index if not exists design_dial_pins_artifact_route
  on design_dial_pins (artifact, route);

create table if not exists design_dial_replies (
  id          uuid primary key,
  pin_id      uuid not null references design_dial_pins (id) on delete cascade,
  author_kind text not null check (author_kind in ('author','guest')),
  author_name text not null,
  body        text not null check (char_length(body) between 1 and 4000),
  created_at  timestamptz not null default now()
);
create index if not exists design_dial_replies_pin on design_dial_replies (pin_id);
...
create table if not exists design_dial_share_links (
  token_hash  text primary key,
  artifact    text not null,
  expires_at  timestamptz,
  created_at  timestamptz not null default now()
);
create index if not exists design_dial_share_links_artifact
  on design_dial_share_links (artifact);

alter table design_dial_pins enable row level security;
alter table design_dial_replies enable row level security;
alter table design_dial_share_links enable row level security;
...
alter publication supabase_realtime add table design_dial_pins;
alter publication supabase_realtime add table design_dial_replies;
