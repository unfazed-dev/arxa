-- Arxa Dial drawings storage amendment (2026-08-23, operator decision):
-- drawings move from arxa_dial_pins.drawing (jsonb column, added by
-- 20260823_arxa_dial_drawings.sql) into their own table. The MODEL is
...
create table if not exists arxa_dial_drawings (
  pin_id     uuid primary key references arxa_dial_pins (id) on delete cascade,
  strokes    jsonb not null,
  created_at timestamptz not null default now()
);

-- Carry over any drawings written while they lived on the pin row.
insert into arxa_dial_drawings (pin_id, strokes)
select id, drawing from arxa_dial_pins where drawing is not null
on conflict (pin_id) do nothing;

alter table arxa_dial_pins drop column if exists drawing;

alter table arxa_dial_drawings enable row level security;
...
alter publication supabase_realtime add table arxa_dial_drawings;
