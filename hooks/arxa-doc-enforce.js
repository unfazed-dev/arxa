#!/usr/bin/env node
// arxa-doc-enforce.js — "always document arxa work."
//
// Port of stacked_kit/hooks/stacked_kit-doc-enforce.js for arxa. Sibling
// of arxa-doc-health.js (which VALIDATES docs after a doc edit); this hook
// notices when arxa SOURCE is edited and blocks Stop until the docs
// validator passes.
//
// Mechanism (one marker file, fail-open everywhere):
//   posttool      — arxa source edit (non-markdown under a source dir)
//                   SETS the marker. Edits outside the source set are ignored.
//   stop          — marker set → run `arxa docs`; on failure BLOCK
...
//
// Source set (repo-relative): kit/ arxa/ arxa-studio/lib/ pipeline/ gates/
// tools/ skills/ config/ — the pieces whose behavior the docs describe.
// archives/ and build outputs never count.
//
// Marker: <repo>/.kimi-code/.arxa_doc_pending
// Bypass: ARXA_DOC_ENFORCE_OFF=1 (fail-open); sessionstart also clears.
'use strict';
const fs = require('fs');
...
const MARKER = path.join(repoRoot, '.kimi-code', '.arxa_doc_pending');
...
if (MODE === 'stop') {
  ...
  if (typeof r.status === 'number' && r.status !== 0) {
    process.stderr.write(
      '[arxa] doc-enforce: arxa SOURCE was edited but the docs validator FAILS — ' +
      'fix the docs before finishing:\n' +
      (r.stdout || '') + (r.stderr || '') +
      'Fix: update docs/INDEX.md (and the affected doc), then re-run: ' +
      'dart run bin/arxa.dart docs  (bypass: ARXA_DOC_ENFORCE_OFF=1)\n'
    );
...
const SOURCE_DIRS = /[\/\\](kit|arxa|pipeline|gates|tools|skills|config)[\/\\]|[\/\\]arxa-studio[\/\\]lib[\/\\]/;
