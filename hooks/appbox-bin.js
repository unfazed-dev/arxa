#!/usr/bin/env node
// appbox-bin.js — resolve the fastest available way to invoke the appbox CLI.
//
...
//
//     dart run bin/appbox.dart --help   ~1420ms   (JIT, every invocation)
//     .build/appbox --help                ~10ms   (AOT, built by install.sh)
//
...
// enforcing nothing.
//
// The binary deliberately lives INSIDE the repo (.build/appbox): the designer
// resolves its runtime assets by walking up from the running executable to the
// directory holding config/appbox.config.json. A binary outside the tree loses
...
/**
 * Build a spawn descriptor for `appbox <argv>`.
 *
 * @param {string} repoRoot absolute path to the app-box checkout
 ...
 */
function appboxInvocation(repoRoot, argv) {
  const bin = path.join(repoRoot, '.build', 'appbox');
  try {
...
function isStale(repoRoot) {
  ...
  for (const sub of ['bin', 'lib']) {
    const dir = path.join(repoRoot, 'appboxd', sub);
...
module.exports = { appboxInvocation, isStale };
