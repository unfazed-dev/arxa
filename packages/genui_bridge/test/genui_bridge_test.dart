...
import 'package:genui_bridge/testing.dart';
import 'package:test/test.dart';

const _catalogId = 'test.com:catalog';

final _catalog = <String, JsonSchema>{
  'Text': {
    'type': 'object',
    'properties': {
      'id': {'type': 'string'},
...
GenuiBridge bridge(FakeChatStream chat) =>
    GenuiBridge(chat: chat, catalog: _catalog, catalogId: _catalogId);

void main() {
  group('GenuiBridge.generate', () {
    test('happy path: valid output passes in one attempt', () async {
      final chat = FakeChatStream()..scriptText('$_create\n$_goodComponents');
      final turn = await bridge(chat).generate([
        const ChatMessage.user('show a greeting'),
      ]);
      expect(turn.attempts, 1);
      expect(turn.messages, hasLength(2));
      expect(turn.messages[0], isA<CreateSurface>());
      expect(turn.messages[1], isA<UpdateComponents>());
    ...
    test('prepends the catalog-bearing system prompt', () async {
      final chat = FakeChatStream()..scriptText(_create);
      await bridge(chat).generate(const [ChatMessage.user('hi')]);
    ...
    test('invalid then repaired: re-asks with the validation error', () async {
      ...
      expect(repair.role, ChatRole.user);
    ...
    test('only valid messages ever leave the bridge', () async {
      ...
      expect(turn.messages.single, isA<UpdateComponents>());
    });

    test('permanently invalid output throws a typed GenuiBridgeFailure',
        () async {
      final chat = FakeChatStream()
    ...
    test('an empty catalog disables per-component validation', () async {
      ...
      final turn =
          await GenuiBridge(chat: chat).generate(const []); // default catalog
      expect(turn.attempts, 1);
      expect(turn.messages, hasLength(2));
    });

    test('prose alongside valid messages passes through as texts', () async {
      final chat = FakeChatStream()..scriptText('Setting that up.\n$_create');
      final turn = await bridge(chat).generate(const []);
