...
Future<List<A2uiStreamEvent>> parse(List<String> chunks) =>
    const A2uiStreamParser().bind(Stream.fromIterable(chunks)).toList();
...
void main() {
  group('A2uiStreamParser', () {
    test('parses a single message in one chunk', () async {
      ...
      expect(events.single, isA<A2uiMessageEvent>());
      expect(
    ...
    test('assembles a message split across chunks', () async {
      ...
      expect(events, hasLength(1));
      final message = (events.single as A2uiMessageEvent).message;
    ...
    test('handles braces and escapes inside strings', () async {
      ...
      expect(events, hasLength(1));
      final message =
          (events.single as A2uiMessageEvent).message as UpdateDataModel;
    ...
    test('parses a JSONL stream of messages', () async {
      ...
      expect(events[0], isA<A2uiMessageEvent>());
      expect(events[1], isA<A2uiMessageEvent>());
      expect(events[2], isA<A2uiMessageEvent>());
    });
    ...
    test('emits prose around messages as text events', () async {
      ...
      final texts = events.whereType<A2uiTextEvent>().map((e) => e.text);
      final messages = events.whereType<A2uiMessageEvent>();
    ...
    test('strips <a2ui_message> protocol tags from text', () async {
      ...
      expect(events, hasLength(1));
      expect((events.single as A2uiTextEvent).text, 'hello');
    });

    test('invalid JSON inside a fence is an error event, not text', () async {
      final events = await parse(['```json\n{"version":"v0.9", oops\n```']);
      expect(events, hasLength(1));
      expect(events.single, isA<A2uiErrorEvent>());
      expect((events.single as A2uiErrorEvent).raw, contains('oops'));
    });
    ...
    test('a structurally invalid A2UI envelope is an error event', () async {
      ...
      expect(events, hasLength(1));
      final error = events.single as A2uiErrorEvent;
      expect(error.error, isA<A2uiFormatException>());
    });

    test('non-A2UI JSON falls through as text', () async {
      final events = await parse(['{"unrelated":true}']);
      expect(events, hasLength(1));
      expect(events.single, isA<A2uiTextEvent>());
    });
    ...
    test('a truncated final message is an error event at stream end', () async {
      ...
      expect(events[0], isA<A2uiMessageEvent>());
      expect(events[1], isA<A2uiErrorEvent>());
      expect(
        (events[1] as A2uiErrorEvent).error.toString(),
        contains('incomplete'),
      );
    });
    ...
    test('a JSON array of messages in a fence emits each message', () async {
      final events = await parse(['```json\n[$_create,$_delete]\n```']);
      expect(events, hasLength(2));
      expect(events.every((e) => e is A2uiMessageEvent), isTrue);
    });
  });
}
