...
/// competing logger. Standalone: no dependency on `stacked_kit` core.
class KitSupportService {
  KitSupportService({
  ...
  /// typed [FeedbackSubmission] once the user submits, or null if they dismiss.
  ///
  /// Requires a `BetterFeedback` ancestor at the app root (host wiring).
  Future<FeedbackSubmission?> startFeedbackFlow(
    BuildContext context, {
    bool? attachDiagnostics,
  }) {
    final completer = Completer<FeedbackSubmission?>();
  ...
  /// Attach a fresh [DiagnosticsBundle] (Talker export) to [submission].
  FeedbackSubmission withDiagnostics(FeedbackSubmission submission) =>
      submission.withDiagnostics(captureDiagnostics());

  /// Snapshot the injected Talker's current log history as a [DiagnosticsBundle],
  /// capped at [maxDiagnosticEntries] (most recent kept).
  DiagnosticsBundle captureDiagnostics({
  ...
  Future<SubmissionResult?> collectAndSubmit(
    BuildContext context, {
    bool? attachDiagnostics,
  }) async {
