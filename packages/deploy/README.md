...
Deploy port for stacked_kit apps. External CLIs run through a
`KitProcessRunner` port, so every command shape is unit-tested with
`ScriptedProcessRunner` — no toolchain needed in CI.

Pure Dart and standalone: no flutter, stacked, or stacked_kit dependency.
...
```dart
import 'package:stacked_kit_deploy/stacked_kit_deploy.dart';
...
CLI: `dart run stacked_kit_deploy doctor` /
`dart run stacked_kit_deploy deploy cloudflare-pages --project-name=showcase`.

## Testing

```dart
import 'package:stacked_kit_deploy/testing.dart';
