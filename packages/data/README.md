...
The data layer for `stacked_kit` apps: generic repositories over one
...
End-to-end UI → backend data-flow diagrams: `docs/plans/stacked-kit-data-flow-diagrams.md`.
...
dependencies:
  stacked_kit_data:
    path: ../stacked_kit/data

# appwrite's transitive graph pins win32 5.x while stacked_kit → talker_flutter
...
4. The kit never self-registers and never imports the host — `main()` calls
   `KitData.initialize` explicitly, matching `stacked_kit`'s contract.

## Further reading

- Architecture and locked decisions: `docs/plans/stacked-kit-data-layer.md`
- Auth seam decisions: `docs/plans/stacked-kit-data-auth.md`
...
- Kit-wide skill (patterns, rules, host-integration contract):
  `core/skill/SKILL.md` → **Data layer (stacked_kit_data)**
