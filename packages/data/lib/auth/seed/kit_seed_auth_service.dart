...
/// Fake, in-memory [KitAuthService] for [KitDataBackend.seed] hosts.
///
...
/// looks up [kKitAuthUsersTable] rows by email (or phone), matching
...
///
/// Users persist in the [KitSeedStore]'s reserved [kKitAuthUsersTable] (and
...
class KitSeedAuthService implements KitAuthService {
  ...
  final BehaviorSubject<KitAuthSession?> _session$ =
      BehaviorSubject<KitAuthSession?>.seeded(null);
  ...
  /// [kKitAuthUsersTable], canonicalizing its `id` via [idService]. Parsed
  /// inline rather than through `KitFixtureLoader` — that loader demands a
  /// registered [KitTableSchema], and this table is deliberately unregistered.
  ...
  Future<void> initialize() async {
    ...
    for (final entry in decoded) {
      ...
      await _upsertUserRow(KitAuthUser(
        id: idService.canonicalId(kKitAuthUsersTable, seedKey),
  ...
  @override
  KitAuthSession? get currentSession => _session$.value;
  ...
  @override
  Future<KitAuthSession> signInWithGoogle() => _resolveAndEmit(
        email: 'google-user@seed.local',
        displayName: 'Google Seed User',
      );

  @override
  Future<KitAuthSession> signInWithApple() => _resolveAndEmit(
        email: 'apple-user@seed.local',
        displayName: 'Apple Seed User',
      );

  @override
  Future<KitAuthSession> signInAnonymously() async {
    ...
    final count = store.tableSnapshot(kKitAuthUsersTable).length;
    final user = KitAuthUser(
      id: idService.canonicalId(kKitAuthUsersTable, 'anon-$count'),
      isAnonymous: true,
    );
    await _upsertUserRow(user);
    final session = KitAuthSession(user: user);
  ...
  Future<void> dispose() async {
    ...
    // in-memory store is dropped by `locator.reset()` regardless.
  ...
  Future<KitAuthSession> _resolveAndEmit({
    String? email,
    String? phone,
    String? displayName,
  }) async {
  ...
  /// Looks up [kKitAuthUsersTable] for a row matching [email]/[phone]
  ...
  Future<KitAuthUser> _resolveOrCreateUser({
    String? email,
    String? phone,
    String? displayName,
  }) async {
    ...
    for (final row in store.tableSnapshot(kKitAuthUsersTable).values) {
    ...
    final user = KitAuthUser(
      id: idService.canonicalId(kKitAuthUsersTable, key),
      email: email,
      phone: phone,
      displayName: displayName,
    );
  ...
  Future<void> _upsertUserRow(KitAuthUser user) =>
      store.upsertRow(kKitAuthUsersTable, {
        'id': user.id,
        'email': user.email,
        'phone': user.phone,
  ...
  KitAuthUser _userFromRow(Map<String, dynamic> row) => KitAuthUser(
        id: row['id'] as String,
        email: row['email'] as String?,
        phone: row['phone'] as String?,
        displayName: row['display_name'] as String?,
