...
/// every `KitSeedRepository` operation; the host owns the profile names,
/// the fixture selection (`empty` is an empty fixture list, not a store
/// behavior), and any persona sign-in after boot.
///
/// Only meaningful when `KitDataConfig.backend` is `KitDataBackend.seed` —
...
class KitSeedException implements Exception {
  final String message;
  final String code;

  const KitSeedException(
    this.message, {
    this.code = 'seed-profile-failure',
  });

  @override
  String toString() => 'KitSeedException($code): $message';
}
...
///
/// The default (`const KitSeedProfile()`) is a pass-through: zero latency,
/// no failure — existing boots are behavior-identical without it. Auth is
/// never affected: `KitSeedAuthService` reads the store directly, not
/// through repositories.
class KitSeedProfile {
  ...
  const KitSeedProfile({
    this.latency = Duration.zero,
    this.error,
  });

  /// The `slow` profile — everything succeeds, [latency] late.
  const KitSeedProfile.slow(this.latency) : error = null;

  /// The `failing` profile — everything fails with [error].
  const KitSeedProfile.failing(this.error) : latency = Duration.zero;
