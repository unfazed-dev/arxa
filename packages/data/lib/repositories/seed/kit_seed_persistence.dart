...
/// Write-through durability contract for [KitSeedStore] — the store's
...
/// [KitNoPersistence]), which is distinct from "persisted and empty".
abstract interface class KitSeedPersistence {
  ...
  /// table -> [fixtureFingerprint] recorded at the last boot. `null` when
  /// never recorded — including snapshots written before fingerprinting
  /// existed, which [resolveBootTables] deliberately treats as "fixtures
...
/// Pure in-memory seed backend (`KitSeedPersistenceMode.none`): every boot
/// reloads fixtures fresh, nothing survives a restart.
class KitNoPersistence implements KitSeedPersistence {
  @override
...
/// Deep key-sorted copy, so [fixtureFingerprint] is insensitive to JSON map
...
String fixtureFingerprint(Map<String, Map<String, dynamic>> rows) {
...
/// actually change.
///
/// Snapshots are written per table ([KitSeedPersistence.persistTable]), so a
/// partial snapshot is normal — e.g. `KitSeedAuthService.initialize`
/// write-throughs only `kit_auth_users` on first boot; rule 1 keeps that
/// partial snapshot from shadowing unseen fixture tables.
KitBootResolution resolveBootTables({
  ...
  final fingerprints = <String, String>{
    for (final entry in fixtures.entries)
      entry.key: fixtureFingerprint(entry.value),
  };
...
/// fixtures per table (see [resolveBootTables]). A corrupt
...
/// unnoticed.
class KitSnapshotPersistence implements KitSeedPersistence {
  /// Boot-metadata sidecar (see [KitSeedPersistence.loadFixtureFingerprints]).
  ...
  KitSnapshotPersistence({Directory? overrideDirectory})
