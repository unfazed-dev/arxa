...
/// delete, or share an object previously written through [KitStorageService].
///
...
class StorageObjectRef {
  ...
  @override
  String toString() => 'StorageObjectRef(bucket: $bucket, path: $path, id: $id)';
}

/// Blob-storage seam mirroring [KitRepository] — one swap surface, three
/// backends (seed / supabase / appwrite), selected by the same
/// [KitDataBackend] as the repositories.
///
...
///   has no HTTP origin.
abstract interface class KitStorageService {
  /// Stores [bytes] at `bucket/path`. Returns the canonical [StorageObjectRef]
  ...
  Future<void> delete(StorageObjectRef ref);
}
