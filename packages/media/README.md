...
playback in `stacked_kit` apps, backed by thin wrappers over native
...
are no separate `stacked_kit_music` / `stacked_kit_video` kits. Video playback
is the next port to implement against the existing `VideoPlayerService` seam.

## Design

- **No dependency on `stacked` / `stacked_services` / `stacked_kit`.** This kit
...
```dart
import 'package:stacked_kit_media/stacked_kit_media.dart';
...
`package:stacked_kit_media/testing.dart` ships scriptable fakes —
