...
Every public symbol of the stacked_kit UI tier — `stacked_kit/ui_library`
plus the `stacked_kit/core` surface it re-exports — and **where each is used**
across this repo. This document is the single source of truth for "what does
the kit ship and who consumes it".

- **Canonical surface:** the barrel `stacked_kit/ui_library/lib/ui_library.dart`
  (41 export files + `export 'package:stacked_kit_core/stacked_kit_core.dart'`).
...
- **Method:** verbatim-symbol search over `.dart` files. Not counted as usage:
  ...
  prose, `stacked_kit/skills` + `stacked_kit/memory` tooling, comment-only
...
- **Consumer-area legend:** **p2 lib/** (feature shell named), **p2 test/**,
  **showcase lib/** + **showcase test/** (`stacked_kit/showcase_app`), **kit
  packages** (other `stacked_kit/<package>` — named per hit).
...
- **`KitAction`** (`kit_action.dart`) — fluent operation facade
  ...
  - kit packages · **data**: `stacked_kit/data/lib/facades/kit_data_facade.dart:46`
...
- **Other kit packages barely consume the UI tier.** Only `stacked_kit/data`
  does (`KitAction.run` in its facade, `locator`); `stacked_kit/notifications`
