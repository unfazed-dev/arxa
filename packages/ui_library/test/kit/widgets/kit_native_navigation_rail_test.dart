...
import 'package:ui_library/widgets/kit_native_navigation_rail.dart';
...
void main() {
  ...
  const destinations = [
    KitRailDestination(icon: Icons.home, label: 'Home'),
    KitRailDestination(icon: Icons.search, label: 'Search'),
  ];
  ...
  testWidgets('Android wantNative → NavigationRailM3E', (tester) async {
    ...
    await tester.pumpWidget(host(const KitNativeNavigationRail(
      selectedIndex: 0,
  ...
  testWidgets('iOS extended → wide Cupertino rail with inline labels',
    ...
    expect(
      tester.getSize(find.byType(KitNativeNavigationRail)).width,
      220,
  ...
  testWidgets('iOS built-in menu button toggles collapsed ↔ extended',
    ...
    await tester.pumpAndSettle();
    expect(tester.getSize(find.byType(KitNativeNavigationRail)).width, 220,
    ...
    await tester.pumpAndSettle();
    expect(tester.getSize(find.byType(KitNativeNavigationRail)).width, 84,
