...
import 'package:ui_library/widgets/kit_chip_carousel.dart';

import 'native_test_helpers.dart';

/// KitChipCarousel tests — rendering, the edge-fade invariant (fade only
...
void main() {
  ...
  testWidgets('renders all chips; no fade when everything fits',
      (tester) async {
    await tester.pumpWidget(host(const KitChipCarousel(children: [
      KitChip(label: 'One'),
      KitChip(label: 'Two'),
      KitChip(label: 'Three'),
    ])));
  ...
  testWidgets('overflowing rail fades edges and scrolls to the last chip',
      (tester) async {
    await tester.pumpWidget(host(KitChipCarousel(children: boxes(12))));
  ...
  testWidgets('snap settles on the nearest chip start', (tester) async {
    await tester.pumpWidget(host(KitChipCarousel(
      snap: true,
      spacing: 8,
  ...
  testWidgets('chips stay tappable through the fade mask', (tester) async {
    var taps = 0;
    await tester.pumpWidget(host(KitChipCarousel(children: [
      for (var i = 0; i < 12; i++)
        KitChip(label: 'Chip $i', onTap: i == 0 ? () => taps++ : null),
    ])));
    await tester.pump();
  ...
  testWidgets('rail height is intrinsic — tallest child wins', (tester) async {
    ...
    expect(tester.getSize(find.byType(KitChipCarousel)).height, 56.0,
