...
import 'package:ui_library/widgets/kit_native_sliver_app_bar.dart';
// Real bar buttons for the leading/trailing symmetry guard — scoped `show` so
// the barrel's own KitNativeSliverAppBar export doesn't clash with the direct
// import above.
...
void main() {
      ...
      MaterialApp(
        home: Scaffold(
          body: CustomScrollView(
            slivers: const [
              KitNativeSliverAppBar(title: 'Folders'),
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: CustomScrollView(
            slivers: [
              const KitNativeSliverAppBar(title: 'T', wantNative: false),
  ...
  test('snap without floating is rejected', () {
    expect(
      () => KitNativeSliverAppBar(floating: false, snap: true),
  ...
  // screen edge — visibly tighter than the fixed KitNativeAppBar
  ...
  // Inter-action spacing: the fixed KitNativeAppBar (CupertinoNavigationBar)
  // gaps its trailing actions by kGap8; the kit sliver bar must match so two
  ...
  testWidgets('Material tier spaces adjacent actions by kGap8', (tester) async {
    ...
    expect(
      ...
      moreOrLessEquals(8, epsilon: 0.5), // kGap8 — matches the CN bar

      reason: 'kit sliver bar must gap adjacent actions by kGap8 to match the '
          'fixed CN bar (Row spacing: kGap8)',
    );
  });
            ...
            slivers: [
              KitNativeSliverAppBar(
                ...
                leading: KitNativeIconButton(
                    glyph: KitGlyphs.back, onPressed: () {}),
                actions: [
                  KitNativeIconButton(glyph: KitGlyphs.more, onPressed: () {}),
                ],
              ),
              const SliverFillRemaining(child: SizedBox.shrink()),
            ],
  ...
  // chrome from the KitNativeIconButton actions. The kit implies its own back
  // button (same helper as KitNativeAppBar). withAndroidFallback: a real
  // KitNativeIconButton builds a CN UiKitView that can't render headless.
  testWidgets('Material tier: implied leading is a KitNativeIconButton + pops',
      (tester) async {
    await withAndroidFallback(() async {
          ...
          builder: (context) => TextButton(
            onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(
              builder: (_) => const Scaffold(
                body: CustomScrollView(
                  slivers: [
                    KitNativeSliverAppBar(title: 'Detail'),
                    SliverFillRemaining(child: SizedBox.shrink()),
                  ],
                ),
              ),
      ...
      expect(find.byType(KitNativeIconButton), findsOneWidget,
          reason: 'implied leading must be the same KitNativeIconButton the '
              'action slots use (parity with KitNativeAppBar)');
