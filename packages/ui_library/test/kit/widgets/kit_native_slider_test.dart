...
import 'package:ui_library/widgets/kit_native_slider.dart';
...
///   (KitNativeSlider → CNSlider) is unchanged — only the CN-internal render
///   path is diverted.
void main() {
  ...
  testWidgets('Android routes to SliderM3E', (tester) async {
    ...
    await tester.pumpWidget(host(const KitNativeSlider(value: 0.5)));
  ...
  testWidgets('default platform routes to CN (not SliderM3E) and builds clean',
    ...
    await withAndroidFallback(() async {
      await tester.pumpWidget(host(const KitNativeSlider(value: 0.3)));
  ...
  testWidgets('onChanged is wired on the M3E tier', (tester) async {
    KitPlatform.override = const KitPlatformOverride(isAndroid: true);
    double? fired;
    await tester.pumpWidget(host(KitNativeSlider(
      value: 0.0,
      onChanged: (v) => fired = v,
    )));
