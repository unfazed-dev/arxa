...
import 'kit_native_textfield.dart';

/// Adaptive pinned input bar — a composite of [KitNativeTextField] with
/// leading/trailing [KitNativeIconButton] action slots, reproducing the chat
...
/// [TextFieldM3E] on Android — see [KitNativeTextField]'s tier docs), so it is
/// pinned chrome and keeps the `KitNative*` name.
///
/// The public surface is **primitives + typed slots** — [leading]/[trailing]
/// take [KitNativeIconButton]s, never arbitrary [Widget]s, so the bar's actions
/// stay on the native icon-button tiers. Actions must NOT set
/// [KitNativeIconButton.size]: the bar enforces the kit's one-size bar-glyph
...
class KitNativeInputBar extends StatelessWidget {
  const KitNativeInputBar({
    super.key,
    this.controller,
    this.hintText,
    this.leading = const [],
  ...
  /// Owns the input text (forwarded to [KitNativeTextField.controller]).
  final TextEditingController? controller;

  /// Placeholder text, passed through to [KitNativeTextField.hintText].
  final String? hintText;

  /// Actions before the field (e.g. a `KitGlyphs.add` attach button). Each
  /// renders at the kit's bar-glyph size; do not set
  /// [KitNativeIconButton.size] (debug-asserted).
  final List<KitNativeIconButton> leading;

  /// Actions after the field (e.g. `KitGlyphs.mic` / `KitGlyphs.send`). Same
  /// one-size contract as [leading].
  final List<KitNativeIconButton> trailing;
  ...
  /// action buttons self-gate per platform like every `KitNative*` sibling.
  ...
  Widget build(BuildContext context) {
    ...
    assert(
      ...
      'KitNativeInputBar actions keep the kit bar-glyph size (18pt default) '
      'so leading/trailing stay aligned — omit `size`.',
    );
    final scheme = Theme.of(context).colorScheme;
    final bar = Padding(
      padding: const EdgeInsets.symmetric(horizontal: kPad12, vertical: kPad8),
      child: Row(
