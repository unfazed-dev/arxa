...
///
/// **Do NOT wrap tab stacks that mount platform views (any `KitNative*` /
...
///     body: KitDirectionalTabTransition(
...
/// )
/// ```
class KitDirectionalTabTransition extends StatefulWidget {
  const KitDirectionalTabTransition({
  ...
  /// [KitMotionScope]).
  ///
  /// Pass this as a `KitMotionScope.driver` inside a tab body to replay its
  /// wake choreography on every tab switch:
  ///
  /// ```dart
  /// KitMotionScope(
  ///   driver: KitDirectionalTabTransition.timelineOf(context),
  ///   child: ...,
  /// )
  /// ```
  ...
  @override
  State<KitDirectionalTabTransition> createState() =>
...
class _KitDirectionalTabTransitionState
    extends State<KitDirectionalTabTransition> {
  /// 0 until the first switch, then +1 (moved to a higher index) or -1.
  int _direction = 0;

  @override
  void didUpdateWidget(KitDirectionalTabTransition oldWidget) {
