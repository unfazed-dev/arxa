...
/// The edge of a scrollable a [KitScrollEdgeEffect] treats.
enum KitScrollEdge {
...
/// Style of a [KitScrollEdgeEffect], mirroring iOS 26's
/// `UIScrollEdgeEffect.Style`.
enum KitScrollEdgeEffectStyle {
...
/// chrome — so the widget is named `Kit*`, never `KitNative*`.
///
...
/// This supersedes [KitScrollOcclusionGate] for scroll duty — chrome stops
...
/// `KitNativeSliverAppBar` / `SliverAppBar` the effect needs **zero
/// configuration**. Use [occlusionPadding] only for chrome overlaid from
/// *outside* the scrollable (e.g. a tab bar stacked over
/// `Scaffold(extendBody: true)` at the [KitScrollEdge.bottom] edge).
///
...
/// ```dart
/// KitScrollEdgeEffect(
///   child: KitGlassCard(...), // frosted-tier content
/// )
/// ```
///
...
///     .scrollEdgeEffect(edge: KitScrollEdge.bottom, occlusionPadding: 56)
/// ```
class KitScrollEdgeEffect extends StatefulWidget {
  const KitScrollEdgeEffect({
    super.key,
    required this.child,
    this.style = KitScrollEdgeEffectStyle.automatic,
    this.edge = KitScrollEdge.top,
  ...
  /// [KitScrollEdgeEffectStyle.automatic], which resolves to `.soft`.
  final KitScrollEdgeEffectStyle style;

  /// Which viewport edge the coverage is measured against. Defaults to
  /// [KitScrollEdge.top].
  final KitScrollEdge edge;
  ...
  @override
  State<KitScrollEdgeEffect> createState() => _KitScrollEdgeEffectState();
}

class _KitScrollEdgeEffectState extends State<KitScrollEdgeEffect> {
  ...
  void _recompute() {
    ...
    final reveal = viewport
        .getOffsetToReveal(box, widget.edge == KitScrollEdge.top ? 0.0 : 1.0)
        .offset;
    final covered = widget.edge == KitScrollEdge.top
  ...
  Widget build(BuildContext context) {
    ...
    final hard = widget.style == KitScrollEdgeEffectStyle.hard;
...
/// the bar collapses.
extension KitScrollEdgeEffectX on Widget {
  Widget scrollEdgeEffect({
    Key? key,
    KitScrollEdgeEffectStyle style = KitScrollEdgeEffectStyle.automatic,
    KitScrollEdge edge = KitScrollEdge.top,
