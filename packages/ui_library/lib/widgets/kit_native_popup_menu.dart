...
/// [label] and/or [icon] / [sfSymbol], and [items] are [KitMenuItem]s — so hosts
/// never import either underlying dep. Each tier maps [KitMenuItem] to its
...
/// consistent with [KitNativeFab] / [KitNativeButton]. [label] renders a text
/// trigger; otherwise an icon trigger built from [icon] / [sfSymbol].
class KitNativePopupMenu extends StatelessWidget {
  const KitNativePopupMenu({
    super.key,
    this.glyph,
    IconData? icon,
    String? sfSymbol,
  ...
  /// Paired Material icon + SF Symbol for the trigger (see [KitGlyphs]). The
  ...
  /// Menu items. Each [KitMenuItem] maps to the tier's native menu type.
  final List<KitMenuItem> items;

  /// Notified with the selected item.
  final ValueChanged<KitMenuItem>? onSelect;
  ...
  Future<void> _openMenu(BuildContext context) async {
    ...
    final selected = await showMenu<KitMenuItem>(
      context: context,
      position: position,
      constraints: BoxConstraints(maxHeight: maxHeight),
      items: [
        for (final item in items)
          PopupMenuItem<KitMenuItem>(
            value: item,
            child: Row(
  ...
  Widget _glass(BuildContext context) {
    ...
    // 18pt matches KitNativeIconButton's Apple bar-glyph default (HIG nav-bar
    ...
    final symbol = sfSymbol == null
        ? null
        // Wire the trigger glyph to the kit accent (mirrors KitNativeIconButton)
    ...
    // (+ modal overlays) so it can't leak over a route slide — KitNativeChromeGate.
    if (label != null) {
      return KitNativeChromeGate(
