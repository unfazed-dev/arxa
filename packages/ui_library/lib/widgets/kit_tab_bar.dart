...
class KitTab {
  const KitTab({
    ...
    String? sfSymbol,
            ...
            glyph != null || icon != null, 'KitTab needs a glyph or an icon'),
        _icon = icon,
        _sfSymbol = sfSymbol;
...
class KitNativeTabBar extends StatelessWidget {
  const KitNativeTabBar({
    super.key,
    required this.tabs,
  ...
  Widget build(BuildContext context) {
    final wantNative = native ?? KitPlatform.supportsNativeChrome;
    ...
    if (wantNative && KitPlatform.supportsLiquidGlass) {
      ...
      // `CNTabBarRouteObserver`.  Our `withNativeChromeHidden` helper bumps
      ...
      //
      // Fix: KitNativeChromeGate hides the bar on `anyModalDepth` at the
...
class _KitFallbackTabBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    switch (KitPlatform.targetPlatform) {
