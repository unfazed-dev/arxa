...
/// M3-Expressive) image surface, so this widget is named `KitImage` (a
/// content widget), **not** `KitNativeImage`. See `core/NATIVE_COMPONENTS.md`
...
/// - **Radius** — [ClipRRect] clips the image to [radius] (default [kRad16]),
...
class KitImage extends StatelessWidget {
  ...
  /// Corner radius. Defaults to [kRad16].
  final double? radius;

  /// Glyph drawn over the fill color before the first frame decodes and on
  /// load error. Defaults to [KitGlyphs.photo].
  final KitGlyph? placeholder;
  ...
  Widget build(BuildContext context) {
    ...
    final glyph = placeholder ?? KitGlyphs.photo;
    final w = size ?? width;
    final h = size ?? height;
    final r = radius ?? kRad16;
