...
///
/// - [KitNativeProgress.linear] — a horizontal bar. Android →
///   [LinearProgressIndicatorM3E]; else → Material [LinearProgressIndicator].
/// - [KitNativeProgress.circular] — a ring. Android →
...
class KitNativeProgress extends StatelessWidget {
  const KitNativeProgress._({
    super.key,
  ...
  factory KitNativeProgress.linear({
    Key? key,
    double? value,
    Color? color,
  }) =>
  ...
  factory KitNativeProgress.circular({
    Key? key,
    double? value,
    Color? color,
  }) =>
  ...
  Widget build(BuildContext context) {
    ...
    if (KitPlatform.supportsComposeM3E) return _m3e(accent);
    switch (_variant) {
