...
import 'kit_native_icon_button.dart';
import 'kit_native_sliver_app_bar.dart';

/// Adaptive top app bar — two-way structural gate (mirrors [KitNativeTabBar]'s
...
/// must collapse/float with the scroll, use [KitNativeSliverAppBar] as the
/// first sliver instead (the kit default there is `pinned; floating + snap off`).
/// [KitNativeAppBar.sliver] delegates to it for back-compat.
class KitNativeAppBar extends StatelessWidget implements PreferredSizeWidget {
  const KitNativeAppBar({
    super.key,
    this.title,
    this.actions,
    this.leading,
  ...
  /// leading is a kit-owned [KitNativeIconButton] with [KitGlyphs.back] (see
  /// [kitImpliedAppBarLeading]) so the back affordance matches the trailing
  ...
  /// [KitNativeSliverAppBar] (the first-class sliver bar; `pinned; floating +
  /// snap off` by default). Prefer constructing [KitNativeSliverAppBar]
  ...
  // must return the enclosing type (`KitNativeAppBar`), but the sliver tier
  // returns a sibling sliver widget. The call site `KitNativeAppBar.sliver(...)`
  // is unchanged.
  ...
  Widget build(BuildContext context) {
    ...
    if (want) {
      ...
      // status bar edge-to-edge. Same fix as KitNativeTabBar._m3e: make the
    ...
    if (KitPlatform.isIOS && wantNative) {
      final scheme = Theme.of(context).colorScheme;
      ...
      // label preserved — see kitImpliedAppBarLeading).
      final effectiveLeading = kitImpliedAppBarLeading(
        context,
        leading: leading,
        automaticallyImplyLeading: automaticallyImplyLeading,
      );
      ...
      //
      // The CN persistent height is a FIXED 44 (== KitNativeIconButton's 44pt
      ...
      // gap of kGap8) so the buttons get the same bottom
        ...
        children: [
          ...
          ColoredBox(
            ...
            child: const SizedBox(
              key: Key('kitNativeAppBarBottomGap'),
              height: kGap8,
            ),
          ),
        ],
    ...
    // tiers (which self-inset 16 + space by kGap8). Mirror KitNativeSliverAppBar's
    // Material-tier guards so every appbar tier lands its bar buttons at the same
    // inset (16), gap (kGap8), and size — the two bars are kept in lockstep
    ...
    // from the KitNativeIconButton trailing actions, so the kit implies its own.
    final effectiveLeading = kitImpliedAppBarLeading(
      context,
      leading: leading,
      automaticallyImplyLeading: automaticallyImplyLeading,
