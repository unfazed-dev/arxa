...
import '../../widgets/kit_native_sheet.dart';

/// Drop-in [BottomSheetService] that presents through [kitShowNativeSheet]
/// instead of `Get.bottomSheet`, so every stacked sheet call site
/// (`showBottomSheet`, `showCustomSheet`, KitAction's NotificationManager)
...
/// useRootNavigator, elevation) are accepted but ignored — kitShowNativeSheet
...
class KitBottomSheetService extends BottomSheetService {
  ...
  Future<SheetResponse?> showBottomSheet({
    ...
    if (context == null) {
      debugPrint('KitBottomSheetService: no navigator context; sheet skipped');
      return Future.value(null);
    }
    return kitShowNativeSheet<SheetResponse>(
      context: context,
  ...
  Future<SheetResponse<T>?> showCustomSheet<T, R>({
    ...
    if (sheetBuilder == null || context == null) {
      debugPrint('KitBottomSheetService: no builder/context; sheet skipped');
      return Future.value(null);
    }
    return kitShowNativeSheet<SheetResponse<T>>(
      context: context,
      isDismissible: barrierDismissible,
