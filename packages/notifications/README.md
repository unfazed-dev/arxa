# stacked_kit_notifications

The **device-notifications** port for `stacked_kit` apps — permission, push
token / foreground-message seams, local notifications, and badge.

## Naming: not the same as core's `KitNotificationService`

`stacked_kit` core already ships `lib/services/notifications/` with
...
- **No dependency on `stacked_kit` core, `stacked`, or `stacked_services`.**
...
`import 'package:stacked_kit_notifications/testing.dart';` for
