...
capability kit with **no dependency on stacked_kit** core.
...
- Deterministic test drivers (in `package:stacked_kit_motion/testing.dart`):
...
- No routing, transitions-between-pages, or navigation — route/tab transitions
  live in `stacked_kit` core (`KitDirectionalTabTransition` et al.); this kit
  only *consumes* their animations as drivers.
- No dependency back on `stacked_kit` core, `stacked_services`, or a host
...
```dart
import 'package:stacked_kit_motion/testing.dart';
...
`stacked_kit_motion` never imports `stacked_kit`. The allowed direction is the
