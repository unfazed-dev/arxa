...
```dart
import 'package:stacked_kit/stacked_kit.dart';
```

## When to use

Use this skill any time you touch a Stacked app with `stacked_kit` in its
...
- Adopting `stacked_kit` into a fresh Stacked project → follow **Host integration**.
...
`docs/plans/stacked-kit-showcase-app-port.md` §7.
...
## Data layer (stacked_kit_data)

`stacked_kit_data` is a sibling package (`stacked_kit/data`, a path
dependency exactly like `stacked_kit` itself) that gives a Stacked host one
...
1. **Dependency.** Add `stacked_kit_data` as a path dep. It pulls in the
   ...
   propagate): *"appwrite's transitive graph pins win32 5.x while stacked_kit
...
- **The kit never self-registers.** `KitData.initialize` only runs when the
  host calls it, matching `stacked_kit`'s host-integration contract.
...
## Showcase & app template (stacked_kit_showcase_app)

**`showcase_app` is the reference template** for bespoke
stacked_kit apps — a standalone runnable app scaffolded and grown *entirely*
via the Stacked CLI (see "Building on the kit" above; the step-by-step playbook
lives in `docs/plans/stacked-kit-showcase-app-port.md` §7). The older
`packages/stacked_kit_showcase` (pluggable, host-embedded) is superseded as the
...
iOS-Notes-style app smoke-testing every stacked_kit_data capability over
...
- **Host plug-in is exactly three moves** (see the package README):
  `await StackedKitShowcase.initialize()` in `main()`, the `/showcase`
...
- **It's a sibling package, not part of stacked_kit** — it depends on
  stacked_kit_data (which depends on stacked_kit); nesting it in the kit
...
1. **Dependency.** Add `stacked_kit` (path/git/pub). Declare in the host only what the host uses directly (commonly `stacked` + `stacked_services`); the kit's own deps resolve transitively — see **Dependency management** below.
...
below are re-exported from the barrel (`package:stacked_kit/stacked_kit.dart`).
...
`stacked_kit_native` plugin was removed); unavailable surfaces fall back to the
built widget.
...
1. **Router shell** (the class can't live in the kit — it `extends` the host's
   ...
   import 'package:stacked_kit/stacked_kit.dart' show KitPlatformPagesMixin;
...
- **Never add a host import** (`package:<app>/...`) inside `stacked_kit/`. The kit stays host-agnostic; reach services only through `locator`.
...
- Data layer source of truth: `stacked_kit/data/lib/`. Architecture + locked decisions: `docs/plans/stacked-kit-data-layer.md`. Auth seam decisions: `docs/plans/stacked-kit-data-auth.md`. Vocabulary (Backend, Seed Key, Canonical ID, Schema Descriptor, Auth Service, Session, Fake Auth, …): `CONTEXT.md`. Canonical-ID derivation: `docs/adr/0001-deterministic-v5-canonical-ids.md`.
- SSOT: this skill lives IN the kit package at `stacked_kit/skills/stacked-kit/SKILL.md`; `.claude/skills/stacked-kit` is a symlink to it (edit either path, same file). When adopting the kit in another project, symlink `stacked_kit/skills/stacked-kit` into that project's `.claude/skills/stacked-kit` — do not copy (copies go stale).
