...
| [plans/rust-port-closure-and-surgical-lens.md](plans/rust-port-closure-and-surgical-lens.md) | decisions locked 2026-08-21 — Rust port CLOSED; evidence bundle + drill-down compare, capture manifest, bounded auto-loop, semantic dictionary + provenance gate (prefix now `arxa-`, see amendment), read-only arxa in using-sessions + `lens eval`, feedback dial + Supabase, a11y toolbox island — **amended 2026-08-23**: feedback dial becomes the Arxa Dial (two modes, Draft Overlay + manual Publish, Share Link guest access, pin kanban) |
