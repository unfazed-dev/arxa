# arxa-guard findings — 2026-08-26
...
`arxa/lib/cdp.dart` (the append itself is deferred — see the tail of this
file). Session mode source: `ARXA_GUARD_MODE=using` in the harness process
env; no `~/.arxa/guard-mode` file exists.
...
`hooks/arxa-guard.js:L139-158` extracts shell write-targets with
...
`echo "ARXA_GUARD_MODE=${ARXA_GUARD_MODE:-<unset>}"` was denied with
...
`ARXA_GUARD_MODE=dev <command>` as the escape hatch for a legitimate
...
Reproduced: `ARXA_GUARD_MODE=dev printf '…' >> arxa/lib/cdp.dart` was
...
`Override: relaunch this session with ARXA_GUARD_MODE=dev, or apply the
write yourself outside the session.` No guard-logic change needed.

## The deferred edit

Requested: append a one-line comment to `arxa/lib/cdp.dart`. The file was
clean at HEAD when the attempt was made. Ready to apply from an arxa-dev
