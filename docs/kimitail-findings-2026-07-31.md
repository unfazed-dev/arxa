...
**Status: DOCUMENTED ONLY — no fixes applied.** Deferred until the full appbox
implementation is complete; re-review then. Scope: `appboxd/`, `kit/`, root
tooling (`hooks/`, `tools/`, `gates/`, `pipeline/`, `config/`). Excluded:
`appbox-studio/`, `designs/appbox-studio/` (active development), `archives/`.
...
**Totals: ≈ −7,600 lines possible** (appboxd −2,450 · kit part 1 −1,500 ·
...
## appboxd/ (−2,450; ≈−1,000 if imminent features are wired instead of cut)

### Three large dead blocks (≈1,450 of the total)

- `appboxd/lib/blueprint.dart:L2964-3505: delete: ~420 lines of data-model projection machinery unreachable — buildBlueprint's only caller never passes dataModelPath, so _entityDart/_supabaseAdapterDart/_driftDbDart/_migrationSql/seed-SQL helpers never run. Delete, or wire a --data-model flag.`
- `appboxd/lib/generate_view.dart:L900-1509 + L3057-3111: delete: ~700 lines of uncalled VM/OTP template emitters (tplVmHomeBase/Stub, tplVmSplash, tplVmAuth, tplVmWelcome, tplVmOtp, tplViewOtp, tplVmBase) — buildView emits only views.`
- `appboxd/lib/runtime_config.dart:L1-126 + harness.dart:L1-34 + credentials.dart:L1-149: delete: three whole files (~310 lines) with zero production callers, test-only — ports landed ahead of wiring. Delete with their tests, or wire them. (If wiring is imminent, hold.)`

### blueprint / emit / generate_view

- `appboxd/lib/blueprint.dart:L3404-3435: yagni: _assemble opts factoryV/generated/stack/appNameOverride never passed; upfront mkdir loop redundant with write loop's recursive create.`
- `appboxd/lib/blueprint.dart:L504-563: yagni: _tplPorts/_tplBaseRepo emit Repository/CrudRepository/BaseSupabaseRepository that no generated code implements or extends.`
- `appboxd/lib/blueprint.dart:L329-358: shrink: _sortedJsonEncode/_prettySortedJson duplicate emit_stage.dart's _sortKeys+_encodeSorted — hoist one shared helper.`
- `appboxd/lib/blueprint.dart:L2644-2698: delete: unused tabScreens param in _tplNavShellView/_tplNavShellVm.`
- `appboxd/lib/emit_stage.dart:L990-1076: yagni: public emit() has one caller; adopt/supabaseUrl/supabasePublishableKey never passed and the .env block is dead — inline into emitStage.`
- `appboxd/lib/emit_stage.dart:L43-48: shrink: _binaryExt list + _isBinary for one extension → endsWith('.png').`
- `appboxd/lib/generate_view.dart:L828-896 + L158-164 + L383-399: delete: seedToSessionLiterals/AnchorDate/seedDefaultMap/dartStrSeed — only callers are tests.`
- `appboxd/lib/generate_view.dart:L1522-1634 + L780-790 + L1512-1518 + L2318-2319: delete: extractCallSites/tplHomeAssetsStub, specProviders, stubIsDefault, children() — zero callers.`
- `appboxd/lib/generate_view.dart:L1705-1751: delete: svgAssets/svgLedger/dropGlyph — write-only state, populated and never read.`
- `appboxd/lib/generate_view.dart:L2097-2102: shrink: !inner.contains('\${') and !inner.contains(r'${') are the same check twice.`
- `appboxd/lib/scaffold.dart:L684-715: yagni: ScaffoldEmitter facade class with zero callers — scaffold_cli calls scaffold() directly.`

### engine / memory / fabric / pipeline

- `appboxd/lib/phases.dart:L102-148: yagni: PipelineRunner/PipelineAvailability — only tests touch them; server.dart uses runPhase.`
- `appboxd/lib/memory_cache.dart:L37-43,L126-151: yagni: cacheBreakpointMarker/assemblePrompt test-only; engine calls cache directly.`
- `appboxd/lib/memory.dart:L10-11,L136-149: delete: dead MemoryKinds.licence/.deploy constants and test-only countByKind.`
- `appboxd/lib/memory_curate.dart:L66-85: yagni: lessonsFor/readFacts test-only; production calls appendLesson only.`
- `appboxd/lib/tier1.dart:L52-71: delete: RealRunner — its own docstring admits it's unused.`
- `appboxd/lib/engine.dart:L29-43: delete: Stage.gate is write-only; Stage.toJson uncalled.`
- `appboxd/lib/trace.dart:L124-128: delete: _routeFactoryRe — declared with // ignore: unused_element.`
- `appboxd/lib/intake.dart:L438,L481: delete: unused ValidationResult.ok getter and IntakeEngine.validate wrapper.`
- `appboxd/lib/fabric.dart:L15-190: yagni: escalation and three notes fields parsed, stored, never read.`
- `appboxd/lib/fabric.dart:L150-151: delete: ModelFabric.paramPolicy — zero callers.`
- `appboxd/lib/pipeline_fsm.dart:L71-206: yagni: initPipeline({targets}) and approvePrototype({note}) — params never passed; targets state key write-only.`
- `appboxd/lib/memory_analytics.dart:L27,L173: delete/yagni: LineCounts.total incremented never read; spend({since}) never passed in production.`
- `appboxd/lib/gen_playbook.dart:L42-319: yagni: loadKitFacts + ~70 lines of README-mining reachable only via it — dead in production (bin reads facts JSON raw). Wire or cut.`
- `appboxd/lib/gen_freshness.dart:L44: shrink: bool? buildFailed — only true/absent occur; make it bool = false.`

### gates

- `appboxd/lib/gate_freeze.dart:L98-308: delete: _checkShape always returns true — dead early-return.`
- `appboxd/lib/gate_freeze.dart:L184-189 + L892-897: shrink: _Derivation/_FutureServer single-use wrapper classes → records.`
- `appboxd/lib/gate_freeze.dart:L767-803 + gate_lens.dart:L482-519: shrink: _bindStaticServer/_mime duplicated near-verbatim — hoist to gates.dart; same for the duplicated _Vp class.`
- `appboxd/lib/gate_freeze.dart:L191-251 + gate_coverage.dart:L379-418: shrink: both re-implement the derivation inherit-walk — one shared resolver.`
- `appboxd/lib/gate_freeze.dart:L419-425,L652-653,L927-939: shrink: _listEq/_setEq/_renderFallback single-use helpers — inline.`
- `appboxd/lib/gate_freeze.dart:L1090 + gate_scaffold.dart:L610: stdlib: hand-rolled basename helpers → p.basename (package:path already a direct dep).`
- `appboxd/lib/gate_scaffold.dart:L687-705: stdlib: _normalize reimplements p.normalize.`
- `appboxd/lib/gate_coverage.dart:L439-447 + gate_deploy.dart:L159-167: shrink: _truthy duplicated verbatim — hoist one.`
- `appboxd/lib/gate_runner.dart:L51,L91-166: delete/shrink: uncalled SuiteResult.allPassed; _runSingleGate alias fold; _tryBashGate extensions map dead except 'review' (and duplicates bin/appbox.dart's _runReviewGate).`

### design server / tools / selftest

- `appboxd/lib/design_server.dart:L575-586: stdlib: _parseCookies hand-parses the Cookie header — dart:io ships req.cookies.`
- `appboxd/lib/design_server.dart:L507-519 + L687-688: shrink/delete: _triedList re-derives resolveArtifact's candidate walk; serveRegistryFileCount uncalled.`
- `appboxd/lib/design_server/worker.dart:L37,L306: delete/yagni: WorkerResponse.locale never read; findWorkerAssetsDir(from:) never passed.`
- `appboxd/lib/design_selftest.dart:L148-172 + design_tools.dart:L72-75,L794-802: shrink: _findRepoRoot, _walkFiles, and recursive copy duplicated across the two files — share one each.`
- `appboxd/lib/design_selftest.dart:L889-891 + L994-998: shrink/delete: 3-line claimed loop is one map().toSet(); mutationLabel uncalled.`
- `appboxd/lib/design_tools.dart:L184-197 + L954-955: shrink/stdlib: designCheckLadder parses ladder.json twice; manual basename → p.basename.`

### lens / cdp

- `appboxd/lib/lens.dart:L178-205: delete: runLensGate — zero callers (only a README mention).`
- `appboxd/lib/cdp.dart:L106-107,L314-315,L405,L540-545,L706: yagni/delete: launch(chromePath:, extraArgs:), events getter, setViewport(deviceScaleFactor:), screencast maxWidth/maxHeight/everyNthFrame, ScreencastFrame.timestamp — all never passed/read.`
- `appboxd/lib/cdp.dart:L584-620: yagni: setEmulatedMedia/elementScreenshot test-only (−36 if cdp.dart is lens substrate, keep if it's a general protocol lib).`
- `appboxd/lib/lens_cli.dart:L253-261 + lens/crawl.dart:L334-341: shrink: _hexToRgb hand-rolled twice — hoist one.`
- `appboxd/lib/lens_cli.dart:L452-457,L654-692: shrink: _color re-implements _emitJson; duplicated rung parsing; _matchRung one-liner with a doc promising unimplemented behavior.`
- `appboxd/lib/lens/tokens.dart:L130-140: delete: tokensToJson — zero callers.`
- `appboxd/lib/lens/native/flutter_vm.dart:L76-77 + sck.dart:L75-79: delete/yagni: uncalled start(); test-only tccOk().`

### misc

- `appboxd/lib/story_map.dart:L335-343: stdlib: _esc hand-rolls HTML escaping — HtmlEscape().convert(s) is byte-identical and dart:convert is already imported.`
- `appboxd/lib/story_map.dart:L40-41,L584-588: delete/shrink: _rankToPriority is a literal copy of _priorityOrder; _FeatureEntry holder → record.`
- `appboxd/lib/crypto_aead.dart:L456-466: yagni: ChaCha20-Poly1305 (12-byte nonce) pair is test-only; production routes through XChaCha. Cut only with the RFC vector test, or keep as the vector seam.`
- `appboxd/lib/deploy.dart:L264-266,L462-466: yagni/stdlib: targetStatus used only by self-test; _utcTimestamp hand-rolls toUtc().toIso8601String() (keep if seconds-precision parity with the Python port matters).`
- `appboxd/lib/synthesize.dart:L445-463: shrink: _dumpSorted → SplayTreeMap key-sort + JsonEncoder.withIndent('  ') (verify against golden).`
- `appboxd/lib/transform_tokens.dart:L71,L109: delete: _clamp255 guards provably-in-range 2-hex-digit parses — dead.`
- `appboxd/lib/palette.dart:L20-82: stdlib: hand-rolled clamps → num.clamp; L84-85 dead if (h < 0) branch — Dart % is euclidean so the result is already non-negative (comment is wrong).`
- `appboxd/lib/intake.dart:L565-575 + kit_facts.dart:L409-419: shrink: _stripAll/_stripEnds duplicate hand-rolled strip → one regex.`
- `appboxd/lib/emit_htmx.dart:L88-90 + emit_playground.dart:L81-83: shrink: stripCssComments + contentRoot duplicated verbatim — hoist one shared copy.`
- `appboxd/lib/kb_check.dart:L18-52: delete: verifyLinks option never passed true; only effect is a TODO warning.`
- `appboxd/lib/server.dart:L14-18 + config.dart:L25,L49 + vault.dart:L61 + docs_lint.dart:L53-54: yagni: never-passed parameters (address, webRoot, service, memoryDir) — inline the defaults.`
- `appboxd/lib/docs_lint.dart:L22-29: shrink: KbIssue is field-for-field DocIssue — reuse it.`
- `appboxd/lib/api_map_scan.dart:L49-56: shrink: _stripDartComments byte-identical to capability_scan.dart's — hoist one.`
- `appboxd/lib/deploy_cli.dart:L121-134 + watermark.dart:L146-162 + bin/appbox.dart:L835: shrink: three private copies of the walk-up repo/licence-path resolver — one shared helper (watermark's fallback also diverges despite the comment).`
- `appboxd/lib/kit_lock.dart:L77-102: shrink: _discoverSkills/_discoverTools same loop, one filter apart — collapse.`
- `appboxd/lib/scaffold_cli.dart:L28-56: shrink: 5 redundant break; statements — Dart 3 switch doesn't fall through.`
- `appboxd/bin/appbox.dart:L481-483: shrink: _runCrud one-line wrapper — inline.`

### Checked and lean (no findings)

gateway.dart, crud.dart, emit_playground.dart, kb_build.dart, licence.dart,
lint_conventions.dart, intake_cli.dart, bin/appboxd.dart, bin/licence_tool.dart,
...
- `emit_stage.dart:L1138-1142` treats `{'isAppbox': false}` as truthy — correctness bug (fixed 2026-07-31).
...
- `kit/auth/lib/src/backends/seed_auth_backend.dart + providers/kit_oauth_provider.dart + apple/ + google/: yagni: 119 lines of throw-only stubs, no real implementors — real seed backend lives in appboxd/lib/tier1.dart. Caveat: tracked in config/kit-registry.json/stub-inventory.md; if that registry is load-bearing, finding is void.`
...
- `hooks/appbox-doc-enforce.js:47-49: shrink: hasMarker() wraps fs.existsSync in try/catch — existsSync never throws. Body is return fs.existsSync(MARKER).`
- `hooks/appbox-doc-health.js:23-31,36-55 + hooks/appbox-doc-enforce.js:84-92,34-36,60-68: shrink: identical stdin-read/JSON.parse/file_path-extract preamble (~10 lines) and identical repoRoot-locate + spawnSync('dart', …'docs') block (~15 lines) duplicated across both hooks. One shared sibling require('./_doc-hook-common.js') halves both files. Modest win; skip if zero-dependency hook files are valued.`
- `tools/sweep_rename.sh:1-67: delete: whole file. All patterns are identity mappings (appbox:appbox — header literally says "rename appbox → appbox"); sed replaces every string with itself; the script can only ever no-op. The rename already happened.`
- `tools/phase5_kit_copy.sh:1-68: delete: one-shot migration, already applied (kit/ exists). docs/plans/appbox-dart-only-tooling.md:227 already classifies both tools as "transient", and its hardcoded $SRC=/Volumes/... abs path violates the repo's own R3 (config/forbidden_abs_prefixes.txt). A re-copy is a 3-line rsync when actually needed.`
...
- `gates/freeze/freeze.sh:121-143: shrink: viewport/inherits derivation from targets.derivation.json implemented a second time here (embedded python vps_of recursion) — appboxd/lib/gate_coverage.dart:44-129 and scaffold_cli.dart:87 already parse the same table in Dart. One derivation should call the other.`
