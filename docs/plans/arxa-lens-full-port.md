# arxa lens — full probe-runner port + skills dartification

**Status:** **COMPLETE** (2026-07-31). Settled with the operator 2026-07-31;
landed in commit 4f9c458 with the full arxa suite green (826/826).
...
outputs (`CdpSession.key()`/`click()` in `arxa/lib/cdp.dart`,
`arxa/tool/lens_check.dart`, `arxa/test/lens_input_test.dart`) are
**inputs** to this plan — never re-implement them, never edit the files that
plan owns while it is in flight.
**Inputs:** [`arxa-dart-only-tooling.md`](arxa-dart-only-tooling.md)
(doctrine, COMPLETE for the pipeline spine) ·
[`../../skills/arxa-lens/SKILL.md`](../../skills/arxa-lens/SKILL.md) +
[`../../skills/arxa-lens/capability-map.md`](../../skills/arxa-lens/capability-map.md)
...
into the arxa lens — improved for arxa, not transliterated — and dartify
the skills layer's remaining non-Dart runtimes (`.mjs`/`.py`/`.sh`). End
state: `skills/arxa-lens/capability-map.md` fully green (every verb
**ported** with a new name or **dropped with reason**), `arxa lens <verb>`
wired in `arxa/bin/arxa.dart`, a real `lens` pipeline gate
(design-vs-built), every skill invoking Dart only, python/node/bash archived
out of `skills/` and grep-clean, `arxa` tests green.

## Architecture

- **Lens subtree.** `arxa/lib/lens/` becomes a package subtree:
  ...
  `native/flutter_vm.dart`. The existing flat `arxa/lib/lens.dart` stays
  ...
  `arxa/lib/cdp.dart` stays the transport; this plan adds domain helpers
...
- **One Chrome per invocation** (existing isolation doctrine), pure `dart:io`
  ...
  sourced from `config/arxa.config.json` / `ladder.json`, never hardcoded
  in app code — R3).
- **CLI.** `arxa lens <verb>` (new `case 'lens':` in
  `arxa/bin/arxa.dart`, dispatch in new `arxa/lib/lens_cli.dart`).
...
- **Native capture stays spawned external binaries driven from Dart**
  ...
  over raw WebSocket JSON-RPC. Lens-named (`arxa lens native adb shot`),
  never "probe".
- **Design artifacts stay JavaScript; the server becomes Dart.** The designs
  ...
  (`arxa/lib/design_server.dart`) therefore executes artifact JS in a
...
- `image` is adopted as arxa's second runtime dependency (after `path`).
...
- **Media-plan files are inputs, not targets.** Do not edit
  `arxa/lib/cdp.dart`'s `key()`/`click()`, `arxa/tool/lens_check.dart`,
  `arxa/test/lens_input_test.dart`, or any designer island file
...
**Interfaces**
- Consumes: existing `arxa/lib/lens.dart` API; media plan's
  `cdp.dart`/`tool/lens_check.dart` untouched.
- Produces: `arxa/lib/lens/pixels.dart`; `image: ^4.5.4` in
  `arxa/pubspec.yaml`; rewired `compareGolden` internals (signatures
  unchanged); `arxa/test/lens_pixels_test.dart`.

### Steps

- [x] 1.1 Write the failing test — `arxa/test/lens_pixels_test.dart`:
  ```dart
  ...
  ```
  Run `cd arxa && dart test test/lens_pixels_test.dart` — expected:
  COMPILE ERROR (`package:arxa/lens/pixels.dart` not found). Red
  confirmed.
- [x] 1.2 Add the one sanctioned dependency to `arxa/pubspec.yaml`
  (dependencies block, after `path: ^1.9.0`):
  ```yaml
    image: ^4.5.4
  ```
  ```sh
  cd /Volumes/developer_ssd/Developer/totem_labs/arxa/arxa && dart pub get
  ```
  Expected: `Got dependencies!` with `image` resolved at 4.5.x.
- [x] 1.3 Create `arxa/lib/lens/pixels.dart` — complete file:
  ```dart
...
- [x] 1.4 Rewire `compareGolden` in `arxa/lib/lens.dart`. Keep the
...
- [x] 1.5 Update the stale docs in `arxa/lib/lens.dart`'s header comment
...
Additive extensions to `arxa/lib/cdp.dart`, appended below the media
...
- Produces: in `arxa/lib/cdp.dart` — `CdpScreencast` +
  `CdpSession.screencast()`, `traceNetwork()`, `getFullAxTree()`,
  `captureDomSnapshot()`, `setEmulatedMedia()`, `elementScreenshot()`;
  `arxa/test/cdp_domains_test.dart`.

### Steps

- [x] 2.1 Write the failing test — `arxa/test/cdp_domains_test.dart`.
  ...
  ```
  Run `cd arxa && dart test test/cdp_domains_test.dart` — expected:
  COMPILE ERROR (`screencast`, `traceNetwork`, … undefined). Red confirmed.
- [x] 2.2 Add to `arxa/lib/cdp.dart`, appended after the media plan's
...
`arxa/lib/lens/{tokens,dom,a11y,net}.dart`. `tokens.dart` is the fully
...
- Produces: the four lib modules + `arxa/test/lens_tokens_test.dart`,
...
- [x] 3.2 Create `arxa/lib/lens/tokens.dart` (exemplar — the clustering
  ...
  ```
  plus a tiny shared writer placed in `arxa/lib/lens.dart` (facade, so
...
- [x] 3.3 Create the three sibling modules — same shape as 3.2 (fixture →
  ...
  exact deltas:
  - `arxa/lib/lens/dom.dart` — `Future<Map<String, dynamic>>
    ...
    evaluate call).
  - `arxa/lib/lens/a11y.dart` — `Future<Map<String, dynamic>>
  ...
  - `arxa/lib/lens/net.dart` — `Future<Map<String, dynamic>>
...
motion + WAAPI oracle) to `arxa/lib/lens/motion.dart`. probe-runner's
...
- Produces: `arxa/lib/lens/motion.dart`;
  `arxa/test/lens_motion_test.dart`.

### Steps

- [x] 4.1 Write the failing test — `arxa/test/lens_motion_test.dart`.
  ...
  confirmed.
- [x] 4.2 Create `arxa/lib/lens/motion.dart` with four public verbs:
  ...
  - `Future<Map<String, dynamic>> captureFlipbook(String url, {required
    ...
    covers arxa's own designs; add frame recovery when a design
...
- Consumes: Task 2's `captureDomSnapshot`/`setEmulatedMedia`, Task 1's
  pixels (state diffs).
- Produces: `arxa/lib/lens/skeleton.dart`, `arxa/lib/lens/states.dart`;
  `arxa/test/lens_skeleton_test.dart`, `lens_states_test.dart`.
...
- [x] 5.2 Create `arxa/lib/lens/skeleton.dart`:
  ...
  - `Map<String, dynamic> diffSkeleton(Map<String, dynamic> a,
    ...
    the self-diff test.)
- [x] 5.3 Create `arxa/lib/lens/states.dart`:
...
machinery, no arxa consumer; recorded in capability-map). This is the
moodboard-at-scale capability the arxa-moodboarder skill fans out
manually today.

**Interfaces**
- Consumes: Task 3's tokens, Task 5's skeleton.
- Produces: `arxa/lib/lens/crawl.dart`;
  `arxa/test/lens_crawl_test.dart`.

### Steps

- [x] 6.1 Write the failing test — `arxa/test/lens_crawl_test.dart`.
...
## Task 7: Wire `arxa lens <verb>` in the CLI

`arxa/bin/arxa.dart` currently lists `lens` as future and falls
...
- Consumes: Tasks 1-6 lib modules; existing `_runEmit`/`_runServe` argv
  style in `bin/arxa.dart`.
- Produces: `arxa/lib/lens_cli.dart` (`Future<int> runLens(List<String>
  args)`), `case 'lens':` in `bin/arxa.dart`, usage text;
  `arxa/test/lens_cli_test.dart`; `tool/lens_shot.dart` unchanged
  (still valid).

### Steps

- [x] 7.1 Write the failing test — `arxa/test/lens_cli_test.dart`:
...
- [x] 7.2 Create `arxa/lib/lens_cli.dart`. Verb table (each verb maps
  to one lib call; argv parsing follows `_runEmit`'s manual style —
  positional args then `--flag=value`):
  ```
  arxa lens shot <url> <out.png> [width] [height] [settleMs] [--full]
                                   → captureGolden
  arxa lens check <url> <out.png> [w] [h] [settle] [--selector=..] [--press=..] [--expect=..]
                                   → media plan's lens_check logic, moved
                                     verbatim into a lib function it and
                                     tool/lens_check.dart both call
  arxa lens compare <url> <golden.png> [w] [h] [--mode=byte|pixel|ssim] [--threshold=0.95]
                                   → compareGolden; exit 1 on fail
  arxa lens shoot <url> [--rungs=compact,medium,expanded] [--out=dir] [--artifact=dir]
                                   → ladder pass: captureGolden per rung
                                     (rung widths from
                                     skills/arxa-designer/runtime/ladder.json
                                     — the file ladder.json already is;
                                     --artifact/_d_meta.json .ladder
                                     override, then $ARXA_LADDER, then
  ...
  arxa lens tokens <url> [w] [h] [--out=path]     → extractTokens
  arxa lens dom <url> [--out=path] [--html]       → extractDom / extractOuterHtml
  arxa lens a11y <url> [--out=path]               → extractA11y
  arxa lens net <url> [--traceMs=3000] [--out=path] → traceNet
  arxa lens record <url> <out.mp4> [seconds] [fps] [w] [h] → recordVideo
  arxa lens burst <url> <out-dir> [count] [intervalMs] [w] [h] → burstFrames
  arxa lens anim <url> [w] [h] [--out=path]       → captureScrollAnim
  arxa lens flipbook <url> --trigger=<css> [--out=path] → captureFlipbook
  arxa lens skeleton <url> [--out=path] [w] [h]   → captureSkeleton
  arxa lens skeleton-diff <a.json> <b.json>       → diffSkeleton; exit 1 when deltas
  arxa lens states <url> --trigger=<css>:<click|hover|focus> [--trigger=..] [--out=path]
                                   → captureStates
  arxa lens crawl <baseUrl> [--routes=/a,/b] [--crawl] [--crawl-max=20] [--out=dir]
...
- [x] 7.3 Edit `arxa/bin/arxa.dart`: add `case 'lens':
  ...
  `lens  Visual gate (arxa lens — future)` usage line with the verb
...
- [x] 7.5 Run `dart test test/lens_cli_test.dart` — pass; full suite
  ...
  sleep 2
  cd arxa && dart run bin/arxa.dart lens shot http://localhost:4399/dashboard /tmp/lens-cli-dash.png 390 844 2000
  dart run bin/arxa.dart lens tokens http://localhost:4399/dashboard 390 844 --out=/tmp/lens-cli-tokens.json
  cd .. && kill %1
  ```
...
`arxa/lib/lens/native/adb.dart`, driven through the `ProcessRunner`
seam (`arxa/lib/process.dart`) so tests use scripted fakes — no
...
- Produces: `arxa/lib/lens/native/adb.dart`;
  `arxa/test/lens_adb_test.dart`; `native adb` cases in
  `lens_cli.dart`'s `runLensNative`.

### Steps

- [x] 8.1 Write the failing test — `arxa/test/lens_adb_test.dart`:
...
- [x] 8.2 Create `arxa/lib/lens/native/adb.dart`:
  ...
  class LensAdb {
    LensAdb({String? serial, ProcessRunner? runner})
        : _serial = serial ?? Platform.environment['ARXA_ADB_SERIAL'],
...
- [x] 8.3 Add `native adb` cases to `runLensNative` in
  `arxa/lib/lens_cli.dart`: `shot <out.png> [--serial=]`, `record
...
`arxa/lib/lens/native/simctl.dart`. All `idb`-dependent verbs are
...
- Produces: `arxa/lib/lens/native/simctl.dart`;
  `arxa/test/lens_simctl_test.dart`; `native ios` cases in
  `runLensNative`.

### Steps

- [x] 9.1 Write the failing test — `arxa/test/lens_simctl_test.dart`
  ...
  Run — expected: COMPILE ERROR. Red confirmed.
- [x] 9.2 Create `arxa/lib/lens/native/simctl.dart` — same class shape
  as `LensAdb`:
  ```dart
...
CLI compiled on demand, plus `arxa/lib/lens/native/sck.dart` and
`arxa/lib/lens/ocr.dart` Dart wrappers. `vision_probe` (face/body/
...
- Produces: `arxa/tool/native/lens_macos.swift` (single-file CLI) (landed
  as `arxa/native/lens_native.swift`; binary `lens_native`,
  `ensureLensNativeBinary()`);
  `arxa/lib/lens/native/sck.dart`; `arxa/lib/lens/ocr.dart`;
  `arxa/test/lens_sck_test.dart`, `lens_ocr_test.dart`; `native macos
...
- [x] 10.1 Write the failing tests:
  - `lens_sck_test.dart` — compile-on-demand: `ensureLensMacosBinary()`
    ...
    `~/.arxa/bin/lens_macos` (or `.dart_tool/` — pick
    `.dart_tool/arxa/lens_macos`, repo-local, gitignored) is missing
  ...
  - `lens_ocr_test.dart` — `ocrText(pngPath)` spawns `<bin> ocr <png>`,
    ...
    returns canned JSON.
  Run — expected: COMPILE ERROR. Red confirmed.
- [x] 10.2 Create `arxa/tool/native/lens_macos.swift` — complete CLI
  (macOS 14+; ~120 lines):
  ```swift
  // lens_macos — arxa lens macOS helper: ScreenCaptureKit one-shot
  ...
  ```
- [x] 10.3 Create `arxa/lib/lens/native/sck.dart`:
  `Future<String> ensureLensMacosBinary({ProcessRunner? runner})` —
  compile-on-demand with mtime check into
  `.dart_tool/arxa/lens_macos`; `class LensSck { LensSck({ProcessRunner?
...
- [x] 10.4 Wire CLI: `arxa lens native macos shot <out.png>
  [--window-id=N]`; `arxa lens ocr <image.png> [--out=path]`;
  `arxa lens text-diff <a.png> <b.png>` (exit 1 when text differs).
...
`arxa/lib/lens/native/flutter_vm.dart`: attach/URL discovery, generic
...
- Consumes: nothing from earlier tasks (pure transport; may run parallel
  to Tasks 8-10).
- Produces: `arxa/lib/lens/native/flutter_vm.dart`;
  `arxa/test/lens_flutter_vm_test.dart` (fake WebSocket server via
  `dart:io`); `native flutter` cases in `runLensNative`.

### Steps

- [x] 11.1 Write the failing test —
  `arxa/test/lens_flutter_vm_test.dart`: stand up a real
  ...
  {"wsUri":"ws://…/ws"}}`).
  Run — expected: COMPILE ERROR. Red confirmed.
- [x] 11.2 Create `arxa/lib/lens/native/flutter_vm.dart` — the RPC
...
- [x] 11.3 Add `native flutter` cases to `runLensNative`: `shot <out.png>
  [--uri=]` (uri or `$ARXA_FLUTTER_VM`, else read
...
first-class gate in the framework of `arxa/lib/gates.dart`.

**Interfaces**
- Consumes: Tasks 1 (pixels), 3 (tokens), 5 (skeleton), 7 (CLI);
  `gates.dart` framework; `phases.dart`/`gate_runner.dart` wiring points.
- Produces: `arxa/lib/gate_lens.dart` (`lensGate(GateContext ctx,
  {bool recaptureGoldens = false})`); wiring in `gate_runner.dart`
  (`gateOrder` + `_tryDartGate` case), `bin/arxa.dart`
  (`_dispatchGate`), `phases.dart` (`phaseGates`);
  `arxa/test/gate_lens_test.dart`; config schema note.

### Steps

- [x] 12.1 Write the failing test — `arxa/test/gate_lens_test.dart`
  ...
  `config/arxa.config.json` carrying a `lens` block:
  ```json
  {
    "targets": ["web"],
    "viewports": {"mobile": {"width": 390, "height": 844}},
    "lens": {
      "goldens": "designs/arxa-studio/goldens",
      "surfaces": {"home": "/"},
  ...
  flow).
  Run — expected: COMPILE ERROR. Red confirmed.
- [x] 12.2 Create `arxa/lib/gate_lens.dart`. Contract (match
...
- [x] 12.3 Wire it: add `'lens'` to `gateOrder` in
  `arxa/lib/gate_runner.dart` after `'native_deps'` (before
  `'deploy'`); add the case in `_tryDartGate`; add `case 'lens': return
  lensGate(ctx);` in `_dispatchGate` (`bin/arxa.dart`); add `lens` to
  the usage gate list (`bin/arxa.dart:132`); in
  `arxa/lib/phases.dart` change the `build` entry to `[native_deps,
  lens]`. Add the selftest path: `arxa gate lens --self-test` builds
...
- [x] 12.4 Run `dart test test/gate_lens_test.dart` — pass; `dart test`
  full suite green; `dart run bin/arxa.dart gate lens --self-test`
...
Rewrite `skills/arxa-lens/capability-map.md` so every probe-runner
verb is **ported** (with its new name) or **dropped with reason**, and
refresh `skills/arxa-lens/SKILL.md` to the `arxa lens` CLI. This is
the plan's scoreboard.

**Interfaces**
- Consumes: Tasks 1-12 (everything the map points at).
- Produces: rewritten `skills/arxa-lens/capability-map.md` +
  `skills/arxa-lens/SKILL.md` (+ mirrors); `docs/VOCABULARY.md` lens
  entry refreshed if it names verbs.

### Steps

- [x] 13.1 Rewrite `skills/arxa-lens/capability-map.md` with three
  ...
  doctrine — one render engine, the one arxa gates on);
  `content_firewall/slots/bundle_writer/motion_adapter/site_chrome`
  (ADR-0001 extraction-pipeline machinery — arxa authors designs,
  ...
  `verbs.json` (tool-routing manifest for a retired tool — the `arxa`
  ...
  arxa's ephemeral-server tests); `templates/dioxus_debug_probe.rs`
  + `dom.py/console.py` (Dioxus/wry — arxa doesn't ship Dioxus);
...
- [x] 13.2 Update `skills/arxa-lens/SKILL.md`: replace the two-entry-
  points section with the `arxa lens` verb table (keep the
...
- [x] 13.3 Mirror:
  ```sh
  cd /Volumes/developer_ssd/Developer/totem_labs/arxa
  cp skills/arxa-lens/capability-map.md .kimi-code/skills/arxa-lens/capability-map.md
  cp skills/arxa-lens/SKILL.md .kimi-code/skills/arxa-lens/SKILL.md
  ```
- [x] 13.4 Audit pass — every script name in
  ...
  for f in archives/tooling-pre-dart/tools/vendor/probe-runner/scripts/*.py; do
    b=$(basename "$f" .py)
    grep -q "$b" skills/arxa-lens/capability-map.md || echo "MISSING: $b"
  done
  ```
...
## Task 14: Dartify story-map generator (`arxa emit story-map`)

Ports `skills/arxa-story-mapper/scripts/generate_story_map.py` (495
lines, pure stdlib) to `arxa/lib/story_map.dart`. Golden rule
...
- Consumes: the archived-at-Task-23 Python script (still live during
  this task); `bin/arxa.dart` `_runEmit` pattern.
- Produces: `arxa/lib/story_map.dart`; `arxa emit story-map`
  dispatch; `arxa/test/story_map_test.dart` + golden fixture
  `arxa/test/fixtures/story_map_sample.json`.
...
- [x] 14.1 Capture the golden baseline from the Python original:
  ```sh
  cd /Volumes/developer_ssd/Developer/totem_labs/arxa
  python3 skills/arxa-story-mapper/scripts/generate_story_map.py --self-test
  mkdir -p arxa/test/fixtures
  # reuse the self-test's sample shape; write arxa/test/fixtures/story_map_sample.json
  # (project + 2 releases + 2 epics incl. one CJK-named feature and one all-wont feature)
  python3 skills/arxa-story-mapper/scripts/generate_story_map.py \
    -i arxa/test/fixtures/story_map_sample.json \
    -o arxa/test/fixtures/story_map_sample.html \
    --data-out arxa/test/fixtures/story_map_sample.data.json \
    --brief-out arxa/test/fixtures/story_map_sample.brief.md
  ```
...
- [x] 14.2 Write the failing test — `arxa/test/story_map_test.dart`:
  ...
  Run — expected: COMPILE ERROR. Red confirmed.
- [x] 14.3 Create `arxa/lib/story_map.dart`: `class StoryMap {
...
- [x] 14.4 Wire `arxa emit story-map` into `_runEmit`
  (`bin/arxa.dart`): flags `-i/--input` (or stdin), `-o/--output`,
...
## Task 15: Dartify the scaffolder emitter (`arxa emit scaffold`)

Ports `skills/arxa-scaffolder/scaffold.py` (860 lines, pure stdlib) to
`arxa/lib/scaffold.dart`. Note the split that already exists:
`arxa/lib/gate_scaffold.dart` is the *coverage gate*; this task ports
the *file emitter*. Golden: the Python `--self-test`'s ~50 assertions
become the Dart test suite, plus a differential run on
`designs/arxa-studio/structure.json`.

**Interfaces**
- Consumes: `pipeline/state/targets.derivation.json` +
  `config/arxa.config.json` (target→form-factor derivation — the same
  inputs the Python reads); `bin/arxa.dart` `_runEmit`.
- Produces: `arxa/lib/scaffold.dart`; `arxa emit scaffold` +
  `--check`; `arxa/test/scaffold_test.dart`.

### Steps

- [x] 15.1 Write the failing test — `arxa/test/scaffold_test.dart`:
  ...
  name).
  Run — expected: COMPILE ERROR. Red confirmed.
- [x] 15.2 Create `arxa/lib/scaffold.dart`: `class ScaffoldEmitter {
  ...
  resolution with cycle guard, `config/arxa.config.json` viewports
...
- [x] 15.3 Wire `arxa emit scaffold --design-dir <d> --app-root <a>
  --targets <t1,t2> [--check] [--self-test]` into `_runEmit` (env
  fallbacks `KIT_DESIGN_DIR`/`ARXA_APP`/`ARXA_TARGETS` preserved;
...
- [x] 15.4 Differential run against the real design (read-only check
  ...
  ```sh
  cd /Volumes/developer_ssd/Developer/totem_labs/arxa
  python3 skills/arxa-scaffolder/scaffold.py --design-dir designs/arxa-studio \
    --app-root arxa-studio --targets macos --check
  cd arxa && dart run bin/arxa.dart emit scaffold \
    --design-dir designs/arxa-studio --app-root ../arxa-studio \
    --targets macos --check
  ```
...
## Task 16: Dartify the deployer (`arxa deploy`)

Ports `skills/arxa-deployer/deploy.py` (486 lines, pure stdlib) to
`arxa/lib/deploy.dart`, reusing the existing
...
**Interfaces**
- Consumes: `lib/process.dart`; `bin/arxa.dart` dispatch pattern.
- Produces: `arxa/lib/deploy.dart`; `arxa deploy doctor|deploy`;
  `arxa/test/deploy_test.dart`.

### Steps

- [x] 16.1 Write the failing test — `arxa/test/deploy_test.dart`:
  ...
  Run — expected: COMPILE ERROR. Red confirmed.
- [x] 16.2 Create `arxa/lib/deploy.dart`: `class Deployer {
  ...
  `$ARXA_DEPLOY_LEDGER`, `DeployHalted` exception. Port
  function-for-function; do not add targets.
- [x] 16.3 Wire top-level `arxa deploy doctor [--config f]` /
  `arxa deploy deploy --target T --version V --account A --approval
  <tok> [--ledger f]` in `bin/arxa.dart` (new `case 'deploy':`,
  exit codes 0 ok / 1 halt / 2 usage; HALT lines to stderr). Note in
  the usage text that `arxa gate deploy` remains the gate.
- [x] 16.4 Run tests — pass; full suite green.

---

## Task 17: Fold KB lint into `arxa docs`; retire lint_kb.py

`skills/arxa-lint/lint_kb.py` (121 lines) overlaps the already-Dart
`arxa/lib/validate_docs.dart` (docs/INDEX.md dead-link + unindexed
validator). The lazy port: extend `validate_docs.dart` with the three
checks it lacks, and retire the script as superseded.

**Interfaces**
- Consumes: `arxa/lib/validate_docs.dart` and its test.
- Produces: extended `validate_docs.dart` + `validate_docs_test.dart`;
  updated `skills/arxa-lint/SKILL.md` (+ mirror). No new module, no
  new CLI. (Landed as a new module after all: `arxa/lib/docs_lint.dart`
  + `docs_lint_test.dart`, wired into `arxa docs`.)

### Steps

- [x] 17.1 Diff the check lists and write failing tests for the gaps in
  `arxa/test/validate_docs_test.dart` (synthetic fixture tempdir,
...
- [x] 17.2 Extend `arxa/lib/validate_docs.dart` with the three
  checks, keeping the existing function shape and output format (`WARN
  …` / `ERROR …` + summary line). Run tests — pass; full suite green.
- [x] 17.3 Update `skills/arxa-lint/SKILL.md`: replace the `python
  skills/arxa-lint/lint_kb.py` invocation lines (and fix the stale
  `skills/lint/` path on line 53) with `arxa docs` (or `dart run
  arxa/bin/arxa.dart docs` from repo root — match the invocation
  style neighboring skills use for arxa), noting lint_kb.py is
  archived and its checks merged into the Dart validator. Mirror:
  `cp skills/arxa-lint/SKILL.md .kimi-code/skills/arxa-lint/SKILL.md`.

---

## Task 18: Port the intake engine (`arxa intake`)

**Briefing gap found during planning:** `arxa/lib/intake.dart` is NOT
a port — its own header says it "drives the elicitation engine
(`skills/arxa-intake/intake.py`) headless" by shelling out. The 669-
...
**Interfaces**
- Consumes: `skills/arxa-intake/intake.py` +
  `skills/arxa-intake/intake.schema.json` (the provenance schema);
  `bin/arxa.dart` dispatch.
- Produces: rewritten `arxa/lib/intake.dart` (`IntakeEngine` —
  validate/emit/seed, pure Dart; the old `IntakeRunner` shell-out
  class deleted and its callers repointed); `arxa intake
  emit|seed|validate|--self-test`; `arxa/test/intake_test.dart`.

### Steps

- [x] 18.1 Find `IntakeRunner`'s callers before touching it:
  `grep -rn 'IntakeRunner' arxa/ skills/ .kimi-code/skills/`.
  Repoint every caller to the new in-process API in this task.
- [x] 18.2 Write the failing test — `arxa/test/intake_test.dart`:
  ...
  Run — expected: COMPILE ERROR. Red confirmed.
- [x] 18.3 Rewrite `arxa/lib/intake.dart`: `class IntakeEngine {
  ...
  gate, already Dart).
- [x] 18.4 Wire `arxa intake emit --answers <f> [--brief-out p]
  [--registry-out p]` / `seed --brief <f>` / `validate <f>` /
  `--self-test` into `bin/arxa.dart` (new `case 'intake':` — check
  for collision with `gate intake`: the gate is `arxa gate intake`,
  no collision).
- [x] 18.5 Update `skills/arxa-intake/SKILL.md:87,91,101,117` and
  `skills/arxa-story-mapper/SKILL.md`'s intake reference to the Dart
  invocation; mirror both files. Run tests — pass; full suite green.

---

## Task 19: Dartify the designer's small runtimes (`arxa design …`)

Ports the designer runtime's zero/low-dependency scripts to
`arxa/lib/design_tools.dart` behind a new `arxa design <verb>`
dispatch: `lint`, `check-ladder`, `check-wiring`, `pseudolocalize`,
`vendor-fetch`, `doctor`. `shoot.mjs` and `console-check.mjs` are NOT
ported — they are superseded by `arxa lens shoot` / `arxa lens
...
- Consumes: Task 7's `lens shoot`/`lens check` (doc replacements);
  `bin/arxa.dart` dispatch.
- Produces: `arxa/lib/design_tools.dart`; `arxa design` dispatch;
  `arxa/test/design_tools_test.dart`.

### Steps

- [x] 19.1 Write the failing test —
  `arxa/test/design_tools_test.dart`. Fixture: a temp artifact dir
...
- [x] 19.2 Capture the pseudolocalize golden from the .mjs original:
  ...
  node skills/arxa-designer/runtime/pseudolocalize.mjs arxa/test/fixtures/ploc_artifact
  cp arxa/test/fixtures/ploc_artifact/l10n/app_qps-ploc.arb arxa/test/fixtures/ploc_golden.arb
  git checkout -- arxa/test/fixtures/ploc_artifact 2>/dev/null || true
  ```
- [x] 19.3 Create `arxa/lib/design_tools.dart` — exemplar, the lint
  port (complete):
  ```dart
  ...
  /// arxa design lint <artifact-dir> — exit 0 clean / 1 findings / 2 usage.
  int designLint(List<String> args) {
    if (args.isEmpty) {
      stderr.writeln('usage: arxa design lint <artifact-dir>');
      return 2;
    }
  ...
  - `pseudolocalize <artifact-dir>`: the fixed lookalike table (copy it
    ...
    `arxa/lib/design_server/l10n.dart` in THIS task with the
  ...
  - `vendor-fetch`: allowlisted pins from vendor/fetch.mjs (htmx.org
    ...
    `crypto`? — arxa has no crypto package; compute SHA-384 via
  ...
  - `doctor`: post-port semantics — checks the DART toolchain (`dart
    ...
    22).
- [x] 19.4 Wire `arxa design <verb>` into `bin/arxa.dart` (new
  `case 'design':` → `runDesign(rest)` in
  `arxa/lib/design_cli.dart` — sibling to `lens_cli.dart`; `serve`
  prints `lands in Task 20` + exit 2 for now). Run tests — pass; full
  suite green.

---

## Task 20: The Dart design server (`arxa design serve`) + eject
...
`lib/timers.mjs` (~810 lines total) → `arxa/lib/design_server.dart` +
`arxa/lib/design_server/` subtree. Golden spec: `serve.test.mjs`'s 15
...
`skills/arxa-designer/runtime/vendor/` like every other island. (Landed
in `arxa/lib/design_server/worker_assets/` instead — server-side worker
...
- Consumes: `cdp.dart` (worker transport); Task 19's l10n `parsePlural`
  ...
  (the behavior spec).
- Produces: `arxa/lib/design_server.dart` (process: target
  resolution, ports, supervisor, hot reload, pidfile registry, state
  restore, exit codes); `arxa/lib/design_server/worker.dart` (CDP
  bridge); `arxa/lib/design_server/router.dart` (route-table match);
  `arxa/lib/design_server/state.dart` (sessions/prefs/timers);
  `arxa/lib/design_server/l10n.dart` (started in Task 19);
  `skills/arxa-designer/runtime/vendor/worker_page.html` +
  `worker_shim.js` + `nunjucks-slim.min.js` (SRI-pinned) (+ mirrors);
  `arxa/test/design_server_test.dart`; `arxa design serve` wired;
  `arxa design eject` (port of eject.mjs).

### Steps

- [x] 20.1 **Worker spike first** (the risk gate; ~200 lines, throwaway
  if it fails): `arxa/tool/_worker_spike.dart` — boot the Dart
  static-file `_bindStaticServer`-pattern server over
  `designs/arxa-studio`, launch `CdpClient`, load a page that
...
- [x] 20.2 Vendor the worker assets (SRI-pinned, the vendor/ contract) —
  landed in `arxa/lib/design_server/worker_assets/` with the FULL
...
- [x] 20.3 Write the failing test —
  `arxa/test/design_server_test.dart`: serve.test.mjs's 15 behaviors,
  verbatim semantics:
  1. bare design name `arxa-studio` resolves (walk-up +
  ...
  14. kill semantics: SIGTERM stops one instance; SIGINT stops every
     ...
     serve marker" check to look for `arxa`/`design_server` instead
     of `serve.mjs`);
  15. pidfiles swept from the registry dir after exit.
  Drive the server as a spawned `dart run bin/arxa.dart design serve
...
- [x] 20.4 Create the Dart server. `design_server.dart` public entry:
  ```dart
  /// The arxa design server (ports skills/arxa-designer/runtime/serve.mjs
  ...
  - **Router** (`design_server/router.dart`) — at boot, ask the worker
    ...
    Hono's `:param` segments only — no wildcards in arxa routes;
  ...
  - **State** (`design_server/state.dart`) — port lib/state.mjs +
    ...
    `${tmp}/arxa-serve-state-<sha1(artifactDir)[:12]>.json` on
    SIGINT/SIGTERM (SHA-1: arxa has SHA-256 in crypto_aead — a 12-
  ...
  - **Supervisor** — port serve.mjs's two modes: `--worker`/`--no-watch`
    ...
    bin/arxa.dart, design, serve, dir, --worker, --port, P, --json])`),
    ...
    `${tmp}/arxa-designer-serve/<pid>.json` with the SIGINT
...
- [x] 20.5 Port `eject.mjs` → `arxa design eject <artifact-dir>
  ...
  `arxa design serve . --no-watch` invocation line — ejected
...
- [x] 20.6 Run the 15-behavior test — all pass; full suite green. Then
  ...
  ```sh
  cd /Volumes/developer_ssd/Developer/totem_labs/arxa/arxa
  dart run bin/arxa.dart design serve arxa-studio --port 4399 &
  sleep 6
  ...
  `arxa/test/fixtures/serve_golden_dashboard.png`).

---

## Task 21: Dartify the designer selftest (`arxa design selftest`)

Ports `skills/arxa-designer/selftest.sh` (468 lines bash, ~25 labeled
...
- Produces: `arxa/lib/design_selftest.dart`; `arxa design selftest
  [artifact-dir] [--negative]`; `arxa/test/design_selftest_test.dart`.

### Steps

- [x] 21.1 Write the failing test —
  `arxa/test/design_selftest_test.dart`: run the selftest against a
  ...
  `--negative` mode below.
  Run — expected: COMPILE ERROR. Red confirmed.
- [x] 21.2 Create `arxa/lib/design_selftest.dart`: port the ~25
...
- [x] 21.3 Wire `arxa design selftest [artifact-dir] [--negative]`
  into `design_cli.dart`. Run against the real skill:
  ```sh
  cd /Volumes/developer_ssd/Developer/totem_labs/arxa/arxa
  dart run bin/arxa.dart design selftest ../skills/arxa-designer/examples/hello-hda
  ```
...
Dispositions for `skills/arxa-designer/agents/*.mjs` (found during
...
- [x] 22.1 Dispositions for `skills/arxa-designer/agents/` — verify
  each claim, then record in a new **`## Skills runtimes`** section
  appended to `skills/arxa-lens/capability-map.md` (created in this
  ...
  - `import-figma.mjs` — **dropped with reason**: statically imports
    ...
    `node skills/arxa-designer/agents/import-figma.mjs 2>&1 | head -3`
  ...
  - `check-design-system.mjs` (110), `record-asset.mjs` (187),
    ...
    `arxa/lib/design_tools.dart` — `dsCheck(projectDir)` (read-only
    ...
    `design_tools_test.dart` with fixture dirs. Wire as `arxa design
    ds-check|record-asset|ds-import`.
- [x] 22.2 Update every skill doc invocation line (enumerated —
  canonical `skills/` then mirror each):
  - `skills/arxa-story-mapper/SKILL.md:170,177,262` → `arxa emit
    story-map -i … -o … --data-out … --brief-out …` and `--self-test`.
  - `skills/arxa-scaffolder/SKILL.md:84,94,109` → `arxa emit
    scaffold --design-dir … --app-root … --targets … [--check]
    [--self-test]`.
  - `skills/arxa-deployer/SKILL.md:83-84` → `arxa deploy doctor` /
    `arxa deploy deploy …` / `--self-test`.
  - `skills/arxa-designer/SKILL.md:97-103,110,135` → serve/lint/
    console-check/shoot/eject/doctor lines become `arxa design
    serve|lint|eject|doctor` and `arxa lens check|shoot`;
    pseudolocalize line → `arxa design pseudolocalize`.
  - `skills/arxa-designer/system-prompt.md`,
  ...
  - `skills/arxa-intake/SKILL.md` (done in Task 18.5 — verify).
- [x] 22.3 Mirror every file touched:
  ```sh
  cd /Volumes/developer_ssd/Developer/totem_labs/arxa
  for f in arxa-story-mapper/SKILL.md arxa-scaffolder/SKILL.md \
           arxa-deployer/SKILL.md arxa-designer/SKILL.md \
           arxa-designer/system-prompt.md; do
...
and rewrites `arxa-lens`'s "extend, don't route around" doc lines that
...
- [x] 23.1 Move (preserve relative paths):
  ...
  mkdir -p $A/arxa-scaffolder $A/arxa-deployer $A/arxa-lint \
           $A/arxa-story-mapper/scripts $A/arxa-intake \
           $A/arxa-designer/runtime/vendor $A/arxa-designer/agents
  git mv skills/arxa-scaffolder/scaffold.py $A/arxa-scaffolder/
  git mv skills/arxa-story-mapper/scripts/generate_story_map.py $A/arxa-story-mapper/scripts/
  git mv skills/arxa-deployer/deploy.py $A/arxa-deployer/
  git mv skills/arxa-lint/lint_kb.py $A/arxa-lint/
  git mv skills/arxa-intake/intake.py $A/arxa-intake/
  git mv skills/arxa-designer/selftest.sh $A/arxa-designer/
  git mv skills/arxa-designer/runtime/serve.mjs skills/arxa-designer/runtime/serve.test.mjs \
         skills/arxa-designer/runtime/lint.mjs skills/arxa-designer/runtime/doctor.mjs \
         skills/arxa-designer/runtime/shoot.mjs skills/arxa-designer/runtime/console-check.mjs \
         skills/arxa-designer/runtime/pseudolocalize.mjs skills/arxa-designer/runtime/eject.mjs \
         skills/arxa-designer/runtime/check_ladder.mjs skills/arxa-designer/runtime/check_wiring.mjs \
         skills/arxa-designer/runtime/verify-interact.mjs skills/arxa-designer/runtime/verify-shots.mjs \
         skills/arxa-designer/runtime/verify-timeline.mjs $A/arxa-designer/runtime/
  git mv skills/arxa-designer/runtime/vendor/fetch.mjs $A/arxa-designer/runtime/vendor/
  git mv skills/arxa-designer/runtime/lib $A/arxa-designer/runtime/lib
  git mv skills/arxa-designer/agents $A/arxa-designer/agents
  ```
  ...
  `mv .kimi-code/skills/arxa-designer/runtime/_*.mjs
  $A/arxa-designer/runtime/` (untracked debug one-offs — archived,
  not deleted).
  And the two per-artifact example scripts (they are node runtimes
  inside `skills/` — the end-state rule covers them):
  ```sh
  git mv skills/arxa-designer/examples/hello-hda/serve.mjs \
    $A/arxa-designer/examples-hello-hda-serve.mjs
  git mv skills/arxa-designer/examples/hello-hda/models/greeting_model/generate.mjs \
    $A/arxa-designer/examples-hello-hda-generate.mjs
  ```
  Replace both with a 5-line note in
  `skills/arxa-designer/examples/hello-hda/README.md` (create if
  absent): serving is `arxa design serve <dir>` (Task 20); the
...
- [x] 24.1 Full arxa suite + analyzer:
  ```sh
  cd /Volumes/developer_ssd/Developer/totem_labs/arxa/arxa
...
- [x] 24.2 Gate wiring smoke:
  ```sh
  cd /Volumes/developer_ssd/Developer/totem_labs/arxa/arxa
  dart run bin/arxa.dart gate lens --self-test
  dart run bin/arxa.dart lens --help 2>&1 | head -5 || true
  dart run bin/arxa.dart design 2>&1 | head -3 || true
  ```
...
- [x] 24.3 End-to-end lens smoke at the ladder against the Dart design
  ...
  for r in 390:844 744:1133 1280:832; do
    w=${r%%:*}; h=${r##*:}
    dart run bin/arxa.dart lens shot http://localhost:4399/dashboard \
      ../designs/arxa-studio/evidence/lens-full-port/dashboard-$w.png $w $h 2500
  done
  dart run bin/arxa.dart lens tokens http://localhost:4399/dashboard 390 844 \
    --out=../designs/arxa-studio/evidence/lens-full-port/tokens-390.json
  kill %1
  ```
...
- `web_states` consent/keyframes sub-cores, `site_chrome` chrome-dedup —
  extraction-pipeline machinery; archive remains the source if arxa
  ever extracts third-party designs.
- `dart compile exe bin/arxa.dart` distribution (the header comment's
