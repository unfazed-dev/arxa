...
1. `designs/appbox-studio/ui/views/main_shell/shared/widgets/design_viewer.html`
   — filmstrip macro: anchor → `#dvt-views--<id>`, drop hx-* attrs, rewrite
   the macro comment.
2. `skills/appbox-designer/runtime/vendor/canvas.js` — strip-sync: rAF scroll
...
3. `designs/appbox-studio/assets/css/viewer.css` + `app.css` — `.dv-tile.on`
   accent outline beside the existing `.dv-thumb.on` rules (both files carry
   the pair deliberately).
4. `skills/appbox-designer/docs/adr/0002-zero-custom-client-js-boundary.md`
...
Serve appbox-studio on a spare port (read-only GETs + view-state JS only;
never the user's 4319 / `~/.appbox/current`). Lens `--expect` asserts:
