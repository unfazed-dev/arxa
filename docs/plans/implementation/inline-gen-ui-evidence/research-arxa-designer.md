# arxa design pipeline — research for the arxa design panel

Repo: `/Volumes/developer_ssd/Developer/totem_labs/arxa` (read-only investigation, nothing modified).
...
## 1. `arxa design serve` — what it is

### Command chain

| stage | file:line |
|---|---|
| top-level CLI verb `design` | `arxa/bin/arxa.dart:88` |
| `design` subcommand dispatcher `designMain` | `arxa/lib/design_cli.dart:78` |
| `case 'serve'` | `arxa/lib/design_cli.dart:103` |
| server implementation | `arxa/lib/design_server.dart` (1420 lines) |
| CDP worker bridge | `arxa/lib/design_server/worker.dart` (1382 lines) |

Invocation used throughout the repo:
`dart run arxa/bin/arxa.dart design serve <artifact-dir|design-name> [--port N] [--host H] [--json] [--no-watch]`
(example at `arxa/lib/probes/probe_base.dart:217`).

### Port

- Default **4319**, overridable by the `PORT` env var, then `--port`:
  `arxa/lib/design_server.dart:151-153` (`Platform.environment['PORT'] != null ? … : 4319`).
...
- Binds **127.0.0.1** by default; `--host 0.0.0.0` exists for phone previews and announces
  itself on stdout (`skills/arxa-designer/runtime/README.md`, Commands section).
- The *ejected* Hono app defaults to **4399**, not 4319 — different thing, don't confuse
  them (`arxa/lib/design_cli.dart` usage block; `arxa/test/design_tools_test.dart:1379`).
...
```json
{"url":"http://127.0.0.1:52953/","port":52953,"host":"127.0.0.1","pid":40311,"artifact":"/…/designs/arxa-app"}
```

Built at `arxa/lib/design_server.dart:1041-1047` — note it registers and reports the
...
`serve` accepts a **path or a bare design name** — `arxa-app` resolves to
`designs/arxa-app`.
...
(`arxa/test/serve_scope_test.dart:1-5`). Scoping is now artifact + port
...
- The artifact's JS (`app.routes.js`, `*_viewmodel.js`, `*.tsx` views) executes in a
  **headless-Chrome tab driven over CDP** — the "worker".
  Header comment: `arxa/lib/design_server.dart:1-19`; worker rationale:
  `arxa/lib/design_server/worker.dart:1-14`.
- History: this is a port of the retired Node runtime `skills/arxa-designer/runtime/serve.mjs`
  (archived under `archives/tooling-pre-dart/`). Stated at
  `skills/arxa-designer/runtime/README.md` ("The server is the Dart design server now",
...
All in `arxa/lib/design_server.dart`:
...
From `arxa/lib/design_cli.dart:89-117` and the usage block at `:42-77`:
...
- Absent from the `switch` in `arxa/lib/design_cli.dart:89-117` and from the usage block.
- The only `patch` verbs in the repo are shorebird OTA (`arxa/lib/deploy.dart:207`, `:569`).
...
- No `text/event-stream`, `EventSource`, `WebSocketTransformer`, or HTTP upgrade handling
  anywhere in `arxa/lib` outside `cdp.dart` and `lens/`, which use WebSocket to talk to
  **Chrome**, not to a design-panel client. The only `WebSocketException` reference is
  ...
  path — `gateway.dart:216` does do SSE, but that is the `arxa serve` LLM gateway, a
  different process and verb entirely; see §4.)
- No `EventSource` / `WebSocket` / `location.reload` / `setInterval` in any worker asset:
  `arxa/lib/design_server/worker_assets/{islands_eager.js,worker_shim.js,worker_page.html,fs_shim.js}`.
- No `hx-ext` / `sse-connect` / `ws-connect` / `hx-trigger="every …"` in any artifact
  (`designs/`, `kit/`, `skills/arxa-designer/examples/`).
- **One nuance worth knowing:** `hx-sse.min.js` and `sse.js` **are vendored** in
  `skills/arxa-designer/runtime/vendor/`. They are unused, and the artifacts' own
  `base.tsx` records why: *"support extensions are gone — hx-ext does not exist in v4"*
  (`designs/arxa-studio/ui/common/base.tsx:79`, same line in `arxa-studio-v2`). So an
...
**`skills/arxa-designer/runtime/ladder.json`** is the runtime contract:
...
Doctrine doc: `skills/arxa-designer/references/viewport-ladder.md` — same table, and it
...
`arxa design check-ladder` (pure core `checkLadderFromInputs` at
`arxa/lib/design_tools.dart:560-613`, CLI wrapper `designCheckLadder` at `:616-645`)
...
`config/arxa.config.json:4-8` carries a *different* map under a *different* vocabulary:

```json
"viewports": { "mobile": {390,844}, "tablet": {744,1133}, "desktop": {1280,800} }
```

**`ladder.json` wins.** `check-ladder` never reads `config/arxa.config.json`, so this
...
(`arxa/lib/lens_cli.dart:825-829`: `_Rung('compact',390,844)`,
`_Rung('medium',744,1133)`, `_Rung('expanded',1280,832)`), the recorded evidence goldens
(`designs/arxa-studio/evidence/tsx-vs-master/README.md:18-29`), and the rungs the arxa
...
Resolution order (`arxa/lib/lens_cli.dart:831-865`):
`--rungs` flag > `$ARXA_LADDER` env > the artifact's `_d_meta.json` `.ladder` key >
...
`grep -n "queryParameters|'vp'|viewport|rung|width|height" arxa/lib/design_server.dart`
...
The same grep for `viewport|rung|'vp'` over `arxa/lib/design_server/worker.dart` returns
...
1. **The lens** — `arxa lens shoot <url> --rungs=compact,medium,expanded`
   (`arxa/lib/lens_cli.dart:35`, loop at `:933`) drives `CdpSession.setViewport` per
   rung and screenshots each. This is capture-time, one navigation per rung.
2. **The artifact's own viewer** — `designs/arxa-studio/ui/common/integration_viewer.md`
...
3. **Responsive view files** — a surface is authored as
   ...
   `main_shell_view.tsx` (see `skills/arxa-designer/examples/hello-hda/ui/views/main_shell/`).
...
`arxa-orchestrator` (Ø) → `arxa-story-mapper` / `arxa-moodboarder` (0, optional) →
`arxa-intake` (1) → **`arxa-designer` (2)** → `arxa-scaffolder` (3) →
`arxa-builder` (4) → `arxa-tester` (5) → `arxa-reviewer` (6) → `arxa-deployer` (9);
cross-cutting `arxa-lint` (7), `arxa-lens` (8), `arxa-cicd` (10).
FSM phases: `arxa/lib/phases.dart`. Map: `docs/arxa-system-map.md`,
...
(`skills/arxa-designer/SKILL.md`, "The output triad"; binding contract in
`skills/arxa-designer/DESIGN-ARCHITECTURE.md`).
...
`arxa/lib/design_server/worker.dart:1-14` and `:395-410`:

1. At boot and on every hot reload, Dart scans the artifact's `.tsx` view inventory.
2. `generateRenderTsx(artifactDir, {projectDir})` (`arxa/lib/design_tools.dart:2305`)
...
by an **agent using the `arxa-designer` skill** — an agent writing `.tsx`/`.js`/`.json`
...
1. **The loopback gateway** — `arxa/lib/gateway.dart`, mounted by
   `arxa/lib/server.dart:12-22` under `arxa serve` (a *different* verb,
   `arxa/bin/arxa.dart:115`). Serves `/llm/v1/chat/completions` (OpenAI shape) and
...
Every emit verb (`arxa emit story-map|scaffold|structure|htmx|playground|synthesize|blueprint|…`,
dispatched at `arxa/bin/arxa.dart:656-712`) is produce-whole-artifact-then-return.
...
**What exists** (`kit/genui_bridge/`, a Flutter/Dart package `arxa_kit_genui_bridge`):

| file | what |
|---|---|
| `lib/src/arxa_kit_chat_stream.dart` | `abstract class ArxaKitChatStream { Stream<String> complete(List<ArxaKitChatMessage>, {ArxaKitJsonSchema? schema}); }` — the one provider-agnostic abstraction |
| `lib/src/adapters/arxa_kit_openai_chat_stream.dart` | `POST {baseUrl}/v1/chat/completions`; also covers Ollama / llama.cpp / vLLM via `baseUrl` |
| `lib/src/adapters/arxa_kit_anthropic_chat_stream.dart` | `POST {baseUrl}/v1/messages` |
| `lib/src/arxa_kit_sse_transport.dart` | injectable SSE HTTP layer (how tests assert request shapes without network) |
| `lib/src/arxa_kit_a2ui_stream_parser.dart` | **incrementally parses LLM text chunks into UI messages** |
| `lib/src/arxa_kit_a2ui_message.dart` | A2UI envelope, protocol version pinned `v0.9` |
| `lib/src/arxa_kit_json_schema.dart` | bounded validator + `ArxaKitSchemaError` for repair loops |

The parser (`arxa_kit_a2ui_stream_parser.dart:44-70`) mirrors flutter/genui's
...
assemble**; JSONL whitespace dropped; prose surfaces as `ArxaKitA2uiTextEvent`; malformed
payloads become `ArxaKitA2uiErrorEvent` (emitted, never thrown) so a consumer can feed
`raw` + `error` into a repair loop. Tests cover chunk-boundary splits down to one byte at
a time (`kit/genui_bridge/test/arxa_kit_a2ui_stream_parser_test.dart:16-100`).

**What does NOT exist — the critical part:**

- `grep -rln "arxa_kit_genui_bridge" --include=pubspec.yaml` over the whole repo returns
  **only `kit/genui_bridge/pubspec.yaml` itself**. Zero dependents.
- `grep -rn "genui_bridge|GenuiBridge|ArxaKitA2ui|a2ui"` over `arxa/lib` and
  `arxa/bin` returns **nothing**. The engine does not import it.
...
5. **`arxa lens shoot --rungs=…`** (`lens_cli.dart:35`, `:933`) already does the
...
How it invokes the engine: `arxa design commission <app-dir>` first (§0 of the skill —
...
`arxa design serve` to preview, `arxa design lint` / `check-wiring` / `check-ladder`
...
### `skills/arxa-lens/` (cross-cutting stage 8)

Three files only: `SKILL.md`, `capability-map.md`, `LENS_playbook.mdx`. It is a **Dart
library first** — `arxa/lib/lens.dart` over `arxa/lib/cdp.dart`, with
`arxa/lib/lens/{a11y,crawl,daemon,dom,motion,net,ocr,pixels,skeleton,states,tokens}.dart`
and `arxa/lib/lens_cli.dart`.
...
Verb table (`skills/arxa-lens/SKILL.md`): `shot`/`compare` → `captureGolden`/`compareGolden(byte|pixel|ssim)`;
...
Standing doctrine in both skills: **for anything regarding arxa, arxa's own tools come
first**; if a verb is missing, extend `arxa/lib/lens.dart` / `cdp.dart` — never reach
...
| concern | file |
|---|---|
| serve entrypoint / HTTP / routing / watcher | `arxa/lib/design_server.dart` |
| CDP worker, esbuild bundling, dispatch, reload | `arxa/lib/design_server/worker.dart` |
| `design` subcommand dispatch | `arxa/lib/design_cli.dart` |
| `generateRenderTsx`, check-ladder, `_d_meta.json`, eject | `arxa/lib/design_tools.dart` |
| l10n / ARB parsing for the server | `arxa/lib/design_server/l10n.dart` |
| worker-side JS assets | `arxa/lib/design_server/worker_assets/` |
| rung ladder (authoritative) | `skills/arxa-designer/runtime/ladder.json` |
| rung doctrine | `skills/arxa-designer/references/viewport-ladder.md` |
| legacy viewport map (do not use for rungs) | `config/arxa.config.json:3-8` |
| per-rung capture | `arxa/lib/lens_cli.dart:813-975` |
| LLM gateway (separate `arxa serve` verb) | `arxa/lib/gateway.dart`, `arxa/lib/server.dart` |
...
| server behavior contract (15 behaviors) | `arxa/test/design_server_test.dart` |
| multi-server / pidfile scoping contract | `arxa/test/serve_scope_test.dart` |
