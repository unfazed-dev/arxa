...
- [ ] **13.1** Add `verification` to every provider entry in the vendored kit
      registry: `stub | port-tested | sim-verified | device-verified`.
- [ ] **13.2** Add the **advertise gate**: no provider may be offered by any
      surface above its recorded tier. One assertion, applied forever.
- [ ] **13.3** Write the rule into the gate itself:
...
- [ ] **13.4** Implement **Tier 1** for payments and auth — a process/SDK port
      plus a scripted fake, mirroring the deploy kit's runner. Runs in CI with
      no toolchain, no credentials, no device.
- [ ] **13.5** Set up **Tier 2** runs. Record what the tier cannot prove, in the
...
- [ ] **13.6** Define **Tier 3** as device-only and mandatory before
...
- [ ] **13.7** Implement `SeedAuthBackend` **first** among the stubs. It needs
      no device and no accounts, and it unblocks the seeded-data story the
      product promises.
- [ ] **13.8** Sequence the rest: ports for payments and auth → Stripe to
      Tier 3 (app_box's own licensing needs it) → auth providers to Tier 3 →
