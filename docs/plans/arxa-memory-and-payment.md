# arxa memory + payment — decisions record
...
`arxa-engine-llm-fabric.md` (engine). Decisions were grilled out with the
...
> `watermark.dart` (still in `arxa/lib/` as of 2026-08-05) is to be
> retired. M1/M2 (the memory module) stand as built.

## M1 — arxa-memory: dedicated arxa module

A **dedicated memory module inside arxa** owns memory read/write/consolidation
across three layers. arxa manages its own memory — it does not depend on
...
- **Coverage mandate (operator, explicit):** logs for **every gate, event, and
  ...
  user and the system. If arxa manages all of it, that satisfies the mandate.
- **Storage, split by volatility:**
  - Raw events, scorecards (`pipeline/state/scorecard.jsonl`,
    `pipeline/state/usage.jsonl`), response cache → arxa data dir (JSONL,
...
- **Encrypt arxa's own sensitive state at rest:** LLM-key vault, licence
...
by arxa. No activation server, no machine limits (unenforceable offline).
...
Cursor-credit trap). An optional hosted tier ("arxa Cloud") for teams is a
