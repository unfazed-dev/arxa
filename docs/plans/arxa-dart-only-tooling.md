# arxa dart-only tooling — retirement of Python/bash/Node

> **Note (2026-07):** probe-runner is superseded by the arxa lens (`arxa/lib/lens.dart`, skill `skills/arxa-lens`); probe-runner mentions below are historical.

**Status:** **COMPLETE**. All Python/bash/Node tooling retired to
`archives/tooling-pre-dart/`. One Dart binary (`arxa`) handles gates,
emitters, lens, and serve. stacked_kit copied in-repo as `kit/`
(24 packages, 511 .dart files, all renamed `arxa_kit_*`). Sweep rename
complete — zero stale `arxa`/`arxa`/`KIT_APP`/`stacked_kit` references
in source. 408/408 arxa tests pass. Flutter analyze clean. Catalogs
...
beats Rust for those here: team language, existing daemon (`arxa/`), no
...
arxa could emit Rust apps: Dioxus/Leptos/Tauri"). A grill session with
...
- Emitter language is orthogonal to emission targets (OpenAPI Generator:
  Java emitting idiomatic Rust/Dart/50+). A Dart arxa can emit Rust
...
2. **Scope — full purity, upstreams absorbed.** `flutter-crew` is retired
   after arxa; `stacked_kit` is copied, improved, and adapted into arxa.
   **arxa becomes the canonical home and depends on neither upstream** —
   `tools/vendor/VENDOR.lock` and `check_freshness.sh` retire with the port
   (there is no upstream left to sync). arxa is self-contained.
   **Renames (settled 2026-07-30):** probe-runner → **`arxa lens`**
   (`lib/lens/`), stacked_kit → **`arxa kit`** (`lib/kit/`),
   flutter-crew → **`arxa crew`** (`lib/crew/`) — modules/subcommands of
   the one binary.
   ...
   **Copy source = live siblings, not the vendored snapshots.** When the copy
   ...
   ArxaKitGlyphs.lucide. `tools/vendor/` snapshots serve only as golden-test
...
3. **Archive policy (very important)** — nothing retired is deleted. All
   ...
   "rm". Existing `archives/` entries (arxa-app, arxa-variants,
   design-v1|v2, spikes) are untouched. **Scope: the arxa repo only.** The
...
6. **probe-runner promoted → `arxa lens`** — from underused vendored skill
   to a first-class design-vs-built gate: pixel (SSIM), skeleton, colour
   (deltaE) against the frozen golden — the gate `designs/arxa/models/intake_model/intake.en.json:403`
...
7. **End state — one `arxa` binary.** `arxa` absorbs the pipeline FSM,
   gates, emitters, lens, and the CDP substrate; ships via
   `dart compile exe` (subcommands: `gate`, `emit`, `serve`, …). Rename
   `arxa` → `arxa` everywhere (docs, file names, config keys) via a
...
- **"Web" = server-hosted** — the binary runs cloud-side or on localhost and
  the browser is the UI client (arxa's existing HTTP shape); an
...
CDP spike → pipeline FSM + gates into `arxa` → emitters in dependency
order (transform_tokens → blueprint → emit → generate_view → emit_htmx /
emit_playground / emit_structure) → arxa lens + visual gate → arxa kit
...
**CDP spike ✅** — `arxa/lib/cdp.dart` (420 lines): Dart CDP-over-WebSocket
...
screenshot at 390×844 (arxa lens pattern), console error capture,
...
**Gate framework ✅** — `arxa/lib/gates.dart`: ports all `_common/` bash
...
**arxa lens ✅** — `arxa/lib/lens.dart`: visual gate using CDP screenshots.
captureGolden + compareGolden (byte/pixel/ssim modes). 4/4 tests green.
Console errors always fail the lens.

**Subcommand dispatch ✅** — `arxa/bin/arxa.dart`: unified entry point
with `arxa gate <name>`, `arxa gate --all`, `arxa emit structure|htmx|
transform_tokens`, `arxa serve`.
...
emitters wired into `arxa emit` dispatch.

**arxa kit copy ✅ (Phase 5)** — 24 library packages from stacked_kit
copied into `kit/` (511 .dart files, 31 pubspecs). All `stacked_kit_*`
identifiers rewritten to `arxa_kit_*`. Flutter app path-depends on
in-repo `kit/i18n`. `flutter pub get` resolves.

**Sweep rename ✅ (Phase 6)** — 1,237 text replacements across 2,410 files.
Physical renames: `config/arxa.config.json`, 10 skill dirs in `skills/`
...
`config/arxa.config.json`. `engine.dart` `loadStages()` uses hardcoded
...
plan ("stacked_kit → arxa kit (`lib/kit/`)"). 22 barrel files renamed
(`lib/stacked_kit_*.dart` → `lib/arxa_kit_*.dart`). 44 stacked_kit text
...
22 `stacked_kit_*` package names → `arxa_kit_*`. `gate_scaffold.dart`
...
32 Python stages vs arxa's 44 Dart files:
- **5 directly ported** (blueprint, transform_tokens, synthesize,
  generate_view, emit_stage).
- **16 covered** by arxa's deliberately different authored-layer
...
- **11 deferred/superseded** (capture_*.py, glass_entrance.py,
  motion_device.py — require the jsx design model arxa doesn't use).
...
ported to `arxa/lib/arch_guard.dart`. Validates the DDD 4-layer contract
...
G5 Supabase confined to infrastructure. Wired into `arxa gate arch`.
...
4. **Archived tools** — all already ported in prior sessions:
   ...
   `run_repo_tests.sh` → `dart test` + `arxa gate --all`.
5. **theme_map.py → Dart ✅** — `arxa/lib/theme_map.dart`. W3C DTCG color
   tokens → ThemeData fragment. CLI: `arxa emit theme-map`. 7 tests.
6. **gen_freshness.py → Dart ✅** — `arxa/lib/gen_freshness.dart`. build_runner
   diff gate for committed generated files. CLI: `arxa gate gen-freshness`.
   5 tests.
7. **trace.py → Dart ✅** — `arxa/lib/trace.dart` (388 lines). View↔VM↔route
   traceability manifest + guard. CLI: `arxa gate trace`. 11 tests.
8. **palette.py → Dart ✅** — `arxa/lib/palette.dart`. HCT seed → DTCG tonal
   palette + APCA contrast. Pure `dart:math`, zero deps. CLI: `arxa emit palette`.
   13 tests.
9. **tier1.py → Dart ✅** — `arxa/lib/tier1.dart`. Tier-1 verification for
   payments + auth SDK patterns + SeedAuthBackend. CLI: `arxa gate tier1`.
   25 tests.

**Final test suite: 408/408 green.** Zero analyzer issues across arxa,
arxa, and all 24 kit/ packages. All Python/bash/Node tooling retired.
...
Dart in arxa. Nine new modules, all wired into `arxa kb` subcommands:
...
CLI: `arxa kb facts|build|check|lock|playbook|conventions`,
`arxa gate capability|api-map`, `arxa docs`.

**showcase_app copied ✅** — 121-file Flutter app from stacked_kit copied
to `kit/showcase_app/` with all `stacked_kit_*` → `arxa_kit_*` renames
(82 files modified). `flutter pub get` resolves, `flutter analyze` clean.
`FLUTTER_API_MAP.md` copied to `kit/core/`.

**Final test suite: 496/496 green.** Zero analyzer issues across arxa
