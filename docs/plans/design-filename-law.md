...
2. **No version/era prefixes** (`v1_`, `v2_`, …). `v1_strings.tsx` → the showcase
   kit name the scaffolder will receive: `arxa_kit_app_strings.tsx` (mirrors
   `arxa_kit_app_strings.dart` in the Flutter kit mirror — SKILL.md §kit).
...
## Renames performed (designs/arxa-studio-v2, git mv)

- `ui/common/v1_strings.tsx` → `ui/common/arxa_kit_app_strings.tsx`
...
- Law prose lives in `skills/arxa-designer/SKILL.md` ("Filename law (locked)").
- SSOT procedure per `docs/plans/no-single-letter-identifiers-law.md`: edit
  `skills/arxa-designer` (git SSOT), rsync → `.claude/skills/arxa-designer`.
