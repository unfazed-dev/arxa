...
unify — kit runtime + arxa governance"), `docs/research/deploy-machinery.md`
(two-implementation inventory), `docs/research/kit-system.md` (catalog/tier
plumbing).

## Scope & non-goals

In scope: merging `arxa/lib/deploy.dart` and `kit/deploy/` into one deploy
engine per D13's three clauses — kit runtime as base, arxa governance
...
**Base: `kit/deploy`'s runtime.** `ArxaKitDeployTarget` port, its five target
...
`ArxaKitDeployService`, and `ArxaKitDeployConfig` survive as the sole target-execution
...
— arxa's parallel target functions (`fastlaneIos`, `shorebirdRelease`,
...
**arxa supplies orchestration, not execution.** Three things wrap the kit
...
1. **Approval gate** — the four-field confirmation (target/version/account/
   ...
   `DeployAttempt` row and returns without calling `ArxaKitDeployService` at all.
2. **Append-only ledger** — `DeployAttempt`/`DeployResult` (the row shape at
   ...
   status) is retained as the ledger schema. `ArxaKitDeployResult`'s `ok`/
...
implementation reads `config/credentials.catalog.json` — arxa relies on
...
the `kit/deploy` module's 10 keys and populates `ArxaKitDeployConfig.environment`
...
1. **Target vocabulary reconciliation.** Verify `ArxaKitDeployService`'s registry
   (keyed by name, `arxa_kit_deploy_service.dart:8`) actually exposes all seven
   ...
   entries each. This is pre-work: read `arxa_kit_deploy_service.dart` and the two
   target files before writing the wrapper, don't assume parity.
2. **Credential resolver.** New module populating `ArxaKitDeployConfig.environment`
...
3. **Orchestration wrapper.** Replace `deploy.dart`'s direct target-function
   calls with calls into `ArxaKitDeployService.deployTo`/`.doctor`, built from a
   `ArxaKitDeployConfig` assembled from the approval-confirmed
   ...
   `DeployHalted` continues to model the halt path, `ArxaKitDeployResult`
...
4. **Version model upgrade.** Replace `deploy.dart`'s opaque `String version`
   with `ArxaKitDeployConfig`'s `releaseVersion`/`patchNumber`/`flutterVersion`
...
5. **Ledger-aware gate.** Extend `gate_deploy.dart` to load the ledger
   ...
   `$ARXA_DEPLOY_LEDGER`, per today's resolution order) and assert a
...
6. **CLI unification.** Single user-facing CLI stays `arxa deploy`
   (`doctor`/`deploy`/`--self-test`, exit contract `0/1/2`). `kit/deploy`'s
   own CLI (`bin/arxa_kit_deploy.dart`, usage exit **64**) is demoted to
   ...
   package tests (`targets_test.dart`, `arxa_kit_deploy_service_test.dart`,
...
- **Exit code divergence (2 vs 64).** If `bin/arxa_kit_deploy.dart` survives
  as a kept-around entrypoint, any script depending on its usage exit code
  silently disagrees with `arxa deploy`'s contract. Resolve explicitly in
...
- **Silent credential fallback.** If the resolver's ambient-env fallback
  ...
  environments that relied on ambient env under the old arxa path,
  masking exactly the gap this unification is meant to close.
- **Ledger schema loss.** `ArxaKitDeployResult.failureReason` is richer than a
...
- Does the credential resolver live inside `kit/deploy` (making the package
  catalog-aware, coupling it to arxa's config layout) or as an
  arxa-side adapter that only populates `ArxaKitDeployConfig.environment`
  (keeping `kit/deploy` standalone)? Affects whether `kit/deploy` remains
  usable outside the arxa tree.
- Is `bin/arxa_kit_deploy.dart` deleted outright in workstream 6, or kept
  as an internal harness solely for `kit/deploy`'s own package tests?
- Does the approval token stay a CLI flag (`--approval`) on `arxa deploy`,
