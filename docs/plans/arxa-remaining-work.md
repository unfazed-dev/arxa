# arxa — remaining work (status 2026-08-05)
...
1. **Dev-key swap** (runbook §9 step 6): replace `Entitlement.publicKey` with
   ...
   `arxa/test/release_gate_test.dart`. After this, dogfooding goes through
...
3. **`arxa login` + silent refresh** (runbook §4/§6): PKCE loopback against
...
10. **`.kimi-code/skills/arxa-designer/` is a stale mirror** of `skills/` —
...
12. **Holdco + operating Pty Ltd** with a Melbourne accountant (Holdco owns
    arxa + cairn IP; opco trades) — see the round-2 grilling decisions in
    `docs/plans/arxa-data-model-decisions.md`.
