# arxa engine + LLM fabric — decisions record
...
Purpose (operator's words): arxa operates **standalone with the user's LLM
...
## E1 — Credential home: arxa vault, N keys

All N provider keys (Kimi, z.ai, Anthropic, OpenAI, DeepSeek, Gemini, Grok, …) live
**only** in arxa's existing vault. Everything else — headless stage runs, genui
...
## E2 — Access: arxa loopback gateway

arxa serves an OpenAI-compatible **and** Anthropic-compatible endpoint on
...
## E3 — Engine: deterministic stage-runner inside arxa

arxa absorbs three things it half-has:
...
Rationale: Anthropic's canon (A) — arxa's pipeline is a *workflow* (finite,
...
- **z.ai**: Coding Plan is tool-restricted by policy (A) — arxa engine use needs
...
- **Built (2026-07-30).** `config/model-fabric.json` (schema_version 1),
  `arxa/lib/fabric.dart`, `arxa/lib/gateway.dart` (E2: scoped tokens,
  routing, param_policy, Anthropic↔OpenAI translation, usage.jsonl attribution),
  `arxa/lib/engine.dart` (E3: registry from `gates/run_all.sh` order —
