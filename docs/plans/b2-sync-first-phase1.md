...
- [ ] Phone-in-hand leg: the app installed (build 08:51) and its tunnel proven up (pairing.json touched 08:53 — token re-hand), but that arrival was outside the boot's 18s window → localOnly for that boot; a foregrounded unlocked boot re-engages sync. Then: registration receipt + doorbell.
  - 2026-09-01 evidence: token registered + rail live all day (receipts seq→161 delivered, latest token ea319dc05f…; decide-loop resolved; zombie persisted 30min; smoke-rounds 4/5).
  - **SILENT DOORBELL DELIVERED LIVE (2026-09-01 05:43, receipts seq 172–174):** metadata `{account:local, table:approvals, lsn:2741–2743}` + a `tasks` wake at 2743 — these are the MIRROR FAN-OUT's silent data doorbells (not the visible B1 push), coalesced per the daemon's retry policy, delivered to the registered token. The full B2 chain — engine mirror-out → /ingest → fan-out → offline-subscription wake → APNs receipt — ran end-to-end on the physical phone. What remains of this checkbox is the owner's eyes: one foregrounded unlocked boot (sync engages inside the 18s window) + watch the approvals card render from SQLite; the rig script is `/tmp/arxa-d68/phone-leg-demo.sh` (later boots that miss the window fall localOnly by design — line 130's known shape).
