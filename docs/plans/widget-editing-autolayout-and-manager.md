...
| 3 | K enforcement | **Scale, not names.** Designer vocabulary steps only through the ax numeric scale (4/8/12/16/24…); a lint rejects off-scale values; the scaffolder maps numbers → `axPad*`/`axGap*` inside kit code where their usage is legal. The kit doc's "no external usage" rule stays intact. |
...
- **ax constants** live in `kit/core/lib/common/appbox_kit_app_constants.dart` (`axPad1–80`, `axGap0–100` even ramp, `axFont2–80` + semantic aliases, `axRad*`, `axSize*`); `kit/ui_library/ui-library-usage-appendix.md:546-551` marks `axPad*`/`axGap*`/etc. **kit-internal only** → decision 3. Designer's 4/8/12/16/24 steps sit inside the kit scale 1:1.
...
- **Figma mechanics** (web, official docs): per-axis Fixed/Hug/Fill; corner-drag forces Fixed on both axes; padding/gap never change on plain resize; Option-drag = padding modifiers; no token-quantized drag in Figma — the ax-scale quantization is our tightening, precedented by Penpot's token-application step.
...
6. **Fonts**: vendor woff2 for the three families into the artifact/runtime; font choice persisted as a token; ax-scale `axFont*` untouched (sizes come from the existing scale).
...
8. **Lints/gates**: ax-scale lint (off-scale pad/gap/size rejected), existing no-custom-JS lint stays green, schema validation for structure@2.
...
ax-scale lint and the provenance router (ARB vs seed), design lint, gate --all,
