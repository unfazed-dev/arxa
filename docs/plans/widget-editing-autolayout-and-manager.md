...
| 12 | Accent SSOT | **Designer 5 wins: cyan, violet, blue, ember, moss.** `appBoxKitAccentOptions` (6-color unconsumed stub) is rewritten to these 5 as 5-role bundles. |
...
> **Decision 14 amendment (ruled, 2026-08-08 grill).** The name authority for chrome copy is **`abxStr` consts**, not ARB. Verified state at ruling time: `kit/` and `kit/showcase_app/` contain zero `.arb` files and zero `AppLocalizations`/l10n references — the only ARB anywhere is the studio's own UI localization (`appbox-studio/lib/l10n/`), a different program. Decision 14's provenance *routing* is unchanged (chrome copy vs seed-bound data, template-inline literals never written); what changes is the write target: the text editor rewrites the **value behind the `abxStr` name** (`appbox_kit_app_strings.dart` app copy). ARB becomes a **derived artifact**: if/when i18n is enabled for an app, the scaffolder derives keys mechanically from const names (`abxStrNotesEmptyTitle` → `notesEmptyTitle`) and swaps consts for l10n lookups at that gate. One authored name; ARB generated, never authored. See `designer-scaffolder-grill-decisions.md` §Named strings vocabulary.
...
- **abx constants** live in `kit/core/lib/common/appbox_kit_app_constants.dart` (`axPad1–80`, `axGap0–100` even ramp, `axFont2–80` + semantic aliases, `axRad*`, `axSize*`); `kit/ui_library/ui-library-usage-appendix.md:546-551` marks `abxPad*`/`abxGap*`/etc. **kit-internal only** → decision 3. Designer's 4/8/12/16/24 steps sit inside the kit scale 1:1.
...
- **Kit theming**: Material 3, explicit `ColorScheme.copyWith` (not fromSeed); `appBoxKitLightTheme/appBoxKitDarkTheme({Color accent})` derive containers via lerp helpers; `appBoxKitAccentOptions` (`appbox_kit_colors.dart:57-64`) is an unconsumed 6-color stub; `AppBoxKitThemeService` manages only ThemeMode. **No fontFamily anywhere in kit; no google_fonts; no bundled text fonts** — fonts need new API surface (a `textTheme`/family param + bundled OFL assets).
...
7. **Pipeline contract**: `structure.json@2` (widgets, theme, fonts) + scaffolder emits token-exact builder guidance + kit API: `appBoxKitAccentOptions` → 5-role bundles (designer-5), `appBoxKitLightTheme/appBoxKitDarkTheme` gain textTheme/family param, OFL fonts bundled in kit assets with LicenseRegistry registration.
