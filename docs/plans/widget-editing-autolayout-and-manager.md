...
| # | Decision | Choice |
|---|---|---|
| 1 | Persistence | **Write-through to source.** Edits rewrite the live-read project's files (`~/.arxa/projects/<name>`) via the existing single write channel (`POST /__project_write`); watch reload propagates to every frame; git is undo. Drag gestures preview client-side, commit on release (same shape as panel rail resize). |
...
| 3 | K enforcement | **Scale, not names.** Designer vocabulary steps only through the ax numeric scale (4/8/12/16/24…); a lint rejects off-scale values; the scaffolder maps numbers → `abxPad*`/`abxGap*` inside kit code where their usage is legal. The kit doc's "no external usage" rule stays intact. |
...
| 12 | Accent SSOT | **Designer 5 wins: cyan, violet, blue, ember, moss.** `arxaKitAccentOptions` (6-color unconsumed stub) is rewritten to these 5 as 5-role bundles. |
...
> **Decision 14 amendment (ruled, 2026-08-08 grill).** The name authority for chrome copy is **`abxStr` consts**, not ARB. Verified state at ruling time: `kit/` and `kit/showcase_app/` contain zero `.arb` files and zero `AppLocalizations`/l10n references — the only ARB anywhere is the studio's own UI localization (`arxa-studio/lib/l10n/`), a different program. Decision 14's provenance *routing* is unchanged (chrome copy vs seed-bound data, template-inline literals never written); what changes is the write target: the text editor rewrites the **value behind the `abxStr` name** (`arxa_kit_app_strings.dart` app copy). ARB becomes a **derived artifact**: if/when i18n is enabled for an app, the scaffolder derives keys mechanically from const names (`abxStrNotesEmptyTitle` → `notesEmptyTitle`) and swaps consts for l10n lookups at that gate. One authored name; ARB generated, never authored. See `designer-scaffolder-grill-decisions.md` §Named strings vocabulary.
...
- **abx constants** live in `kit/core/lib/common/arxa_kit_app_constants.dart` (`axPad1–80`, `axGap0–100` even ramp, `axFont2–80` + semantic aliases, `axRad*`, `axSize*`); `kit/ui_library/ui-library-usage-appendix.md:546-551` marks `abxPad*`/`abxGap*`/etc. **kit-internal only** → decision 3. Designer's 4/8/12/16/24 steps sit inside the kit scale 1:1.
...
- **Pipeline**: freeze is hash-locked + human-gated; `structure.json@1` = registry/shellRoots/screens/flows — no widgets/theme/fonts; `arxa/lib/scaffold.dart:338-345` names the ceiling ("emission lands when the design-side widget map reaches structure.json"). ARB crosses verbatim (`scaffold.dart:360-405`); seeds do NOT cross (design-layer only). Palette/Type emissions are placeholder comments (`scaffold.dart:290-322`).
- **Kit theming**: Material 3, explicit `ColorScheme.copyWith` (not fromSeed); `arxaKitLightTheme/arxaKitDarkTheme({Color accent})` derive containers via lerp helpers; `arxaKitAccentOptions` (`arxa_kit_colors.dart:57-64`) is an unconsumed 6-color stub; `ArxaKitThemeService` manages only ThemeMode. **No fontFamily anywhere in kit; no google_fonts; no bundled text fonts** — fonts need new API surface (a `textTheme`/family param + bundled OFL assets).
...
- **Figma mechanics** (web, official docs): per-axis Fixed/Hug/Fill; corner-drag forces Fixed on both axes; padding/gap never change on plain resize; Option-drag = padding modifiers; no token-quantized drag in Figma — the abx-scale quantization is our tightening, precedented by Penpot's token-application step.
...
6. **Fonts**: vendor woff2 for the three families into the artifact/runtime; font choice persisted as a token; abx-scale `abxFont*` untouched (sizes come from the existing scale).
7. **Pipeline contract**: `structure.json@2` (widgets, theme, fonts) + scaffolder emits token-exact builder guidance + kit API: `arxaKitAccentOptions` → 5-role bundles (designer-5), `arxaKitLightTheme/arxaKitDarkTheme` gain textTheme/family param, OFL fonts bundled in kit assets with LicenseRegistry registration.
8. **Lints/gates**: abx-scale lint (off-scale pad/gap/size rejected), existing no-custom-JS lint stays green, schema validation for structure@2.
...
abx-scale lint and the provenance router (ARB vs seed), design lint, gate --all,
