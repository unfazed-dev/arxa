# arxa-studio artifact inventory

Scope: everything under `designs/arxa-studio/`. Purpose: establish what the
...
Source: `designs/arxa-studio/models/screens_model/registry.json` (326
lines), cross-checked against `designs/arxa-studio/structure.json` (476
...
(likely the `arxa-designer`/`arxa-scaffolder` skills, not this data file).
...
`design`, `build`, `app`, `workspace` (`designs/arxa-studio/app.routes.js:17-23`
...
Canonical vocabulary lives in `designs/arxa-studio/ui/common/_integration_panels.md`
and is restated as always-loaded context in the `arxa-designer` skill's
...
arxa writes build evidence yet, so EVERY project reads empty today"* — the
...
tree (per the `arxa-scaffolder` skill: *"the scaffolder PRODUCES the tree;
...
3. **Repository** (`services/repositories/*_repository.js`) — the only layer
   ...
   (`~/.arxa/projects/<name>/...`, served at `/project/<rel>` by the design
...
- `designs/arxa-studio/models/screens_model/registry.json` — add one or
...
- `designs/arxa-studio/structure.json` — mirror the same entries
...
- `designs/arxa-studio/models/screens_model/flows.json` — replace the
...
`build/loop/`):**
- New `designs/arxa-studio/ui/views/main_shell/scaffold/` directory:
  ...
  fan-out.
- `designs/arxa-studio/app.routes.js` — import and spread the new route
...
- `designs/arxa-studio/ui/views/main_shell/shared/widgets/chrome.html` —
...
- `designs/arxa-studio/l10n/app_en.arb` (+ `app_pl.arb`, `app_qps-ploc.arb`)
