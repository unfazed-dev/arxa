# Paygate hook points in arxa

## Summary

The brief's premise — "confirm absence if none" — is **wrong**. arxa already ships a
complete monetization system: an Ed25519-signed, fully-offline licence
(`arxa/lib/licence.dart`), a five-state verdict with a 30-day grace period, and an
enforced paywall. The paywall is **already installed at first deploy**
(`arxa/lib/gate_deploy.dart:9`), fail-closed, and it is the first thing `deployGate`
checks (`:23-28`).
...
No arxa-owned server exists. Every outbound call is third-party. Any online
...
| Component | Citation |
|---|---|
| Licence value type + status enum | `arxa/lib/licence.dart:49` — `enum LicenceStatus { valid, expired, grace, invalid, none }` |
| Verdict class | `arxa/lib/licence.dart:51` — `class LicenceVerdict` |
| Grace period | `arxa/lib/licence.dart:81` — `static const gracePeriod = Duration(days: 30)` |
| Embedded public key | `arxa/lib/licence.dart:83-90` — "The arxa licence PUBLIC key (Ed25519, 32 bytes)" |
| Offline verification | `arxa/lib/licence.dart:96` — `static LicenceVerdict verify(List<int> fileBytes, {DateTime? now})` |
| File-based verification | `arxa/lib/licence.dart:172` — `static LicenceVerdict verifyFile(String path, ...)` |
| Signature scheme | `arxa/lib/licence.dart:2` — "Ed25519-signed licence file, verified fully offline by arxa" |
| Canonical signing bytes | `arxa/lib/licence.dart:23-26` — payload re-encoded sorted-keys/no-whitespace/UTF-8 |
| Ed25519 implementation | `arxa/lib/ed25519.dart` (imported at `licence.dart:47`) |
| Tier plans | `arxa/lib/licence.dart:14` — `"tier": "annual"` \| `"perpetual"` |
| Free/paid tier watermarking | `arxa/lib/watermark.dart:5-6, :16-17, :32-33, :54-59` |
| Watermark string | `arxa/lib/watermark.dart:33` — `'Built with arxa (free tier) — https://arxa.dev'` |
| Provenance manifest | `arxa/lib/watermark.dart:36, :99, :137` — `.arxa-provenance.json` `{tier, ts, files[]}` |
| Tier resolution from licence | `arxa/lib/watermark.dart:101-106` — valid/grace → paid, else free |
| Licence CLI | `arxa/bin/licence_tool.dart:37, :39` — `status`, `verify` subcommands only |
| Watermark CLI flag | `arxa/bin/arxa.dart:695-704` — `arxa watermark <rootDir> [--licence <path>]` |
| Tests | `arxa/test/watermark_test.dart:35, :67` — free-tier watermark, paid-tier clean |

### Licence payload shape (`arxa/lib/licence.dart:10-19`)
...
### Licence file discovery (`arxa/lib/gate_deploy.dart:145-152`)

Mirrors `licence_tool`'s `_defaultLicencePath`: `~/.arxa/licence.json`, then
`<repoRoot>/pipeline/state/licence.json`. First existing candidate wins.

### Dev bypass

`arxa/lib/gate_deploy.dart:15` — "ARXA_DEV_LICENCE=1 is the documented dev/dogfood
...
`arxa/lib/gate_deploy.dart:9-11` — "the paywall sits at first deploy … failing
...
|---|---|---|---|---|
| 1 | `scaffoldGate(ctx)` gate dispatch | `arxa/bin/arxa.dart:352-353` — `case 'scaffold': return scaffoldGate(ctx);` | Mirror `gate_deploy.dart:23-28`: licence assertion first, `GateResult.fail` on miss | **Yes** — only guards the *gate*, not the emitter |
| 2 | `scaffoldMain(rest)` emitter | `arxa/bin/arxa.dart:606-607` — `case 'scaffold': exit(scaffoldMain(rest));` | Assert inside `scaffoldMain` before any file write | Harder — this is the real `arxa emit scaffold` work entry |
| 3 | Phase declaration | `arxa/lib/phases.dart:17` — `'scaffold'` in phase list; `:58` — `'scaffold': ['scaffold', 'coverage']` | Phase-transition guard on entering `scaffold` | **Yes** — phases.dart is a declarative phase/gate map, not an execution funnel |
| 4 | Deploy gate (existing) | `gate_deploy.dart:23-28` | Already implemented | Only by `ARXA_DEV_LICENCE=1` |
...
separable from the emitter. A caller can invoke `arxa emit scaffold` (#2) and never
touch `scaffoldGate`. The only defensible single insertion is **inside `scaffoldMain`**,
before it writes output. Even then, arxa ships as source-available Dart the user runs
locally — any check is removable by editing and recompiling. Enforcement here is a
speed bump, not a boundary.

---

## Q3 — The free web path (`arxa design eject`)
...
- Eject walks the artifact directory and rewrites files in place:
  `arxa/lib/design_tools.dart:280` (`var d = Directory(dir);`), `:297`
...
- `arxa design serve` lives in `arxa/lib/design_server.dart` (file watchers at
...
Read in full: `config/arxa.config.json` (15 lines). Contains `version`, `targets`,
...
Other outbound references in `arxa/`: OSM/Mapbox tiles
(`arxa/lib/tier1.dart:468, :501`), jsdelivr/npm (`design_tools.dart:948-949`), and a
DevTools protocol doc comment (`cdp.dart:11`).

`https://arxa.dev` appears **only** as a watermark string literal
(`watermark.dart:33`) — it is never fetched.

`arxa/bin/licence_tool.dart` exposes only `status` (`:37`) and `verify` (`:39`) — no
issuance, no signing, no network. Licence *signing* (the private key) lives outside this
repo.

**Conclusion: there is no arxa-owned endpoint, no update check, and no telemetry.**
...
**No machine identity exists.** A grep of `arxa/lib/secure_store.dart` for `uuid`,
...
- `arxa/lib/secure_store.dart:34` — `class SecureStore`
- `arxa/lib/secure_store.dart:131` — `class SealedFileVault implements Vault`
- `:88` — encrypts and writes an `ABX1` envelope at mode `0600`
- `arxa/lib/crypto_aead.dart` — XChaCha20-Poly1305 seal (`:14`), `sha256` (`:78`),
...
(copies freely), and arxa executes locally as user-modifiable Dart. Nothing here is
...
2. **`scaffoldMain` (`arxa.dart:606-607`), assertion before first file write** — best
...
3. **Existing deploy-gate assertion (`gate_deploy.dart:23-28`)** — proven, fail-closed,
   already shipped. Weakened by `ARXA_DEV_LICENCE=1` (`:97`); resistance rises
   immediately if that env bypass is compiled out of release builds.
4. **`scaffoldGate` (`arxa.dart:352-353`)** — cheap and consistent with existing gate
...
5. **Watermark/provenance degradation (`watermark.dart:54-59`)** — already shipped and
   ...
   strippable, but it is *tamper-evident*: the `.arxa-provenance.json` sha256 manifest
