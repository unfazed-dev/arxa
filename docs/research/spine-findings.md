# app_box spine — measured findings

Scored against `app-box-spine-rubric.md`, which was written first. Every number
...
Neither overturns the call. Both are inherited debt app_box must close rather than
...
opportunity app_box exists to close.

*Update — this is now closed.* The end-to-end run was done on p2's
`train_shell_today_view`; see `app-box-headtohead-train-shell.md`. Summary:
...
The exact input is unreproducible. app_box must vendor from a clean tag.
