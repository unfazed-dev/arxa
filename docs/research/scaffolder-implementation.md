...
scaffolder (`arxa emit scaffold`), which was the primary subject of this
brief, and (B) the eject/productionize pipeline (`arxa design eject`), which
...
- `arxa emit scaffold --design-dir <d> --app-root <a> --targets <t1,t2> [--check] [--self-test]`
  - Dispatch: `arxa/bin/arxa.dart:606-609` (`case 'scaffold': ... emitStructure(...)` for `emit structure`, and the `emit scaffold` case calling `scaffoldMain(rest)`).
  - CLI parsing: `arxa/lib/scaffold_cli.dart:21-94` (`scaffoldMain`).
  - Engine: `arxa/lib/scaffold.dart:556-671` (`scaffold(...)`), a Dart port of `skills/arxa-scaffolder/scaffold.py`.
- `arxa gate scaffold` — validator, not a generator: `arxa/bin/arxa.dart:352-353` dispatches to `arxa/lib/gate_scaffold.dart` (`scaffoldGate`, 1126 lines; checks §6/SN/S5/S8/S10/S6c/S0/S1/S4/S2/S6/S7/S9/S3 per its header, lines 12-32).
- `arxa emit structure [--check]` — the producer feeding the scaffolder: `arxa/bin/arxa.dart:606-609` → `arxa/lib/emit_structure.dart:377-411` (`emitStructure`), building `structure.json` from the authored htmx layer via `buildStructure` (`emit_structure.dart:37-339`).
- Design doc: `.kimi-code/skills/arxa-scaffolder/SKILL.md` (166 lines) — states the scaffolder "turn[s] a FROZEN design into the per-surface Flutter file set the coverage gate asserts" and explicitly scopes out widget bodies, business logic, and platform ceremony files (lines 74-78).

**Pipeline B — eject/productionize (not Flutter)**
- `arxa design eject <artifact-dir> <out-dir>` — dispatch `arxa/lib/design_cli.dart:98-99` (`case 'eject': return _emit(designEject(rest));`), implementation `arxa/lib/design_tools.dart:1208-1303` (`designEject`).
- Doc: `.claude/skills/arxa-designer/built-in-skills/productionize.md` (45 lines).
- This pipeline turns the htmx artifact into a self-contained, hardened, still-htmx production app served by the same Dart design server (`arxa/lib/design_server.dart`) — it is not a candidate for "the scaffolder" in the Flutter sense and is documented here only to prevent it being mistaken for the missing piece.
...
Targets/form-factors: `deriveFactors(targets, derivationPath, configPath)` (`scaffold.dart:62-97`) reads `viewports` (never `ceremonies`) from `pipeline/state/targets.derivation.json`, plus the `viewports` ordering table from `config/arxa.config.json`, resolving `inherits` chains (e.g. `pwa` inherits `web`).
...
- `--targets` / `ARXA_TARGETS` — **required explicitly**; the CLI refuses to fall back to ambient pipeline state, citing the "stale-green" defect (lines 62-76 of `scaffold_cli.dart`): "a scaffold run that reads ambient state is the stale-green defect (§16)".
- `--design-dir` / `KIT_DESIGN_DIR` (default `design`) — must stay app-root-relative (R3, lines 79-83).
- `--app-root` / `ARXA_APP` (default `.`).
...
4. **The SKILL.md "Route table contract" is aspirational, not implemented.** `.kimi-code/skills/arxa-scaffolder/SKILL.md` (lines 80-105) describes a go_router-shaped route compiler reading `intake/registry.json` + `intake/flows.json`, with typed nav actions (`push|replace|back|modal|system`), `requiresAuth` guards, and `tab` data. Grep across `arxa/lib`, `arxa/bin`, `arxa/test` finds `go_router`/`requiresAuth`/`GoRouter` only in `decision_log.dart` and `intake.dart` (which merely validate/record the flags, not compile a route table) — and the real `designs/arxa-studio/models/screens_model/flows.json` only ever declares a `trigger` key on edges, no `action` field. So the design data model does not even carry the shape this contract would need; the feature is fully unimplemented, and its own SKILL.md is currently inaccurate about present-tense capability.
