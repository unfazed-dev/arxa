...
**Correction to the brief's hypothesis: Shorebird / fastlane / Vercel / Cloudflare are NOT absent — they are the entire deploy runtime, and there are TWO independent implementations of it.** `appboxd/lib/deploy.dart:275-283` declares seven wired targets: `fastlane-ios`, `fastlane-android`, `shorebird-release`, `shorebird-patch`, `cloudflare-pages`, `cloudflare-workers`, `vercel`. Each is a function that shells the real CLI (`fastlane run gym`, `vercel deploy --prod`, etc.). Separately, **`kit/deploy/` is a full Dart package** implementing the same seven targets behind a proper port interface, with its own CLI and tests (see "Two implementations" below). The two have divergent contracts and neither calls the other.
...
## Q1 — What does `appbox deploy` do today?

**Dispatch.** `appboxd/bin/appbox.dart:84-85` routes `deploy` to `deployMain(rest)`; `appbox.dart:23` imports `deploy_cli.dart`. Help text at `appbox.dart:144`: "deploy `<sub>` — Deploy runtime — doctor, deploy, --self-test".

**Three verbs** (`appboxd/lib/deploy_cli.dart:42-62`), exit contract `0 ok / 1 halt / 2 usage`, HALT to stderr (`deploy_cli.dart:4`, `36`):
...
**Ledger resolution** (`deploy_cli.dart:113-136`): `--ledger` flag → `$APPBOX_DEPLOY_LEDGER` → repo-root default `pipeline/state/deploy-ledger.json` (`deploy.dart:379`).
...
## Two implementations — `appboxd/lib/deploy.dart` vs `kit/deploy/`
...
- **Own CLI** `bin/appbox_kit_deploy.dart`: `doctor` (`:46`), `deploy <target>` (`:54`), flags `--project-name`, `--release-version`, `--flutter-version` (`:69-72`); usage exit **64** (`:11`, `:63`) — different from `deploy_cli`'s 2.
...
| | `appboxd/lib/deploy.dart` | `kit/deploy/` |
|---|---|---|
...
`kit/deploy` has the better runtime model (real doctor, explicit credentials, structured config, no-throw); `appboxd` has the governance (approval gate, ledger, licence check). Neither is a superset.
...
`appboxd/lib/phases.dart:13` declares `phases`, with `'deploy'` at `:20` (last). `phaseGates` (`:54`) maps `'deploy': ['deploy', 'advertise']` (`:61`) — the deploy phase must pass **two** gates. `runPhase` looks up `phaseGates[phase]` at `:69`.

**The gate** — `appboxd/lib/gate_deploy.dart`, dispatched from `appbox.dart:344-345` (`deployGate(ctx)`):

1. **Licence assertion first, fail-closed** (`gate_deploy.dart:24-31`, `94-135`). Only `status == 'paid'` permits deploy (`:117-125`); `none`/`free` → "PRECONDITION NOT MET" (`:134`). `APPBOX_DEV_LICENCE=1` is a documented dev bypass (`:97-102`, "dev/dogfood only, never ship").
...
`designs/appbox-studio/models/screens_model/registry.json:228-235` registers:
...
`surface: null` and **no view file exists** — `find designs/appbox-studio/ui -ipath '*ship*' -o -ipath '*deploy*'` returns nothing. The screen is a registry placeholder only.
...
0. **Decide which runtime the shell drives first.** Two implementations exist with incompatible contracts (see table above). This is the blocking design question — a shell built on `appboxd` inherits the approval gate and ledger but a fake `doctor`; one built on `kit/deploy` inherits real readiness checks but no governance. Merging them is plausible (kit runtime + appboxd governance) but is a code change, not a shell.
