...
| `intake.state.schema.json` | JSON Schema for the intake state slot (plan 10.1): elicited answers + emitted artefact paths. The answers shape references `skills/arxa-intake/intake.schema.json` (the engine's artefact format); this schema binds answers into pipeline state. |
...
- `viewports` — viewport **names** (mobile/tablet/desktop) the target implies;
  the actual widths live in `config/arxa.config.json`, never here (R3).
...
(`skills/arxa-intake/intake.schema.json`) defines the **artefact format** —
...
`arxa intake emit` wrote. The registry seed lands at the design root
