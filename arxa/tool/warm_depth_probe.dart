...
//
// Run:  cd arxa && dart run tool/warm_depth_probe.dart
...
(int browserKb, int summedKb, int count) sampleRss(String userDataDir, int? browserPid) {
  ...
  for (final line in (ps.stdout as String).split('\n')) {
    ...
    if (at < 0) continue;
    // Whole dir, not a prefix — `arxa-cdp-AB` must not claim `…-ABC`'s rows.
...
String buildSection({
  ...
  final b = StringBuffer()
    ...
    ..writeln('### Setup')
    ..writeln()
    ..writeln('- Probe: `arxa/tool/warm_depth_probe.dart`')
    ..writeln('- Reproduce: `cd arxa && dart run tool/warm_depth_probe.dart`')
