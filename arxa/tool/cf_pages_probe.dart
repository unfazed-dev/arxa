...
Future<void> main() async {
  final store =
      await defaultCredentialStore('${Platform.environment['HOME']!}/.arxa');
  ...
  stdout.writeln('pages projects visible to this token: ${(projects['result'] as List)
  ...
  final deps = await get('/accounts/$acc/pages/projects/architect-gallore/deployments');
  final list = deps['result'] as List? ?? [];
  stdout.writeln('  deployments:      ${list.length}${list.isEmpty ? '  (inert — nothing published, by design)' : ''}');
  client.close();
}
