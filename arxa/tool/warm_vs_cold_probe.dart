...
//
// This gates the `arxa lens` daemon plan: holding ONE warm Chrome and reusing
...
//
// Run:  cd arxa && dart run tool/warm_vs_cold_probe.dart
...
// ── hashing ───────────────────────────────────────────────────────────────
// FNV-1a 64. package:crypto is not a dependency of arxa; this is the
...
/// against. `arxa/lib/cdp.dart` is under active edit by other work, and the
...
/// `arxa/`, so a wrong cwd cannot silently scatter the report.
File reportFile() {
  var dir = Directory.current.absolute;
  for (var i = 0; i < 6; i++) {
    if (Directory('${dir.path}/docs').existsSync() &&
        Directory('${dir.path}/arxa').existsSync()) {
  ...
  throw StateError('repo root (a dir holding both docs/ and arxa/) not found '
...
Future<int> run() async {
  ...
  say(verdict);
  say('');
  say('Reproduce: cd arxa && dart run tool/warm_vs_cold_probe.dart');
