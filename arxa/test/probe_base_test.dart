...
void main() {
  ...
  group('resolveTarget — rule 3: --base/--port beat ARXA_BASE beats the default', () {
    test('--port wins over ARXA_BASE', () {
      final t = resolveTarget(['--port', '4371'],
          env: {'ARXA_BASE': 'http://localhost:9999'});
      expect(t.base, 'http://localhost:4371');
      expect(t.via, '--port');
    });

    test('--base wins over ARXA_BASE', () {
    ...
    test('ARXA_BASE wins over the default', () {
      final t = resolveTarget([], env: {'ARXA_BASE': 'http://localhost:9999'});
      expect(t.base, 'http://localhost:9999');
      expect(t.via, 'ARXA_BASE');
    });
  ...
  group('disposableVerdict — refusing to mutate a project that is not disposable', () {
    test('rejects a project whose name does not end in -probe or -test', () {
      ...
      expect(v.message, contains('cp -R ~/.arxa/projects/portalo'));
    });
