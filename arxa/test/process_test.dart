import 'package:arxa/harness.dart';
import 'package:arxa/phases.dart';
  ...
  group('HarnessAuth', () {
    test('presence probes PATH via the seam', () async {
      final harness = HarnessAuth(
        command: 'arxa-harness',
        envHint: 'ARXA_HARNESS_PATH_TEST_ABSENT',
        runner: runner,
      );
      runner.handler = (_, _) => const RunnerResult(1, '', '');
      expect(await harness.harnessPresent, isFalse);
      expect(runner.calls.single, ['command', '-v', 'arxa-harness']);
    });
    ...
    test('harnessAction shells out to the harness, holding no token', () async {
      ...
      expect(runner.calls.single, ['arxa-harness', 'generate']);
    });
  });
}
