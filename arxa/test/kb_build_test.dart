...
void main() {
  ...
  group('injectReferences', () {
    test('idempotent — running twice yields identical output', () {
      ...
      final facts = <String, Fact>{
        'auth': const Fact(kit: 'auth', name: 'arxa_kit_auth'),
      };
  ...
  group('buildKb', () {
    test('writes KB.md + 5 domain pages and reports counts', () {
      ...
      expect(kb.readAsStringSync(), contains('# arxa kit knowledge base'));
    ...
    test('creates kb/ dir when it does not exist', () {
      _fact(tmp, 'auth', {'kit': 'auth', 'name': 'arxa_kit_auth'});
    ...
    test('domain pages group mined packages by kit (sorted)', () {
      ...
      expect(flutter, contains('backing dependency of arxa_kit_forms'));
  ...
  group('buildToc', () {
    test('emits playbooks.md table + llms.txt sections', () {
      _fact(tmp, 'forms', {'kit': 'forms', 'name': 'arxa_kit_forms',
        'description': 'Forms.'});
      _fact(tmp, 'auth', {
        'kit': 'auth',
        'name': 'arxa_kit_auth',
      ...
      expect(r.playbooksMd.indexOf('arxa_kit_auth'),
          lessThan(r.playbooksMd.indexOf('arxa_kit_forms')));

      // llms.txt
      expect(r.llmsTxt, startsWith('# arxa kit'));
      expect(r.llmsTxt, contains('## Playbooks'));
      expect(
          r.llmsTxt,
          contains('- [arxa_kit_auth playbook](auth/auth_playbook.mdx): '
              'Auth and session.'));
    ...
    test('role falls back to readmeFirst then name when description is absent', () {
      ...
      expect(r.playbooksMd, contains('| `arxa_kit_auth` | Readme-first role. |'));
      // playbooks.md + llms.txt: name fallback for forms (no description).
      expect(r.playbooksMd, contains('arxa_kit_forms` | arxa_kit_forms |'));
      expect(r.llmsTxt,
          contains(': arxa_kit_forms'));
    });
