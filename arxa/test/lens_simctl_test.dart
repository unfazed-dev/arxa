// iOS-simulator capture wrappers — TDD port (Task 9 of the arxa lens
      ...
      const json = '''
{
  ...
  group('iosOpenUrl', () {
    test('runs simctl openurl <udid> <url>', () async {
      final runner = _ScriptedRunner([
        (
          ['xcrun', 'simctl', 'openurl', 'booted', 'arxa://stage/proof'],
          const RunnerResult(0, '', ''),
        ),
      ]);
      await LensSimctl(runner: runner).openUrl('arxa://stage/proof');
    });
  });
}
