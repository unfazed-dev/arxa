// `arxa credentials` tests — list/check against a fake store
...
void main() {
  ...
  Future<CredentialStore> fakeStore() async {
    final store =
        CredentialStore(vault: InMemoryVault(), keyPrefix: 'arxa.');
    await store.hydrate();
    return store;
  }
  ...
  group('supabaseMachineSatisfied (list provenance)', () {
    test('env wins; file backfills the rest', () {
      ...
      final viaEnv = supabaseMachineSatisfied({
        'ARXA_SUPABASE_URL': 'https://env.example',
      }, home.path);
      expect(viaEnv['SUPABASE_URL'], 'env ARXA_SUPABASE_URL');
      expect(viaEnv['SUPABASE_SERVICE_ROLE_KEY'], '~/.arxa/supabase');

      final viaFile = supabaseMachineSatisfied(const {}, home.path);
      expect(viaFile['SUPABASE_URL'], '~/.arxa/supabase');
      expect(viaFile['SUPABASE_SERVICE_ROLE_KEY'], '~/.arxa/supabase');
    });
  ...
  group('exec', () {
    ...
    test('exit 127 when the command does not exist', () async {
      ...
      expect(
        await credentialsMain(
            ['exec', 'KIMI_API_KEY', '--', 'arxa-no-such-binary-xyz'],
            store: store, catalogPath: catalogPath),
        127,
      );
    });
