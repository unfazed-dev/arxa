...
// brand never appears (arxa_kit does), and test doubles are split out of the
// public API table.
  ...
  group('generatePlaybook', () {
    test('renders header, role, public-surface table, and references from facts',
      ...
      final facts = <String, dynamic>{
        ...
        'version': '0.1.0',
        'description':
            'Reusable Stacked MVVM toolkit. Holds ArxaKitAction and reactive services.',
        'hasTesting': true,
        'publicSurface': [
          {'name': 'ArxaKitPlatform', 'kind': 'class', 'file': 'platform/arxa_kit_platform.dart'},
          {'name': 'ArxaKitTier', 'kind': 'enum', 'file': 'platform/arxa_kit_platform.dart'},
        ],
      ...
      // Header + version line.
      expect(mdx, contains('# arxa_kit_core — Per-Package Playbook'));
      ...
      expect(mdx, contains('ArxaKitPlatform'));
      expect(mdx, contains('ArxaKitTier'));
...
Plugin-neutral maps port for arxa_kit apps.

## Usage

Render `ArxaKitMapView` and pass a `ArxaKitMapConfig`.
...
''');
      ...
      File(p.join(factsDir.path, 'maps.json')).writeAsStringSync(jsonEncode({
        ...
        'publicSurface': [
          {'name': 'ArxaKitMapView', 'kind': 'class', 'file': 'arxa_kit_map_view.dart'},
        ],
      }));
      ...
      expect(mdx, contains('Render `ArxaKitMapView` and pass a `ArxaKitMapConfig`.'));
    ...
    test('never references stacked_kit; branding is arxa_kit', () {
      final facts = <String, dynamic>{
        'name': 'arxa_kit_auth',
        'version': '1.0.0',
        'description': 'Auth kit.',
        'publicSurface': [
          {'name': 'ArxaKitAuth', 'kind': 'class', 'file': 'arxa_kit_auth.dart'},
        ],
      ...
      expect(mdx, contains('arxa_kit_auth'));
    ...
    test('splits test doubles out of the public API surface', () {
      final facts = <String, dynamic>{
        'name': 'arxa_kit_haptics',
        'version': '0.1.0',
        'description': 'Haptics kit.',
        'hasTesting': true,
        'publicSurface': [
          {'name': 'ArxaKitHaptics', 'kind': 'class', 'file': 'arxa_kit_haptics.dart'},
      ...
      expect(mdx, contains('ArxaKitHaptics'));
