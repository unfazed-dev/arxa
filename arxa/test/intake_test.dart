// Tests for the Dart intake engine — port of skills/arxa-intake/intake.py.
//
...
void main() {
  ...
  group('IntakeEngine', () {
    ...
    setUp(() {
      ...
      tmp = Directory.systemTemp.createTempSync('arxa_intake_test');
    });
    tearDown(() {
  ...
  group('emit — settings sync (targets/locales wiring)', () {
    late Directory home;
    setUp(() {
      home = Directory.systemTemp.createTempSync('arxa_sync_test');
      arxaHomeOverride = '${home.path}/.arxa';
    });
    tearDown(() {
  ...
  group('defaultRegistryOut — mirrors the gate resolver', () {
    ...
    setUp(() {
      ...
      tmp = Directory(Directory.systemTemp
          .createTempSync('arxa_registry_out')
          .resolveSymbolicLinksSync());
      prevDir = Directory.current;
      File('${tmp.path}/config/arxa.config.json').createSync(recursive: true);
    ...
    test('design root without structure.json -> models/screens_model default', () {
      Directory('${tmp.path}/designs/arxa-studio').createSync(recursive: true);
      expect(defaultRegistryOut(),
          '${tmp.path}/designs/arxa-studio/models/screens_model/registry.json');
    });

    test('structure.json "registry" field wins', () {
      Directory('${tmp.path}/designs/arxa-studio').createSync(recursive: true);
      File('${tmp.path}/designs/arxa-studio/structure.json')
          .writeAsStringSync('{"registry": "custom/reg.json"}');
      expect(defaultRegistryOut(),
          '${tmp.path}/designs/arxa-studio/custom/reg.json');
    });
  });
  ...
  group('intakeMain — exit codes', () {
    ...
    setUp(() => tmp = Directory.systemTemp.createTempSync('arxa_intake_cli'));
    tearDown(() {
      if (tmp.existsSync()) tmp.deleteSync(recursive: true);
    });
  ...
  group('confirmFlow — dual-writes answers.json (D2 site 7)', () {
    ...
    setUp(() {
      tmp = Directory.systemTemp.createTempSync('arxa-intake-confirm-');
...
void _mergeRegistryTests() {
  group('mergeRegistry — foreign keys survive a re-emit', () {
    ...
    late String path;
    setUp(() {
      tmp = Directory.systemTemp.createTempSync('arxa_merge_reg');
  ...
  // run inside any arxa repo. It must refuse instead.
...
String _schemaPath() {
  ...
  while (dir.parent.path != dir.path) {
    if (File('${dir.path}/config/arxa.config.json').existsSync()) break;
    dir = dir.parent;
  }
  return '${dir.path}/skills/arxa-intake/intake.schema.json';
}
