...
void main() {
  ...
  void seed() {
    f('kit/core/pubspec.yaml', "name: arxa_kit_core\nversion: 0.1.0\n");
    f('skills/arxa-builder/SKILL.md', '# Builder\n');
    f('skills/arxa-reviewer/SKILL.md', '# Reviewer\n');
  ...
  test('pins the kit version read from core/pubspec.yaml', () {
    ...
    expect(lock['kit'], {'name': 'arxa_kit', 'version': '0.1.0'});
  });
  ...
  test('discovers skills and tools, sorted, with sha256 content hashes', () {
    ...
    // Sorted: arxa-builder before arxa-reviewer.
    expect(skills.keys.toList(), ['arxa-builder', 'arxa-reviewer']);
    expect(skills['arxa-builder']!['path'], 'skills/arxa-builder/SKILL.md');
    expect(
        (skills['arxa-builder']!['sha256'] as String).length, 64); // hex
  ...
  test('changing a SKILL.md changes its hash but not the structure', () {
    ...
    f('skills/arxa-builder/SKILL.md', '# Builder (edited)\n');
    ...
    expect(
      (a['skills']! as Map)['arxa-builder'],
      isNot(equals((b['skills']! as Map)['arxa-builder'])),
    );
    // Reviewer hash unchanged, version unchanged.
    expect(
      (a['skills']! as Map)['arxa-reviewer'],
      equals((b['skills']! as Map)['arxa-reviewer']),
    );
  ...
  test('output uses only repo-relative paths (no absolute paths)', () {
    ...
    expect(out, contains('skills/arxa-builder/SKILL.md'));
