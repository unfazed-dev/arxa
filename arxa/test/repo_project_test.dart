// repo_project + moodboard_check — the arxa law tests.
//
// repo mode: marker resolution, 8-stage skeleton, README authorship, the
// ~/.arxa shadow hard error, kind validation, sync. moodboard: totals
...
void main() {
  late Directory tmp;
  late Directory home; // isolated ARXA_HOME

  setUp(() {
    tmp = Directory.systemTemp.createTempSync('arxa_repo_law');
    home = Directory('${tmp.path}/home')..createSync();
    arxaHomeOverride = home.path;
  });
  ...
  group('ensureRepoProject', () {
    test('creates marker + all 8 stage folders + READMEs', () {
      ...
      expect(p.kind, 'site');
      final marker = jsonDecode(File('${tmp.path}/landing/arxa.json').readAsStringSync());
    ...
    test('HARD ERROR when a ~/.arxa shadow exists — the law', () {
      ...
      File('${tmp.path}/landing/arxa.json').writeAsStringSync(
          '{"name": "landing", "kind": "site", "targets": [], "locales": []}');
      expect(() => resolveRepoProject('${tmp.path}/landing'),
          throwsA(isA<StateError>().having(
              (e) => e.message, 'message', contains('arxa law'))));
    });
  });
  ...
  group('findRepoProject', () {
    ...
    test('a corrupt marker is skipped, not fatal', () {
      ...
      File('${tmp.path}/broken/arxa.json').writeAsStringSync('not json');
  ...
  group('updateRepoProject', () {
    test('syncs kind/targets/locales and preserves the name', () {
      ...
      final m = jsonDecode(File('${tmp.path}/landing/arxa.json').readAsStringSync()) as Map;
  ...
  group('commissionMain', () {
    Directory seedApp() {
      ...
      File('${app.path}/arxa.json').writeAsStringSync(jsonEncode({
        'name': 'site',
        'kind': 'site',
        'targets': ['web'],
        'locales': ['en', 'fr'],
