...
//
// Ports skills/arxa-scaffolder/scaffold.py `_self_test` to package:test in a
...
void main() {
  ...
  setUp(() {
    ...
    configPath = '${tmp.path}/arxa.config.json';
  ...
  group('design-system.md: token-exact emission from structure@2', () {
    ...
    test('theme present -> authored swatches + accent API, no placeholder', () {
      ...
      for (final doc in [surfaceDoc(app), shellDoc(app)]) {
        ...
        expect(doc, contains("arxaKitAccentByName('cyan')"),
            reason: 'default swatch reachable through the real API');
        expect(doc, contains('arxaKitDefaultAccent'));
        // The placeholder must be GONE — that is the whole point of Inc 6.
        expect(doc,
            isNot(contains('<!-- builder: ArxaKitColors.* tokens this surface uses -->')),
            reason: 'placeholder replaced by token-exact guidance');
      }
    });

    test('fonts present -> declared families + arxaKitFontById, no placeholder', () {
      final des = plantDesign('${tmp.path}/d', struct: themed());
      final app = '${tmp.path}/app1';
      expect(scaffold(des, app, ['macos'], derivationPath, configPath), 0);
      ...
      for (final doc in [surfaceDoc(app), shellDoc(app)]) {
        ...
        expect(doc, contains("arxaKitFontById('lexend').cssName"));
    ...
    // The live designs/arxa-studio/structure.json carries `theme` but NO
    ...
    test('fonts absent -> Type placeholder survives verbatim', () {
      ...
      for (final doc in [surfaceDoc(app), shellDoc(app)]) {
        ...
        expect(doc, isNot(contains('arxaKitFontById')),
    ...
    test('structure@1 (no theme, no fonts) -> both placeholders survive', () {
      ...
      for (final doc in [surfaceDoc(app), shellDoc(app)]) {
        expect(doc,
            contains('<!-- builder: ArxaKitColors.* tokens this surface uses -->'));
        expect(doc, contains('<!-- builder: TextTheme roles -->'));
        expect(doc, isNot(contains('arxaKitAccentByName')),
  ...
  group('macos scaffold -> [desktop] -> 3 files/surface', () {
    ...
    test('shell-level design-system.md + *_chrome.dart', () {
      ...
      expect(ds.contains('## Palette') && ds.contains('ArxaKitColors'), isTrue,
          reason: 'shell design-system.md carries Palette + ArxaKitColors (S4)');
  ...
  group('kit-manifest sidecar (D8)', () {
    ...
    const resolved = [
      {'id': 'core', 'package': 'arxa_kit_core', 'provenance': 'declared', 'auto': false},
      {'id': 'maps', 'package': 'arxa_kit_maps', 'provenance': 'declared', 'auto': false},
      {'id': 'data', 'package': 'arxa_kit_data', 'provenance': 'inferred', 'auto': true},
    ];
  ...
  group('red-first test seeds (behavior-TDD canon)', () {
    ...
    test('no story map -> single placeholder skipped test, run-intake comment', () {
      ...
      final t = File('$app/test/viewmodels/projects_home_viewmodel_test.dart')
          .readAsStringSync();
      expect(t, contains('run `arxa intake` first'));
