...
// (arxa lens shoot), which the verification step of this slice performs.
...
final String _fixture = p.absolute('../skills/arxa-designer/examples/hello-hda');
...
void main() {
  group('design dial over a real server', () {
    ...
    test('framed CSS lands in HEAD so an hx-boost swap keeps it', () async {
      ...
      expect(code, 200);
      expect(html, contains('__arxa_framed'));
      final headEnd = html.indexOf('</head>');
      expect(headEnd, greaterThan(0));
      expect(html.indexOf('__arxa_framed'), lessThan(headEnd),
  ...
  //
  // `DesignServer.start` builds its dial store from ~/.arxa/supabase when
