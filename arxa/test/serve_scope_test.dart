...
// is not: every design server in this repo serves `designs/arxa-studio`, so
...
Future<Process> _fakeServer() => Process.start(
    '/bin/sh', ['-c', 'while :; do sleep 1; done', 'arxa-design-serve-probe']);
...
void main() {
  ...
  // Real `arxa design serve` subprocesses on spare ports (4370+). 4319 is the
  ...
  group('real design servers coexist', () {
    ...
    Future<Process> serve(String dir, int port) async {
      final proc = await Process.start(
          Platform.resolvedExecutable, ['run', 'bin/arxa.dart', 'design', 'serve', dir, '--port', '$port'],
