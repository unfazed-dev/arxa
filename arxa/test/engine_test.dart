...
import 'package:arxa/gateway.dart' show TokenMinter;
import 'package:path/path.dart' as p;
import 'package:test/test.dart';

/// Walks up from the cwd to the repo root (where config/arxa.config.json
/// lives), same convention as bin/arxa.dart.
String repoRoot() {
  var root = Directory.current.path;
  while (!File(p.join(root, 'config', 'arxa.config.json')).existsSync()) {
    final parent = p.dirname(root);
    if (parent == root) fail('config/arxa.config.json not found above $root');
    root = parent;
  }
  return root;
}
...
void main() {
  group('stage registry', () {
    test('returns the 14 Dart gates in dependency order', () {
  ...
  group('headless runner (fake kimi on PATH)', () {
    ...
    test('runs print mode, writes manifest + scorecard, keeps stderr',
      ...
      expect(config, contains('api_key = "arxa-stage-token-TODO"'));
