...
import 'package:arxa/design_server/worker.dart' show JsWorker;
...
void main() {
  ...
  group('LensDaemon lifecycle', () {
    ...
    test('a live daemon is a protected dir; a stopped one is not', () async {
      ...
      // is the sweep's orphan signal for over-age arxa-cdp- Chromes.
      final s = await LensDaemon.start();
      try {
    ...
    test('a stale state file is reaped, and acquire falls back to a launch',
      ...
      // what a stray `pkill -f arxa-cdp-` does.
      final s = await LensDaemon.start();
  ...
  group('sweep classification (synthetic ps, nothing is killed)', () {
    ...
    final workerDir = '$tmp/arxa-design-worker-AAA111';
    final cdpDir = '$tmp/arxa-cdp-BBB222';
