...
// (~/.arxa/projects/<name>/intake) instead of the studio: answers.json,
...
GateResult intakeGate(GateContext ctx, {String? project}) {
  ...
  if (!Directory(designRoot).existsSync() &&
      Directory('$repoRoot/designs/arxa-studio').existsSync()) {
    designRoot = '$repoRoot/designs/arxa-studio';
  }
  ...
  if (project != null) {
    ...
    // path-traversal name from escaping ~/.arxa/projects), then existence.
    ...
    if (!Directory(projectDir(project)).existsSync()) {
      return GateResult.env('intake: no such project "$project" — nothing at '
          '${projectDir(project)} (run arxa project init $project)');
    }
  ...
  if (sourceLabel == 'intake answers' && briefPath != null) {
    const chainHint = 're-emit via the chain: arxa emit story-map '
  ...
  if (!File(registryPath).existsSync()) {
    ...
    fail('$sourceLabel has ${sourceIds.length} surface(s) but registry not found '
        'at $registryPath — run arxa intake emit to seed it');
    return GateResult.fail('intake: FAIL ($fails check(s))', details);
  }
