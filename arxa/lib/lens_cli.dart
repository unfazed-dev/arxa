// lens_cli — `arxa lens <verb>` dispatcher.
//
...
Native verbs (arxa lens native <family> <verb> …):
...
/// Entry point for `arxa lens`. Returns the process exit code.
Future<int> runLensCli(List<String> args) async {
  ...
  switch (verb) {
    ...
    case 'text-diff':
      return _textDiff(a);
    default:
      stderr.writeln('arxa lens: unknown verb "$verb"');
      stderr.writeln(_usage);
      return 2;
  }
}
...
int _usageErr(String syntax) {
  stderr.writeln('arxa lens: usage: $syntax');
  return 2;
}
...
Future<int> _daemon(_Args a) async {
  ...
  switch (sub) {
    ...
    case 'status':
      ...
      return alive ? 0 : 1;
    default:
      stderr.writeln('usage: arxa lens daemon <start|status|stop>');
      return 64;
  }
}
...
/// The built-in ladder (mirrors skills/arxa-designer/runtime/ladder.json).
...
/// Resolve the active rungs: --rungs > $ARXA_LADDER > --artifact
...
List<_Rung> _resolveRungs(_Args a, String repoRoot) {
  ...
  } else if (Platform.environment['ARXA_LADDER'] != null &&
      Platform.environment['ARXA_LADDER']!.isNotEmpty) {
    requested = Platform.environment['ARXA_LADDER']!
        .split(',')
        .map((s) => s.trim())
        .where((s) => s.isNotEmpty)
        .toList();
...
/// Load the ladder from skills/arxa-designer/runtime/ladder.json, falling
/// back to [_defaultLadder] when the file is absent or malformed.
List<_Rung> _loadLadder(String repoRoot) {
  final f = File('$repoRoot/skills/arxa-designer/runtime/ladder.json');
...
Future<int> _runNative(List<String> args) async {
  if (args.isEmpty) {
    stderr.writeln('arxa lens native: missing <family> (adb|ios|macos|flutter)');
    stderr.writeln(_usage);
    return 2;
  }
  switch (args.first) {
    ...
    default:
      stderr.writeln('arxa lens native: unknown family "${args.first}"');
      return 2;
  }
}
...
Future<int> _nativeAdb(_Args a) async {
  if (a.positional.isEmpty) {
    stderr.writeln('arxa lens native adb: missing verb '
  ...
  try {
    switch (verb) {
      ...
      case 'ui-tree':
        ...
        return 0;
      default:
        stderr.writeln('arxa lens native adb: unknown verb "$verb"');
        return 2;
    }
  } on LensNativeException catch (e) {
...
Future<int> _nativeIos(_Args a) async {
  if (a.positional.isEmpty) {
    stderr.writeln(
        'arxa lens native ios: missing verb (shot|record|devices|open-url|status-bar|appearance)');
    return 2;
  }
  final verb = a.positional.first;
  ...
  try {
    switch (verb) {
      ...
      case 'appearance':
        ...
        return 0;
      default:
        stderr.writeln('arxa lens native ios: unknown verb "$verb"');
        return 2;
    }
  } on LensNativeException catch (e) {
...
Future<int> _nativeMacos(_Args a) async {
  if (a.positional.isEmpty) {
    stderr.writeln('arxa lens native macos: missing verb (only "shot")');
    return 2;
  }
  final verb = a.positional.first;
  if (verb != 'shot') {
    stderr.writeln('arxa lens native macos: only "shot" is supported (got "$verb")');
    return 2;
  }
  if (a.positional.length < 2) {
...
Future<int> _nativeFlutter(_Args a) async {
  if (a.positional.isEmpty) {
    stderr.writeln('arxa lens native flutter: missing verb '
    ...
    try {
      switch (verb) {
        ...
        case 'flag':
          ...
          return 0;
        default:
          stderr.writeln('arxa lens native flutter: unknown verb "$verb"');
          return 2;
      }
    } finally {
      await vm.dispose();
