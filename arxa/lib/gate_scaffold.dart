...
//         and the declared ArxaKitDataBackend has its artifacts (apps without the
//         tool have no data layer — skip, never false-fail)
//   S8  — peer-service registration (a registered ArxaKit* service's
//         arxaKitLocator<Y>() peers are also registered;
//         setupArxaKitSnackbars() called when used)
...
GateResult scaffoldGate(GateContext ctx) {
  ...
  // layer keeps `arxa gate scaffold` / phase runs honest.
  ...
  // Apps with a data layer declare ArxaKitTableSchema(s) under lib/data/schemas/
  ...
  if (!generateTool.existsSync()) {
    ...
    if (d1Bad == 0) {
      ...
      // ArxaKitDataConfig in lib/app/app_data.dart) must have its artifacts in
      ...
      if (backend == null) {
        ...
        for (final artifact in backendArtifacts[backend] ?? const <String>[]) {
          if (!File('$app/data/generated/$artifact').existsSync()) {
            fail('backend coherence (D1): app boots ArxaKitDataBackend.$backend '
  ...
  for (final s in shells) {
    ...
    if (!ds.existsSync()) {
      ...
      final hasColor = RegExp(r'ax[A-Z][A-Za-z0-9]*|ArxaKitColors').hasMatch(dst);
      if (hasHeading && hasColor) {
        ...
        fail("$s: design-system.md lacks a '## Palette' / '## Tokens' heading "
            'with a kit color reference (ax*/ArxaKitColors) (S4)');
      }
    }
...
void _runS8(String app, void Function(String) ok, void Function(String) warn,
  ...
  for (final p in pkgs.cast<Map<String, dynamic>>()) {
    ...
    if (!name.startsWith('arxa_kit_')) continue;
  ...
  var bad = 0;
  for (final kt in have.where((t) => t.startsWith('ArxaKit')).toList()..sort()) {
    final hit = index[kt];
    if (hit == null) continue;
    // read comment-free source: an arxaKitLocator<> inside a doc comment is not a peer edge
    final peers = _captures(_stripComments(hit.text),
            RegExp(r'arxaKitLocator<([A-Z][A-Za-z0-9_]*)>'))
        .toSet()
        .toList()
      ..sort();
    for (final peer in peers) {
      ...
      fail('$kt is registered but its peer $peer is not (S8) — $kt calls '
          'arxaKitLocator<$peer>(); add LazySingleton(classType: $peer) to @StackedApp '
          '[${_basename(hit.file)}]');
      bad++;
    }
  }

  // setupArxaKitSnackbars(): the kit registers a SnackbarConfig per ArxaKitSnackbarType.
  // Asserting the CALL, not a main.dart template.
  if (have.contains('ArxaKitNotificationService')) {
    var called = false;
    for (final x in _findFiles(
        Directory('$app/lib'), (f) => f.path.endsWith('.dart'))) {
      if (_readOrEmpty(x).contains('setupArxaKitSnackbars')) {
        called = true;
        break;
      }
    }
    if (!called) {
      fail('ArxaKitNotificationService is registered but setupArxaKitSnackbars() is never '
          'called in lib/ (S8) — the ArxaKitSnackbarType SnackbarConfig variants stay '
  ...
  if (bad == 0) ok('peer services registered for every ArxaKit* service (S8)');
}
...
/// (does this class resolve `arxaKitLocator<X>()`?) so it must read comment-free source — a
