// The designer selftest, ported from skills/arxa-designer/selftest.sh
// (Task 21). `arxa design selftest [artifact-dir] [--negative]`.
//
...
import 'package:arxa/design_selftest_kit_catalog_mirror.dart';
...
import 'package:arxa/gate_design_widgets.dart'
...
class _Route {
  ...
  /// Project-backed shells render from the overlaid `~/.arxa` project, so
...
List<_Check> _buildChecks({required bool skipRender}) {
  return [
    ...
    // another agent held arxa/lib) — the registration is here because
    ...
    _Check(_lRender, _Section.render, (art, skill, src) async {
      ...
      try {
        ...
        // overlaid (the studio live-reads ~/.arxa) when one exists — the
...
///   defaults to the repo's `skills/arxa-designer`.
...
/// `arxa design selftest [artifact-dir] [--negative]` — returns the exit code.
