...
//
// WHY THIS EXISTS. `arxa design serve` binds loopback and has always treated
...
//   * The callers in that category are the arxa CLI, the probe suite, lens
...
class BrowserTrust {
  ...
  /// (`~/.arxa/trusted-origins` + `--trusted-origin`) governs both who may
  ...
  /// `GET /__projects` enumerates every project in ~/.arxa; a cross-site
  ...
  static bool guardedRequest(String method, String path) {
    ...
    // ~/.arxa and state changes; a fixed module bundle enumerates
    ...
    if (secFetchSite != null) {
      ...
      if (!ownPage.contains(secFetchSite) && allowedOrigin == null) {
        return const TrustVerdict.deny(
            ...
            '~/.arxa/trusted-origins (one per line), or pass '
