...
import 'package:arxa/lens/daemon.dart' show LensDaemon;
import 'package:arxa/design_server/l10n.dart' show parseArb;
...
/// only thing separating a live `arxa-cdp-` Chrome from a leaked one, so a
...
Future<_Prefetch> _scanArtifact(String artifactDir, String origin,
  ...
  // ---- the live-read project overlay (~/.arxa/projects/<name>/) ---------
...
Future<String> _buildRenderBundle(
  ...
  if (nodeModulesDir == null || !Directory(nodeModulesDir).existsSync()) {
    throw StateError(
        ...
        'skills/arxa-designer/runtime/node_modules. Run `npm install` there.');
  }
  ...
  final buildDir =
      await Directory.systemTemp.createTemp('arxa-render-build');
  try {
...
class _ChromeHandle {
  ...
  /// Markers identifying a Chrome arxa launched: our own `--user-data-dir`
  ...
  /// own Chrome under `arxa-cdp-` and leaks it by exactly the same routes.
  ...
  static const _uddWorker = 'arxa-design-worker-';
  static const _uddCdp = 'arxa-cdp-';
  ...
  ///
  ///  * `arxa-design-worker-` — a live worker is always a child of its own
  ///    Dart server, so ppid==1 is a sound orphan signal on its own.
  ///  * `arxa-cdp-` — `CdpClient` launches with `--no-startup-window`, and
  ...
  static void sweepOrphans() {
    ...
    for (final pid in orphans) {
      try {
        ...
        stderr.writeln('design serve: reaped orphaned arxa chrome $pid');
      } catch (_) {}
    }
...
/// The arxa package root (…/arxa/arxa), resolved through the
/// RUNNING isolate's package config — the cwd-independent anchor.
///
/// Skills invoke `arxa` from wherever the engagement lives: a client
/// checkout, ~/.arxa, anywhere. The cwd walks in this file only find the
...
/// config of the script the wrapper execs (`dart run …/arxa/bin/arxa.dart`)
...
String? arxaPackageDir() {
  Uri? uri;
  try {
    uri = Isolate.resolvePackageUriSync(Uri.parse(
        'package:arxa/design_server/worker_assets/worker_page.html'));
  } catch (_) {
    uri = null;
  }
  if (uri != null) {
    // …/arxa/lib/design_server/worker_assets/worker_page.html → …/arxa
    var dir = File.fromUri(uri).parent; // worker_assets
    for (var i = 0; i < 3; i++) {
      dir = dir.parent; // design_server → lib → arxa
    }
    if (dir.existsSync()) return dir.path;
  }
  // AOT exe: no package config at runtime, so the resolver above is null and
  // a cwd walk from a client repo never reaches arxa. The compiled binary
  // lives inside the repo (.build/arxa), so the exe path itself is the
...
/// looking for `arxa/pubspec.yaml`. Split out so tests can drive the AOT
...
String? packageDirFromScript(Uri script) {
  ...
  for (var i = 0; i < 12; i++) {
    final pkg = p.join(dir.path, 'arxa');
...
/// bundled inside the arxa package.
///
/// Anchor 1 is the package itself ([arxaPackageDir]) — cwd-independent.
/// Anchor 2 is the original walk up from [from] looking for
/// `arxa/pubspec.yaml`, kept for shells where no package config exists
/// but cwd sits inside the repo.
String? findWorkerAssetsDir([String? from]) {
  final pkg = arxaPackageDir();
  if (pkg != null) {
  ...
  var dir = Directory(from ?? Directory.current.path);
  for (var i = 0; i < 12; i++) {
    final candidate = File(p.join(dir.path, 'arxa', 'pubspec.yaml'));
    if (candidate.existsSync()) {
      final assets = p.join(dir.path, 'arxa', 'lib', 'design_server',
