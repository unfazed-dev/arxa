...
CmdResult? routeTextToArbForFile(
  ...
  if (!arbDir.existsSync()) {
    return CmdResult(5, stderrLines: [
      'arxa design patch: "$id" reads t()-backed copy but no l10n/ directory exists under ${dir.path}'
    ]);
  }
  ...
  if (locale != null) {
    ...
    if (named.isEmpty) {
      return CmdResult(5, stderrLines: [
        'arxa design patch: no l10n/*.arb file for locale "$locale" (found: ${targets.map((a) => p.basename(a.path)).join(', ')})'
      ]);
    }
    targets = named;
  }
  ...
  if (key != null) {
    ...
    if (missing.isNotEmpty) {
      return CmdResult(5, stderrLines: [
        'arxa design patch: key "$key" missing in: ${missing.join(', ')} - add it there and retry'
      ]);
    }
  ...
  if (was == null || was.isEmpty) {
    return CmdResult(5, stderrLines: [
      'arxa design patch: --text refused: "$id" reads a dynamic t() '
  ...
  if (matchedKeys.isEmpty) {
    return CmdResult(5, stderrLines: [
      'arxa design patch: --text refused: no ARB key matches --was for '
          '"$id"${loose.isEmpty ? ' (stale anchor? draft and l10n have diverged)' : ' ending in "$tail"'}'
    ]);
  }
  if (matchedKeys.length > 1) {
    return CmdResult(5, stderrLines: [
      'arxa design patch: --text ambiguous for "$id" - '
  ...
  if (missing.isNotEmpty) {
    return CmdResult(5, stderrLines: [
      'arxa design patch: key "$k" missing in: ${missing.join(', ')} - add it there and retry'
    ]);
  }
  final how = t.isEmpty
      ? 'value match'
...
CmdResult? _routeConditional(File hit, Directory dir, int at, String id,
  ...
  if (total == 0) {
    if (was.isEmpty) {
      return CmdResult(5, stderrLines: [
        'arxa design patch: --text refused: "$id" is a conditional '
    ...
    return CmdResult(5, stderrLines: [
      'arxa design patch: --text refused: the conditional on "$id" has '
  ...
  if (total > 1) {
    ...
    return CmdResult(5, stderrLines: [
      'arxa design patch: --text ambiguous for "$id": $total branches '
          'match --was:',
      ...parts,
    ]);
  }
...
CmdResult? routeTextToSeed(
  ...
  if (resolved.isEmpty) {
    return CmdResult(5, stderrLines: [
      'arxa design patch: --text refused: "$id" reads seed data (binding ends in "$hint") but no seed path matched${was == null || was.isEmpty
  ...
  if (resolved.length > 1) {
    return CmdResult(5, stderrLines: [
      'arxa design patch: --text ambiguous for "$id" — ${resolved.length} seed paths match:',
      ...seen,
  ...
  if (locale != null) {
    if (!locs.containsKey(locale)) {
      return CmdResult(5, stderrLines: [
        'arxa design patch: locale "$locale" has no slice in the "$stem" seed spine (${allLocales.join(', ')})'
      ]);
    }
    writeLocales = [locale];
  ...
  for (final loc in writeLocales) {
    ...
    for (final (file, pathSegs) in targets) {
      ...
      if (span == null) {
        return CmdResult(5, stderrLines: [
          'arxa design patch: seed path "${segs.join(".")}" missing or not a ' 'string in ${p.relative(file.path, from: dir.path)}'
        ]);
      }
...
/// `arxa design patch <artifactDir> <id> [--set n=v]… [--rm n]…`
...
CmdResult patchMain(List<String> args) {
  ...
  if (!dir.existsSync()) {
    return CmdResult(2,
        stderrLines: ['arxa design patch: no such dir: ${positional[0]}']);
  }
  ...
  if (tokens.isNotEmpty) {
    ...
    if (css == null) {
      return CmdResult(5, stderrLines: [
        'arxa design patch: no tokens.css / :root stylesheet found under '
            '${dir.path}'
      ]);
    }
  ...
  if (stampHits.length > 1) {
    return CmdResult(4, stderrLines: [
      'arxa design patch: id "$id" found in ${stampHits.length} files '
  ...
  if (stampHits.length == 1) {
    ...
    if (src.indexOf('$attr="$id"', at + 1) >= 0) {
      return CmdResult(4, stderrLines: [
        'arxa design patch: $attr="$id" appears twice in '
    ...
    if (sites.isEmpty) {
      return CmdResult(3, stderrLines: [
        'arxa design patch: no element carries data-el="$id" and no '
    ...
    if (sites.length > 1) {
      ...
      return CmdResult(4, stderrLines: [
        'arxa design patch: authored identity "$id" resolves to '
    ...
    return CmdResult(3, stderrLines: [
      'arxa design patch: no element carries $attr="$id" '
          'under ${positional[0]}'
    ]);
  }
  ...
  if (edits.text != null) {
    ...
    if (innerTrim.startsWith('{') && innerTrim.endsWith('}')) {
      ...
      return CmdResult(5, stderrLines: [
        'arxa design patch: --text refused: "$id" is expression-backed '
  ...
  if (res.error != null) {
    return CmdResult(5, stderrLines: ['arxa design patch: ${res.error}']);
  }
