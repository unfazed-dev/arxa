...
/// docs/plans/arxa-engine-llm-fabric.md). arxa owns the pipeline FSM:
...
class Engine {
  ...
  /// ARXA_STAGE_TOKEN env override wins (manual runs); without a minter
  ...
  String stageToken(Stage stage) {
    final override = Platform.environment['ARXA_STAGE_TOKEN'];
    if (override != null) return override;
    final minter = _minter;
    final tier = stage.tier;
    if (minter == null || tier == null) return 'arxa-stage-token-TODO';
    return minter.mint(
  ...
  Directory _writeKimiHome(String token) {
    final dir = Directory.systemTemp.createTempSync('arxa-kimi-');
    File(p.join(dir.path, 'config.toml')).writeAsStringSync('''
default_model = "arxa/stage"
