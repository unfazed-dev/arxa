// project_cli — CLI entry for `arxa project`.
//
//   arxa project init <name> [--targets ios,android] [--locales en]
//   arxa project init --repo <dir> --kind site|app [--name n]
//                        [--targets web] [--locales en,fr]
//   arxa project sync <dir>  (kind/targets/locales from intake answers)
//   arxa project resolve     (report repo-mode binding from cwd)
//   arxa project use <name>
//   arxa project list
//
// The arxa law in two modes: `init <name>` creates an arxa-NATIVE
// project under ARXA_HOME (no repo behind it); `init --repo` binds an
...
// ~/.arxa shadow is a hard error (repo_project.dart).
//
// All commands honor ARXA_HOME (see project.dart). Exit: 0 ok · 2 usage.

import 'dart:convert';
import 'dart:io';

import 'package:arxa/project.dart';
import 'package:arxa/repo_project.dart';
...
int _init(List<String> args) {
  ...
  for (var i = 1; i < args.length; i++) {
    switch (args[i]) {
      ...
      default:
        stderr.writeln('arxa project init: unknown flag ${args[i]}');
        return 2;
    }
  }
  try {
    ...
    stderr.writeln('arxa project init: ${e.message}');
    return 2;
  }
...
int _initRepo(List<String> args) {
  ...
  for (var i = start; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--locales':
        ...
        locales = args[++i].split(',').map((s) => s.trim()).toList();
      default:
        stderr.writeln('arxa project init --repo: unknown flag ${args[i]}');
        return 2;
    }
  }
  if (dir == null) {
    stderr.writeln('arxa project init --repo: --repo <dir> is required');
    return 2;
  }
  if (kind == null) {
    stderr.writeln('arxa project init --repo: --kind site|app is required '
  ...
  try {
    ...
    stderr.writeln('arxa project init --repo: ${e.message}');
    return 2;
  } on StateError catch (e) {
    stderr.writeln(e.message);
    return 2;
...
int _sync(List<String> args) {
  if (args.isEmpty) {
    stderr.writeln('arxa project sync: usage: sync <app-dir>');
    return 2;
  }
  final dir = args.first;
  final answersFile = File('$dir/intake/answers.json');
  if (!answersFile.existsSync()) {
    stderr.writeln('arxa project sync: no intake/answers.json under $dir — '
        'run intake first');
    return 2;
  }
  ...
  try {
    ...
    stderr.writeln('arxa project sync: cannot read answers ($e)');
    return 2;
  }
  String? kind;
  ...
  if (kind == null && targets == null && locales == null) {
    stderr.writeln('arxa project sync: nothing to sync (no product.kind, '
  ...
  try {
    ...
    stderr.writeln('arxa project sync: ${e.message}');
    return 2;
  }
  final done = [
...
int _use(List<String> args) {
  ...
  try {
    ...
    stderr.writeln('arxa project use: ${e.message}');
    return 2;
  }
...
int _missing(String flag) {
  stderr.writeln('arxa project: $flag requires a value');
  return 2;
}
...
Subcommands:
  init <name> [--targets ios,android] [--locales en]
               Create ~/.arxa/projects/<name>/ (intake/design/build/settings)
               — arxa-NATIVE projects only; never for a repo-backed app
  init --repo <dir> --kind site|app [--name n] [--targets web]
               [--locales en,fr]
               Bind an EXISTING repo app dir as the single home of its
               pipeline state: arxa.json marker + the 8 stage folders
               (intake moodboard design scaffold build test review deploy)
               with kind-aware READMEs. HARD ERROR when a same-named project
               shadows it in ~/.arxa/projects/ — the arxa law
  sync <dir>   Push kind/targets/locales from <dir>/intake/answers.json
               into the arxa.json marker
  resolve      Report repo-mode binding from cwd (or ~/.arxa fallback)
