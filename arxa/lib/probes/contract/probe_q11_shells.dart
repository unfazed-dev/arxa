...
//
// Suite: kSuiteContract. A contract probe "asserts the arxa opinion against ANY
// served design"; this one asserts the arxa opinion — the Q8 manifest — against a
...
/// The runner may be invoked from `arxa/` or from the repo root.
Directory? _repoRoot() {
...
void _verdictInspectAttrs(ProbeReport report, Directory root, Directory showcaseLib) {
  ...
  // own invention:
  //   (a) the Dart shape is `ArxaKitInspectAttrs` in kit core;
  //   (b) the anatomy-node vocabulary is a CLOSED set at
  //       skills/arxa-scaffolder/kind-resolution.registry.json (v1.2.0).
  ...
  if (vocab.isEmpty) {
    report.check(
      ...
      'skills/arxa-scaffolder/kind-resolution.registry.json#/anatomyNodes/vocabulary. '
...
Set<String> _anatomyVocabulary(Directory root) {
  final f = File('${root.path}/skills/arxa-scaffolder/kind-resolution.registry.json');
