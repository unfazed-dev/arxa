...
//      const BASE = process.env.ARXA_BASE || 'http://localhost:4319';
...
//   3. --base / --port beat ARXA_BASE, which beats the 4319 default.
//
...
/// applicable checks lead, so a design that violates the arxa opinion fails
...
class ProbeTarget {
  ...
  /// How it was resolved: `--base`, `--port`, `ARXA_BASE` or `default`.
  final String via;
...
ProbeTarget resolveTarget(List<String> args, {Map<String, String>? env}) {
  ...
  final envBase = environment['ARXA_BASE'];
  final resolved = base ??
...
/// global `~/.arxa/current` marker, which lies in both directions:
...
class Probe {
  ...
  ///
  /// [kSuiteContract] — asserts the arxa opinion against ANY served design,
