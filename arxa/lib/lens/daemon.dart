...
class LensDaemon {
  ...
  /// `ARXA_LENS_DAEMON_STATE` overrides the path for subprocesses.
  static String statePath() {
    if (statePathOverride != null) return statePathOverride!;
    final override = Platform.environment['ARXA_LENS_DAEMON_STATE'];
    if (override != null && override.isNotEmpty) return override;
    final home = Platform.environment['HOME'] ??
        Platform.environment['USERPROFILE'] ??
        '.';
    return p.join(home, '.arxa', 'lens-daemon.json');
  }
