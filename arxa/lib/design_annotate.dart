...
/// `arxa design annotate <artifactDir>` — derive the semantic layer on
...
CmdResult annotateMain(List<String> args) {
  if (args.isEmpty) {
    return CmdResult(2, stderrLines: [
      'arxa design annotate: missing <artifactDir> '
  ...
  if (!dir.existsSync()) {
    return CmdResult(2,
        stderrLines: ['arxa design annotate: no such dir: ${args.first}']);
  }
  final files = dir
  ...
  if (files.isEmpty) {
    return CmdResult(2,
        stderrLines: ['arxa design annotate: no .tsx under ${args.first}']);
  }
  final out = <String>[];
