...
//
// Tool-owned like the emit manifest: `arxa project init --repo` (re)writes
...
+ style tokens). The `arxa-designer` skill consumes the commission as a
binding contract before authoring anything; locked intake requirements are
non-deferrable. Serve with `arxa design serve <this-dir>`; verify with
`arxa design lint` and the lens. When intake locks motion, the lens must
...
What starts it: a scaffolded site in `../scaffold/`. The `arxa-builder`
...
What starts it: a served site (design or build). The `arxa-tester`
...
`arxa-deployer` skill owns the mechanics. A live domain is PRODUCTION:
...
What starts it: a scaffolded app with views filled by the `arxa-builder`
...
What starts it: a buildable app. The `arxa-tester` skill owns all three
