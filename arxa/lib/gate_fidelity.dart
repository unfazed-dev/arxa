...
//   1. Map legality: `fidelity` values in config/arxa.config.json must be
...
//   3. Const-drift: the kit reads ARXA_FIDELITY via String.fromEnvironment
//      with defaultValue `mix`. Every emitted `ARXA_FIDELITY=<v>` under the
...
GateResult fidelityGate(GateContext ctx) {
  ...
  final defineRe = RegExp(r'ARXA_FIDELITY=([A-Za-z_]+)');
  ...
  for (final v in foundValues) {
    ...
    if (!legal.contains(v)) {
      failures.add('fidelity.define_drift: emitted ARXA_FIDELITY=$v matches '
  ...
  if (nonMix.isNotEmpty && defineHits.isEmpty) {
    failures.add('fidelity.define_drift: '
        ...
        'no ARXA_FIDELITY= define emitted anywhere under $appRoot — the kit '
