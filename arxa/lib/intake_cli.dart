...
int intakeMain(List<String> args) {
  ...
  switch (cmd) {
    ...
    default:
      stderr.writeln('arxa intake: unknown subcommand "$cmd"');
      _usage();
      return 2;
  }
}
...
void _usage() {
  stderr.writeln('''
Usage: arxa intake <subcommand> [options]
...
Subcommands:
  emit --answers <f> [--project <name>] [--brief-out p] [--registry-out p]
                        ...
                        ~/.arxa intake/ dir (answers/brief/registry/flows),
...
int _cmdValidate(List<String> args) {
  if (args.isEmpty) {
    stderr.writeln('arxa intake validate: missing <answers> path');
    return 2;
  }
  ...
  for (var i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--registry-out':
        ...
        break;
      default:
        stderr.writeln('arxa intake emit: unknown flag ${args[i]}');
        return 2;
    }
  }
  if (answersPath == null) {
    stderr.writeln('arxa intake emit: --answers <f> is required');
    return 2;
  }
  final Map<String, dynamic> answers;
  try {
...
int _emitMissingValue(String flag) {
  stderr.writeln('arxa intake emit: $flag requires a value');
  return 2;
}
...
int _cmdFlows(List<String> args) {
  if (args.isEmpty || args.first != 'confirm') {
    stderr.writeln('arxa intake flows: only subcommand is "confirm"');
    return 2;
  }
  ...
  for (var i = 1; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--as':
        ...
        break;
      default:
        stderr.writeln('arxa intake flows confirm: unknown flag ${args[i]}');
        return 2;
    }
  }
  if (project == null || flowId == null || as == null) {
    stderr.writeln('arxa intake flows confirm: --project, --flow and --as are required');
    return 2;
  }
...
int _flowsMissing(String flag) {
  stderr.writeln('arxa intake flows confirm: $flag requires a value');
  return 2;
}
...
int _cmdSeed(List<String> args) {
  ...
  for (var i = 0; i < args.length; i++) {
    switch (args[i]) {
      case '--brief':
        if (i + 1 >= args.length) {
          stderr.writeln('arxa intake seed: --brief requires a value');
          return 2;
        }
        briefPath = args[++i];
        break;
      case '--registry-out':
        if (i + 1 >= args.length) {
          stderr.writeln('arxa intake seed: --registry-out requires a value');
          return 2;
        }
        registryOut = args[++i];
        break;
      default:
        stderr.writeln('arxa intake seed: unknown flag ${args[i]}');
        return 2;
    }
  }
  if (briefPath == null) {
    stderr.writeln('arxa intake seed: --brief <f> is required');
    return 2;
  }
  final res =
...
int _selfTest() {
  try {
    ...
    final tmp = Directory.systemTemp.createTempSync('arxa_intake_selftest');
    try {
