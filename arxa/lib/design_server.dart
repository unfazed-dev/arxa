...
const _msgServerError =
    'Something went wrong on the arxa server. The serve log has the detail.';
...
class _ServeArgs {
  ...
  List<String> trustedHosts = [
    ..._splitList(Platform.environment['ARXA_TRUSTED_HOSTS']),
  ];
  List<String> trustedOrigins = [
    ..._splitList(Platform.environment['ARXA_TRUSTED_ORIGINS']),
  ];
  String? error;
}
...
/// `~/.arxa/trusted-origins`, `#` starts a comment.
///
...
/// in, and `config/arxa.config.json` is the pipeline's SSOT — an operator
/// preference has no business in it. ~/.arxa is where arxa already keeps
...
List<String> readTrustedOriginsFile() {
  final f = File(p.join(arxaHome(), kTrustedOriginsFile));
...
class DesignServer {
  ...
  /// The dial's store: Supabase when ARXA_SUPABASE_URL +
  /// ARXA_SUPABASE_SERVICE_KEY are set, else the honest per-process memory
  ...
  /// tests); null when no arxa.json sits above the artifact dir.
  ...
  /// overlays the live-read project (.arxa/projects/name) — see
  ...
  static Future<DesignServer> start({
    ...
    if (dial) {
      stderr.writeln('[design-server] dial store: ${srv.dialStore.kind}'
          '${srv.dialStore.kind == 'memory' ? ' (set ARXA_SUPABASE_URL + ARXA_SUPABASE_SERVICE_KEY, or ~/.arxa/supabase, for the shared store)' : ''}');
  ...
  Future<void> _handle(HttpRequest req) async {
    try {
      ...
      if (!verdict.allowed) {
        ...
        // ONLY the 403s (2026-08-26). browser_trust has three deny paths and
        // they are not the same question: 421 asks "is this connection even
        // for me" (hostAllowed), the two 403s ask "may that page read me"
        // (browser_trust.dart:277 cross-site, :285 the older-browser variant).
        // The softening exists for ONE reason — a sandboxed frame's console —
        // which is the 403 question. A DNS-rebinding probe is a plain GET with
        // a forged Host and no Origin, i.e. the exact shape tested below, so
        // it used to match and turn its 421 into a 200: a security refusal
        // answered OK. Written as an ALLOW-list, not `!= 421`: a deny reason
        // added later must then be softened deliberately rather than by
        // default.
        if (method == 'GET' &&
            verdict.status == 403 &&
          ...
          // the operator to "add the origin to ~/.arxa/trusted-origins"
      ...
      // The dashboard's live project grid: every project in ~/.arxa with
      ...
      // #21): a contract probe asserts the arxa opinion against ANY served
      ...
      if (method == 'GET' && path == '/__projects') {
        ...
        req.response.write(jsonEncode({
          ...
          // `current` is a live read of the GLOBAL ~/.arxa/current marker,
          ...
          // (--project > ARXA_PROJECT > the marker as it stood then) and
      ...
      // arxa-studio's app.unknown) has authored where a whole-page miss
  ...
  static const String _kFramedCss = '<style id="__arxa_framed">'
  ...
  // fixture under ~/.arxa/projects/<name> re-scans and re-boots the worker.
...
Future<int> designServe(List<String> args) async {
  ...
  // The live-read project: --project wins, then ARXA_PROJECT, then the
  // ~/.arxa current project when it exists; none → artifact-only serving.
  final projectName = a.project ??
      Platform.environment['ARXA_PROJECT'] ??
...
List<String> humanReadyLines(
    ...
    required int port}) {
  final out = <String>[
    'arxa-designer serving $artifactDir',
    '→ $url',
  ];
...
Directory get serveRegistryDir =>
    Directory(p.join(Directory.systemTemp.path, 'arxa-designer-serve'));
...
/// every server in this repo serves the same studio design dir
/// (`designs/arxa-studio`; v2 held it 2026-08-16 → 2026-09-07), so
...
String? _findRuntimeVendorDir() {
  ...
  for (var i = 0; i < 12; i++) {
    final c =
        p.join(dir.path, 'skills', 'arxa-designer', 'runtime', 'vendor');
    if (Directory(c).existsSync()) return c;
