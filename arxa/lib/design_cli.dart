// `arxa design` — CLI entry for the ported designer runtimes (Task 19).
//
// Dispatches the six ported verbs and returns the process exit code (the
// caller — `bin/arxa.dart` — wraps the return in `exit(...)`). Exit codes
...
//
// Usage:
//   arxa design lint <artifact-dir>
//   arxa design check-ladder [--config <p>] [--doc <p>]
//   arxa design check-wiring <artifact-dir> <fragments|mutations-posted|urls-resolve|targets-exist>
//   arxa design pseudolocalize <artifact-dir>
//   arxa design vendor-fetch [--vendor <dir>]
//   arxa design doctor
//   arxa design serve <dir|name> [--port N] [--host H] [--json] [--no-watch]
//                                  [--trusted-host NAME]… [--trusted-origin URL]…
//   arxa design eject <artifact-dir> <out-dir> [--target=node|cloudflare|vercel] [--kits=a,b]
//   arxa design ds-check <projectDir> [--verbose]
//   arxa design record-asset <projectDir> <htmlPath> [flags]
//   arxa design record-asset <projectDir> --remove [<htmlPath>] [flags]
//   arxa design ds-import <dsDir> <projectDir> [--primary]
//   arxa design --self-test
...
import 'package:arxa/design_stamp.dart';
import 'package:arxa/design_selftest.dart';
import 'package:arxa/design_tools.dart';
import 'package:arxa/probes/probe_cli.dart';
...
Subcommands:
  ...
  probe <name…|all> [--port <n> | --base <url>] [--project <name>]
                                     ...
                                     (arxa design probe --help)
  --self-test                        Run the embedded invariant self-test

Note: `arxa design serve` runs the Dart design server (Task 20);
`shoot`/`console-check` are `arxa lens shoot`/`arxa lens check`.''';

/// Entry point for `arxa design`. Returns the process exit code.
Future<int> designMain(List<String> args) async {
  ...
  switch (cmd) {
    ...
    default:
      stderr.writeln("arxa design: unknown subcommand '$cmd'");
      stderr.writeln(_usage);
      return 2;
  }
}
