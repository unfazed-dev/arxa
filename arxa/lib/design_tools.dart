...
/// skills/arxa-designer/runtime/ — exit codes and stdout/stderr contracts
/// are preserved. `shoot.mjs` / `console-check.mjs` are deliberately NOT
/// ported (superseded by `arxa lens shoot` / `arxa lens check`); the
/// `verify-*.mjs` debug scripts are dropped (`lens check` covers them). See
/// plan §19.
///
/// SRI uses SHA-384; arxa has no crypto package with sha384 (only
...
export 'package:arxa/design_server/l10n.dart' show parsePlural, Plural, parseArb;
...
List<LintFinding> _clientJsCeilingFindings(String artifactDir,
  ...
  final vendorDir = Directory(p.join(
      repo, 'skills', 'arxa-designer', 'runtime', 'vendor'));
  var total = 0;
...
/// `arxa design lint <artifact-dir> [--coverage-b]` — exit 0 clean /
...
CmdResult designLint(List<String> args) {
  ...
  if (rest.isEmpty) {
    return CmdResult(2, stderrLines: const ['usage: arxa design lint <artifact-dir>']);
  }
...
/// `arxa design check-ladder [--config <p>] [--doc <p>]` — exit 0/1.
...
/// `arxa design check-wiring <artifact-dir> <property>` — exit 2 usage /
/// 1 problems / 0 ok.
CmdResult designCheckWiring(List<String> args) {
  if (args.length < 2 || !_wiringProps.contains(args[1])) {
    return CmdResult(2, stderrLines: [
      'usage: arxa design check-wiring <artifact-dir> <${_wiringProps.join('|')}>',
    ]);
  }
...
/// `arxa design pseudolocalize <artifact-dir>` — exit 64 usage / 66 nothing /
/// 0 wrote.
CmdResult designPseudolocalize(List<String> args) {
  if (args.isEmpty) {
    return CmdResult(64,
        stderrLines: const ['usage: arxa design pseudolocalize <artifact-dir>']);
  }
...
String vendorSriDoc(List<Map<String, String>> manifest,
  ...
  final b = StringBuffer()
    ..write('# Vendored client libraries (the `fetch.mjs` updater was archived;'
        ' re-vendor htmx + extensions via `arxa design vendor-fetch`)\n\n')
  ...
  b.write('\n## Local patches — files that DIVERGE from the upstream release\n\n'
      'Each entry below is re-applied automatically by `arxa design '
...
/// ships openssl as a base tool; arxa has no sha384 in-tree.
...
/// `arxa design vendor-fetch [--vendor <dir>]` — exit 0 vendored / 1 failed.
...
/// The designer skill's runtime/ dir resolved from the arxa INSTALLATION,
/// not the CWD: CWD walk-up first (in-repo use unchanged), then the running
/// script's location (the PATH wrapper runs
/// `dart run /…/arxa/arxa/bin/arxa.dart` from any folder — a
...
/// script's file. Walk-up stops at the repo marker config/arxa.config.json.
///
...
/// invoke `arxa` from a client checkout or ~/.arxa, and a CWD walk never
/// reaches arxa from there.
String? scriptRepoRoot() {
  for (final anchor in <Uri?>[
    () {
      try {
        return Isolate.resolvePackageUriSync(
            Uri.parse('package:arxa/design_tools.dart'));
      } catch (_) {
        return null;
      }
    }(),
    ...
    while (true) {
      if (File(p.join(dir.path, 'config', 'arxa.config.json'))
          .existsSync()) {
        return dir.path;
      }
...
/// `arxa design doctor` — exit 0 all-present / 1 missing.
...
// ══ eject (Task 20.5) ═══════════════════════════════════════════════════
// Ported from skills/arxa-designer/runtime/eject.mjs. Same steps (copy the
...
// a checked `arxa design serve . --no-watch` line: ejected artifacts run on
...
/// binary (skills/arxa-designer/runtime devDependency), fall back to npx
/// with an exact pinned version — never a floating `npx esbuild`.
(String, List<String>) _esbuildCmd(String repo) {
  final local = p.join(repo, 'skills', 'arxa-designer', 'runtime',
...
Future<CmdResult> designEject(List<String> args) async {
  ...
  if (positional.length < 2) {
    return CmdResult(2,
        stderrLines: const [
          'usage: arxa design eject <artifact-dir> <out-dir> '
  ...
  final ejectSrc = p.join(repo, 'skills', 'arxa-designer', 'runtime', 'eject');
...
void _generateRouteManifest(String artifactDir, String out) {
  ...
  File(p.join(runtimeDir.path, 'routes.js')).writeAsStringSync(
    ...
    '// Regenerate via `arxa design eject`.\n\n'
...
Map<String, String> _emitKitsAndEnv(String out, String repo, String target, List<String> kits) {
  ...
  final kitFacadesDir = p.join(repo, 'skills', 'arxa-designer', 'runtime', 'kit-facades');
  ...
  if (publishableKeys.isNotEmpty) {
    ...
    File(p.join(out, 'runtime', 'client_config.js')).writeAsStringSync(
      ...
      '// Regenerate via `arxa design eject`. Secrets never appear here.\n'
      '\n'
      '/**\n'
...
Ejected from a arxa-designer artifact — a self-contained server-rendered
htmx app (zero custom client-side JavaScript). It runs on a real Hono runtime
on $target.

**One-way eject:** this tree is yours. arxa will never re-import it. Edit
...
4. **Tests** — `arxa lens check http://localhost:${isWorkers ? '8787' : isVercel ? '3000' : '4399'}/`
...
// Ported from skills/arxa-designer/agents/{check-design-system,record-asset,
...
CmdResult _recordUsage(String msg) {
  ...
  err.addAll([
    'Usage:',
    '  arxa design record-asset <projectDir> <htmlPath> [--name N] [--inherit-from P]',
    '       [--subtitle S] [--status needs-review|approved|changes-requested]',
    '       [--width W] [--height H] [--section S] [--chat-id ID]',
    '  arxa design record-asset <projectDir> --remove [<htmlPath>] [--name N] [--path P]',
  ]);
  return CmdResult(64, stderrLines: err);
}

/// `arxa design record-asset <projectDir> <htmlPath> [flags]` (or
...
/// `arxa design ds-check <projectDir> [--verbose]` — read-only validator.
...
CmdResult designDsCheck(List<String> args) {
  ...
  if (projectDir == null) {
    return CmdResult(64,
        stderrLines: const [
          'Usage: arxa design ds-check <projectDir> [--verbose]'
        ]);
  }
...
/// `arxa design ds-import <dsDir> <projectDir> [--primary]` — sync a
...
CmdResult designDsImport(List<String> args) {
  ...
  if (positional.length < 2) {
    return CmdResult(64,
        stderrLines: const [
          'Usage: arxa design ds-import <dsDir> <projectDir> [--primary]'
        ]);
  }
