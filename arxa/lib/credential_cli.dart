// `arxa credentials` — the unified credential manager.
//
...
// are never printed.
//
//   arxa credentials list [--module <m>]    catalog + set/unset status
//   arxa credentials check [--module <m>]   missing required keys (exit 1)
//   arxa credentials set <KEY> [value]      store (stdin when value omitted)
//   arxa credentials unset <KEY>            remove
//   arxa credentials exec <KEY>... -- <cmd> run <cmd> with those keys in its
...
const _usage = '''
Usage: arxa credentials <verb> [args]
...
Verbs:
  ...
  exec <KEY>... -- <cmd> [args]
                         ...
                         e.g. arxa credentials exec ZAI_API_KEY -- dsh
''';
...
/// the dial was happily connected through `~/.arxa/supabase` — the
...
Map<String, String> supabaseMachineSatisfied(
    Map<String, String> env, String arxaHome) {
  var url = env['ARXA_SUPABASE_URL'];
  var key = env['ARXA_SUPABASE_SERVICE_KEY'];
  var urlVia = 'env ARXA_SUPABASE_URL';
  var keyVia = 'env ARXA_SUPABASE_SERVICE_KEY';
  final file = File(p.join(arxaHome, 'supabase'));
  if (file.existsSync()) {
    final parsed = parseSupabaseCredentials(file.readAsStringSync());
    if ((url == null || url.isEmpty) && parsed.url != null) {
      url = parsed.url;
      urlVia = '~/.arxa/supabase';
    }
    if ((key == null || key.isEmpty) && parsed.key != null) {
      key = parsed.key;
      keyVia = '~/.arxa/supabase';
    }
  }
  return {
...
Future<CredentialStore> defaultCredentialStore(String stateDir) async {
  ...
  final store = CredentialStore(vault: vault, keyPrefix: 'arxa.');
  await store.hydrate();
  return store;
}

/// Entry point for `arxa credentials`. Returns the process exit code.
Future<int> credentialsMain(
  ...
  if (verb == 'exec') {
    ...
    if (sep < 0) {
      stderr.writeln('arxa credentials exec: missing `--` before the command');
      stderr.writeln('  usage: arxa credentials exec <KEY>... -- <cmd> [args]');
      return 2;
    }
  ...
  store ??= await defaultCredentialStore(p.join(_homeDir(), '.arxa'));

  switch (verb) {
    case 'list':
      final setIds = store.credentials.map((c) => c.id).toSet();
      final machineStore = supabaseMachineSatisfied(
          Platform.environment, p.join(_homeDir(), '.arxa'));
    ...
    case 'set':
      if (positional.isEmpty) {
        stderr.writeln('arxa credentials set: missing <KEY>');
        return 2;
      }
      final key = positional.first;
      if (!catalog.any((e) => e.key == key)) {
        stderr.writeln('arxa credentials set: "$key" is not in the catalog');
        return 2;
      }
      ...
      if (value.isEmpty) {
        stderr.writeln('arxa credentials set: empty value, nothing stored');
        return 2;
      }
    ...
    case 'unset':
      if (positional.isEmpty) {
        stderr.writeln('arxa credentials unset: missing <KEY>');
        return 2;
      }
      final key = positional.first;
      if (!catalog.any((e) => e.key == key)) {
        stderr.writeln(
            'arxa credentials unset: "$key" is not in the catalog');
        return 2;
      }
    ...
    case 'exec':
      if (positional.isEmpty) {
        stderr.writeln('arxa credentials exec: missing <KEY> before `--`');
        return 2;
      }
      if (childArgv!.isEmpty) {
        stderr.writeln('arxa credentials exec: missing command after `--`');
        return 2;
      }
      ...
      for (final key in positional) {
        if (!catalog.any((e) => e.key == key)) {
          stderr.writeln(
              'arxa credentials exec: "$key" is not in the catalog');
          return 2;
        }
      }
      ...
      for (final key in positional) {
        ...
        if (value == null || value.isEmpty) {
          ...
          stderr.writeln('arxa credentials exec: "$key" is not set in the '
              'vault — refusing to run the command');
          stderr.writeln('  store it with: arxa credentials set $key');
          return 2;
        }
        childEnv[key] = value;
      }
      try {
        ...
        stderr.writeln('arxa credentials exec: cannot run '
            '"${childArgv.first}": ${e.message}');
        return 127; // conventional "command not found"
      }
    default:
      stderr.writeln('arxa credentials: unknown verb "$verb"');
      stderr.write(_usage);
      return 2;
  }
}
