...
class ArxadConfig {
  ...
  /// Loads config/arxa.config.json under [repoRoot] when present; the
  /// optional `daemon` object may carry `port` and `webRoot` (relative to the
  /// repo root). Anything missing falls back to defaults.
  static ArxadConfig load(String repoRoot, {int? port, String? webRoot}) {
    repoRoot = p.canonicalize(repoRoot);
    var cfgPort = defaultPort;
    var cfgRoot = p.join(repoRoot, 'arxa-studio', 'build', 'web');

    final file = File(p.join(repoRoot, 'config', 'arxa.config.json'));
    if (file.existsSync()) {
      try {
