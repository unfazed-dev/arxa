...
void main() {
  ...
  test('refresh heal gives up after one failed retry (no loops)', () async {
    ...
    await expectLater(
        () => repo.refresh(), throwsA(isA<ApprovalsOfflineException>()));
    expect(transport.resumeCount, 1, reason: 'one heal per refresh, never a loop');
    expect(cache.rows.keys, ['kept'],
        reason: 'a failed heal leaves the cached projection alone');
  });

  test('refresh heal rethrows the original error when the tunnel never returns',
      () async {
    final api = _FakeApi(const [])
    ...
    final cache = _FakeCache()..rows['kept'] = _approval('kept');
    final repo = ApprovalsRepository(
        cache: cache, api: api, healWait: const Duration(milliseconds: 50));
  ...
  test('refresh does not heal when unpaired (nothing to resume into)', () async {
  ...
  test('refresh does not heal on a remote error (the engine answered)', () async {
    final transport = FakeTransportService();
    await transport.beginPairing('ticket');
    final api = _FakeApi(const [])
      ..transport = transport
    ...
    await expectLater(
        () => repo.refresh(), throwsA(isA<ApprovalsRemoteException>()));
    expect(transport.resumeCount, 0,
