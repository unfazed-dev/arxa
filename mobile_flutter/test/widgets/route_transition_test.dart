...
Future<void> _pumpPushUnderTheme(WidgetTester tester, ThemeData theme) async {
  ...
  tester
      ...
      .push(
        MaterialPageRoute(
          builder: (_) => const Scaffold(body: Text('a thread')),
        ),
      );
  await tester.pump();
  await tester.pump();
...
void main() {
  testWidgets('the app theme pushes routes with no entrance animation', (
    ...
    try {
      ...
      expect(
        find.byType(CupertinoPageTransition),
        findsNothing,
        reason: 'the pushed thread mounts untouched — fully there on frame one',
      );
    } finally {
