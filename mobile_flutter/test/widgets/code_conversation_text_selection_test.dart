...
void main() {
  testWidgets(
    'hold-then-scroll on a bubble scrolls the thread and starts no selection (iOS)',
    (tester) async {
      ...
      try {
            ...
            's1': [
              ...
              for (var i = 0; i < 8; i++)
                ConversationMessage(
                  ...
                  text:
                      'filler message row number $i enough rows that the transcript genuinely overflows and scrolls',
                ),
            ],
          };
        ...
        expect(
          _transcript(tester).controller!.position.pixels,
          greaterThan(40),
        ...
        expect(
          ...
          findsNothing,
          reason:
              'thread bubbles carry no text-editing machinery at all '
  ...
  testWidgets('the sheet text is equally inert under a hold-then-drag', (
    tester,
  ) async {
    debugDefaultTargetPlatformOverride = TargetPlatform.iOS;
    try {
