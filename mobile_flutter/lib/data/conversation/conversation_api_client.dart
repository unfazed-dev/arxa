...
class ConversationApiClient {
  ...
  Future<(String?, List<ConversationMessage>)> messages(
    String sessionId, {
    int limit = 200,
  }) async {
    final body = await _json(
      'GET',
      '/__arxa/conversations/$sessionId/messages?limit=$limit',
    );
    final messages = [
  ...
  Future<void> selectModel(
    ...
    await _json(
      'POST',
      '/__arxa/conversations/$sessionId/model',
  ...
  Future<void> setMode(String sessionId, String mode) async {
    await _json(
      'POST',
      '/__arxa/conversations/$sessionId/mode',
      body: {'mode': mode},
    );
  }
  ...
  Future<void> send(
    ...
    await _json(
      'POST',
      '/__arxa/conversations/$sessionId/messages',
      body: {
        'text': text,
        'mode': mode,
  ...
  Future<Map<String, dynamic>> attachment(
    ...
    return _json(
      'GET',
      '/__arxa/conversations/$sessionId/attachments/$attachmentId',
    );
  }
  ...
  Future<List<EngineCommand>> commands(String sessionId) async {
    final body = await _json(
      'GET',
      '/__arxa/conversations/$sessionId/commands',
    );
    return [
      for (final record in (body['commands'] as List<dynamic>? ?? <dynamic>[]))
  ...
  Future<String> exportTranscript(String sessionId, String savePath) async {
    ...
    try {
      final request = await _httpClient
          .openUrl(
            'GET',
            base.resolve('/__arxa/conversations/$sessionId/export'),
          )
