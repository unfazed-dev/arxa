...
kit-native widgets (`ArxaKitNativeAppBar`, `ArxaKitGlassCard`, `ArxaKitListTile`,
...
**Validate after any registry edit** (a real gate — it also rides `arxa gate --all` and CI):

```
arxa gate kind_registry
```

It checks the registry against its two ground truths — the partials directory
(`arxa-designer/starter-partials/widgets/_<kind>.tsx` *is* the vocabulary; never
