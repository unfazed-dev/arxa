...
SUPERSEDED 2026-08: promoted to a real gate — run `arxa gate kind_registry`
(arxa/lib/gate_kind_registry.dart), which rides `gate --all` and CI. This
script stays as the port's reference implementation; behavior is parity-locked
by arxa/test/gate_kind_registry_test.dart.

Manual for now — nothing in CI executes this yet. Run it after any registry edit.

Ground truth 1 — VOCABULARY: skills/arxa-designer/starter-partials/widgets/_<kind>.tsx.
...
PARTIALS = os.path.normpath(os.path.join(SKILL, "..", "arxa-designer", "starter-partials", "widgets"))
...
def main():
    ...
    for kind in sorted(kinds):
        ...
        refs += [x for x in (v.get("presentation") or {}).values()
                 if isinstance(x, str) and x.startswith("ArxaKit")]
    ...
    if not shape_pkg:
        ...
        if not os.path.isfile(shape_path):
            ...
            if "class ArxaKitInspectAttrs" not in src:
                bad.append(f"{shape_pkg} does not declare class ArxaKitInspectAttrs")
            ...
            barrel = os.path.join(KIT_ROOT, "core", "lib", "arxa_kit_core.dart")
            if os.path.isfile(barrel) and os.path.basename(rel) not in open(barrel, errors="ignore").read():
                bad.append("inspectAttrs shape is not exported from arxa_kit_core.dart — importers cannot reach the single home")
