...
- [`references/project-layout.md`](references/project-layout.md) — load when you're about to run `emit`/`seed` and need the exact `~/.appbox/projects/<name>/intake/` file layout, or need to trace which skill (`appbox-story-mapper` vs `appbox-moodboarder`) owns `map.json` / `moodboard.json`.
