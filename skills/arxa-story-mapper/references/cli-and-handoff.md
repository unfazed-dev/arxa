...
echo '{"project":"demo",...}' | arxa emit story-map \
  --output docs/intake/story_map.html \
  --data-out docs/intake/story-map.json \
  --brief-out docs/intake/brief.md
```
...
Standard arxa layout: all three under `docs/intake/` — the gate's default
paths, so `arxa gate intake` needs no flags.
...
- **`brief.md`** → `arxa-designer` reads it as its requirements source; the
