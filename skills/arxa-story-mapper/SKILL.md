...
hands those requirements to `arxa-designer`.

## Where this sits in the arxa pipeline

```
arxa intake (answers)  →  arxa emit story-map  →  docs/intake/brief.md (unified) + story-map.json + story_map.html  →  arxa-designer
```
...
- It also remains valid **standalone** (no answers): `arxa-intake` is
...
- The emitted `brief.md` carries a **`## Surface inventory`** table (and
  ...
  `arxa gate intake` (pure Dart, `arxa/lib/gate_intake.dart`; plan 10.6)
...
- **Upstream:** the client's requirements (`data.json`) — or `arxa-intake` answers (`--answers`), which make the emitted brief the unified one.
- **Downstream:** `arxa-moodboarder` slices the emitted `story-map.json` per epic; `arxa-designer` consumes `docs/intake/brief.md` as its input contract.
...
6. **Self-check**: `arxa emit story-map --self-test` asserts the handoff contract — ids match the gate pattern, are unique, all-`wont` features stay out of the surface table, and the gate's own parse would recover exactly the emitted ids
7. **Licensed MIT** — third-party skill, adapted for arxa. See `LICENSE.txt` and the repository's `THIRD-PARTY-NOTICES.md`

## References

- `references/pipeline-and-mapping.md` — load when resolving how a feature's `id` pins to (or fails to match) an intake-declared surface, or when the exact Epic/Feature/Story → arxa mapping rule is needed.
...
- `references/cli-and-handoff.md` — load when running `arxa emit story-map` (full command forms, `--input`/`--output`/`--answers`/etc. argument reference, the unified-brief section order) or handing `brief.md` / `story-map.json` / `story_map.html` off to the designer.
