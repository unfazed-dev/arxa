# Capability map — probe-runner (archived) → arxa lens
...
against the arxa lens — `arxa/lib/lens.dart` over `arxa/lib/cdp.dart`,
the `arxa/lib/lens/` modules, and `arxa/lib/lens/native/`. The
...
- **ported** — the capability lives in Dart. Invoke via the lens library API
  (`arxa/lib/lens/*.dart`) or the `tool/lens_*.dart` one-off drivers.
...
- **dropped** — not ported, with a one-line reason (another tool owns it, or it
  is research/extraction machinery with no arxa consumer).

The `arxa lens <verb>` CLI dispatch is **live** in `arxa/bin/arxa.dart`
(`case 'lens'` → `runLensCli`); every ported verb is reachable as
`arxa lens <verb>` (see `lens_cli.dart`). The library beneath every verb is
...
| `dom` / `console` | Dioxus/wry debug-probe pair (`templates/dioxus_debug_probe.rs`); arxa doesn't ship Dioxus |
| `content_firewall` / `slots` / `bundle_writer` / `motion_adapter` / `site_chrome` | ADR-0001 extraction-pipeline machinery — arxa authors designs; nothing consumes content-free bundles |
...
| `safari_inspect` + Safari fallback paths in `web_*` | Chrome-only doctrine — one render engine, the one arxa gates on |
...
| probe-runner verbs | reason |
|---|---|
| `pr-decide` | tool-routing manifest for a retired tool; the `arxa` CLI dispatch replaces it |
...
- Dropped (research / no consumer): `_chrome_dedup` (chrome-instance dedup
  ...
  design-token transform lives in `arxa`, not the lens), `_virt`
...
tool. Not ported — Dart unit tests in `arxa/test/lens_*_test.dart` replace
...
extraction-pipeline and research machinery (ADR-0001) have no arxa consumer
...
|---|---|---|---|
| `agents/check-design-system.mjs` (110) | `design_tools.dart` → `dsCheck(projectDir)` (read-only DS validator, exit-64 usage contract) | `arxa design ds-check` | ported |
| `agents/record-asset.mjs` (187) | `design_tools.dart` → `recordAsset(...)` (appends the deliverable record into `_d_meta.json`; JSON shape ported exactly) | `arxa design record-asset` | ported |
| `agents/import-design-system.mjs` (274) | `design_tools.dart` → `dsImport(projectDir, slug)` (syncs compiled DS into `_ds/<slug>/` + `_d_meta.json` note) | `arxa design ds-import` | ported |
...
> (`ds-check`/`record-asset`/`ds-import`) are wired into `arxa design`.

### Designer `agents/` — dropped with reason

| runtime file | reason |
|---|---|
| `agents/import-figma.mjs` (740) | The offline `.fig` decoder (kiwi + zstd/deflate → node tree → React JSX / token CSS emit); a babel/JSX concern, not a Dart port. **Archived** under `archives/tooling-pre-dart/skills-pre-dart/arxa-designer/agents/` — restorable if Figma import returns. |
...
| `agents/build-preview.mjs` (1919), `agents/lib/ds-core.mjs` (750), `agents/lib/ds-prompt.mjs` (290), `agents/lib/asset-store.mjs` (177) | The design-system **preview compiler** — a self-contained ~3k-line subsystem (parse/compile/preview/prompt/asset store). No pipeline gate consumes it. Archived under `archives/tooling-pre-dart/skills-pre-dart/arxa-designer/agents/` — port on demand (the capability-map convention) only if a gate or the designer's DS flow asks for it. |

### Designer `runtime/` — ported

| runtime file | Dart target | CLI | status |
|---|---|---|---|
| `runtime/serve.mjs` (387) | `design_server.dart` → `designServe` (Task 20; loopback bind, `--port 0 --json`, hot-reload supervisor) | `arxa design serve` | ported (the Dart design server **supersedes** it; archived by Task 23 under `archives/tooling-pre-dart/skills-pre-dart/arxa-designer/runtime/`) |
| `runtime/serve.test.mjs` (195) | `design_server` Dart tests (serve contract) | — | ported |
| `runtime/lint.mjs` (53) | `design_tools.dart` → `designLint` (ADR-0002 no-custom-JS check) | `arxa design lint` | ported |
| `runtime/console-check.mjs` (32) | the lens — `consoleErrors`/`pageErrors` auto-fail (`cdp.dart`); navigate + assert | `arxa lens check` | ported |
| `runtime/shoot.mjs` (100) | the lens — viewport-ladder capture pass (`lens_cli.dart`) | `arxa lens shoot` | ported |
| `runtime/pseudolocalize.mjs` (119) | `design_tools.dart` → `pseudolocalizeDir` (en → `qps-ploc`) | `arxa design pseudolocalize` | ported |
| `runtime/doctor.mjs` (92) | `design_tools.dart` → `designDoctor` (preflight the toolchain the gates use) | `arxa design doctor` | ported |
| `runtime/check_ladder.mjs` (43) | `design_tools.dart` → `designCheckLadder` (ladder config ↔ doctrine drift) | `arxa design check-ladder` | ported |
| `runtime/check_wiring.mjs` (134) | `design_tools.dart` → `designCheckWiring` (fragments/mutations-posted/urls-resolve/targets-exist) | `arxa design check-wiring` | ported |
| `runtime/verify-interact.mjs` (41), `verify-shots.mjs` (37), `verify-timeline.mjs` (13) | folded into `design_selftest.dart` (structural-contract selftest + falsifiability) | `arxa design selftest` | ported |
| `runtime/vendor/fetch.mjs` | `design_tools.dart` → `designVendorFetch` (fetch + SRI-pin vendored client libs) | `arxa design vendor-fetch` | ported |
| `runtime/eject.mjs` (153) | `design_tools.dart` → `designEject` (self-contained Hono eject) | `arxa design eject` | ported |
...
| `runtime/vendor/` (htmx, islands, lucide, …) | **not a runtime** — served, SRI-pinned *client assets* (browser data), nothing to port; `fetch.mjs` above is the one tool that touched them | served by `arxa design serve` | n/a (data) |

### Designer `selftest.sh` — ported

| runtime file | Dart target | CLI | status |
|---|---|---|---|
| `selftest.sh` | `design_selftest.dart` (structural-contract selftest + `--negative` falsifiability; the `check_ladder`/`check_wiring`/`lint` calls it shelled out to are the Dart functions above) | `arxa design selftest [<dir>] [--negative]` / `arxa design --self-test` | ported |

### The `.py` skill runtimes — ported

| runtime file | Dart target | CLI | status |
|---|---|---|---|
| `arxa-scaffolder/scaffold.py` | `scaffold.dart` → `scaffoldMain` | `arxa emit scaffold --design-dir … --app-root … --targets … [--check] [--self-test]` | ported |
| `arxa-story-mapper/scripts/generate_story_map.py` | `story_map.dart` → `storyMapMain` | `arxa emit story-map -i … -o … --data-out … --brief-out … [--self-test]` | ported |
| `arxa-deployer/deploy.py` | `deploy.dart` → `deployMain` (fastlane/shorebird/wrangler over a `ProcessRunner` port) | `arxa deploy doctor` / `arxa deploy deploy …` / `arxa deploy --self-test` | ported |
| `arxa-lint/lint_kb.py` | `docs_lint.dart` → `lintKb` (dead links, orphans, half-wired supersede pointers, unresolved wikilinks) | `arxa docs [root]` | ported |
| `arxa-intake/intake.py` | `intake.dart` → `intakeMain` | `arxa intake emit --answers …` / `seed --brief …` / `validate …` / `--self-test` | ported |
...
The `arxa` CLI dispatch (`arxa/bin/arxa.dart`) routes every ported verb:
`arxa design <sub>` (Task 19-21 + 22.1), `arxa lens check|shoot` (Task 7),
`arxa emit story-map|scaffold` (Tasks 14-16), `arxa deploy` / `arxa intake`
/ `arxa docs` (the `.py` ports). Task 23 moved every file in this section to
