...
Before building, confirm with `AskUserQuestion`:
...
If the user picks no design system, skip the rest of this flow (steps 3–8) and design normally — no `_ds/` folder, `designSystems: []`, `primaryDesignSystem: null`. You **still** get a `_d_meta.json`: `arxa design record-asset` creates it (with `type`/`createdAt`/`updatedAt`) the first time you record a deliverable as an asset — see ["Recording deliverables as assets"](#recording-deliverables-as-assets) below.
...
```
arxa design ds-import <dsDir> <projectDir> [--primary]
```
...
Ignore the `<script>` wiring lines the import prints — they describe the React bundle path, which does not exist here. Global **CSS** is shared scope: `<link>` order is the precedence control (see "Multiple design systems" below). Any global CSS classes the system ships work as-is in templates; only the JS-component layer needs step 5.
...
```
arxa design ds-check <projectDir>/_ds/<slug>
```
...
- `--chat-id <id>` — optional; normally omitted (no chat-id surface in this environment), reserved for hosts that have one.
...
```
arxa design record-asset <projectDir> --remove [<htmlPath>] [--name "<n>"] [--path <relPath>]
```
...
A project may consume several systems at once. Artifacts never load the bundles, so the only shared surface is **global CSS**: every system's `:root` tokens and base rules land in the same scope, so a later `<link>` overrides an earlier one's same-named variables. Mitigations, in order of preference:

1. Treat one system as **primary** (its `<link>` group last, its tokens win) and pull only specific *components* from the others (as partials — step 5).
