...
> **Archived (dart-only port).** The `compile-design-system.mjs` node compiler
> was dropped and archived under
> `archives/tooling-pre-dart/skills-pre-dart/arxa-designer/agents/`. Only the
> read-only checker survived, as the Dart command `arxa design ds-check`. The
> command examples below are the archived tool's reference interface, not active
> invocations.

## This project is a design system

You are authoring the design system itself, not consuming one. `_ds_bundle.js`, `_ds_manifest.json`, and `_adherence.oxlintrc.json` are **generated artifacts** — never hand-edit them. They were (re)generated by the portable compiler, formerly a node call (now archived):
