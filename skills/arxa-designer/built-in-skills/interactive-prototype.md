...
5. Serve, then verify per surface: `arxa design lint`, `arxa lens check`, playwright screenshot + `ReadMediaFile`.
...
`arxa design lint <artifact-dir>` → `arxa lens check <url>` → `npx playwright screenshot <url> /tmp/x.png` + `ReadMediaFile`. All three pass, then give the user the served URL.
