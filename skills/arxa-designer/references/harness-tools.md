...
arxa is **harness-agnostic**. This file names the *capability* you need; the
...
| Screenshot a page | shell → `arxa lens shoot <url>` |
...
arxa design serve designs/<project> --port 4319
```
...
The target may also be a bare design name (`arxa-app` → `designs/arxa-app`).
...
```sh
arxa design serve <project> --port 0 --json
# {"url":"http://127.0.0.1:52953/","port":52953,…}  ← printed once it is LISTENING
```

Read that one line and you have the URL to hand to `arxa lens shoot` or a browser.
...
arxa lens shoot http://localhost:4319/<route> --out /tmp/shots

# a derived subset (e.g. targets: ios -> compact + medium)
arxa lens shoot http://localhost:4319/<route> --rungs compact,medium
```

Widths come from `runtime/ladder.json`; **never pass a pixel value**. `arxa lens shoot`
...
- **No-JS lint:** `arxa design lint <artifact-dir>` — the
  zero-custom-client-JS check (ADR-0002). Must pass before surfacing.
- **Console errors:** `arxa lens check
...
- **Structure:** `arxa design selftest`-style checks — the registry parses,
  every viewmodel declares a `surfaceId`, `shellRoots` is non-empty. See
  [`app-architecture.md`](app-architecture.md).
- **Wiring:** `arxa design check-wiring <artifact> <property>`
...
- **Falsifiability:** `arxa design selftest <artifact> --negative` re-runs the
...
- **Design-system checking:** run the `arxa design ds-check` CLI, or
...
`arxa design eject <artifact-dir> <out-dir>` — see
