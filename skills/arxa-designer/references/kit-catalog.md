...
`kit/core/lib/common/`: colors (`arxa_kit_colors.dart`), spacing/ui helpers
(`arxa_kit_ui_helpers.dart`), app constants, glyphs (incl. lucide) and fonts —
...
app is a verbatim kit copy (plus the app's `arxa_kit_app_strings.dart`),
...
**Not mirrored, by decision:** `ArxaKitNative*` and `arxa_kit_ui_library`
...
|---|---|---|---|
| `core` | `arxa_kit_core` | MVVM plumbing; error/theme services; design tokens; locator; input formatters; ArxaKitGlyphs; ArxaKitPlatform | — |
| `ui_library` | `arxa_kit_ui_library` | ArxaKit* adaptive port widgets; ArxaKitNative* widgets; UI-coupled services (navigation / sheet / notifications toast) | — |
| `state` | `arxa_kit_state` | async state vocabulary (idle/loading/error); retry policy; persistence | — |
| `data` | `arxa_kit_data` | repositories; schema descriptors; seed/Supabase/Appwrite backends; canonical IDs; codecs; seeder | — |
| `auth` | `arxa_kit_auth` | email/OAuth auth seam; typed AuthResult; session stream | SeedAuthBackend: port-tested; Apple SignIn: port-tested; Google SignIn: port-tested |
| `forms` | `arxa_kit_forms` | form field state; sync/async validation; ArxaKitFieldController; error messages | — |
| `permissions` | `arxa_kit_permissions` | OS permissions (camera/location/...); typed permission + status | — |
| `media` | `arxa_kit_media` | camera/photos; audio record; audio+video playback | — |
| `documents` | `arxa_kit_documents` | doc pick; scan (stub); OCR (stub); PDF | — |
| `notifications` | `arxa_kit_notifications` | device push/local notifications; tokens; badge | — |
| `analytics` | `arxa_kit_analytics` | analytics event fan-out | — |
| `payments` | `arxa_kit_payments` | Apple Pay; Google Pay | Stripe: port-tested; PayPal: port-tested; Apple Pay: stub |
| `maps` | `arxa_kit_maps` | native maps; ArxaKitMapView | OpenStreetMap: port-tested; Mapbox: port-tested |
| `deploy` | `arxa_kit_deploy` | release automation (fastlane/shorebird/CF Views+Workers/Vercel) | Vercel: port-tested; Cloudflare Views: port-tested; Cloudflare Workers: port-tested; fastlane: port-tested; Shorebird: port-tested |
| `haptics` | `arxa_kit_haptics` | haptic feedback | — |
| `bluetooth` | `arxa_kit_bluetooth` | bluetooth adapter state; BLE scan/GATT (stub) | — |
| `wifi` | `arxa_kit_wifi` | Wi-Fi state; network info; settings escort | — |
| `support` | `arxa_kit_support` | in-app support; feedback + Talker export; submission sinks | — |
| `security` | `arxa_kit_security` | biometrics; secure storage; crypto; app-lock; device integrity (stub) | — |
| `compliance` | `arxa_kit_compliance` | ToS/privacy/EULA; consent gates; OSS licenses | — |
| `branding` | `arxa_kit_branding` | app icons; native/in-Flutter splash; brand colors (codegen); BrandSplash | — |
| `motion` | `arxa_kit_motion` | entrance/exit choreography; gesture drivers; motion scopes; ArxaKitWake | — |
| `i18n` | `arxa_kit_i18n` | i18n | — |
| `showcase_app` | `arxa_kit_showcase_app` | integration surface / reference app (proves every kit) | — |

This table is checked against the registry by `kitCatalogMirrorCheck`
(`arxa/lib/design_selftest_kit_catalog_mirror.dart`) — every `dir` in
...
(`arxa/lib/intake.dart:59` — `loading | empty | error`) a surface
declaring that kit must have, whether or not the registry entry says so
explicitly. This is the source arxa-designer reads for D10 ("Feedback &
...
Verified against portalo's real registry (`~/.arxa/projects/portalo/intake/registry.json`),
...
values in Dart; `skills/arxa-intake/intake.schema.json` (owned by another
...
The pipeline takes it from there: `arxa emit structure` validates the names
...
- Keys are managed outside the design: the `arxa credentials list` /
  `arxa credentials check` CLI reports what each declared module needs and
