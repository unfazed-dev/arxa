...
**Serve CLI**:
The command that runs an Artifact against the Runtime (`arxa design serve <artifact-dir>`).
...
**Client-JS-Free**:
The skill's boundary: zero hand-written client-side JavaScript — no `<script>` blocks, no `hx-on:*`, no `js:`-prefixed attributes, no `[expr]` trigger filters. Libraries (htmx + Allowlisted Extensions) are exempt. Enforced by `arxa design lint`; htmx 4 evaluates no attribute expressions (the v2 `allowEval:false` switch has no v4 equivalent). Named islands (vendored, enumerated in ADR-0002's islands amendment) are the only permitted extension: third-party declarative web components and first-party data-attribute init modules, all loaded from /assets/vendor/.
