...
> `arxa design serve` → `arxa/lib/design_server.dart` (artifact JS runs
...
```sh
arxa design serve <artifact-dir|design-name> [--port 4319] [--host 127.0.0.1] [--json] [--no-watch]
arxa design lint  <artifact-dir>                 # zero-custom-client-JS check
arxa design check-wiring <artifact-dir> <property>
#   fragments | mutations-posted | urls-resolve | targets-exist
arxa design pseudolocalize <artifact-dir>        # en → qps-ploc pseudo-locale
arxa design vendor-fetch                         # (re)vendor htmx + extensions
```

`serve` takes a path **or** a bare design name — `arxa-app` resolves to
`designs/arxa-app` — because anything offering "preview this prototype" has
a name, not a path.
...
```sh
arxa design serve arxa-app --port 0 --json
...
`<system-temp>/arxa-designer-serve/` (one file per instance, so no shared
...
app's `arxa design serve . --no-watch` uses it; keep it for anything
productionized.
...
```
<artifact>/
├── serve.mjs                     # formerly `node serve.mjs` (archived) — now served via `arxa design serve <dir>`
...
(`arxa design ds-import`) and are served at `/_ds/` — wire their CSS
...
- **ViewRefs keep the `.html` name for registry parity.** Viewmodels render
  ...
  the generated render registry (`generateRenderTsx`, arxa/lib/
...
`runtime/vendor/lucide/icons/<kebab-name>.svg` by `arxa design vendor-fetch` and
...
to `en`. `arxa design pseudolocalize <artifact-dir>` derives
...
`hx-on:*`, `js:`-prefixed
attributes, `[expr]` trigger filters. `arxa design lint` enforces it;
