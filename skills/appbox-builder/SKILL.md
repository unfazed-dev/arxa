...
- **Prose comment canon (canon: `BUILDER_playbook.mdx` → Prose comment canon):** every prose `///` block is plain language — user-visible behavior, present tense, no framework/API names or banned jargon; primary class gets no class doc, member docs are `/// [N. Requirement name] One plain sentence.`; exemplar `showcase_note_editor_viewmodel.dart`; G13-language enforces it via `appbox gate arch`.
