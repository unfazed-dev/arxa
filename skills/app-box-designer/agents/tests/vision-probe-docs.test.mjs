...
test('the harness reference gates image reads behind the vision probe', () => {
  const app-box-designer = readSkill('references/harness-tools.md');

  assert.match(app-box-designer, /Vision probe/);
  assert.match(app-box-designer, /VISION_OK/);
  assert.match(app-box-designer, /per session/i);
});
...
test('vision probe uses a committed PNG asset, not a hardcoded /tmp write', () => {
  ...
  assert.match(app-box-designer, /agents\/assets\/vision-probe\.png/);
  assert.match(probe, /agents\/assets\/vision-probe\.png/);
  assert.doesNotMatch(app-box-designer, /\/tmp\/the upstream MIT project-vision-probe\.png/);
  assert.doesNotMatch(app-box-designer, /writeFileSync/);
