...
2. empty View/ViewModel **stubs** (`@arxa-extension-point`).
...
- **Upstream:** `arxa-scaffolder` — the stub tree (`@arxa-extension-point` View/ViewModel pairs) plus the `deps` recorded in each stub header.
- **Downstream:** `arxa-tester` — filled bodies handed over arch_guard-clean (pre-check `arxa gate arch` here; the stage-6 `arxa-reviewer` owns the binding verdict — its violations rewind to this skill). The build phase's `arxa-lens` goldens compare design-vs-built.
...
All native glass comes from the kit wrappers (`ArxaKit*`) over the vendored
...
- The emitted target (extension-point files carry `@arxa-extension-point`; `lib/ui/primitives.dart` is generated — read it for the available widgets).
...
- **File structure (canon: `BUILDER_playbook.mdx` → File structure):** every covered file (`*_view.dart`, `*_viewmodel.dart`, facade/adapter/repository services, `*_widget.dart`) carries a semantic library doc comment above `library;` — layer intro → plain paragraph (fixed role name) → requirements (`N. [Name] — story-id`) → relationships diagram + inventory → history — and locked body sections per kind with `// ── Name ──` separators. Models get the light variant (role paragraph + field comments). The pilot VM is the living reference.
- **Prose comment canon (canon: `BUILDER_playbook.mdx` → Prose comment canon):** every prose `///` block is plain language — user-visible behavior, present tense, no framework/API names or banned jargon; primary class gets no class doc, member docs are `/// [N. Requirement name] One plain sentence.`; exemplar `showcase_note_editor_viewmodel.dart`; G13-language enforces it via `arxa gate arch`.
...
- **ViewModels are context-free** — they read copy through a tiny app-layer service holding the latest `AppLocalizations`, attached once from the root view's builder:
  ...
  ```
- **Capability wiring:** register `ArxaKitI18n` + `ArxaKitLocaleStore` (arxa_kit_i18n) in the stacked locator, and give the settings surface a language row — System / English / Polski (system locale + persisted override, live switch).
- **Generative UI:** every generative-UI prompt includes `arxaKitI18n.llmLocaleDirective()` in the system prompt, so generated copy lands in the active locale.
