...
```sh
appbox design serve examples/hello-hda
```

The former `serve.mjs` node runtime was archived to
`archives/tooling-pre-dart/skills-pre-dart/appbox-designer/` — serving is now
the Dart `appbox design serve` command (the Task 20 `DesignServer`).
...
`archives/tooling-pre-dart/skills-pre-dart/appbox-designer/examples-hello-hda-generate.mjs`).
