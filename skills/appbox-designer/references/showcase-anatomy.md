...
this vocabulary *and* the scaffolder's kind→`appbox_kit` resolution registry,
in the same change.

**Where resolution lives (do not duplicate it here).** The kind→widget mapping
is owned solely by `skills/appbox-scaffolder/kind-resolution.registry.json`
(`sharedWith.consumedBy` names this skill). Designers name kinds; they never
name `AppBoxKit*` widgets. An unmapped kind exits non-zero via that file's
...
| Dart source | mirrors |
|---|---|
| `appbox_kit_colors.dart` | colors |
| `appbox_kit_ui_helpers.dart` | spacing / ui helpers |
...
appbox-scaffolder and consumed by this skill. Read the templates from that
