...
The swap unit is the **Named Fragment** — a macro inside the surface's own view file, renderable alone via `view.html#macroName` for `HX-Request` swaps. Shared cross-surface fragments are `_name.html` partials living at their placement-law tier (`references/app-architecture.md`), pulled in with `{% include %}`. Icons come from the runtime's vendored Lucide set via the `{{ icon('name') }}` global (inlined server-side, `currentColor`, kebab-case names) — never emoji, never hand-drawn SVG glyphs.
...
- **[widgets/](starter-partials/widgets/)** — the UI library: drop-in `_name.html` partials (nav rail, bottom nav, tabs, app bar, list row, card, form field, dialog, bottom sheet, toast, empty state) + a shared `widgets.css`, matching the recipes in `references/ui-recipes.md`. The starting point of the widget-library pass — copy into the artifact's widget tree at the tier its consumers require (`references/app-architecture.md`); dialog/bottomsheet partials are subfolders inside that tier's `widgets/` dir, not a separate root.
...
- **[artboards.html](starter-partials/artboards.html)** — Static side-by-side comparison page for design variations — replaces the design-canvas pan/zoom widget, which was dropped as JS-bound.
