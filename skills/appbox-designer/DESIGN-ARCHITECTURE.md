...
- **Strings** (chrome, labels, any copy in templates) live in `l10n/app_<locale>.arb` catalogs and render through `helpers.translate(context)` — a string literal in a template is a bug the pseudo-locale (`qps-ploc`, generated by `appbox design pseudolocalize`) exists to expose. Jargon variants are key suffixes (`keyPlain` / `keyTechnical`; base key = balanced). Key parity across catalogs is a selftest check.
...
binding (`{project.tagline}`) is routed by `appbox design patch` into the seed
...
- Reference implementation: the appbox design shell's own stub views (`screen_stub_view.tsx`) carry the full set.
