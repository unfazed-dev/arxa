# arxa behavior-TDD canon

The rules every arxa-scaffolded app and every kit package test suite follows.
Enforced by `arxa gate tests` (rule IDs **T1/T2/T3** below).
...
- **Shape** = plain `flutter_test` group/test in blocTest arrange/act/assert
  ...
  test('notes.folders.create-folder — creates folder and emits it in folder list', () async {
    // given
    final repo = FakeArxaKitRepository<Folder>();
...
- **Kit fakes beat mocks.** When a package ships fakes in its
  `arxa_kit_testing.dart`, use them — they script real behavior:
  - `ui_library`: `FakeArxaKitNotificationService`,
    `FakeArxaKitBottomSheetService`, `FakeArxaKitNavigationControllerService`
  - `data`: `FakeArxaKitRepository<T>`, `FakeArxaKitDataFacade`
...
```dart
setUpAll(() => ArxaKitData.initialize());        // once per file
tearDownAll(() => ArxaKitData.resetForTesting());
setUp(() => arxaKitLocator.reset());             // per test
```
- Platform-dependent UI tests: `ArxaKitPlatform.override(...)` in arrange,
...
| visual/smoke | optional per app | arxa lens |
