 ...
 *
 * Loads harness/pi/appbox-gate.ts through jiti — the SAME loader Pi uses to run
...
console.log('== pi appbox-gate extension ==');
...
let factory;
try {
  const mod = load(join(here, 'appbox-gate.ts'));
...
const run = async () => {
  ...
  const blocked = await call('edit', { file_path: join(REPO, 'appboxd', 'lib', 'x.dart') }, 'using');
  ...
  const dev = await call('edit', { file_path: join(REPO, 'appboxd', 'lib', 'x.dart') }, 'dev');
  ...
  const sh = await call('bash', { command: `echo x > ${join(REPO, 'appboxd', 'z.dart')}` }, 'using');
  sh && sh.block === true
  ...
  process.env.APPBOX_GUARD_PATH = '/nonexistent/guard.js';
  const open = await call('edit', { file_path: join(REPO, 'appboxd', 'lib', 'x.dart') }, 'using');
  delete process.env.APPBOX_GUARD_PATH;
  open === undefined
