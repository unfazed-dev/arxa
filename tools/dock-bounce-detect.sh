#!/bin/bash
# Dock-bounce detector for arxa-launched Chrome.
#
...
# only — resolved by the unique `--user-data-dir` prefix arxa launches with.
#
...
#   e.g.   tools/dock-bounce-detect.sh dart run arxa/bin/arxa.dart \
...
(
  while :; do
    # Every arxa Chrome alive right now (browser processes only — renderers
    # and helpers carry --type= and never own a tile).
    pids=$(ps -eo pid=,command= \
           | grep -E 'arxa-cdp-|arxa-design-worker-' \
...
if [ "$MAX" -ge 10 ]; then
  ...
  echo "  VERDICT          : clean — no arxa Chrome ever held a Dock tile"
fi
rm -f "$HITS" "$HITS.pids"
exit $RC
