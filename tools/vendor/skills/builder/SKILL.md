...
2. empty View/ViewModel **stubs** (`@crew-extension-point`).
...
- **expressive** (Android) — real native Material 3 Expressive via Jetpack **Compose**, embedded as a Flutter `PlatformView` (`_ExpressiveView` → `AndroidView 'crew/expressive'`; Kotlin `ExpressivePlatformView.kt` is emitted by `emit.py`, NOT the snapshot golden — it patches the `stacked create` scaffold: Compose plugin + `material3:1.5.0-alpha*` + AGP≥9.1 + Gradle≥9.3.1 + compileSdk 37). Compose-embed covers the LEAF primitives whose whole surface is the touch target — button, segmented, progress. Composite primitives (cards-with-content, text fields) and content-width inline buttons stay Flutter Material 3 (a platform view is a leaf — can't host Flutter children — and content-width needs an intrinsic-sizing leaf). All Material 3; diverges from iOS glass.
...
- The emitted target (extension-point files carry `@crew-extension-point`; `lib/ui/primitives.dart` is generated — read it for the available widgets).
...
- **Don't touch generated-layer files** (`AUTO-GENERATED` marker) except `primitives.dart` when adding a primitive. Edit `@crew-extension-point` files freely.
