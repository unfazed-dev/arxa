#!/usr/bin/env node
// appbox-designer Runtime — Serve CLI.
//
//   node serve.mjs <artifact-dir|design-name> [--port N] [--host H] [--json] [--no-watch]
//
// The target may be a path, or the bare name of a design — `appbox-app`
// resolves to `designs/appbox-app`. A UI offering "preview this prototype"
...
// are tracked as pidfiles under os.tmpdir()/appbox-designer-serve/ (one
...
if (worker || noWatch) {
  ...
  const stateFile = path.join(
    os.tmpdir(),
    `appbox-serve-state-${createHash('sha1').update(artifactDir).digest('hex').slice(0, 12)}.json`,
  );
  try {
  ...
  const server = serve({ fetch: app.fetch, port, hostname: host }, (info) => {
    ...
    if (asJson) {
      ...
      console.log(`appbox-designer serving ${artifactDir}`);
      console.log(`→ ${url}`);
  ...
  const registryDir = path.join(os.tmpdir(), 'appbox-designer-serve');
