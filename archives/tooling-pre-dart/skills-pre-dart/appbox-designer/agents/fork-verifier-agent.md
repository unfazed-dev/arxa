...
nothing else. Resolve every tool named below via `references/harness-tools.md`: a
generic action like "show the file" or "evaluate JS in-page" maps to the the harness
Code preview / eval path described there.
...
1. Show the file the main agent built/edited (file path + served URL for the
   user; load it yourself over the served URL — upstream `show_html`).
