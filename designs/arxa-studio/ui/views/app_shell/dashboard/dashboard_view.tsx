...
// needs-you strip (gates), the LIVE project grid (~/.arxa/projects), an
      ...
      surface={
        <Open>
          <section class="mp-content">
            ...
            <div class="dash-head">
              <div>
                <Heading name="app-dashboard:greeting" level={1} class="display">{translate('dash.greeting', { name: account.name }) as string}</Heading>
                <Txt name="app-dashboard:greeting-sub" class="muted">
                  {translate('dash.gatesNeedYou', { count: gateCount }) as string} ·{' '}
                  {translate('dash.projectsTailnet', { count: projects.length }) as string}
                </Txt>
              </div>
            ...
            <section class="dash-section" aria-labelledby="wizard-h">
              <Heading name="app-dashboard:wizard-h" level={2} id="wizard-h">{translate('action.newProject') as string}</Heading>
              <div class="wizard">
                ...
                <form class="wizard-form" method="post" action="/dashboard/projects">
                  <label class="auth-label" for="wizard-name" {...inspectAttrs('app-dashboard:wizard-name-label', { role: 'label' })}>{translate('wizard.nameLabel') as string}</label>
                  <input
                    id="wizard-name"
                    type="text"
                    name="name"
