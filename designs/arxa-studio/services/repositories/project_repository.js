...
//
// Written by `arxa intake emit --project <name>` from the SAME answers.json
// (arxa/lib/intake.dart:1144-1148, emitters in intake_artifacts.dart). The
// `mapping`, `brief` and `moodboard` panels read these; before Slice B3 they
// read the studio's own intake fixture, which showed arxa's product plan
...
// (arxa/lib/intake.dart:625) REGENERATES flows.json from answers, so an edit
...
// Every project in ~/.arxa with its derived stage + output counts, plus the
// current marker. No arxa home / no endpoint → an empty honest grid.
