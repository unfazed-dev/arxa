# appbox-studio-v2 — intake design brief
...
This buildout is a smoke test of the refactored appbox-designer skill and
...
- Artifact: `designs/appbox-studio-v2/` — appbox studio itself, designed as
  an appbox app per the showcase recipe.
...
- Recipe grammar sits at the **artifact root** `designs/appbox-studio-v2/`
...
- v1 (`designs/appbox-studio/`) is candidates-only reference. Re-derive
  from scratch; never copy v1 structure or file layout.

## Constants & strings (locked)

- All layout/spacing values via appbox kit `abx*` constants (abxPad16,
...
- Studio is a UI to operate on the appbox pipeline; each shell maps to
...
- Designer manages assets; defaults from `kit/assets_default/` (appbox
...
- `appbox design lint <artifact-dir>` clean.
