...
export const page = (c, h) => h.render(c, VIEW, facade.pairingContext(h.session(c).data, h.prefs(c)));
...
export const confirm = async (c, h) => {
  ...
  const ok = facade.confirmPairing(h.session(c).data, String(form.code || ''));
  if (ok) return c.redirect('/pair', 303);
  return h.render(c, VIEW, facade.pairingContext(h.session(c).data, h.prefs(c)), 422);
};
