...
export const page = (c, h) =>
  h.render(c, VIEW, { activeTab: 'app', ...facade.dashboardContext(h.session(c).data, h.prefs(c)) });

// Needs-you quick actions — seeded decision, 303 back to the dashboard.
export const decide = async (c, h) => {
  const form = await h.form(c);
  facade.decideGate(h.session(c).data, String(form.gate || ''), String(form.decision || ''));
