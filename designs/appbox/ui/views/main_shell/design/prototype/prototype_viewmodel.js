...
export const page = (c, h) =>
  h.render(c, VIEW, { activeShell: 'design', ...facade.stageContext(h.session(c).data, { line: 'prototype', pin: c.req.query('screen') ?? null }, h.prefs(c), h.t(c), h.locale(c)) });
...
export const rail = (c, h) =>
  h.render(c, `${VIEW}#filterSwap`, facade.setRailFilter(h.session(c).data, c.req.query('epic') ?? 'all', h.prefs(c), h.t(c), h.locale(c)));

// Multi-view rail carousel: screens / artifacts / files — body swap plus
// head/bar out-of-band (the rail owns its active-icon/label state).
