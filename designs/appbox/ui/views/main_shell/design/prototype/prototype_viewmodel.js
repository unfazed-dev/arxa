...
// The design tab opens on the chat: the first agent message offers to draft
// every surface at once. ?screen=<id> deep-links a context pin.
export const page = (c, h) =>
  h.render(c, VIEW, { activeTab: 'design', ...facade.stageContext(h.session(c).data, { line: 'prototype', pin: c.req.query('screen') ?? null }, h.prefs(c)) });
...
export const railView = (c, h) =>
  h.render(c, `${VIEW}#railBody`, facade.setRailView(h.session(c).data, c.req.param('view'), h.prefs(c)));
