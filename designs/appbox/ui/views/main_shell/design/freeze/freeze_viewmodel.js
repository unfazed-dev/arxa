...
export const page = (c, h) =>
  h.render(c, VIEW, { activeShell: 'design', ...facade.freezeContext(h.session(c).data, h.prefs(c), h.t(c), h.locale(c)) });
...
export const send = async (c, h) => {
  ...
  return h.render(c, `${VIEW}#stageSwap`, facade.freezeContext(h.session(c).data, h.prefs(c), h.t(c), h.locale(c)));
};
...
export const close = (c, h) => {
  facade.closeChat(h.session(c).data, {}, {}, h.t(c), h.locale(c));
  return h.render(c, `${VIEW}#stageSwap`, facade.freezeContext(h.session(c).data, h.prefs(c), h.t(c), h.locale(c)));
};
