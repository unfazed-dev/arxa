...
export const send = async (c, h) => {
  ...
  facade.sendChat(h.session(c).data, text, h.prefs(c));
...
// Filmstrip × on THIS surface: toggle the pin, re-render the freeze stage
// (the shared toggle mutates the session; freezeContext rebuilds the rest).
export const context = (c, h) => {
  facade.toggleContext(h.session(c).data, c.req.param('id'), c.req.query('state') ?? 'toggle', h.prefs(c));
  return h.render(c, `${VIEW}#stageSwap`, facade.freezeContext(h.session(c).data, h.prefs(c)));
};
