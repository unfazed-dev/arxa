...
export const depth = async (c, h) => {
  const form = await h.form(c);
  return h.render(c, `${VIEW}#stageSwap`, facade.chooseDepth(h.session(c).data, String(form.depth || ''), h.prefs(c), h.t(c), h.locale(c)));
};
...
export const answer = async (c, h) => {
  ...
  if (!text) return h.noContent(c);
  return h.render(c, `${VIEW}#stageSwap`, facade.answerQuestion(h.session(c).data, String(form.q || ''), text, h.prefs(c), h.t(c), h.locale(c)));
};

// A question card's skip affordance.
export const skip = async (c, h) => {
  const form = await h.form(c);
  return h.render(c, `${VIEW}#stageSwap`, facade.skipQuestion(h.session(c).data, String(form.q || ''), h.prefs(c), h.t(c), h.locale(c)));
};

// Revisit an answered/skipped card: ?q=<id> re-opens its form.
export const edit = (c, h) =>
  h.render(c, `${VIEW}#stageSwap`, facade.editQuestion(h.session(c).data, c.req.query('q'), h.prefs(c), h.t(c), h.locale(c)));

// The approval gate: the "approve the story map" quick-reply flips seed state.
export const approve = (c, h) =>
  h.render(c, `${VIEW}#stageSwap`, facade.approveMap(h.session(c).data, h.prefs(c), h.t(c), h.locale(c)));
