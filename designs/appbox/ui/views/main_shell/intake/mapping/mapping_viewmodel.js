...
// Clicking a rail card (or a story card in the grid) swaps the canvas.
...
// Rail top-bar filter: ?release=<name>|all — re-frames the swimlanes.
export const filter = (c, h) =>
  h.render(c, `${VIEW}#filterSwap`, facade.setFilter(h.session(c).data, S, 'release', c.req.query('release') ?? 'all', h.prefs(c)));
