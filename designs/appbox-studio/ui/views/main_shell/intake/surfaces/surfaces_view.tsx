...
function SurfaceRow({ s, t }: { s: Surface; t: TFn }) {
  return (
    <li class="surface-row">
      ...
      {s.release && <span class="chip chip--muted">{s.release}</span>}
      <SH.ProvChip p={s.provenance} t={t} />
      <span class="surface-states">
        <span class="fact-label">{t('intake.surfaces.statesLabel') as string}</span>
        {(s.states ?? []).map((st, i) => <span key={i} class="chip chip--sm chip--muted">{st}</span>)}
      </span>
    </li>
  );
}
  ...
  return (
    <article class={`artifact surface-group-card is-${item.state}`}>
      <header class="artifact-head">
        ...
        <span class="chip chip--muted">{t('intake.surfaces.surfaceCount', { count: (item.surfaces ?? []).length }) as string}</span>
...
function SummaryCard({ c, item, t }: { c: Ctx; item: SurfaceGroup; t: TFn }) {
  return (
    <div class={`q-card is-${item.state}`}>
      <p class="q-text">{item.label} <span class="chip chip--muted">{t('intake.surfaces.surfaceCount', { count: (item.surfaces ?? []).length }) as string}</span> {item.edited && <span class="chip chip--muted">{t('intake.item.edited') as string}</span>}</p>
      <SH.ItemActions c={c} item={item} t={t} />
    </div>
  );
}
...
function SurfacesStage({ c, t }: { c: Ctx; t: TFn }) {
  ...
  return (
    <SH.StepStage c={c} t={t}>
      {step.complete ? (
        <Fragment>
          <p class="artifact-lede">{t('intake.step.allConfirmed', { total: step.total }) as string}</p>
