...
import { CtaLink, inspectAttrs, Label, Heading } from '../../../../common/widgets/primitives.tsx';
...
function ProvChip({ k, t }: { k: Kit; t: TFn }) {
  ...
  return (
    <span class={`chip prov-chip prov-${k.provenance}`} title={title} {...inspectAttrs('scaffold.picker:prov-chip', { role: 'label' })}>
      ...
      {k.confidence && <Fragment> <Label name="scaffold.picker:prov-conf" class="prov-conf">{t(`scaffold.picker.prov.confidence.${k.confidence}`) as string}</Label></Fragment>}
    </span>
  );
}
  ...
  return (
    ...
    >
      ...
      {!k.readinessAxis?.ready && (
        <p class="kit-keys">
          <Label name="scaffold.picker:keys-note" class="muted">{t('scaffold.picker.readiness.informOnly') as string}</Label>
          {' '}
      ...
      <footer class="kit-foot" {...inspectAttrs('scaffold.picker:kit-foot', { role: 'group' })}>
        {k.essential ? (
          <span class="chip chip--muted" {...inspectAttrs('scaffold.picker:essential-badge', { role: 'label' })}><Icon name="lock" size={13} /> {t('scaffold.picker.essential') as string}</span>
        ) : k.auto ? (
          <span class="chip chip--muted" title={t('scaffold.picker.auto.why', { kits: (k.requiredBy ?? []).join(', ') }) as string} {...inspectAttrs('scaffold.picker:auto-badge', { role: 'label' })}>
            <Icon name="git-merge" size={13} /> {t('scaffold.picker.auto') as string}
          </span>
        ) : c.gated ? (
          <span class="chip chip--muted" {...inspectAttrs('scaffold.picker:gated-badge', { role: 'label' })}><Icon name="eye" size={13} /> {t('scaffold.picker.gated.badge') as string}</span>
        ) : k.selected ? (
...
function AutoNotice({ c, t }: { c: PickerViewProps; t: TFn }) {
  ...
  return (
    <aside class="picker-notice picker-notice--auto">
      <h4 class="fact-label" {...inspectAttrs('scaffold.picker:auto-notice-title', { role: 'heading' })}><Icon name="git-merge" size={14} /> {t('scaffold.picker.autoNotice.title') as string}</h4>
      <Txt name="scaffold.picker:auto-notice-body">{t('scaffold.picker.autoNotice.body') as string}</Txt>
...
function RemoveConfirm({ c, t }: { c: PickerViewProps; t: TFn }) {
  ...
  return (
    <aside class="picker-confirm" role="alertdialog" aria-labelledby="confirm-title">
      <h3 class="display" id="confirm-title" {...inspectAttrs('scaffold.picker:confirm-title', { role: 'heading' })}>{t('scaffold.picker.confirm.title', { kit: cf.kit.id }) as string}</h3>
      {cf.blockedBy?.length ? (
        <Fragment>
        ...
        <Fragment>
          {cf.screens?.length ? (
            <Txt name="scaffold.picker:confirm-screens">{t('scaffold.picker.confirm.declared', { screens: cf.screens.join(', ') }) as string}</Txt>
          ) : null}
          {cf.forcesFallback && (
          ...
          <div class="confirm-actions">
            <form method="post" action={cf.confirmHref} hx-post={cf.confirmHref} hx-target="#panels" hx-swap="outerMorph">
              <button type="submit" class="cta-ghost is-destructive" {...inspectAttrs('scaffold.picker:confirm-yes', { role: 'action' })}>{t('scaffold.picker.confirm.yes') as string}</button>
            </form>
            <form method="post" action={cf.cancelHref} hx-post={cf.cancelHref} hx-target="#picker-grid" hx-swap="outerHTML">
              <button type="submit" class="cta-ghost" {...inspectAttrs('scaffold.picker:confirm-cancel-alt', { role: 'action' })}>{t('scaffold.picker.confirm.cancel') as string}</button>
            </form>
          </div>
        </Fragment>
      )}
...
function GatedNotice({ c, t }: { c: PickerViewProps; t: TFn }) {
  ...
  return (
    <aside class="picker-notice picker-notice--gated" data-gated={c.gatedReason}>
      <h4 class="fact-label" {...inspectAttrs('scaffold.picker:gated-title', { role: 'heading' })}><Icon name="lock" size={14} /> {t(`${k}.title`) as string}</h4>
      <Txt name="scaffold.picker:gated-body">{t(`${k}.body`) as string}</Txt>
...
function ManifestPanel({ c, t }: { c: PickerViewProps; t: TFn }) {
  return (
    <section class="picker-manifest">
      <h4 class="fact-label" {...inspectAttrs('scaffold.picker:manifest-title', { role: 'heading' })}><Icon name="file-json" size={14} /> {t('scaffold.picker.manifest.title') as string}</h4>
      <Txt name="scaffold.picker:manifest-lede" class="muted">{t('scaffold.picker.manifest.lede') as string}</Txt>
      <dl class="manifest-facts">
        <dt {...inspectAttrs('scaffold.picker:manifest-resolved-label', { role: 'label' })}>{t('scaffold.picker.manifest.resolved') as string}</dt>
        ...
        {c.manifest?.wishlist?.length ? (
          <Fragment>
            <dt {...inspectAttrs('scaffold.picker:manifest-wishlist-label', { role: 'label' })}>{t('scaffold.picker.manifest.wishlist') as string}</dt>
        ...
        {c.manifest?.todos?.length ? (
          <Fragment>
            <dt {...inspectAttrs('scaffold.picker:manifest-todos-label', { role: 'label' })}>{t('scaffold.picker.manifest.todos') as string}</dt>
...
function PickerGrid({ c, t }: { c: PickerViewProps; t: TFn }) {
  return (
    <div class="picker-grid" id="picker-grid" {...inspectAttrs('scaffold.picker:grid', { role: 'group' })}>
      {c.loading ? (
        <p class="picker-loading" aria-busy="true" {...inspectAttrs('scaffold.picker:loading', { role: 'text' })}><Icon name="loader" size={16} /> {t('scaffold.picker.loading') as string}</p>
      ) : c.error ? (
        <aside class="picker-notice picker-notice--error" role="alert">
          <h4 class="fact-label" {...inspectAttrs('scaffold.picker:error-title', { role: 'heading' })}><Icon name="circle-alert" size={14} /> {t('scaffold.picker.error.title') as string}</h4>
          <Txt name="scaffold.picker:error-body">{t('scaffold.picker.error.body') as string}</Txt>
        ...
        <Fragment>
          {c.state === 'empty' && (
            <aside class="picker-notice">
              <h4 class="fact-label" {...inspectAttrs('scaffold.picker:empty-title', { role: 'heading' })}>{t('scaffold.picker.empty.title') as string}</h4>
              <Txt name="scaffold.picker:empty-body">{t('scaffold.picker.empty.body') as string}</Txt>
            </aside>
          )}
          <AutoNotice c={c} t={t} />
          ...
          {(c.groups ?? []).map((g) => (
            <section class="kit-group" id={`group-${g.id}`} key={g.id}>
              <h3 class="fact-label kit-group-head" {...inspectAttrs('scaffold.picker:group-head', { role: 'heading' })}>
                {g.label}
                <Label name="scaffold.picker:group-count" class="chip chip--sm chip--muted">{t('scaffold.picker.groupCount', { selected: g.selectedCount, count: g.count }) as string}</Label>
              </h3>
    ...
    <section class="mp-content" id="mp-content" aria-live="polite">
      <div class="picker-stage">
        <header class="picker-head">
          <Heading name="scaffold.picker:page-title" level={1} class="display">{t('scaffold.picker.pageTitle') as string}</Heading>
          <Txt name="scaffold.picker:lede" class="artifact-lede">{t('scaffold.picker.lede') as string}</Txt>
          <p class="picker-counts">
            <Label name="scaffold.picker:selected-count" class="chip chip--accent">{t('scaffold.picker.selectedCount', { selected: c.counts?.selected, total: c.counts?.total }) as string}</Label>
            <Label name="scaffold.picker:summary" class="chip chip--muted">{t('scaffold.picker.summary', { declared: c.counts?.declared, inferred: c.counts?.inferred, requested: c.counts?.requested }) as string}</Label>
            {c.counts?.auto ? <Label name="scaffold.picker:auto-count" class="chip chip--muted">{t('scaffold.picker.autoCount', { count: c.counts.auto }) as string}</Label> : null}
            {c.counts?.unready ? <Label name="scaffold.picker:unready-count" class="chip chip--sm kit-readiness kit-readiness--missing-keys">{t('scaffold.picker.unreadyCount', { count: c.counts.unready }) as string}</Label> : null}
          </p>
        </header>
        ...
        <footer class="picker-foot" {...inspectAttrs('scaffold.picker:foot', { role: 'group' })}>
          {c.canContinue ? (
            ...
            <span class="cta-main is-disabled" aria-disabled="true" {...inspectAttrs('scaffold.picker:continue-blocked', { role: 'action' })}>{t('scaffold.picker.continue.blocked') as string}</span>
          )}
        </footer>
      </div>
    </section>
