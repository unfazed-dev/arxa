...
Studio served with the portalo project mounted (the `~/.appbox/current`
marker), never touching the user's server on 4319:

```sh
dart run appboxd/bin/appbox.dart design serve designs/appbox-studio --port 4422
dart run appboxd/tool/lens_shot.dart http://127.0.0.1:4422/<route> <out.png> <w> <h> <settleMs>
```
...
Diff metric: exact-pixel diff, `dart run appboxd/tool/png_diff.dart a.png b.png`
(tolerance 0).
