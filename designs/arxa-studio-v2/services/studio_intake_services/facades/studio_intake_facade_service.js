...
import * as repo from '../repositories/studio_intake_repository_service.js';
...
function stepItems(step, activeLocale) {
  if (step === 'personas') {
    ...
    // same rule the mapping/brief/moodboard panels now follow: arxa's own
    // personas are arxa's, and walking a client through them as if they were
...
// arxa product plan and arxa's own reference captures, rendered under the
...
function resolveArtifact(surface, ref, translate, activeLocale) {
  ...
  if (surface === 'brief') {
    ...
    // arxa-story-mapper · …") is deliberately NOT reproduced: it is a
...
function activityViewFor(sd, surface, base, lv, translate, activeLocale) {
  ...
  if (active === 'artifacts') {
    ...
    // arxa's story map and arxa's moodboard. Rendered beside a client
