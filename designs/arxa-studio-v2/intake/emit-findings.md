...
Structure emits through the v2-native chain (`arxa emit structure` reads
...
- **F0 — BLOCKER: `intake/registry.json` does not exist and the designer is
  ...
  - The skill's run-kind table admits exactly two inputs; the one matching this
    ...
    v1 (`designs/arxa-studio/intake/`) has none either.
...
- **F3 — SECOND BLOCKER (verified, not assumed): the Q-v2-5 inspect rename has
  not landed.** Measured spellings across `arxa/`, the skill and `kit/`:
  ...
  | **`data-inspect-screen`** | **2** — `arxa/lib/probes/studio/probe_inspect.dart`, `references/app-architecture.md` |
  | **`data-inspect-view`** | **0** |
...
- **F5 — tooling invocation.** `arxa` is not on `PATH`. The package is
  `arxa` (`arxa/pubspec.yaml`) with entrypoints `arxa/bin/arxa.dart`
  and `arxa/bin/arxad.dart`; `dart` is at
  `/Users/unfazed-mac/fvm/default/bin/dart`. Brief gate commands
  (`arxa design lint …`, `arxa design serve …`, `arxa lens …`) must be
  issued as `dart run` against `arxa/bin/arxa.dart`.

  **Toolchain preflighted and GREEN** —
  `dart run arxa/bin/arxa.dart design doctor` (exit 0):
...
- **F6 — INCIDENT: commit `49249e9` over-captured another agent's in-flight
  ...
  - Before committing I checked `git status --short designs/arxa-studio-v2/`,
    which reported a single line — `?? designs/arxa-studio-v2/`. Git collapses
  ...
  - Concurrency itself *is* evidenced, by later timestamps rather than by the
    ...
    `cc0afeb` "feat: emit arxa-studio-v2 hub and startup ceremony shell per
    Q-v2-1..5", landed at 15:49:14. Another agent is actively building here.
  - `git add designs/arxa-studio-v2/` therefore swept in 9 files I did not
  ...
  - **CLOSED 2026-08-12 — operator ruling (grill D8, see
    ...
    correction: `49249e9` ("docs: record arxa-studio-v2 emit blockers…")
    ...
    designs/arxa-studio-v2/` after `git status --short` collapsed the
