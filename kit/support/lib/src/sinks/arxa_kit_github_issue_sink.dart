...
import 'arxa_kit_submission_sink.dart';

/// STUB — GitHub-issue submission sink.
///
/// TODO(arxa_kit_support): open an issue from the submission. Implement via
...
///   - title       -> derived from ArxaKitFeedbackSubmission.text (first line, capped)
...
///   - ArxaKitSubmissionResult.reference -> the created issue number; .location -> its
///     html_url.
///
/// This stub imports no HTTP client, so no networking dependency is pulled until
/// a host opts in.
class ArxaKitGithubIssueSink implements ArxaKitSubmissionSink {
  ArxaKitGithubIssueSink({this.id = 'github-issue', this.owner, this.repo});
  ...
  Future<ArxaKitSubmissionResult> submit(ArxaKitFeedbackSubmission submission) async =>
      throw UnimplementedError('ArxaKitGithubIssueSink.submit is a stub');
}
