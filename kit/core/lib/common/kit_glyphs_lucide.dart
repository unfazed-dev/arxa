...
import 'kit_glyphs_lucide_map.g.dart';

export 'kit_glyphs_lucide_map.g.dart';

/// Lucide icon namespace — resolves a Lucide design name (kebab-case, as on
/// lucide.dev, e.g. `'arrow-left'`) to its [IconData]. Reached as
/// `KitGlyphs.lucide('arrow-left')`.
///
/// Backed by `lucide_flutter`; the name→glyph map ([kitLucideGlyphMap]) is
...
/// this one (exposed as another `KitGlyphs.<library>` accessor) — no
/// call-site changes.
///
...
final class KitLucideGlyphs {
  const KitLucideGlyphs();
  ...
  IconData call(String name) {
    final icon = kitLucideGlyphMap[name];
    assert(icon != null, 'KitGlyphs.lucide: unknown Lucide icon "$name"');
    return icon ?? kitLucideGlyphMap[_fallback]!;
  }
}
