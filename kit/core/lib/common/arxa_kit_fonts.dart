...
/// and licences. It does NOT bundle font binaries. [arxaKitFontIsBundled]
...
const List<ArxaKitFontFamily> arxaKitFontOptions = [
  ArxaKitFontFamily(
    id: 'lexend',
    label: 'Lexend',
    cssName: 'Lexend',
    role: ArxaKitFontRole.ui,
    license: 'OFL-1.1',
  ),
  ArxaKitFontFamily(
    id: 'grotesk',
  ...
  ArxaKitFontFamily(
    ...
    role: ArxaKitFontRole.mono,
    license: 'OFL-1.1',
  ),
];

/// Mirrors fonts.json `default`.
const String arxaKitDefaultFont = 'lexend';

/// The face with [id], or the [arxaKitDefaultFont] one when unknown — a persisted
/// setting naming a removed face must not crash a host.
ArxaKitFontFamily arxaKitFontById(String id) {
  for (final f in arxaKitFontOptions) {
    if (f.id == id) return f;
  }
  for (final f in arxaKitFontOptions) {
    if (f.id == arxaKitDefaultFont) return f;
  }
  return arxaKitFontOptions.first;
}

/// The first face declared for [role], or null when none is.
ArxaKitFontFamily? arxaKitFontForRole(ArxaKitFontRole role) {
  for (final f in arxaKitFontOptions) {
...
Future<bool> arxaKitFontIsBundled(ArxaKitFontFamily family) async {
  try {
    await rootBundle.load('packages/arxa_kit_core/fonts/${family.id}.ttf');
    return true;
  } catch (_) {
    return false;
  }
...
void registerArxaKitFontLicenses() {
  LicenseRegistry.addLicense(() async* {
    for (final f in arxaKitFontOptions) {
      if (!await arxaKitFontIsBundled(f)) continue;
      final text =
          await rootBundle.loadString('packages/arxa_kit_core/fonts/${f.id}.LICENSE');
      yield LicenseEntryWithLineBreaks(<String>['arxa_kit_core', f.cssName], text);
    }
  });
}
...
/// platform default silently (see [arxaKitFontIsBundled]).
///
...
/// by passing [arxaKitDefaultGoogleFontFamily] to `arxaKitLightTheme` /
/// `arxaKitDarkTheme`.
///
...
String arxaKitGoogleFontFamily(ArxaKitFontFamily face) =>
    GoogleFonts.getFont(face.cssName).fontFamily ?? face.cssName;

/// The [arxaKitDefaultFont] face (`fonts.json` `default`), resolved and
/// registered through `google_fonts` — the app-wide `ui`-role family a host
/// passes to the kit theme builders.
String arxaKitDefaultGoogleFontFamily() =>
    arxaKitGoogleFontFamily(arxaKitFontById(arxaKitDefaultFont));
