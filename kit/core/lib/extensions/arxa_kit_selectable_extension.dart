...
enum ArxaKitSelectablePosition {
  topLeft,
  topCenter,
  topRight,
  center,
...
/// Configuration for ArxaKitSelectable
class ArxaKitSelectableOptions {
  ...
  final ArxaKitSelectablePosition position;

  /// Callback when selection changes
  final Function(bool)? onSelectionChanged;

  /// Constructor with default values
  const ArxaKitSelectableOptions({
    this.borderColor,
    this.borderWidth = 3.0,
    this.borderRadius,
    this.position = ArxaKitSelectablePosition.topRight,
    this.onSelectionChanged,
  });
}

/// Service to manage selectable widgets state
class ArxaKitSelectableService with ListenableServiceMixin {
  static const String widgetId = 'kit_selectable_service';

  ArxaKitSelectableService() {
...
extension ArxaKitSelectableExtension on Widget {
  /// Make this widget selectable with a checkbox
  Widget withSelectable({
    required String id,
    ArxaKitSelectableOptions options = const ArxaKitSelectableOptions(),
  }) {
...
class _KitSelectableViewModel extends ReactiveViewModel {
  ...
  final _errorService = arxaKitLocator<ArxaKitErrorService>();
  final _selectableService = arxaKitLocator<ArxaKitSelectableService>();

  late String _id;
  late ArxaKitSelectableOptions _options;
  ...
  void init(String id, ArxaKitSelectableOptions options) {
    _id = id;
    _options = options;
    _setupStreams();
  }
...
class _KitSelectableWidget extends StackedView<_KitSelectableViewModel> {
  final Widget child;
  final String id;
  final ArxaKitSelectableOptions options;
  ...
  double? _getLeftPosition(ArxaKitSelectablePosition position) {
    switch (position) {
      case ArxaKitSelectablePosition.topLeft:
      case ArxaKitSelectablePosition.bottomLeft:
        return 4;
      case ArxaKitSelectablePosition.topCenter:
      case ArxaKitSelectablePosition.center:
      case ArxaKitSelectablePosition.bottomCenter:
        return 0;
      case ArxaKitSelectablePosition.topRight:
      case ArxaKitSelectablePosition.bottomRight:
        return null;
    }
  }

  double? _getRightPosition(ArxaKitSelectablePosition position) {
    switch (position) {
      case ArxaKitSelectablePosition.topRight:
      case ArxaKitSelectablePosition.bottomRight:
        return 4;
  ...
  double? _getTopPosition(ArxaKitSelectablePosition position) {
    switch (position) {
      case ArxaKitSelectablePosition.topLeft:
      case ArxaKitSelectablePosition.topCenter:
      case ArxaKitSelectablePosition.topRight:
  ...
  double? _getBottomPosition(ArxaKitSelectablePosition position) {
    switch (position) {
      case ArxaKitSelectablePosition.bottomLeft:
      case ArxaKitSelectablePosition.bottomCenter:
      case ArxaKitSelectablePosition.bottomRight:
