...
class ArxaKitPlatform {
  ...
  static ArxaKitPlatformOverride? override;
  ...
  // ArxaKitFidelity.validateAtRoot and the debug assert below; widgets
  ...
  static bool get supportsLiquidGlass =>
      ArxaKitFidelity.allowsNative && liquidGlassCapability;

  /// Capability ∧ fidelity policy.
  static bool get supportsComposeM3E =>
      ArxaKitFidelity.allowsNative && composeM3ECapability;
  ...
  static bool get supportsNativeChrome {
    ...
    assert(() {
      if (ArxaKitFidelity.strict && !nativeChromeCapability) {
        throw FidelityViolation(
          'native fidelity with no native chrome capability reached a tier '
          'gate — ArxaKitFidelity.validateAtRoot() was not called at app '
...
/// Test override for [ArxaKitPlatform]: force identity, iOS major version, and/or
/// the widget-tree [TargetPlatform]. All fields optional.
class ArxaKitPlatformOverride {
  const ArxaKitPlatformOverride({
    bool? isIOS,
    bool? isAndroid,
    int? iosMajor,
...
enum ArxaKitTier { native, fallback }
