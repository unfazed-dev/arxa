...
import 'appbox_kit_platform.dart';
...
/// scaffolder via `--dart-define=APPBOX_FIDELITY=<mode>` and read here as a
...
enum AppBoxKitFidelityMode {
  ...
  /// init via [AppBoxKitFidelity.validateAtRoot] (QF-1 amendment: throw
  /// site moved from first-gate-evaluation to root init).
  native,
}

/// Thrown (debug AND release) when a `native`-fidelity app runs on a target
/// without native chrome capability, or the `APPBOX_FIDELITY` define is not
/// a recognized mode.
...
/// [AppBoxKitPlatform.supportsNativeChrome] (and friends), which compose
/// [allowsNative] with raw capability. The generated app root calls
/// [validateAtRoot] once in `main()`.
class AppBoxKitFidelity {
  AppBoxKitFidelity._();

  static const String _raw =
      String.fromEnvironment('APPBOX_FIDELITY', defaultValue: 'mix');

  /// The mode declared at build time. Unrecognized raw values resolve to
  /// [AppBoxKitFidelityMode.mix] here so const evaluation stays total;
  /// [validateAtRoot] is what rejects them loudly.
  static const AppBoxKitFidelityMode declared = _raw == 'flutter'
      ? AppBoxKitFidelityMode.flutter
      : _raw == 'native'
          ? AppBoxKitFidelityMode.native
          : AppBoxKitFidelityMode.mix;
  ...
  @visibleForTesting
  static AppBoxKitFidelityMode? debugOverride;
  ...
  static AppBoxKitFidelityMode get mode {
    if (kDebugMode) {
  ...
  /// every [AppBoxKitPlatform] `supports*` getter.
  static bool get allowsNative => mode != AppBoxKitFidelityMode.flutter;

  /// True only in `native` mode (error-not-fallback semantics).
  static bool get strict => mode == AppBoxKitFidelityMode.native;
  ...
  static AppBoxKitFidelityMode parse(String raw) {
    switch (raw) {
      case 'flutter':
        return AppBoxKitFidelityMode.flutter;
      case 'mix':
        return AppBoxKitFidelityMode.mix;
      case 'native':
        return AppBoxKitFidelityMode.native;
      default:
        throw FidelityViolation(
          'unrecognized APPBOX_FIDELITY "$raw" — expected flutter|mix|native',
        );
    }
  }
