import 'package:appbox_kit_state/appbox_kit_state.dart';
import 'package:appbox_kit_state/testing.dart';
import 'package:test/test.dart';

void main() {
  group('KitState equality', () {
    test('same case and payload are equal', () {
      expect(const KitSuccess<int>(5), const KitSuccess<int>(5));
      expect(const KitLoading<int>(0.5), const KitLoading<int>(0.5));
      expect(const KitIdle<int>(), const KitIdle<int>());
      expect(
        const KitError<int>(KitFailure(code: 'x', message: 'm')),
        const KitError<int>(KitFailure(code: 'x', message: 'm')),
      );
    });

    test('different payloads are not equal', () {
      expect(const KitSuccess<int>(5) == const KitSuccess<int>(6), isFalse);
      expect(const KitLoading<int>(0.1) == const KitLoading<int>(0.2), isFalse);
    });
  });

  group('combinators', () {
    test('when folds the matching case', () {
      const KitState<int> state = KitSuccess(7);
      final label = state.when(
        idle: () => 'idle',
        pending: () => 'pending',
        loading: (p) => 'loading',
        success: (d) => 'success:$d',
        error: (f) => 'error',
      );
      expect(label, 'success:7');
    });

    test('dataOrNull / failureOrNull', () {
      expect(const KitSuccess<int>(3).dataOrNull, 3);
      expect(const KitIdle<int>().dataOrNull, isNull);
      expect(
        const KitError<int>(KitFailure(code: 'e', message: 'm')).failureOrNull?.code,
        'e',
      );
    });
  });

  group('transition guard', () {
    test('legal transition passes (idle -> loading)', () {
      final notifier = KitStateNotifier<int>();
      notifier.emit(const KitLoading<int>());
      expect(notifier.state, const KitLoading<int>());
      notifier.dispose();
    });

    test('legal transition passes (success -> loading)', () {
      final notifier = KitStateNotifier<int>(initial: const KitSuccess(1));
      notifier.emit(const KitLoading<int>());
      expect(notifier.state, const KitLoading<int>());
      notifier.dispose();
    });

    test('illegal transition trips an assertion in debug (idle -> success)', () {
      final notifier = KitStateNotifier<int>();
      expect(
        () => notifier.emit(const KitSuccess<int>(1)),
        throwsA(isA<AssertionError>()),
      );
      notifier.dispose();
    });

    test('scripted notifier bypasses the guard', () {
      final notifier = KitScriptedStateNotifier<int>();
      notifier.scriptAll(const [
        KitSuccess<int>(1),
        KitError<int>(KitFailure(code: 'x', message: 'x')),
      ]);
      expect(
        notifier.state,
        const KitError<int>(KitFailure(code: 'x', message: 'x')),
      );
      notifier.dispose();
    });
  });

  group('track', () {
    test('drives loading -> success and returns the value', () async {
      final notifier = KitStateNotifier<int>();
      final value = await notifier.track(Future.value(42));
      expect(value, 42);
      expect(notifier.state, const KitSuccess<int>(42));
      notifier.dispose();
    });

    test('drives loading -> error on throw', () async {
      final notifier = KitStateNotifier<int>();
      final value = await notifier.track(Future<int>.error(Exception('boom')));
      expect(value, isNull);
      expect(notifier.state.isError, isTrue);
      notifier.dispose();
    });
  });

  group('KitStateRecorder', () {
    test('captures the emitted sequence with value equality', () async {
      final notifier = KitStateNotifier<int>();
      final recorder = KitStateRecorder<int>(notifier);
      notifier.emit(const KitLoading<int>());
      notifier.emit(const KitSuccess<int>(7));
      await Future<void>.delayed(Duration.zero);
      expect(recorder.states, const [
        KitIdle<int>(),
        KitLoading<int>(),
        KitSuccess<int>(7),
      ]);
      await recorder.dispose();
      notifier.dispose();
    });
  });

  group('KitRetryPolicy', () {
    test('delay curve grows by backoff and caps at maxDelay', () {
      const policy = KitRetryPolicy(
        initialDelay: Duration(milliseconds: 100),
        backoffFactor: 2,
        maxDelay: Duration(milliseconds: 350),
      );
      expect(policy.delayForAttempt(1), Duration.zero);
      expect(policy.delayForAttempt(2), const Duration(milliseconds: 100));
      expect(policy.delayForAttempt(3), const Duration(milliseconds: 200));
      expect(policy.delayForAttempt(4), const Duration(milliseconds: 350));
    });
  });
}
