...
import '../crypto/arxa_kit_crypto_key.dart';
import '../crypto/arxa_kit_crypto_service.dart';
import '../storage/arxa_kit_secure_storage_service.dart';
import 'arxa_kit_pin_verifier.dart';

/// A [ArxaKitPinVerifier] that stores a per-PIN-salted HMAC of the PIN in
/// [ArxaKitSecureStorageService], hashing via [ArxaKitCryptoService].
///
...
class SecureStoragePinVerifier implements ArxaKitPinVerifier {
  SecureStoragePinVerifier({
    required ArxaKitSecureStorageService storage,
    required ArxaKitCryptoService crypto,
    String keyPrefix = 'arxa_kit_security.app_lock',
  })  : _storage = storage,
        _crypto = crypto,
        _saltKey = '$keyPrefix.pin_salt',
        _digestKey = '$keyPrefix.pin_digest';

  final ArxaKitSecureStorageService _storage;
  final ArxaKitCryptoService _crypto;
  ...
  Future<bool> verifyPin(String pin) async {
    ...
    final actual = await _crypto.hmacSha256(
      utf8.encode(pin),
      key: ArxaKitCryptoKey(base64Decode(saltB64)),
    );
