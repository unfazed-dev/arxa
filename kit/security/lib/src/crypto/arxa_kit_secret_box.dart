...
/// [ArxaKitSecretBox.fromConcatenated] reverses it.
@immutable
class ArxaKitSecretBox {
  ...
  ///
  /// Throws [ArxaKitCryptoFailure] with [ArxaKitCryptoFailureReason.malformed] when the
  /// buffer is too short to hold both.
  factory ArxaKitSecretBox.fromConcatenated(
    List<int> bytes, {
    int nonceLength = 12,
    int macLength = 16,
  }) {
  ...
  @override
  String toString() =>
      'ArxaKitSecretBox(nonce: ${nonce.length}B, cipherText: ${cipherText.length}B, '
      'mac: ${mac.length}B)';
}
