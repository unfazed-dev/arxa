...
/// Exercises the *real* [CryptographyArxaKitCryptoService]. The `cryptography`
...
void main() {
  const plainText = 'arxa_kit_security round-trip ✅';
  late ArxaKitCryptoService crypto;
  late ArxaKitCryptoKey key;
  ...
  group('tamper / wrong-key detection (authenticated encryption)', () {
    test('kit.security.crypto — flipping a ciphertext byte fails authentication', () async {
      ...
      await expectLater(
        ...
        throwsA(isA<ArxaKitCryptoFailure>()),
      );
    });
    ...
    test('kit.security.crypto — flipping a MAC byte fails authentication', () async {
      ...
      await expectLater(
        crypto.decryptBytes(bad, key: key),
        throwsA(
          isA<ArxaKitCryptoFailure>().having(
            (f) => f.reason,
            'reason',
  ...
  group('concatenated box', () {
    test('kit.security.crypto — round-trips through nonce || cipherText || mac', () async {
      final box = await crypto.encryptString(plainText, key: key);
      final rejoined = ArxaKitSecretBox.fromConcatenated(box.concatenated);
      expect(rejoined, box);
    ...
    test('kit.security.crypto — rejects a too-short buffer as malformed', () async {
      await expectLater(
        () => ArxaKitSecretBox.fromConcatenated(<int>[0, 1, 2]),
        throwsA(
          isA<ArxaKitCryptoFailure>().having(
            (f) => f.reason,
            'reason',
