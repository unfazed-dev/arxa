...
import 'package:arxa_kit_maps/src/providers/tiled/arxa_kit_tiled_map_view.dart';

/// Serves an in-memory 1x1 transparent PNG for every tile — no HTTP.
class FakeArxaKitTileProvider extends TileProvider {
  @override
...
final _config = ArxaKitMapConfig(
  initialCameraPosition: ArxaKitCameraPosition(
    target: ArxaKitLatLng(51.5074, -0.1278),
    zoom: 12,
  ),
  markers: {
    ArxaKitMapMarker(id: 'london', position: ArxaKitLatLng(51.5074, -0.1278)),
    ArxaKitMapMarker(id: 'greenwich', position: ArxaKitLatLng(51.4769, -0.0005)),
  },
);
...
void main() {
  group('ArxaKitOpenStreetMapProvider', () {
    testWidgets('kit.maps.tiled-providers — builds a flutter_map with OSM tiles, UA, attribution, '
      ...
      final provider = ArxaKitOpenStreetMapProvider(
        userAgentPackageName: 'com.example.test',
        tileProvider: FakeArxaKitTileProvider(),
      );
      ...
      expect(arxaKitTileProviderUsed(tester), isA<FakeArxaKitTileProvider>());
  ...
  group('ArxaKitMapboxProvider', () {
    ...
    testWidgets('kit.maps.tiled-providers — mapType selects the Mapbox style', (tester) async {
      ...
      String templateFor(ArxaKitMapType type) {
        final map =
            provider.buildMap(config: _config.copyWith(mapType: type));
        return (map as ArxaKitTiledMapView).urlTemplate;
      }

      expect(templateFor(ArxaKitMapType.normal), contains('streets-v12'));
      expect(templateFor(ArxaKitMapType.satellite), contains('satellite-v9'));
      expect(
        templateFor(ArxaKitMapType.hybrid),
        contains('satellite-streets-v12'),
      );
      expect(templateFor(ArxaKitMapType.terrain), contains('outdoors-v12'));
    });
  });

  group('ArxaKitTiledMapView controller', () {
    testWidgets('kit.maps.tiled-providers — onMapCreated delivers a controller that moves the camera',
      ...
      await controller!.moveCamera(
        const ArxaKitCameraPosition(target: ArxaKitLatLng(48.8566, 2.3522), zoom: 9),
      );
      await tester.pump();
...
TileProvider arxaKitTileProviderUsed(WidgetTester tester) =>
