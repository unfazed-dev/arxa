...
/// callbacks into a single [onResult] taking a typed [ArxaKitPaymentResult].
///
...
/// from `package:arxa_kit_payments/arxa_kit_payments.dart`.
class ArxaKitPayButton extends StatelessWidget {
  /// Apple Pay profile — used when running on iOS.
  final ArxaKitApplePayConfig? applePay;

  /// Google Pay profile — used when running on Android.
  final ArxaKitGooglePayConfig? googlePay;

  /// Line items shown in the sheet.
  final List<ArxaKitPaymentItem> items;

  /// Called with a [ArxaKitPaymentSuccess] on a minted token, or a [ArxaKitPaymentError] /
  /// [ArxaKitPaymentCancelled] when the plugin reports a failure. (Apple/Google Pay
  /// do not distinguish decline client-side; those arrive as [ArxaKitPaymentError].)
  final ValueChanged<ArxaKitPaymentResult> onResult;
      ...
      case TargetPlatform.iOS:
        ...
        return _resolve(
          ...
          (configuration) => pay.ApplePayButton(
            ...
            onPaymentResult: _handleSuccess(ArxaKitPaymentMethod.applePay),
      ...
      case TargetPlatform.android:
        ...
        return _resolve(
          ...
          (configuration) => pay.GooglePayButton(
            ...
            margin: margin,
            loadingIndicator: loader,
            onPaymentResult: _handleSuccess(ArxaKitPaymentMethod.googlePay),
            onError: _handleError,
          ),
        );
      default:
  ...
  void Function(Map<String, dynamic>) _handleSuccess(ArxaKitPaymentMethod method) =>
      (result) => onResult(
            ArxaKitPaymentSuccess(
  ...
  void _handleError(Object? error) =>
      onResult(ArxaKitPaymentError(error?.toString() ?? 'Payment failed', cause: error));
