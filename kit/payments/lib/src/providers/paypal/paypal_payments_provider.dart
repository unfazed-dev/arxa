...
class PayPalOrderApproval {
  ...
  const PayPalOrderApproval({required this.orderId, required this.approvalUrl});
}
...
abstract interface class KitPayPalBackend {
  ...
  /// back-out; build them from the app's [PayPalPaymentsProvider]
  /// callback scheme.
  Future<PayPalOrderApproval> createOrder({
  ...
  Future<PayPalCaptureResult> captureOrder(String orderId);
}
...
class WebAuthCancelled implements Exception {
  ...
  @override
  String toString() => 'WebAuthCancelled()';
}

/// In-app browser seam (ASWebAuthenticationSession / Chrome Auth Tab). The
/// real implementation is [FlutterWebAuth2Authenticator]; tests substitute a
/// mock so no platform channel is ever touched.
abstract interface class KitWebAuthenticator {
  /// Opens [url] and completes with the callback URL the flow redirected to.
  /// Throws [WebAuthCancelled] when the user dismisses the session.
  Future<Uri> authenticate({required Uri url, required String callbackUrlScheme});
}

/// [KitWebAuthenticator] over `flutter_web_auth_2`.
class FlutterWebAuth2Authenticator implements KitWebAuthenticator {
  const FlutterWebAuth2Authenticator();
...
/// order, the buyer approves it in an in-app browser via [KitWebAuthenticator]
/// (deep-link return), and the backend Port captures it.
///
/// PayPal is a redirect rail, not a native wallet sheet, so this provider
/// reports only [KitPaymentMethod.payPal] — never Apple/Google Pay.
...
/// [PaymentSuccess] with the capture id as the token; buyer back-out →
/// [PaymentCancelled]; failed capture → [PaymentDeclined].
class PayPalPaymentsProvider implements KitPaymentsProvider {
  /// Server-side Orders v2 seam.
  final KitPayPalBackend backend;
  ...
  final KitWebAuthenticator _authenticator;

  PayPalPaymentsProvider({
    required this.backend,
    required this.callbackUrlScheme,
    KitWebAuthenticator? authenticator,
  }) : _authenticator = authenticator ?? const FlutterWebAuth2Authenticator();

  @override
  String get id => 'paypal';

  @override
  Set<KitPaymentMethod> get supportedMethods => const {KitPaymentMethod.payPal};
  ...
  Future<PaymentResult> requestPayment({
    ...
    if (config.method != KitPaymentMethod.payPal || config is! PayPalConfig) {
      return PaymentError(
          'PayPalPaymentsProvider requires a PayPalConfig, got ${config.runtimeType}');
    }
    final amount = _total(items);
    if (amount == null) {
      return const PaymentError(
          'PayPalPaymentsProvider requires final-priced items');
    }
    try {
      ...
      if (status == 'COMPLETED') {
        return PaymentSuccess(
          ...
          method: KitPaymentMethod.payPal,
          raw: {
            'provider': 'paypal',
      ...
      if (status.contains('DECLIN') || status.contains('FAIL')) {
        return PaymentDeclined(reason: capture.declineReason);
      }
      return PaymentError('PayPal capture ended in status ${capture.status}');
    } catch (error) {
  ...
  /// failing.
  static String? _total(List<KitPaymentItem> items) {
    var total = 0.0;
