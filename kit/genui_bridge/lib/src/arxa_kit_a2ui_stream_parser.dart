...
/// One event from an [ArxaKitA2uiStreamParser]: a parsed message, a chunk of prose,
/// or a malformed attempted message.
sealed class ArxaKitA2uiStreamEvent {
  const ArxaKitA2uiStreamEvent();
}

/// A complete, structurally valid [ArxaKitA2uiMessage] parsed from the stream.
final class ArxaKitA2uiMessageEvent extends ArxaKitA2uiStreamEvent {
  const ArxaKitA2uiMessageEvent(this.message);

  /// The parsed message (already structurally valid; catalog validation is
  /// the bridge's job, not the parser's).
  final ArxaKitA2uiMessage message;
}

/// Non-message text the model emitted alongside (or instead of) A2UI.
final class ArxaKitA2uiTextEvent extends ArxaKitA2uiStreamEvent {
  const ArxaKitA2uiTextEvent(this.text);
...
final class ArxaKitA2uiErrorEvent extends ArxaKitA2uiStreamEvent {
  const ArxaKitA2uiErrorEvent(this.error, this.raw);

  /// What went wrong (typically an [ArxaKitA2uiFormatException] or
...
/// Incrementally parses a stream of LLM text chunks into [ArxaKitA2uiStreamEvent]s.
///
...
/// - anything else is prose and surfaces as [ArxaKitA2uiTextEvent].
///
...
///
/// - a fenced JSON block that fails `jsonDecode` is an [ArxaKitA2uiErrorEvent];
/// - a trailing partial `{...` when the input closes is an [ArxaKitA2uiErrorEvent]
///   (truncated message) instead of text.
class ArxaKitA2uiStreamParser extends StreamTransformerBase<String, ArxaKitA2uiStreamEvent> {
  const ArxaKitA2uiStreamParser();

  @override
  Stream<ArxaKitA2uiStreamEvent> bind(Stream<String> stream) =>
      _A2uiParserStream(stream).stream;
}

class _A2uiParserStream {
  _A2uiParserStream(Stream<String> input) {
    _controller = StreamController<ArxaKitA2uiStreamEvent>(
      onListen: () {
  ...
  late final StreamController<ArxaKitA2uiStreamEvent> _controller;
  ...
  Stream<ArxaKitA2uiStreamEvent> get stream => _controller.stream;
  ...
  void _onDone() {
    ...
    if (rest.contains('{')) {
      ...
      _controller.add(ArxaKitA2uiErrorEvent(
  ...
  void _emitText(String text) {
    ...
    if (clean.isNotEmpty) _controller.add(ArxaKitA2uiTextEvent(clean));
  }
  ...
  void _emitPayload(String content) {
    ...
    try {
      ...
      _controller.add(ArxaKitA2uiErrorEvent(e, content));
      _wasLastEventA2ui = false;
      return;
    }
  ...
  void _emitOne(Map<String, dynamic> json, String raw) {
    try {
      _controller.add(ArxaKitA2uiMessageEvent(ArxaKitA2uiMessage.fromJson(json)));
      _wasLastEventA2ui = true;
    } on ArxaKitA2uiFormatException catch (e) {
      if (json.keys.any(_a2uiMessageKeys.contains)) {
        _controller.add(ArxaKitA2uiErrorEvent(e, raw));
      } else {
        // Some other JSON object the model happened to emit; not A2UI.
        _controller.add(ArxaKitA2uiTextEvent(jsonEncode(json)));
      }
      _wasLastEventA2ui = false;
    }
  }
...
class _Match {
  ...
  /// also what an [ArxaKitA2uiErrorEvent] reports as its raw payload.
