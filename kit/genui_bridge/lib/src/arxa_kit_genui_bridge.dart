...
import 'arxa_kit_a2ui_message.dart';
import 'arxa_kit_a2ui_stream_parser.dart';
...
/// never fetched.
const String arxaKitA2uiBasicCatalogId =
    'https://a2ui.org/specification/v0_9/catalogs/basic/catalog.json';

/// The validated result of one [ArxaKitGenuiBridge.generate] turn.
class ArxaKitA2uiTurn {
  const ArxaKitA2uiTurn({
  ...
  /// partially emitted.
  final List<ArxaKitA2uiMessage> messages;
...
class ArxaKitGenuiBridgeFailure implements Exception {
  const ArxaKitGenuiBridgeFailure({
    required this.attempts,
  ...
  @override
  String toString() => 'ArxaKitGenuiBridgeFailure: still invalid after $attempts '
      'attempt(s):\n${errors.join('\n')}';
}

/// The reliability core: drives a [ArxaKitChatStream], parses its output into A2UI
...
class ArxaKitGenuiBridge {
  /// [catalog] maps component type names (`Text`, `Card`, …) to their JSON
  /// schemas (runtime inputs — flatten any `$ref`s first; see
  /// [ArxaKitJsonSchemaValidator] for the supported subset). An empty catalog
  /// disables per-component validation. [systemPrompt] overrides the prompt
  /// built from the catalog when you need full control.
  ArxaKitGenuiBridge({
    required ArxaKitChatStream chat,
    Map<String, ArxaKitJsonSchema> catalog = const {},
    this.catalogId = arxaKitA2uiBasicCatalogId,
    this.maxRepairs = 2,
    String? systemPrompt,
    ArxaKitJsonSchemaValidator validator = const ArxaKitJsonSchemaValidator(),
  })  : _chat = chat,
        _catalog = catalog,
        _validator = validator,
        systemPrompt = systemPrompt ??
            arxaKitBuildA2uiSystemPrompt(catalogId: catalogId, catalog: catalog);

  final ArxaKitChatStream _chat;
  final Map<String, ArxaKitJsonSchema> _catalog;
  final ArxaKitJsonSchemaValidator _validator;

  /// The catalogId the prompt instructs the model to use, and the one
  /// [ArxaKitCreateSurface] messages are checked against.
  ...
  /// turn, or throws [ArxaKitGenuiBridgeFailure] after 1 + [maxRepairs] attempts.
  Future<ArxaKitA2uiTurn> generate(List<ArxaKitChatMessage> messages) async {
    final attemptMessages = <ArxaKitChatMessage>[
      ArxaKitChatMessage.system(systemPrompt),
      ...messages,
    ];
    final errors = <String>[];
    var rawOutput = '';
    ...
    for (var attempt = 1; attempt <= maxRepairs + 1; attempt++) {
      final events = await const ArxaKitA2uiStreamParser()
          .bind(_chat.complete(attemptMessages))
          .toList();

      final messagesOut = <ArxaKitA2uiMessage>[];
      final texts = <String>[];
      ...
      for (final event in events) {
        switch (event) {
          case ArxaKitA2uiMessageEvent(:final message):
            messagesOut.add(message);
            raw.writeln(message.toJsonLine());
          case ArxaKitA2uiTextEvent(:final text):
            texts.add(text);
            raw.write(text);
          case ArxaKitA2uiErrorEvent(:final error, :final raw):
      ...
      if (attempt <= maxRepairs) {
        ...
        attemptMessages
          ..add(ArxaKitChatMessage.assistant(rawOutput))
          ..add(ArxaKitChatMessage.user(_repairInstruction(attemptErrors)));
      }
    }

    throw ArxaKitGenuiBridgeFailure(
  ...
  List<String> _validateCatalog(List<ArxaKitA2uiMessage> messages) {
    final errors = <String>[];
    for (final message in messages) {
      switch (message) {
        case ArxaKitCreateSurface(:final catalogId):
          if (catalogId != this.catalogId) {
            errors.add('createSurface: catalogId must be "${this.catalogId}"'
                ' (got "$catalogId")');
          }
        case ArxaKitUpdateComponents(:final components):
        ...
        case ArxaKitUpdateDataModel() || ArxaKitDeleteSurface():
  ...
  static String _repairInstruction(List<String> attemptErrors) =>
      ...
      'Output only JSONL: one {"version":"$arxaKitA2uiVersion",...} object per line, '
...
String arxaKitBuildA2uiSystemPrompt({
  String catalogId = arxaKitA2uiBasicCatalogId,
  Map<String, ArxaKitJsonSchema> catalog = const {},
}) {
...
Every message is an envelope {"version":"$arxaKitA2uiVersion","<verb>":{...}} with
