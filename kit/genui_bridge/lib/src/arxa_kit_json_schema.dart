...
typedef ArxaKitJsonSchema = Map<String, Object?>;

/// One schema violation found by [ArxaKitJsonSchemaValidator].
class ArxaKitSchemaError {
  const ArxaKitSchemaError(this.path, this.message);
...
/// Validates decoded JSON values against caller-supplied [ArxaKitJsonSchema]s.
///
...
class ArxaKitJsonSchemaValidator {
  const ArxaKitJsonSchemaValidator();

  /// Returns every violation of [value] against [schema]; empty means valid.
  List<ArxaKitSchemaError> validate(Object? value, ArxaKitJsonSchema schema,
          [String path = r'$']) =>
      _validate(value, schema, path);

  List<ArxaKitSchemaError> _validate(Object? value, ArxaKitJsonSchema schema, String path) {
    final errors = <ArxaKitSchemaError>[];

    final const_ = schema['const'];
    if (schema.containsKey('const') && value != const_) {
      errors.add(ArxaKitSchemaError(path, 'must be $const_'));
    ...
    if (enum_ is List && !enum_.any((e) => _deepEquals(e, value))) {
      errors.add(ArxaKitSchemaError(path, 'must be one of $enum_'));
      return errors;
    }
    ...
    if (type != null && !_matchesType(value, type)) {
      errors.add(
          ArxaKitSchemaError(path, 'must be of type $type, got ${_typeName(value)}'));
    ...
    if (value is Map<String, dynamic>) {
      ...
      if (required is List) {
        for (final key in required) {
          if (!value.containsKey(key)) {
            errors.add(ArxaKitSchemaError(path, 'missing required property "$key"'));
          }
        }
      }
      ...
      if (properties is Map<String, dynamic>) {
        for (final entry in properties.entries) {
          if (value.containsKey(entry.key) && entry.value is ArxaKitJsonSchema) {
            errors.addAll(_validate(value[entry.key],
                entry.value! as ArxaKitJsonSchema, '$path.${entry.key}'));
          }
        }
      }
      if (schema['additionalProperties'] == false && properties is Map) {
        for (final key in value.keys) {
          if (!properties.containsKey(key)) {
            errors.add(ArxaKitSchemaError(path, 'unexpected property "$key"'));
          }
        }
      }
    }
    ...
    if (value is String) {
      ...
      if (minLength is int && value.length < minLength) {
        errors
            .add(ArxaKitSchemaError(path, 'must be at least $minLength character(s)'));
      }
    }
    ...
    if (value is num) {
      ...
      if (minimum is num && value < minimum) {
        errors.add(ArxaKitSchemaError(path, 'must be >= $minimum'));
      }
      final maximum = schema['maximum'];
      if (maximum is num && value > maximum) {
        errors.add(ArxaKitSchemaError(path, 'must be <= $maximum'));
      }
    }
