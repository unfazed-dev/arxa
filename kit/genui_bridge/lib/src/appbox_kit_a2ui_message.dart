...
const String appBoxKitA2uiVersion = 'v0.9';

/// Thrown when a JSON envelope is not a well-formed [AppBoxKitA2uiMessage].
///
...
class AppBoxKitA2uiFormatException extends FormatException {
  AppBoxKitA2uiFormatException(super.message, [this.details]);

  /// The JSON payload that failed to parse, when available.
  final Object? details;

  @override
  String toString() => 'AppBoxKitA2uiFormatException: $message';
}
...
/// `surfaceUpdate` (→ [AppBoxKitUpdateComponents]) and `dataModelUpdate`
/// (→ [AppBoxKitUpdateDataModel]) are accepted as aliases. Writing always uses the
/// canonical v0.9 names. (v0.8's `beginRendering` is NOT aliased — its body
/// shape differs from [AppBoxKitCreateSurface]; a pre-v0.9 message without
/// `version: "v0.9"` is rejected outright.)
sealed class AppBoxKitA2uiMessage {
  const AppBoxKitA2uiMessage();
  ...
  ///
  /// Throws [AppBoxKitA2uiFormatException] on any structural problem — callers feeding
  /// LLM output should treat that as a repairable validation failure.
  static AppBoxKitA2uiMessage fromJson(Map<String, dynamic> json) {
    final Object? rawVersion = json['version'];
    if (rawVersion is! String || rawVersion != appBoxKitA2uiVersion) {
      throw AppBoxKitA2uiFormatException(
        'A2UI message must have version "$appBoxKitA2uiVersion" '
    ...
    switch (verb) {
      ...
      case 'updateComponents':
        ...
        return AppBoxKitUpdateComponents(
          surfaceId: _requiredString(body, 'surfaceId', verb, json),
          components: [
      ...
      case 'updateDataModel':
        ...
        return AppBoxKitUpdateDataModel(
          surfaceId: _requiredString(body, 'surfaceId', verb, json),
          path: path as String?,
          value: body['value'],
        );
      case 'deleteSurface':
        return AppBoxKitDeleteSurface(
          surfaceId: _requiredString(body, 'surfaceId', verb, json),
        );
      default:
...
final class AppBoxKitCreateSurface extends AppBoxKitA2uiMessage {
  const AppBoxKitCreateSurface({
...
final class AppBoxKitUpdateComponents extends AppBoxKitA2uiMessage {
  const AppBoxKitUpdateComponents({required this.surfaceId, required this.components});
...
/// [path].
final class AppBoxKitUpdateDataModel extends AppBoxKitA2uiMessage {
  const AppBoxKitUpdateDataModel({required this.surfaceId, this.path, this.value});
...
final class AppBoxKitDeleteSurface extends AppBoxKitA2uiMessage {
  const AppBoxKitDeleteSurface({required this.surfaceId});
