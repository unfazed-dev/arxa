...
const String arxaKitA2uiVersion = 'v0.9';

/// Thrown when a JSON envelope is not a well-formed [ArxaKitA2uiMessage].
///
...
class ArxaKitA2uiFormatException extends FormatException {
  ArxaKitA2uiFormatException(super.message, [this.details]);

  /// The JSON payload that failed to parse, when available.
  final Object? details;

  @override
  String toString() => 'ArxaKitA2uiFormatException: $message';
}
...
/// `surfaceUpdate` (→ [ArxaKitUpdateComponents]) and `dataModelUpdate`
/// (→ [ArxaKitUpdateDataModel]) are accepted as aliases. Writing always uses the
/// canonical v0.9 names. (v0.8's `beginRendering` is NOT aliased — its body
/// shape differs from [ArxaKitCreateSurface]; a pre-v0.9 message without
/// `version: "v0.9"` is rejected outright.)
sealed class ArxaKitA2uiMessage {
  const ArxaKitA2uiMessage();
  ...
  ///
  /// Throws [ArxaKitA2uiFormatException] on any structural problem — callers feeding
  /// LLM output should treat that as a repairable validation failure.
  static ArxaKitA2uiMessage fromJson(Map<String, dynamic> json) {
    final Object? rawVersion = json['version'];
    if (rawVersion is! String || rawVersion != arxaKitA2uiVersion) {
      throw ArxaKitA2uiFormatException(
        'A2UI message must have version "$arxaKitA2uiVersion" '
    ...
    switch (verb) {
      ...
      case 'updateComponents':
        ...
        return ArxaKitUpdateComponents(
          surfaceId: _requiredString(body, 'surfaceId', verb, json),
          components: [
      ...
      case 'updateDataModel':
        ...
        return ArxaKitUpdateDataModel(
          surfaceId: _requiredString(body, 'surfaceId', verb, json),
          path: path as String?,
          value: body['value'],
        );
      case 'deleteSurface':
        return ArxaKitDeleteSurface(
          surfaceId: _requiredString(body, 'surfaceId', verb, json),
        );
      default:
...
final class ArxaKitCreateSurface extends ArxaKitA2uiMessage {
  const ArxaKitCreateSurface({
...
final class ArxaKitUpdateComponents extends ArxaKitA2uiMessage {
  const ArxaKitUpdateComponents({required this.surfaceId, required this.components});
...
/// [path].
final class ArxaKitUpdateDataModel extends ArxaKitA2uiMessage {
  const ArxaKitUpdateDataModel({required this.surfaceId, this.path, this.value});
...
final class ArxaKitDeleteSurface extends ArxaKitA2uiMessage {
  const ArxaKitDeleteSurface({required this.surfaceId});
