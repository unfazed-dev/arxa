...
import '../forms/arxa_kit_form_controller.dart';

/// STUB (scheduled: multi-step form flow phase).
///
/// Sequences several [ArxaKitFormController]s into a wizard with per-step submit
...
/// a later phase.
class ArxaKitMultiStepFormController extends ChangeNotifier {
  ArxaKitMultiStepFormController(this.steps)
      : assert(steps.isNotEmpty, 'A multi-step form needs at least one step');

  final List<ArxaKitFormController> steps;
  int _index = 0;
  ...
  ArxaKitFormController get current => steps[_index];
