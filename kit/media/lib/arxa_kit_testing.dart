/// Scriptable in-memory fakes for the arxa_kit_media ports — no plugins, no
/// OS, no MethodChannels. Drive streams and script outcomes from tests.
///
/// ```dart
/// final capture = FakeArxaKitMediaCaptureService(hasCamera: false);
/// capture.scriptedResult = const ArxaKitMediaCapturePermissionDenied();
///
/// final rec = FakeArxaKitAudioRecorderService()..permission = false;
///
/// final player = FakeArxaKitAudioPlayerService();
...
import 'arxa_kit_media.dart';

/// In-memory [ArxaKitCapturedMedia]. [saveTo] writes [bytes] to disk so a caller that
/// moves the capture produces a real file the test can assert on.
class FakeArxaKitCapturedMedia implements ArxaKitCapturedMedia {
  FakeArxaKitCapturedMedia(this.path, {this.name = 'fake.jpg', Uint8List? bytes})
...
/// Scriptable [ArxaKitMediaCaptureService]. Set [scriptedResult] to force any outcome;
/// otherwise a camera-on-no-camera returns unavailable and everything else
/// returns a [FakeArxaKitCapturedMedia].
class FakeArxaKitMediaCaptureService implements ArxaKitMediaCaptureService {
  FakeArxaKitMediaCaptureService({this.hasCamera = true, this.scriptedResult});

  @override
  bool hasCamera;

  /// When non-null, [capturePhoto] returns this verbatim.
  ArxaKitMediaCaptureResult? scriptedResult;

  /// Sources passed to [capturePhoto], in call order.
  final List<ArxaKitMediaSource> requestedSources = [];
  ...
  Future<ArxaKitMediaCaptureResult> capturePhoto({
    ...
    if (source == ArxaKitMediaSource.camera && !hasCamera) {
      return const ArxaKitMediaCaptureUnavailable('fake: no camera');
    }
    return ArxaKitMediaCaptured(FakeArxaKitCapturedMedia('/fake/photo.jpg'));
  }
}

/// Scriptable [ArxaKitAudioRecorderService]. Drive [elapsed$]/[amplitude$] with
/// [driveElapsed]/[driveAmplitude]; script [hasPermission] via [permission] and
/// the [stop] return via [scriptedStopResult].
class FakeArxaKitAudioRecorderService implements ArxaKitAudioRecorderService {
  FakeArxaKitAudioRecorderService({this.permission = true, this.scriptedStopResult});
  ...
  ArxaKitRecordingResult? scriptedStopResult;
  ...
  @override
  Future<ArxaKitRecordingResult?> stop() async {
    _recording = false;
    _elapsed.add(null);
    ...
    final path = startedPaths.isNotEmpty && startedPaths.last != null
        ? startedPaths.last!
        : '/fake/rec.m4a';
    return ArxaKitRecordingResult(path: path, duration: _elapsed$value);
  }
...
/// Scriptable [ArxaKitAudioPlayerService]. Drive [position$]/[duration$]/[state$] with
/// [drivePosition]/[driveDuration]/[driveState]; [play]/[pause]/[stop] flip
/// [isPlaying] and emit a ready [ArxaKitPlaybackState].
class FakeArxaKitAudioPlayerService implements ArxaKitAudioPlayerService {
  ...
  /// Emit an explicit [ArxaKitPlaybackState] (e.g. a completed state).
  void driveState(ArxaKitPlaybackState value) => _state.add(value);
...
/// In-memory [ArxaKitVideoPlayerService] — scriptable streams, recorded [load]s, no
/// render surface ([videoView] is an empty box).
class FakeArxaKitVideoPlayerService implements ArxaKitVideoPlayerService {
  final StreamController<Duration> _position =
      StreamController<Duration>.broadcast();
  final StreamController<Duration?> _duration =
      StreamController<Duration?>.broadcast();
