...
class ArxaKitScannedPage {
  const ArxaKitScannedPage({
...
class ArxaKitScannedDocument {
  const ArxaKitScannedDocument({required this.pages});

  /// Pages in capture order.
  final List<ArxaKitScannedPage> pages;
}
...
abstract interface class ArxaKitDocumentScanService {
  ...
  Future<ArxaKitScannedDocument?> scan({int? pageLimit});
}

/// Not-yet-implemented [ArxaKitDocumentScanService] — throws so callers fail loudly.
class UnimplementedArxaKitDocumentScanService implements ArxaKitDocumentScanService {
  const UnimplementedArxaKitDocumentScanService();

  // TODO(arxa_kit_documents): implement over VisionKit (iOS) / ML Kit (Android).
  @override
  Future<ArxaKitScannedDocument?> scan({int? pageLimit}) =>
      throw UnimplementedError('ArxaKitDocumentScanService.scan');
}
