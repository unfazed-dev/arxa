...
/// Claims-based RBAC for a arxa_kit app — one declarative object consumed by
...
class ArxaKitAccessPolicy {
  const ArxaKitAccessPolicy({
    required this.roles,
  ...
  /// The [ArxaKitAuthUser.metadata] key holding the role claim.
  ...
  String? roleOf(ArxaKitAuthUser? user) {
  ...
  bool can(String action, ArxaKitAuthUser? user) {
  ...
  bool canRoute(String routeName, ArxaKitAuthUser? user) {
  ...
  /// Authority: throw [ArxaKitAccessDeniedError] if [user] may not perform
  /// [action]. Facades call this on every mutating op so a bypassed UI still
  /// fails — the C15 bypass test proves it.
  void enforce(String action, ArxaKitAuthUser? user) {
    if (!can(action, user)) {
      throw ArxaKitAccessDeniedError(action: action, role: roleOf(user));
    }
  }
}

/// Typed denial from [ArxaKitAccessPolicy.enforce]. Facades let this propagate or
/// map it onto [ArxaKitFeedback] (CONTEXT.md) — never swallow it silently.
class ArxaKitAccessDeniedError implements Exception {
  final String action;
  final String? role;
  const ArxaKitAccessDeniedError({required this.action, required this.role});

  @override
  String toString() =>
      role == null
          ? 'ArxaKitAccessDeniedError: signed-out user cannot perform "$action"'
          : 'ArxaKitAccessDeniedError: role "$role" cannot perform "$action"';
}
