...
import '../service/arxa_kit_auth_service.dart';
...
/// depends on it). This is the genuine port of the `arxa/lib/tier1.dart`
/// ArxaKitSeedAuthBackend spec into the kit's [ArxaKitAuthService] interface — same
...
class ArxaKitSeedAuthBackend implements ArxaKitAuthService {
  ...
  ArxaKitAuthUser? currentUserFor(String uid) {
    final session = _sessionsByUid[uid];
    if (session == null) return null;
    return _userFor(uid, session.email);
  }
  ...
  /// [ArxaKitAuthFailureReason.tokenExpired].
  Future<ArxaKitAuthResult> refreshToken(String token) async {
    _assertUsable();
  ...
  @override
  Future<ArxaKitAuthResult> signInWithApple() => _resolveProviderUser(
        'apple',
        'apple_demo_user',
        'relay@apple.example',
      );
  ...
  @override
  Future<ArxaKitAuthResult> signInWithGoogle() => _resolveProviderUser(
        'google',
        'google_demo_user',
        'demo@google.example',
      );
  ...
  /// is what the Apple/Google [ArxaKitOAuthProvider]s are for.
  ...
  ArxaKitAuthResult _emitSession(_Session session) {
  ...
  ArxaKitAuthUser _userFor(String uid, String email) => ArxaKitAuthUser(
        id: uid,
        email: email,
  ...
  void _assertUsable() {
    if (_disposed) {
      throw StateError('ArxaKitSeedAuthBackend used after dispose()');
    }
  }
}
