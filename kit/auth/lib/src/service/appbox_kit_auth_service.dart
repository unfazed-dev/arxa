...
///
/// Failures are returned as [AppBoxKitAuthFailure], not thrown — the only exceptions a
...
abstract interface class AppBoxKitAuthService {
  ...
  Stream<AppBoxKitAuthUser?> get authStateChanges;

  /// The signed-in user, or null. Synchronous snapshot of the stream's latest.
  AppBoxKitAuthUser? get currentUser;

  /// Registers a new identity. Fails with [AppBoxKitAuthFailureReason.emailAlreadyInUse]
  /// or [AppBoxKitAuthFailureReason.weakPassword].
  Future<AppBoxKitAuthResult> signUp(AppBoxKitEmailPasswordCredentials credentials);

  /// Authenticates an existing identity. Fails with
  /// [AppBoxKitAuthFailureReason.userNotFound] or [AppBoxKitAuthFailureReason.invalidCredentials].
  Future<AppBoxKitAuthResult> signIn(AppBoxKitEmailPasswordCredentials credentials);
