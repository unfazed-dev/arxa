...
/// [AppBoxKitAuthFailureReason.invalidCredentials] — but it is NOT secure: passwords
...
/// always resolves to the same [AppBoxKitAuthUser.id] within and across runs.
class InMemoryAppBoxKitAuthService implements AppBoxKitAuthService {
  ...
  /// a realistic [AppBoxKitAuthSession.expiresAt].
  ...
  AppBoxKitAuthSession? _session;
  bool _disposed = false;

  InMemoryAppBoxKitAuthService({
  ...
  @override
  AppBoxKitAuthUser? get currentUser => _session?.user;
  ...
  Future<AppBoxKitAuthResult> signUp(AppBoxKitEmailPasswordCredentials credentials) async {
    ...
    if (credentials.password.length < minPasswordLength) {
      return AppBoxKitAuthFailure(
        AppBoxKitAuthFailureReason.weakPassword,
    ...
    final user = AppBoxKitAuthUser(id: _idFor(email), email: email);
  ...
  @override
  Future<AppBoxKitAuthResult> signInWithApple() =>
      _resolveProviderUser('apple', 'apple-user@local', 'Apple User');

  @override
  Future<AppBoxKitAuthResult> signInWithGoogle() =>
  ...
  AppBoxKitAuthResult _emitSession(AppBoxKitAuthUser user) {
    final session = AppBoxKitAuthSession(
      user: user,
    ...
    return AppBoxKitAuthSuccess(session);
  }
  ...
  void _assertUsable() {
    if (_disposed) {
      throw StateError('InMemoryAppBoxKitAuthService used after dispose()');
    }
  }
}
