...
The **device-notifications** port for `arxa_kit` apps — permission, push
token / foreground-message seams, local notifications, and badge.

## Naming: not the same as core's `ArxaKitNotificationService`

`arxa_kit` core already ships `lib/services/notifications/` with
**`ArxaKitNotificationService`** (singular) — that is the **in-app transient
...
This package's service is **`ArxaKitNotificationsService`** (plural) — **OS-level
...
- **`ArxaKitNotificationsService`** (port): `initialize`, `requestPermission`
  (typed `ArxaKitNotificationPermissionResult`), `permissionStatus`, `tokenStream`,
  `currentToken`, `foregroundMessages`, `showLocalNotification`, `cancel`,
  `cancelAll`, `setBadgeCount`, `clearBadge`, `dispose`.
- **`LocalArxaKitNotificationsService`** — the working default, wrapping
...
- **`ArxaKitOutboundMessageSink`** — a separate port for outbound one-to-one
...
- **Push is a stub.** `tokenStream` / `foregroundMessages` are seams the FCM/APNs
  backend owns; the local default emits nothing on them. `ArxaKitFcmPushBackend`
...
- **Standalone badge is unsupported by the local default.** flutter_local_
  ...
  `LocalArxaKitNotificationsService.setBadgeCount` / `clearBadge` **throw
  `UnsupportedError`** — set `ArxaKitLocalNotification.badgeCount` on a shown
  notification, or own the badge from the push payload. A push backend may
  override these with a real implementation.
- **Outbound SMS/email are stubs** (`ArxaKitSmsBackend`, `ArxaKitEmailBackend`) — provider-side
...
**0.1.0 — priority implemented.** `LocalArxaKitNotificationsService` (permission,
show/cancel, per-notification badge) is complete and analyzer-clean.
`ArxaKitFcmPushBackend`, `ArxaKitSmsBackend`, `ArxaKitEmailBackend` are real-signature stubs.
