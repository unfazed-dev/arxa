...
/// Map AppBoxKitOutboundMessage.to -> recipient, .subject -> subject, .body -> body,
/// .metadata -> { from, replyTo, templateId, ... }.
///
/// This stub imports no mail client, so no SMTP/HTTP dependency is pulled until
/// a host opts in.
class AppBoxKitEmailBackend implements AppBoxKitOutboundMessageSink {
  AppBoxKitEmailBackend();
  ...
  Future<void> send(AppBoxKitOutboundMessage message) async =>
      throw UnimplementedError('AppBoxKitEmailBackend.send is a stub');
}
