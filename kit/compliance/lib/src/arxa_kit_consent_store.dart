...
/// Persistence port for [ArxaKitConsentRecord]s.
///
...
abstract interface class ArxaKitConsentStore {
  /// Appends [record] to the log.
  Future<void> record(ArxaKitConsentRecord record);
  ...
  /// anonymous), or null if none. Ties on [ArxaKitConsentRecord.acceptedAt] break
  /// by insertion order.
  Future<ArxaKitConsentRecord?> latestFor(String documentId, {String? userId});
  ...
  Future<List<ArxaKitConsentRecord>> history({String? documentId, String? userId});
}

/// The working default [ArxaKitConsentStore] — an in-memory append-only log.
///
...
class InMemoryArxaKitConsentStore implements ArxaKitConsentStore {
  final List<_LoggedRecord> _log = <_LoggedRecord>[];
  int _seq = 0;

  @override
  Future<void> record(ArxaKitConsentRecord record) async {
  ...
  Future<void> withdraw({
    ...
    _log.add(_LoggedRecord(
      ArxaKitConsentRecord(
        ...
        method: ArxaKitConsentMethod.withdrawn,
  ...
  @override
  Future<ArxaKitConsentRecord?> latestFor(String documentId,
      {String? userId}) async {
    _LoggedRecord? best;
  ...
  @override
  Future<List<ArxaKitConsentRecord>> history(
    ...
    return List<ArxaKitConsentRecord>.unmodifiable(
        [for (final entry in matches) entry.record]);
  }

  /// The full append-ordered log across **all** tracks (audit/debug only).
  List<ArxaKitConsentRecord> get records =>
      List<ArxaKitConsentRecord>.unmodifiable(
...
class _LoggedRecord {
  ...
  final ArxaKitConsentRecord record;
  final int seq;
}
