  ...
  return MaterialApp(
    home: Builder(
      builder: (context) => MediaQuery(
        ...
        child: ArxaKitMotionScope(driver: driver, spec: spec, child: child),
      ),
    ),
  );
}
...
void main() {
  ...
  group('ArxaKitWake stagger', () {
    testWidgets('kit.motion.wake — later orders wake later on the timeline', (tester) async {
      driver.value = 0.12;
      await tester.pumpWidget(_host(
        driver: driver,
        child: const Column(children: [
          ArxaKitWake(order: 0, child: SizedBox(key: Key('a'), height: 10)),
          ArxaKitWake(order: 5, child: SizedBox(key: Key('b'), height: 10)),
        ]),
      ));
    ...
    testWidgets('kit.motion.wake — settles to identity at driver == 1', (tester) async {
      ...
      await tester.pumpWidget(_host(
        driver: driver,
        spec: const ArxaKitMotionSpec(scale: 0.9),
    ...
    testWidgets('kit.motion.wake — auto-claims sequential orders in build order',
      ...
      await tester.pumpWidget(_host(
        driver: driver,
        child: const Column(children: [
          ArxaKitWake(child: SizedBox(key: Key('first'), height: 10)),
          ArxaKitWake(child: SizedBox(key: Key('second'), height: 10)),
        ]),
      ));
  ...
  group('safe degrades', () {
    ...
    testWidgets('kit.motion.wake — spec.enabled == false renders children untouched',
      ...
      await tester.pumpWidget(_host(
        driver: driver,
        spec: const ArxaKitMotionSpec(enabled: false),
        child: const ArxaKitWake(
            order: 0, child: SizedBox(key: Key('a'), height: 10)),
      ));
      expect(
    ...
    testWidgets('kit.motion.wake — no scope above renders children untouched', (tester) async {
      await tester.pumpWidget(const MaterialApp(
        home: ArxaKitWake(order: 0, child: SizedBox(key: Key('a'), height: 10)),
      ));
  ...
  group('extensions', () {
    testWidgets('kit.motion.wake — .wake() wraps in ArxaKitWake', (tester) async {
      driver.value = 1.0;
      await tester.pumpWidget(_host(
        driver: driver,
        child: const SizedBox(key: Key('a'), height: 10).wake(order: 1),
      ));
      expect(find.byType(ArxaKitWake), findsOneWidget);
      expect(_opacityOf(tester, const Key('a')), 1.0);
    });
    ...
    testWidgets('kit.motion.wake — .wakeAll() assigns sequential explicit orders',
      ...
      final wakes =
          tester.widgetList<ArxaKitWake>(find.byType(ArxaKitWake)).toList();
      ...
      await tester.pumpWidget(_host(
        ...
        child: Builder(builder: (context) {
          return const SizedBox(key: Key('a'), height: 10)
              .animate(adapter: ArxaKitMotionAdapter.of(context, order: 0))
              .fadeIn();
        }),
      ));
  ...
  group('testing helpers', () {
    testWidgets('kit.motion.wake — staticArxaKitMotionScope pins the timeline', (tester) async {
      await tester.pumpWidget(MaterialApp(
        home: staticArxaKitMotionScope(
          t: 0.0,
          child: const ArxaKitWake(
              order: 0, child: SizedBox(key: Key('a'), height: 10)),
        ),
  ...
  group('ArxaKitMotionSpec', () {
    test('kit.motion.wake — startFor staggers and clamps', () {
      const spec = ArxaKitMotionSpec();
    ...
    test('kit.motion.wake — lerp interpolates continuous channels', () {
      final mid = const ArxaKitMotionSpec()
          .lerp(const ArxaKitMotionSpec(offset: Offset(0, 0.16)), 0.5);
      expect(mid.offset.dy, closeTo(0.12, 1e-9));
    });

    test('kit.motion.wake — theme extension resolution falls back to standard', () {
      expect(
        ThemeData().extension<ArxaKitMotionSpec>(),
        isNull,
      );
    });
  });
