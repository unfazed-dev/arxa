...
import 'package:arxa_kit_ui_library/utils/arxa_kit_view_model.dart';

/// Tests for owner-identity ArxaKitAction: derived keys (no hand-written
/// widgetIds), parallel ops across owners, the awaitable builder, and
/// auto-dispose via `ArxaKitViewModel.dispose` → `ArxaKitAction.disposeOwner`.
class _FakeVm extends ArxaKitViewModel {}
...
void main() {
  ...
  group('owner-identity keys', () {
    test(
        'kit.ui-library.action-owner — action(name, operation) state is observable via actionState\$',
        () async {
      final vm = _FakeVm();
    ...
    test(
        'kit.ui-library.action-owner — awaiting the same builder twice runs the operation once',
        () async {
      final vm = _FakeVm();
      var runs = 0;
    ...
    test(
        'kit.ui-library.action-owner — same name on two owners runs in parallel (independent keys)',
        () async {
      final vmA = _FakeVm();
      final vmB = _FakeVm();
      final gate = Completer<void>();
  ...
  group('auto-dispose', () {
    test(
        'kit.ui-library.action-owner — ArxaKitViewModel.dispose cancels owner watches (no callback after)',
        () async {
      final vm = _FakeVm();
    ...
    test(
        'kit.ui-library.action-owner — disposeOwner closes the op state subject (fresh state after)',
        () async {
      final vm = _FakeVm();
