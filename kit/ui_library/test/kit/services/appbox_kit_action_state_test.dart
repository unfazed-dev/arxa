...
void main() {
  ...
  group('AppBoxKitAction.state\$', () {
    ...
    test('busy true while running, false after success, no error', () async {
      ...
      expect(AppBoxKitAction.state$(widgetId: 'state.success').value.busy, isTrue);
      gate.complete();
      expect(await future, 'ok');
      await Future<void>.delayed(Duration.zero);

      expect(AppBoxKitAction.state$(widgetId: 'state.success').value.busy, isFalse);
      expect(AppBoxKitAction.state$(widgetId: 'state.success').value.errorMessage, isNull);
    ...
    test('dispose closes the state subject', () async {
      ...
      expect(AppBoxKitAction.state$(widgetId: 'state.dispose').value.busy, isFalse);
      AppBoxKitAction.dispose(widgetId: 'state.dispose');
    });
  });
}
