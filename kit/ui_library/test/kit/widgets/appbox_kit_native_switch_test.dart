...
import 'package:appbox_kit_ui_library/widgets/appbox_kit_native_switch.dart';

import 'appbox_kit_native_test_helpers.dart';

/// AppBoxKitNativeSwitch is a **thin wrap** over [CNSwitch] with no M3E branch, so
...
/// (tap → onChanged). The kit branch (AppBoxKitNativeSwitch → CNSwitch) is unchanged
...
void main() {
  ...
  testWidgets('kit.ui-library.native-switch — builds clean', (tester) async {
    await withAndroidFallback(() async {
      ...
      expect(find.byType(AppBoxKitNativeSwitch), findsOneWidget);
      expect(
        find.byType(Switch),
        findsOneWidget,
