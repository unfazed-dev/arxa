...
import 'package:arxa_kit_ui_library/widgets/arxa_kit_animated_tab_stack.dart';

// ArxaKitAnimatedTabStack owns its tab bodies: each must be inflated exactly once
...
void main() {
  ...
  testWidgets(
    ...
    Color runColor() => tester
        .widget<ColoredBox>(find.descendant(
            of: find.byType(ArxaKitAnimatedTabStack),
  ...
  // `ArxaKitPlatform.isIOS` reads `dart:io` and is false — which is also why
  // every test above still exercises the animated path unchanged.
  group('native cross-cut (iOS)', () {
    setUp(() => ArxaKitPlatform.override =
        const ArxaKitPlatformOverride(isIOS: true));
