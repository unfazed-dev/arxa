...
import 'package:arxa_kit_ui_library/widgets/arxa_kit_native_navigation_rail.dart';
...
void main() {
  ...
  const destinations = [
    ArxaKitRailDestination(icon: Icons.home, label: 'Home'),
    ArxaKitRailDestination(icon: Icons.search, label: 'Search'),
  ];

  testWidgets(
      'kit.ui-library.native-navigation-rail — Android wantNative → NavigationRailM3E',
      (tester) async {
    ArxaKitPlatform.override =
        const ArxaKitPlatformOverride(isAndroid: true);
    await tester.pumpWidget(host(const ArxaKitNativeNavigationRail(
      selectedIndex: 0,
  ...
  testWidgets(
      'kit.ui-library.native-navigation-rail — iOS extended → wide Cupertino rail with inline labels',
      (tester) async {
    ...
    expect(
      tester.getSize(find.byType(ArxaKitNativeNavigationRail)).width,
      220,
  ...
  testWidgets(
      'kit.ui-library.native-navigation-rail — Android rail container is rounded (kit-side corner.large clip)',
      (tester) async {
    ArxaKitPlatform.override =
        const ArxaKitPlatformOverride(isAndroid: true);
    await tester.pumpWidget(host(const ArxaKitNativeNavigationRail(
  ...
  testWidgets(
      'kit.ui-library.native-navigation-rail — iOS built-in menu button toggles collapsed ↔ extended',
      (tester) async {
    ArxaKitPlatform.override = const ArxaKitPlatformOverride(isIOS: true);
    await tester.pumpWidget(host(const ArxaKitNativeNavigationRail(
      selectedIndex: 0,
    ...
    await tester.pumpAndSettle();
    expect(
        tester.getSize(find.byType(ArxaKitNativeNavigationRail)).width, 220,
    ...
    await tester.pumpAndSettle();
    expect(tester.getSize(find.byType(ArxaKitNativeNavigationRail)).width, 84,
        reason: 'menu_open button collapses it back');
  });

  testWidgets(
      'kit.ui-library.native-navigation-rail — iOS collapsed → 84-wide Cupertino rail',
      (tester) async {
    ArxaKitPlatform.override = const ArxaKitPlatformOverride(isIOS: true);
    await tester.pumpWidget(host(const ArxaKitNativeNavigationRail(
      selectedIndex: 0,
    ...
    expect(
        tester.getSize(find.byType(ArxaKitNativeNavigationRail)).width, 84);
  });

  testWidgets(
      'kit.ui-library.native-navigation-rail — default → Material NavigationRail, builds clean',
      (tester) async {
    await tester.pumpWidget(host(const ArxaKitNativeNavigationRail(
      selectedIndex: 0,
      destinations: destinations,
