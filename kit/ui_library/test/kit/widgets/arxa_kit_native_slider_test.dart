...
import 'package:arxa_kit_ui_library/widgets/arxa_kit_native_slider.dart';
...
///   (ArxaKitNativeSlider → CNSlider) is unchanged — only the CN-internal render
///   path is diverted.
void main() {
  ...
  testWidgets('kit.ui-library.native-slider — Android routes to SliderM3E',
    ...
    await tester.pumpWidget(host(const ArxaKitNativeSlider(value: 0.5)));
  ...
  testWidgets(
    ...
    await withAndroidFallback(() async {
      await tester.pumpWidget(host(const ArxaKitNativeSlider(value: 0.3)));
  ...
  testWidgets(
    ...
    ArxaKitPlatform.override =
        const ArxaKitPlatformOverride(isAndroid: true);
    double? fired;
    await tester.pumpWidget(host(ArxaKitNativeSlider(
      value: 0.0,
      onChanged: (v) => fired = v,
    )));
  ...
  testWidgets(
    ...
    await withAndroidFallback(() async {
      await tester.pumpWidget(host(const SingleChildScrollView(
        child: ArxaKitNativeSlider(value: 0.5),
      )));
