...
import 'package:arxa_kit_ui_library/widgets/arxa_kit_native_app_bar.dart';
...
void main() {
  ...
  testWidgets(
    ...
    await tester.pumpWidget(host(const ArxaKitNativeAppBar(title: 'Hi')));
    ...
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        body: CustomScrollView(
          slivers: [
            ArxaKitNativeAppBar.sliver(title: 'Sliver'),
          ],
        ),
      ),
    ));
  ...
  testWidgets(
    ...
    expect(bar.right - action.right, moreOrLessEquals(16, epsilon: 0.5),
        reason: 'kit app bar must inset trailing actions by 16 to match the '
            'CN/M3E tiers (parity with ArxaKitNativeSliverAppBar)');
  });
  ...
  testWidgets(
    ...
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        appBar: ArxaKitNativeAppBar(
          title: 'T',
          leading: const ArxaKitNativeIconButton(glyph: ArxaKitGlyphs.back),
          actions: const [
            ArxaKitNativeIconButton(glyph: ArxaKitGlyphs.more)
          ],
        ),
      ),
    ));
  ...
  // A plain SizedBox stands in for the action — a real ArxaKitNativeIconButton on
  ...
  testWidgets(
    ...
    await tester.pumpWidget(host(const ArxaKitNativeAppBar(
      title: 'T',
    ...
    final gap = find.byKey(const Key('arxaKitNativeAppBarBottomGap'));
  ...
  // slot. The kit implies its OWN back button (a ArxaKitNativeIconButton with
  // ArxaKitGlyphs.back) so leading + actions match; pop behavior + the localized
  // 'Back' a11y label are preserved. Real ArxaKitNativeIconButtons need
  ...
  testWidgets(
      'kit.ui-library.native-app-bar — iOS tier: implied leading is a ArxaKitNativeIconButton',
      (tester) async {
    ArxaKitPlatform.override = const ArxaKitPlatformOverride(isIOS: true);
    await withAndroidFallback(() async {
      await pushPage(tester,
          const Scaffold(appBar: ArxaKitNativeAppBar(title: 'Detail')));
      ...
      expect(
        find.descendant(
          ...
          matching: find.byType(ArxaKitNativeIconButton),
        ),
        findsOneWidget,
        reason:
  ...
  testWidgets(
    ...
    await withAndroidFallback(() async {
          ...
          appBar: ArxaKitNativeAppBar(
            ...
            actions: [
              ArxaKitNativeIconButton(
                  key: a, glyph: ArxaKitGlyphs.more, onPressed: () {}),
              ArxaKitNativeIconButton(
                  key: b, glyph: ArxaKitGlyphs.back, onPressed: () {}),
            ],
          ),
        ),
      );
      ...
      expect(find.byType(ArxaKitNativeIconButton), findsNWidgets(3),
