...
import 'package:arxa_kit_ui_library/widgets/arxa_kit_chip_carousel.dart';

import 'arxa_kit_native_test_helpers.dart';

/// ArxaKitChipCarousel tests — rendering, the edge-fade invariant (fade only
...
void main() {
  ...
  testWidgets(
      'kit.ui-library.chip-carousel — renders all chips; no fade when everything fits',
      (tester) async {
    await tester.pumpWidget(host(const ArxaKitChipCarousel(children: [
      ArxaKitChip(label: 'One'),
      ArxaKitChip(label: 'Two'),
      ArxaKitChip(label: 'Three'),
    ])));
    await tester.pump();
  ...
  testWidgets(
      'kit.ui-library.chip-carousel — overflowing rail fades edges and scrolls to the last chip',
      (tester) async {
    await tester.pumpWidget(host(ArxaKitChipCarousel(children: boxes(12))));
  ...
  testWidgets(
      'kit.ui-library.chip-carousel — snap settles on the nearest chip start',
      (tester) async {
    await tester.pumpWidget(host(ArxaKitChipCarousel(
      snap: true,
      spacing: 8,
  ...
  testWidgets(
      'kit.ui-library.chip-carousel — without snap, a slow drag parks where released',
      (tester) async {
    await tester.pumpWidget(host(ArxaKitChipCarousel(
      spacing: 8,
      padding: EdgeInsets.zero,
  ...
  testWidgets(
      'kit.ui-library.chip-carousel — chips stay tappable through the fade mask',
      (tester) async {
    var taps = 0;
    await tester.pumpWidget(host(ArxaKitChipCarousel(children: [
      for (var i = 0; i < 12; i++)
        ArxaKitChip(label: 'Chip $i', onTap: i == 0 ? () => taps++ : null),
    ])));
    await tester.pump();
  ...
  testWidgets(
      'kit.ui-library.chip-carousel — rail height is intrinsic — tallest child wins',
      (tester) async {
    await tester.pumpWidget(host(const ArxaKitChipCarousel(children: [
      ArxaKitChip(label: 'Small'),
      SizedBox(height: 56, width: 80),
    ])));
    await tester.pump();

    expect(tester.getSize(find.byType(ArxaKitChipCarousel)).height, 56.0,
