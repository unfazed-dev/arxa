...
void main() {
      ...
      home: Scaffold(
        body: CustomScrollView(
          ...
          slivers: [
            ...
            SliverToBoxAdapter(
              child: ArxaKitScrollOcclusionGate(
                key: gateKey,
                hideMode: hideMode,
  ...
  testWidgets(
    ...
    await tester.pumpWidget(harness(
      controller: controller,
      hideMode: ArxaKitChromeHideMode.unmount,
    ));
  ...
  testWidgets(
    ...
    final gated = const SizedBox(key: childKey, height: 40).scrollOcclusion(
        occlusionPadding: 8, hideMode: ArxaKitChromeHideMode.unmount);

    expect(gated, isA<ArxaKitScrollOcclusionGate>());
    final gate = gated as ArxaKitScrollOcclusionGate;
    expect(gate.occlusionPadding, 8);
    expect(gate.hideMode, ArxaKitChromeHideMode.unmount);

    await tester.pumpWidget(MaterialApp(home: Scaffold(body: gated)));
    expect(
      find.ancestor(
        of: find.byKey(childKey),
        matching: find.byType(ArxaKitScrollOcclusionGate),
      ),
      findsOneWidget,
    );
  });
