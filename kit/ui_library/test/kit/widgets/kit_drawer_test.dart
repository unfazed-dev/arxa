...
import 'package:ui_library/widgets/kit_drawer.dart';
import 'package:ui_library/widgets/kit_frosted_surface.dart';

/// KitDrawer tests — variant rendering (glassPeek geometry + skin vs the
/// plain stock look), the stock Scaffold machinery (edge swipe / drag close
/// / scrim dismiss), the mirrored end-drawer corner, and the KitMotionScope
...
void main() {
  ...
  testWidgets(
    ...
    await tester.pumpWidget(host(const KitDrawer(child: Text('menu'))));
    await openDrawer(tester);

    expect(find.byType(KitFrostedSurface), findsOneWidget,
  ...
  testWidgets('glassPeek mirrors the rounded corner for an endDrawer',
      (tester) async {
    await tester
        .pumpWidget(host(const KitDrawer(child: Text('menu')), end: true));
  ...
  testWidgets('plain renders the stock themed Drawer look', (tester) async {
    await tester.pumpWidget(host(const KitDrawer(
      variant: KitDrawerVariant.plain,
      child: Text('menu'),
    )));
    await openDrawer(tester);

    expect(find.byType(KitFrostedSurface), findsNothing,
  ...
  testWidgets(
      'motionDriver wraps the content in a KitMotionScope driven by '
      'the passed driver (the driver path)', (tester) async {
    final driver = KitGestureDriver(vsync: tester, initialValue: 0.4);
    addTearDown(driver.dispose);
    await tester.pumpWidget(host(KitDrawer(
      motionDriver: driver,
    ...
    expect(
      tester.widget<KitMotionScope>(find.byType(KitMotionScope)).driver,
      same(driver),
    ...
    // 0.4 the woken row renders at easeOutCubic(0.4). Scoped to the KitWake
  ...
  testWidgets(
      ...
      'gestureKitMotionScope', (tester) async {
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        body: gestureKitMotionScope(
          t: 0.5,
          child: Column(
            children: [
