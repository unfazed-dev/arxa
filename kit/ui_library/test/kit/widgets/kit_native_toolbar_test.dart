...
import 'package:ui_library/widgets/kit_native_toolbar.dart';
...
/// a [ToolbarM3E] whose `actions` are mapped 1:1 from [KitToolbarAction]. The
...
void main() {
  ...
  testWidgets('Android wantNative → ToolbarM3E', (tester) async {
    ...
    await tester.pumpWidget(host(KitNativeToolbar(
      actions: [
        KitToolbarAction(icon: Icons.add, onPressed: () {}),
        KitToolbarAction(icon: Icons.delete, onPressed: () {}),
      ],
    )));
  ...
  testWidgets('iOS glass tier pins button minHeight to height', (tester) async {
    ...
    await withAndroidFallback(() async {
      ...
      await tester.pumpWidget(host(KitNativeToolbar(
        actions: [KitToolbarAction(icon: Icons.add, onPressed: () {})],
      )));
