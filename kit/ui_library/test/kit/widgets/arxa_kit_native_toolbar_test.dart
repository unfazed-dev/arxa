...
import 'package:arxa_kit_ui_library/widgets/arxa_kit_native_toolbar.dart';
...
/// a [ToolbarM3E] whose `actions` are mapped 1:1 from [ArxaKitToolbarAction]. The
...
void main() {
  ...
  testWidgets('kit.ui-library.native-toolbar — Android wantNative → ToolbarM3E',
    ...
    await tester.pumpWidget(host(ArxaKitNativeToolbar(
      actions: [
        ArxaKitToolbarAction(icon: Icons.add, onPressed: () {}),
        ArxaKitToolbarAction(icon: Icons.delete, onPressed: () {}),
      ],
    )));
  ...
  testWidgets(
    ...
    await withAndroidFallback(() async {
      ...
      await tester.pumpWidget(host(ArxaKitNativeToolbar(
        actions: [ArxaKitToolbarAction(icon: Icons.add, onPressed: () {})],
      )));
