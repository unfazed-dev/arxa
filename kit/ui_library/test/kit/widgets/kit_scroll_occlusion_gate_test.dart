...
void main() {
  ...
  testWidgets('unmount mode swaps in a same-size placeholder when covered',
    ...
    await tester.pumpWidget(harness(
      controller: controller,
      hideMode: KitChromeHideMode.unmount,
    ));
  ...
  testWidgets('.scrollOcclusion() extension wraps child in a gate',
    ...
    final gated = const SizedBox(key: childKey, height: 40)
        .scrollOcclusion(occlusionPadding: 8, hideMode: KitChromeHideMode.unmount);

    expect(gated, isA<KitScrollOcclusionGate>());
    final gate = gated as KitScrollOcclusionGate;
    expect(gate.occlusionPadding, 8);
    expect(gate.hideMode, KitChromeHideMode.unmount);

    await tester.pumpWidget(MaterialApp(home: Scaffold(body: gated)));
    expect(
      find.ancestor(
        of: find.byKey(childKey),
        matching: find.byType(KitScrollOcclusionGate),
      ),
      findsOneWidget,
    );
  });
