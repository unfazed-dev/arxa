...
import 'package:appbox_kit_ui_library/widgets/appbox_kit_image.dart';

import 'appbox_kit_native_test_helpers.dart';

/// AppBoxKitImage tests.
///
...
void main() {
  ...
  testWidgets('kit.ui-library.image — empty asset shows the placeholder glyph and never throws',
    ...
    expect(find.byIcon(AppBoxKitGlyphs.photo.icon), findsOneWidget,
        reason: 'the default placeholder (AppBoxKitGlyphs.photo) is shown');
  ...
  testWidgets('kit.ui-library.image — honors a custom placeholder glyph', (tester) async {
    await tester.pumpWidget(host(const AppBoxKitImage(
      asset: '',
      size: 48,
      placeholder: AppBoxKitGlyphs.cart,
    )));
    await tester.pump();

    expect(find.byIcon(AppBoxKitGlyphs.cart.icon), findsOneWidget);
