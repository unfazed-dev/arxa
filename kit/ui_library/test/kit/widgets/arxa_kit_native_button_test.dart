...
/// Native-widget tests for [ArxaKitNativeButton]. Two tiers:
///
...
void main() {
  ...
  Future<ButtonM3E> pumpM3E(
      WidgetTester tester, ArxaKitButtonStyle style) async {
    ArxaKitPlatform.override =
        const ArxaKitPlatformOverride(isAndroid: true);
    await tester.pumpWidget(host(ArxaKitNativeButton(
      label: 'Go',
      style: style,
      onPressed: () {},
    )));
  ...
  testWidgets('kit.ui-library.native-button — Android routes to ButtonM3E',
      (tester) async {
    await pumpM3E(tester, ArxaKitButtonStyle.glass);
  });

  testWidgets(
      'kit.ui-library.native-button — emphasis map: plain → text (never a filled CTA)',
      (tester) async {
    final button = await pumpM3E(tester, ArxaKitButtonStyle.plain);
    expect(
      button.style,
      ButtonM3EStyle.text,
  ...
  testWidgets(
    ...
    for (final style in [
      ...
      ArxaKitButtonStyle.filled,
      ArxaKitButtonStyle.borderedProminent,
    ]) {
  ...
  testWidgets(
    ...
    for (final style in [
      ...
      ArxaKitButtonStyle.gray,
      ArxaKitButtonStyle.tinted,
    ]) {
      final button = await pumpM3E(tester, style);
  ...
  testWidgets(
      ...
      (tester) async {
    final button = await pumpM3E(tester, ArxaKitButtonStyle.bordered);
  ...
  testWidgets(
    ...
    await withAndroidFallback(() async {
      ...
      // pin (bright-base demotion is pinned in arxa_kit_glass_luminance_test).
      for (final style in ArxaKitButtonStyle.values) {
        ...
        expect(
          ...
          reason:
              'ArxaKitButtonStyle is a 1:1 name mirror of CNButtonStyle and '
  ...
  testWidgets(
    ...
    await withAndroidFallback(() async {
      for (final style in [
        ...
        // now (arxa_kit_glass_luminance_test).
        await tester.pumpWidget(MaterialApp(
          theme: ThemeData.dark(),
          home: Scaffold(
              body: Center(
