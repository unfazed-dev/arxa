...
import 'package:ui_library/utils/kit_native_overlay.dart';
...
void main() {
  ...
  test('balances depth for a plain future that completes on dismiss',
      () async {
    expect(depth(), 0);
    await withNativeChromeHidden(() async {});
    expect(depth(), 0);
  });

  test('balances depth when present() returns null', () async {
    await withNativeChromeHidden(() => null);
    expect(depth(), 0);
  });

  test('balances depth when present() throws', () async {
    await expectLater(
      withNativeChromeHidden(() => Future.error(StateError('boom'))),
      throwsStateError,
    );
    expect(depth(), 0);
  });
  ...
  test(
    ...
    unawaited(withNativeChromeHidden(() async => controller));
