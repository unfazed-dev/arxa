...
The shared `Kit*` widget surface for `arxa_kit` apps — the UI widgets split
out of `core` in the 2026-07 refactor. A standalone kit.

## Reactive views

- `ArxaKitStreamBuilder<T>` — thin `StreamBuilder` wrapper that auto-seeds from a
...
View bodies render kit-data / async streams via `ArxaKitStreamBuilder`, never raw
