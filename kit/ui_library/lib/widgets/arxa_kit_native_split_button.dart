...
/// [ArxaKitMenuItem] — so hosts never import either underlying dep.
///
...
/// keeps a 1:1 mapping across all three tiers — see [ArxaKitNativePopupMenu] for
/// the same rationale.
class ArxaKitNativeSplitButton extends StatelessWidget {
  const ArxaKitNativeSplitButton({
    super.key,
    this.label,
    this.glyph,
    IconData? icon,
  ...
  /// Paired Material icon + SF Symbol for the action face (see [ArxaKitGlyphs]).
  ...
  /// Menu items revealed by the chevron. Each [ArxaKitMenuItem] maps to the tier's
  /// native menu type.
  final List<ArxaKitMenuItem> menuItems;

  /// Notified with the selected menu item.
  final ValueChanged<ArxaKitMenuItem>? onMenuSelected;
  ...
  Widget build(BuildContext context) {
    ...
    if (ArxaKitPlatform.isIOS) return _glass(context);
    return _material(context);
  }

  Widget _m3e(BuildContext context) => SplitButtonM3E<ArxaKitMenuItem>(
        ...
        items: [
          for (final item in menuItems)
            SplitButtonM3EItem<ArxaKitMenuItem>(
              value: item,
              child: item.label,
            ),
        ],
  ...
  Widget _glass(BuildContext context) {
    ...
    return CNSplitButton(
      ...
      items: [
        for (final item in menuItems)
          // Same mapping as ArxaKitNativeFabMenu: SF Symbol wins on Apple, the
  ...
  Widget _material(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        PopupMenuButton<ArxaKitMenuItem>(
