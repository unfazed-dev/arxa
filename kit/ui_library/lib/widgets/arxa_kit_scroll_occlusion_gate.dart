...
import 'arxa_kit_native_chrome_gate.dart' show ArxaKitChromeHideMode;
...
/// blur/fade of the *content* — [ArxaKitScrollEdgeEffect] — not an alpha-hide of
/// the surface. New scroll-under-pinned-chrome work must use
/// [ArxaKitScrollEdgeEffect]; the reviewer's objective gate
...
/// (`ArxaKitGlassCard`'s `BackdropFilter`) hits the same class of artifact while
/// any platform view exists in the tree (flutter/flutter #175048, open).
///
/// **Fix pattern** (same machinery as [ArxaKitNativeChromeGate]): drive the child
...
/// [ArxaKitScrollEdgeEffect]).** Wrap a glass surface that lives inside a
/// scrollable where Flutter must paint over chrome:
///
/// ```dart
/// ArxaKitScrollOcclusionGate(
///   child: ArxaKitNativeIconButton(...), // or ArxaKitGlassCard, CNButton, maps…
/// )
/// ```
///
...
///
/// Pair with [ArxaKitNativeChromeGate] (compose both) when the same surface must
/// also vanish under modal blurs.
class ArxaKitScrollOcclusionGate extends StatefulWidget {
  const ArxaKitScrollOcclusionGate({
    super.key,
    required this.child,
    this.occlusionPadding = 0.0,
    this.hideMode = ArxaKitChromeHideMode.keepAlive,
  });
  ...
  /// How pixels come off screen once fully covered. [ArxaKitChromeHideMode
  /// .keepAlive] (default) keeps the native view instance alive — restore is
  /// jank-free. [ArxaKitChromeHideMode.unmount] destroys it and holds layout with
  /// a same-size placeholder (escape hatch if a specific native view
  /// misbehaves at alpha 0).
  final ArxaKitChromeHideMode hideMode;

  @override
  State<ArxaKitScrollOcclusionGate> createState() =>
      _KitScrollOcclusionGateState();
}

class _KitScrollOcclusionGateState extends State<ArxaKitScrollOcclusionGate> {
  ScrollPosition? _position;

  /// 1.0 = fully shown, 0.0 = fully covered (paint skipped, native detached).
  double _alpha = 1.0;

  /// Depth at mount — the baseline (mirrors [ArxaKitNativeChromeGate]). A gate
  /// mounted inside an already-open modal stays visible; anything opened
  /// *after* us (kit sheets bump the depth via `arxaKitShowSheet`) counts
  /// as covering us.
  ...
  Widget build(BuildContext context) {
    final unmounting = widget.hideMode == ArxaKitChromeHideMode.unmount;
...
///
/// ```dart
/// ArxaKitNativeSearchBar(...).scrollOcclusion().wake(order: 1)
/// ```
///
/// Retired from scroll duty by ADR 0010 together with
/// [ArxaKitScrollOcclusionGate] — scrolled content under pinned chrome uses
/// `.scrollEdgeEffect()` ([ArxaKitScrollEdgeEffect]) instead; this remains only
...
extension ArxaKitScrollOcclusionX on Widget {
  Widget scrollOcclusion({
    Key? key,
    double occlusionPadding = 0.0,
    ArxaKitChromeHideMode hideMode = ArxaKitChromeHideMode.keepAlive,
  }) =>
      ArxaKitScrollOcclusionGate(
        key: key,
