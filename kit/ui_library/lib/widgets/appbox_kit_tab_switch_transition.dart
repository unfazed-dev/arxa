...
/// [AppBoxKitDirectionalTabTransition] for setups WITHOUT a tabs-router animation —
...
///
/// **Superseded by [AppBoxKitAnimatedTabStack] for kept-alive tab stacks.** This
/// widget receives the ALREADY-SWAPPED stack, so it animates only the incoming
/// tab — the outgoing one vanishes with the index swap. [AppBoxKitAnimatedTabStack]
...
/// that mount platform views (`AppBoxKitNative*` / UiKitView chrome). Opacity-animating
...
///
/// ```dart
/// AppBoxKitTabSwitchTransition(
///   activeIndex: viewModel.currentIndex, // IndexTrackingViewModel
///   child: IndexedStack(index: viewModel.currentIndex, children: tabs),
/// )
/// ```
class AppBoxKitTabSwitchTransition extends StatefulWidget {
  const AppBoxKitTabSwitchTransition({
...
class _KitTabSwitchTransitionState extends State<AppBoxKitTabSwitchTransition>
  ...
  @override
  void didUpdateWidget(AppBoxKitTabSwitchTransition oldWidget) {
