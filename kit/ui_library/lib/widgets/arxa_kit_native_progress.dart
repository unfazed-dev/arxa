...
///
/// - [ArxaKitNativeProgress.linear] — a horizontal bar. Android →
///   [LinearProgressIndicatorM3E]; else → Material [LinearProgressIndicator].
/// - [ArxaKitNativeProgress.circular] — a ring. Android →
...
class ArxaKitNativeProgress extends StatelessWidget {
  const ArxaKitNativeProgress._({
    super.key,
  ...
  factory ArxaKitNativeProgress.linear({
    Key? key,
    double? value,
    Color? color,
  }) =>
      ArxaKitNativeProgress._(
        key: key,
  ...
  factory ArxaKitNativeProgress.circular({
    Key? key,
    double? value,
    Color? color,
  }) =>
  ...
  Widget build(BuildContext context) {
    ...
    if (ArxaKitPlatform.supportsComposeM3E) return _m3e(accent);
    switch (_variant) {
