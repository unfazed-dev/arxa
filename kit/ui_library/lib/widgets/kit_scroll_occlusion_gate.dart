...
import 'kit_native_chrome_gate.dart' show KitChromeHideMode;
...
/// blur/fade of the *content* — [KitScrollEdgeEffect] — not an alpha-hide of
/// the surface. New scroll-under-pinned-chrome work must use
/// [KitScrollEdgeEffect]; the reviewer's objective gate
...
/// (`KitGlassCard`'s `BackdropFilter`) hits the same class of artifact while
/// any platform view exists in the tree (flutter/flutter #175048, open).
///
/// **Fix pattern** (same machinery as [KitNativeChromeGate]): drive the child
...
/// [KitScrollEdgeEffect]).** Wrap a glass surface that lives inside a
/// scrollable where Flutter must paint over chrome:
///
/// ```dart
/// KitScrollOcclusionGate(
///   child: KitNativeIconButton(...), // or KitGlassCard, CNButton, maps…
/// )
/// ```
///
...
///
/// Pair with [KitNativeChromeGate] (compose both) when the same surface must
/// also vanish under modal blurs.
class KitScrollOcclusionGate extends StatefulWidget {
  const KitScrollOcclusionGate({
    super.key,
    required this.child,
    this.occlusionPadding = 0.0,
    this.hideMode = KitChromeHideMode.keepAlive,
  });
  ...
  /// How pixels come off screen once fully covered. [KitChromeHideMode
  /// .keepAlive] (default) keeps the native view instance alive — restore is
  /// jank-free. [KitChromeHideMode.unmount] destroys it and holds layout with
  /// a same-size placeholder (escape hatch if a specific native view
  /// misbehaves at alpha 0).
  final KitChromeHideMode hideMode;

  @override
  State<KitScrollOcclusionGate> createState() => _KitScrollOcclusionGateState();
}

class _KitScrollOcclusionGateState extends State<KitScrollOcclusionGate> {
  ScrollPosition? _position;

  /// 1.0 = fully shown, 0.0 = fully covered (paint skipped, native detached).
  double _alpha = 1.0;

  /// Depth at mount — the baseline (mirrors [KitNativeChromeGate]). A gate
  /// mounted inside an already-open modal stays visible; anything opened
  /// *after* us (kit sheets bump the depth via `kitShowNativeSheet`) counts
  /// as covering us.
  ...
  Widget build(BuildContext context) {
    final unmounting = widget.hideMode == KitChromeHideMode.unmount;
...
///
/// ```dart
/// KitNativeSearchBar(...).scrollOcclusion().wake(order: 1)
/// ```
///
/// Retired from scroll duty by ADR 0010 together with
/// [KitScrollOcclusionGate] — scrolled content under pinned chrome uses
/// `.scrollEdgeEffect()` ([KitScrollEdgeEffect]) instead; this remains only
...
extension KitScrollOcclusionX on Widget {
  Widget scrollOcclusion({
    Key? key,
    double occlusionPadding = 0.0,
    KitChromeHideMode hideMode = KitChromeHideMode.keepAlive,
  }) =>
      KitScrollOcclusionGate(
        key: key,
