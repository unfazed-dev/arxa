...
import 'arxa_kit_opaque_bar_base.dart';
import 'arxa_kit_input_tap_behavior.dart' show abxInputTapGroupId;
import 'arxa_kit_native_icon_button.dart';
import 'arxa_kit_native_textfield.dart';

/// Adaptive pinned input bar — a composite of [ArxaKitNativeTextField] with
/// leading/trailing [ArxaKitNativeIconButton] action slots, reproducing the chat
...
/// [TextFieldM3E] on Android — see [ArxaKitNativeTextField]'s tier docs), so it is
/// pinned chrome and keeps the `ArxaKitNative*` name.
///
/// The public surface is **primitives + typed slots** — [leading]/[trailing]
/// take [ArxaKitNativeIconButton]s, never arbitrary [Widget]s, so the bar's actions
/// stay on the native icon-button tiers. Actions must NOT set
/// [ArxaKitNativeIconButton.size]: the bar enforces the kit's one-size bar-glyph
...
/// ([ArxaKitInputTapBehavior]). Standalone [ArxaKitNativeIconButton]s
...
class ArxaKitNativeInputBar extends StatelessWidget {
  ...
  /// Owns the input text (forwarded to [ArxaKitNativeTextField.controller]).
  final TextEditingController? controller;

  /// Placeholder text, passed through to [ArxaKitNativeTextField.hintText].
  ...
  /// Actions before the field (e.g. a `ArxaKitGlyphs.add` attach button). Each
  /// renders at the kit's bar-glyph size; do not set
  /// [ArxaKitNativeIconButton.size] (debug-asserted).
  final List<ArxaKitNativeIconButton> leading;

  /// Actions after the field (e.g. `ArxaKitGlyphs.mic` / `ArxaKitGlyphs.send`). Same
  /// one-size contract as [leading].
  final List<ArxaKitNativeIconButton> trailing;
  ...
  /// action buttons self-gate per platform like every `ArxaKitNative*` sibling.
  ...
  Widget build(BuildContext context) {
    ...
    assert(
      ...
      'ArxaKitNativeInputBar actions keep the kit bar-glyph size (18pt default) '
    ...
    // The opaque base is [ArxaKitOpaqueBarBase] — the shared pinned-strip
    ...
    final Widget backed = opaqueGlass
        ? ArxaKitOpaqueBarBase(child: SafeArea(top: false, child: bar))
