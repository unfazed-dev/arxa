...
enum AppBoxKitDrawerVariant {
  ...
  /// [AppBoxKitFrostedSurface] skin (Flutter-drawn frosted glass, the ADR 0010
  ...
  /// width, 16dp trailing radius) — kept as a `AppBoxKitDrawer` so hosts swap
...
/// so this widget is named `AppBoxKitDrawer`, **not** `AppBoxKitNativeDrawer` (the
/// `no_invented_native_widgets` allowlist).
///
/// **Composition, not re-implementation:** `AppBoxKitDrawer` builds the stock
...
/// `AppBoxKitListSection`s of glyph-leading `AppBoxKitListTile` menu rows:
///
/// ```dart
/// Scaffold(
///   drawer: AppBoxKitDrawer(
///     child: ListView(
///       children: [
...
///           showDividers: false,
///           children: [
///             AppBoxKitListTile(glyph: AppBoxKitGlyphs.home, title: 'Home', onTap: () {}),
///             AppBoxKitListTile(glyph: AppBoxKitGlyphs.settings, title: 'Settings',
///                 onTap: () {}),
///           ],
///         ),
///       ],
...
/// `AppBoxKitGestureDriver` (with `AppBoxKitSprings.snappy`, the drawer-settle preset)
/// from your gesture layer and pass it as [motionDriver]; the drawer's
/// content is wrapped in a `AppBoxKitMotionScope` and `AppBoxKitWake`-marked rows scrub
/// with your driver:
///
/// ```dart
/// final driver = AppBoxKitGestureDriver(vsync: this); // settleSpring: snappy
/// AppBoxKitDrawer(
///   motionDriver: driver,
...
/// driver would fight. See `appbox_kit/motion/README.md` §5.
///
/// kimitail: no width cap on the 85% fraction — the idiom is phone-first;
/// add `max(width, cap)` if a tablet shell ever needs one.
class AppBoxKitDrawer extends StatelessWidget {
  const AppBoxKitDrawer({
    super.key,
    required this.child,
    this.variant = AppBoxKitDrawerVariant.glassPeek,
    this.width,
  ...
  /// `AppBoxKitListSection` menu groups, and a pinned footer. The app owns the
  /// content; the drawer owns the panel.
  final Widget child;

  /// Which drawer skin to build. Defaults to [AppBoxKitDrawerVariant.glassPeek],
  /// the video idiom.
  final AppBoxKitDrawerVariant variant;

  /// Panel width in dp. `null` (default) = the variant's width: 85% of the
  /// screen width for [AppBoxKitDrawerVariant.glassPeek], the stock themed width
  /// (304dp, or the host's `DrawerTheme`) for [AppBoxKitDrawerVariant.plain].
  final double? width;

  /// The trailing-edge corner radius for [AppBoxKitDrawerVariant.glassPeek].
  /// Defaults to 28 (the kit's large sheet radius). Ignored by
  /// [AppBoxKitDrawerVariant.plain], which takes the themed shape.
  final double cornerRadius;

  /// Optional explicit motion driver for the drawer's *content*: when set,
  /// [child] is wrapped in a `AppBoxKitMotionScope(driver: …)` so `AppBoxKitWake`-marked
  /// rows choreograph against it. Intended for hosts running custom open /
  /// close choreography through a `AppBoxKitGestureDriver` — see the class doc.
  /// `null` (default) = no scope; stock `Scaffold` drawer motion only.
  final Animation<double>? motionDriver;

  /// The [AppBoxKitDrawerVariant.glassPeek] panel width as a fraction of the
  ...
  Widget build(BuildContext context) {
    final content = motionDriver != null
        ? AppBoxKitMotionScope(driver: motionDriver, child: child)
        : child;

    return switch (variant) {
      AppBoxKitDrawerVariant.plain => Drawer(width: width, child: content),
      AppBoxKitDrawerVariant.glassPeek => _buildGlassPeek(context, content),
    };
  }
