...
/// [AppBoxKitNativeAppBar]. Drop it in as the FIRST sliver of a [CustomScrollView] so
...
///
/// Tiers (mirrors [AppBoxKitNativeAppBar]):
...
///   [AppBoxKitNativeAppBar]'s `automaticallyImplyLeading` on M3E.
...
///
/// Prefer this over [AppBoxKitNativeAppBar] in `Scaffold.appBar` when the body is a
/// [CustomScrollView] and you want the bar to scroll with the content. For a
/// fixed bar over any body, use [AppBoxKitNativeAppBar] in `Scaffold.appBar`.
class AppBoxKitNativeSliverAppBar extends StatelessWidget {
  const AppBoxKitNativeSliverAppBar({
    super.key,
    this.title,
    this.actions,
    this.leading,
  ...
  Widget build(BuildContext context) {
    ...
    if (wantNative && AppBoxKitPlatform.supportsComposeM3E) {
      ...
      // here (documented gap — same shape as AppBoxKitNativeAppBar's
    ...
    // from the AppBoxKitNativeIconButton trailing actions, so the kit implies its
    ...
    return SliverAppBar(
      ...
      // trailing menu button sits tighter than the iOS fixed bar (AppBoxKitNativeAppBar
