...
const double kArxaKitFloatingBarBlockHeight = 44 + abxGap8;
...
const Duration kArxaKitFloatingBarMotionDuration =
    Duration(milliseconds: 400);
const Curve kArxaKitFloatingBarMotionCurve = Curves.easeOutCubic;

/// Which end of [ArxaKitNativeFloatingBar] tucks away when minimized.
enum ArxaKitFloatingBarTuck {
  ...
  bool get _tucksLeading =>
      this == ArxaKitFloatingBarTuck.leading ||
      this == ArxaKitFloatingBarTuck.both;
  bool get _tucksTrailing =>
      this == ArxaKitFloatingBarTuck.trailing ||
      this == ArxaKitFloatingBarTuck.both;
}
...
/// body's `MediaQuery.padding.top` by [kArxaKitFloatingBarBlockHeight] so
...
/// on `ArxaKitPlatform.supportsLiquidGlass`.
class ArxaKitNativeFloatingBar extends StatelessWidget {
  const ArxaKitNativeFloatingBar({
    super.key,
    this.leading,
    this.title,
    this.actions,
    this.tuck = ArxaKitFloatingBarTuck.none,
  });
  ...
  /// [ArxaKitFloatingChrome], or directly for custom hosts — but note the
  ...
  final ArxaKitFloatingBarTuck tuck;
            ...
            children: [
              if (leading != null) ...[
                AnimatedSlide(
                  ...
                  duration: kArxaKitFloatingBarMotionDuration,
                  curve: kArxaKitFloatingBarMotionCurve,
                  child: IgnorePointer(
              ...
              if (title != null)
                ...
                // 'Arxa Showcase' rename overflowed the 358pt gallery bar
...
class ArxaKitTopEdgeScrim extends StatelessWidget {
  const ArxaKitTopEdgeScrim({
    super.key,
    this.fadeExtent = kArxaKitFloatingBarBlockHeight,
  });
...
/// Bottom-edge counterpart of [ArxaKitTopEdgeScrim]: a Flutter-DRAWN
...
///
/// Exists because the per-child [ArxaKitScrollEdgeEffect] is deliberately
...
/// the RAW device inset, so read it OUTSIDE [ArxaKitExtendBodyFabLift] (or
...
class ArxaKitBottomEdgeScrim extends StatelessWidget {
  const ArxaKitBottomEdgeScrim({
    super.key,
    this.fadeExtent = kArxaKitFloatingBarBlockHeight,
  });
...
/// Overlays [ArxaKitBottomEdgeScrim] on a full-bleed body: the body fills,
...
/// OUTSIDE any viewPadding-raising wrapper ([ArxaKitExtendBodyFabLift]),
...
class ArxaKitBottomEdgeScrimHost extends StatelessWidget {
  const ArxaKitBottomEdgeScrimHost({
    super.key,
    required this.child,
    this.enabled = true,
    this.fadeExtent = kArxaKitFloatingBarBlockHeight,
      ...
      children: [
        ...
        if (enabled)
          Positioned(
            ...
            left: 0,
            right: 0,
            child: ArxaKitBottomEdgeScrim(fadeExtent: fadeExtent),
          ),
      ],
    );
  }
...
/// How [ArxaKitFloatingChrome] reacts to the body scrolling away from the
/// top.
enum ArxaKitFloatingBarBehavior {
...
/// Full-bleed body + [ArxaKitNativeFloatingBar] + the scroll wiring, in one
...
///   [kArxaKitFloatingBarBlockHeight], so descendants (nested Scaffolds,
...
class ArxaKitFloatingChrome extends StatefulWidget {
  const ArxaKitFloatingChrome({
    super.key,
    required this.body,
    this.leading,
    this.title,
    this.actions,
    this.behavior = ArxaKitFloatingBarBehavior.pinned,
  });

  /// Full-bleed content the bar floats over.
  final Widget body;

  /// See [ArxaKitNativeFloatingBar.leading] — it tucks off the leading
  /// edge together with the title pill under the leading-tucking minimize
  /// variants, and rides the whole-bar slide under
  /// [ArxaKitFloatingBarBehavior.hide].
  final Widget? leading;

  /// See [ArxaKitNativeFloatingBar.title].
  final String? title;

  /// See [ArxaKitNativeFloatingBar.actions].
  final List<Widget>? actions;

  /// Scroll reaction; [ArxaKitFloatingBarBehavior.pinned] by default.
  final ArxaKitFloatingBarBehavior behavior;

  @override
  State<ArxaKitFloatingChrome> createState() =>
      _ArxaKitFloatingChromeState();
}

class _ArxaKitFloatingChromeState extends State<ArxaKitFloatingChrome> {
  bool _away = false;
  ...
  bool _onScroll(ScrollNotification notification) {
    if (widget.behavior == ArxaKitFloatingBarBehavior.pinned) return false;
  ...
  Widget build(BuildContext context) {
    ...
    final hide = widget.behavior == ArxaKitFloatingBarBehavior.hide;
    Widget bar = ArxaKitNativeFloatingBar(
      leading: widget.leading,
      title: widget.title,
      actions: widget.actions,
      tuck: !_away
          ? ArxaKitFloatingBarTuck.none
          : switch (widget.behavior) {
              ArxaKitFloatingBarBehavior.minimize =>
                ArxaKitFloatingBarTuck.both,
              ArxaKitFloatingBarBehavior.minimizeTrailing =>
                ArxaKitFloatingBarTuck.trailing,
              ArxaKitFloatingBarBehavior.minimizeLeading =>
                ArxaKitFloatingBarTuck.leading,
              _ => ArxaKitFloatingBarTuck.none,
            },
    );
    if (hide) {
      bar = AnimatedSlide(
    ...
    //   ArxaKitTopEdgeScrim are siblings below, structurally unreachable
    //   by this clip.
    ...
    // same SDK verification ArxaKitNativeChromeGate cites), so the native
    ...
    bar = ClipRect(
      ...
      clipper: const _ArxaKitHorizontalContainmentClipper(),
      child: bar,
    );
    return NotificationListener<ScrollNotification>(
      ...
      child: Stack(
        children: [
            ...
            child: MediaQuery(
              data: media.copyWith(
                padding: media.padding.copyWith(
                  top: media.padding.top + kArxaKitFloatingBarBlockHeight,
                ),
              ),
              child: widget.body,
            ),
          ...
          const Positioned(
              top: 0, left: 0, right: 0, child: ArxaKitTopEdgeScrim()),
...
class _ArxaKitHorizontalContainmentClipper extends CustomClipper<Rect> {
  const _ArxaKitHorizontalContainmentClipper();
  ...
  @override
  bool shouldReclip(_ArxaKitHorizontalContainmentClipper oldClipper) => false;
}
