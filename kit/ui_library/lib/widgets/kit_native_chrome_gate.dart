...
/// How [KitNativeChromeGate] takes the child's pixels off screen.
enum KitChromeHideMode {
...
///   sheet/dialog routes AND by `withNativeChromeHidden` (a snackbar's full
///   on-screen lifetime).
...
/// - *No layout shift, ever*: in [KitChromeHideMode.keepAlive] the child
///   never stops laying out, so its footprint is simply still there (the
///   scale is a paint-time transform, not a layout change); in
///   [KitChromeHideMode.unmount] a measured same-size placeholder stands in.
...
class KitNativeChromeGate extends StatefulWidget {
  const KitNativeChromeGate({
    super.key,
    required this.child,
  ...
  @override
  State<KitNativeChromeGate> createState() => _KitNativeChromeGateState();
}

class _KitNativeChromeGateState extends State<KitNativeChromeGate>
  ...
  @override
  void didUpdateWidget(KitNativeChromeGate oldWidget) {
...
/// Fluent sugar for wrapping a native-glass tier in [KitNativeChromeGate] — the
/// kit-wide "autoHideOnPageTransition" every `KitNative*` Liquid Glass widget
...
/// + modal depth).
extension KitNativeChromeGateX on Widget {
  Widget chromeGated({
