...
import 'arxa_kit_menu_item.dart';
...
/// [label] and/or [icon] / [sfSymbol], and [items] are [ArxaKitMenuItem]s — so hosts
/// never import either underlying dep. Each tier maps [ArxaKitMenuItem] to its
...
/// consistent with [ArxaKitNativeFab] / [ArxaKitNativeButton]. [label] renders a text
/// trigger; otherwise an icon trigger built from [icon] / [sfSymbol].
class ArxaKitNativePopupMenu extends StatelessWidget {
  const ArxaKitNativePopupMenu({
    super.key,
    this.glyph,
    IconData? icon,
    String? sfSymbol,
  ...
  /// Paired Material icon + SF Symbol for the trigger (see [ArxaKitGlyphs]). The
  ...
  /// Menu items. Each [ArxaKitMenuItem] maps to the tier's native menu type.
  final List<ArxaKitMenuItem> items;

  /// Notified with the selected item.
  final ValueChanged<ArxaKitMenuItem>? onSelect;
  ...
  Future<void> _openMenu(BuildContext context) async {
    ...
    final selected = await showMenu<ArxaKitMenuItem>(
      context: context,
      position: position,
      constraints: BoxConstraints(maxHeight: maxHeight),
      items: [
        for (final item in items)
          PopupMenuItem<ArxaKitMenuItem>(
            value: item,
            child: Row(
  ...
  Widget _trigger(BuildContext context, {required VoidCallback onTap}) {
    ...
    // IconButtonM3E here.
    return ArxaKitNativeIconButton(
  ...
  Widget _glass(BuildContext context) {
    ...
    // 18pt matches ArxaKitNativeIconButton's Apple bar-glyph default (HIG nav-bar
    ...
    final symbol = sfSymbol == null
        ? null
        // Wire the trigger glyph to the kit accent (mirrors ArxaKitNativeIconButton)
    ...
    // (+ modal overlays) so it can't leak over a route slide — ArxaKitNativeChromeGate.
    ...
    if (label != null) {
      return ArxaKitNativeChromeGate(
