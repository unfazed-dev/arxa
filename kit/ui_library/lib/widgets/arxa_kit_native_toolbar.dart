...
/// deps. Mirrors [ArxaKitMenuItem]'s shape: the kit owns the vocabulary, each tier
/// maps it to its native action type (1:1).
class ArxaKitToolbarAction {
  const ArxaKitToolbarAction({
    this.label,
    this.glyph,
    IconData? icon,
    String? sfSymbol,
  ...
  /// Paired Material icon + SF Symbol (see [ArxaKitGlyphs]). The preferred way to
  ...
  @override
  String toString() =>
      'ArxaKitToolbarAction(label: $label, sfSymbol: $sfSymbol, icon: $icon, '
...
///   [ArxaKitToolbarAction] is the exact shape [ToolbarActionM3E] accepts.
/// - **iOS 26 Liquid Glass** — [CNGlassButtonGroup]: one [CNButtonData] per
///   [ArxaKitToolbarAction], blended into a single glass pill via a shared
///   `glassEffectUnionId` (mirrors [ArxaKitNativeSplitButton._glass]).
...
///
/// The public surface is **primitives only** — [ArxaKitToolbarAction], not `Widget`
...
/// shape) could not honor either. See [ArxaKitNativeSplitButton] for the same
/// rationale.
class ArxaKitNativeToolbar extends StatelessWidget {
  const ArxaKitNativeToolbar({
    super.key,
  ...
  /// Trailing actions. Each [ArxaKitToolbarAction] maps to the tier's native action
  /// type — never an arbitrary [Widget].
  final List<ArxaKitToolbarAction> actions;
  ...
  Widget build(BuildContext context) {
    // ponytail: gate order mirrors ArxaKitNativeSplitButton — M3E first (Android),
    // then Liquid Glass (iOS), else Material fallback.
    if (wantNative && ArxaKitPlatform.supportsComposeM3E)
      return _m3e(context);
    if (wantNative && ArxaKitPlatform.isIOS) return _glass(context);
  ...
  Widget _m3e(BuildContext context) => ToolbarM3E(
        ...
        // the exact shape ArxaKitToolbarAction carries, so the mapping is 1:1.
