...
import 'arxa_kit_native_app_bar.dart';
...
///   the body goes full-bleed under [ArxaKitFloatingChrome] — native
...
///   law rule 4).
/// - **Android**: boxed [ArxaKitNativeAppBar], which renders `AppBarM3E` —
...
///   [ArxaKitNativeAppBar] fallback tiers.
///
...
/// [ArxaKitNativeAppBar], which is exactly the hand-assembly the law's
...
/// `extendBehindTopBar: true` to their `ArxaKitEdgeAwareListView` for
/// materialization headroom.
class ArxaKitChromeScaffold extends StatelessWidget {
  const ArxaKitChromeScaffold({
    super.key,
    required this.body,
    this.leading,
    this.title,
    ...
    this.behavior = ArxaKitFloatingBarBehavior.minimize,
  ...
  /// see [ArxaKitNativeFloatingBar.leading].
  ...
  /// `ArxaKitPlatform.supportsLiquidGlass` cannot vary (widget tests).
  @visibleForTesting
  final bool? debugForceGlassTier;

  @override
  Widget build(BuildContext context) {
    final glass = debugForceGlassTier ?? ArxaKitPlatform.supportsLiquidGlass;
    return Scaffold(
      appBar: glass
          ? null
          : ArxaKitNativeAppBar(
