...
/// luminance once and every [AppBoxKitNativeButton] / [AppBoxKitNativeIconButton]
/// adapts automatically — one publisher, N consumers.
///
/// **Publishers.** [AppBoxKitFrostedSurface] publishes on both of its
...
/// [AppBoxKitGlassLuminance] directly.
///
/// **Fallback.** Where NO scope exists, [appBoxKitGlassDemotesOnBrightBase]
...
/// bright imagery).
class AppBoxKitGlassLuminance extends InheritedWidget {
  const AppBoxKitGlassLuminance({
    super.key,
  ...
  static AppBoxKitGlassLuminance? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<AppBoxKitGlassLuminance>();

  @override
  bool updateShouldNotify(AppBoxKitGlassLuminance oldWidget) =>
...
/// demote to the bright-base idiom? The nearest [AppBoxKitGlassLuminance]
/// declaration wins; absent one, the theme's scaffold background (an opaque
/// base by definition) decides.
bool appBoxKitGlassDemotesOnBrightBase(BuildContext context) {
  final scope = AppBoxKitGlassLuminance.maybeOf(context);
