...
/// The edge of a scrollable a [ArxaKitScrollEdgeEffect] treats.
enum ArxaKitScrollEdge {
...
/// Style of a [ArxaKitScrollEdgeEffect], mirroring iOS 26's
/// `UIScrollEdgeEffect.Style`.
enum ArxaKitScrollEdgeEffectStyle {
...
/// chrome — so the widget is named `Kit*`, never `ArxaKitNative*`.
///
...
/// This supersedes [ArxaKitScrollOcclusionGate] for scroll duty — chrome stops
...
/// [ArxaKitNativeChromeGate]), so there the surface fades as one piece.
/// Geometry comes from
...
/// `ArxaKitNativeSliverAppBar` / `SliverAppBar` the effect needs **zero
/// configuration**. Use [occlusionPadding] only for chrome overlaid from
/// *outside* the scrollable (e.g. a tab bar stacked over
/// `Scaffold(extendBody: true)` at the [ArxaKitScrollEdge.bottom] edge).
///
...
///   child: ArxaKitGlassCard(...), // frosted-tier content
/// )
/// ```
///
...
///     .scrollEdgeEffect(edge: ArxaKitScrollEdge.bottom, occlusionPadding: 56)
/// ```
class ArxaKitScrollEdgeEffect extends StatefulWidget {
  const ArxaKitScrollEdgeEffect({
    super.key,
    required this.child,
    this.style = ArxaKitScrollEdgeEffectStyle.automatic,
    this.edge = ArxaKitScrollEdge.top,
  ...
  /// [ArxaKitScrollEdgeEffectStyle.automatic], which resolves to `.soft`.
  final ArxaKitScrollEdgeEffectStyle style;

  /// Which viewport edge the coverage is measured against. Defaults to
  /// [ArxaKitScrollEdge.top].
  final ArxaKitScrollEdge edge;
  ...
  @override
  State<ArxaKitScrollEdgeEffect> createState() => _KitScrollEdgeEffectState();
}

class _KitScrollEdgeEffectState extends State<ArxaKitScrollEdgeEffect> {
  ...
  void _recompute() {
    ...
    final reveal = viewport
        .getOffsetToReveal(
            box, widget.edge == ArxaKitScrollEdge.top ? 0.0 : 1.0)
        .offset;
    final covered = widget.edge == ArxaKitScrollEdge.top
  ...
  Widget build(BuildContext context) {
    final t = _t;
    final hard = widget.style == ArxaKitScrollEdgeEffectStyle.hard;
    // Blur is a FROSTED-TIER effect only. On the Liquid Glass tier the kit's
    // content surfaces (ArxaKitGlassCard -> LiquidGlassContainer) are
    ...
    // applies to platform views reliably (see ArxaKitNativeChromeGate), so on
    ...
    final blurs = !ArxaKitPlatform.supportsLiquidGlass;
...
///
/// ```dart
/// ArxaKitGlassCard(...).scrollEdgeEffect()
/// ```
///
...
/// the bar collapses.
extension ArxaKitScrollEdgeEffectX on Widget {
  Widget scrollEdgeEffect({
    Key? key,
    ArxaKitScrollEdgeEffectStyle style =
        ArxaKitScrollEdgeEffectStyle.automatic,
    ArxaKitScrollEdge edge = ArxaKitScrollEdge.top,
