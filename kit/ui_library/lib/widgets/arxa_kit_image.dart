...
/// M3-Expressive) image surface, so this widget is named `ArxaKitImage` (a
/// content widget), **not** `ArxaKitNativeImage`. See `core/NATIVE_COMPONENTS.md`
...
class ArxaKitImage extends StatelessWidget {
  const ArxaKitImage({
    super.key,
    required this.asset,
    this.size,
  ...
  /// load error. Defaults to [ArxaKitGlyphs.photo].
  final ArxaKitGlyph? placeholder;
  ...
  Widget build(BuildContext context) {
    ...
    final glyph = placeholder ?? ArxaKitGlyphs.photo;
    final w = size ?? width;
    final h = size ?? height;
