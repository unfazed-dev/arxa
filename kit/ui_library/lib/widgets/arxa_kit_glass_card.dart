...
///   [ArxaKitPlatform.supportsLiquidGlass]; below iOS 26 the container degrades to
///   a bare child (no card chrome), so the kit falls through to the frosted
///   tier there — mirrors [ArxaKitNativeTabBar]'s glass gate. **Inside a
...
/// - **Flutter-drawn frosted** — [ArxaKitFrostedSurface] (BackdropFilter blur +
...
class ArxaKitGlassCard extends StatelessWidget {
  const ArxaKitGlassCard({
    super.key,
    required this.child,
  ...
  /// maps to [ArxaKitFrostedSurface.borderRadius]. `null` = 16 dp on both tiers
  ...
  Widget build(BuildContext context) {
    ...
    if (wantNative && !inScrollable && ArxaKitPlatform.supportsLiquidGlass) {
    ...
    if (opaqueGlass) {
      ...
      return Builder(
        builder: (BuildContext context) => ArxaKitFrostedSurface(
