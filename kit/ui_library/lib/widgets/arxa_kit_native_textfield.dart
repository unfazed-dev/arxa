...
import 'arxa_kit_input_tap_behavior.dart';
import 'arxa_kit_native_chrome_gate.dart';

/// Adaptive text field — three-tier, matching the rest of the `ArxaKitNative*`
...
class ArxaKitNativeTextField extends StatelessWidget {
  const ArxaKitNativeTextField({
    super.key,
    this.controller,
    this.placeholder,
    this.hintText,
  ...
  /// for symmetry with [ArxaKitNativeSearchBar.hint].
  ...
  /// Tap-outside keyboard dismissal default (see [ArxaKitInputTapBehavior]).
  ...
  Widget build(BuildContext context) {
    ...
    final useNativeTier = wantNative && ArxaKitPlatform.isIOS;
    if (useNativeTier) {
    ...
    // like every ArxaKitNative* sibling. `wantNative: false` skips it to the
  ...
  /// The per-input dismissal default (see [ArxaKitInputTapBehavior]) — one
  ...
  Widget _withInputTapBehavior(Widget tier) => ArxaKitInputTapBehavior(
