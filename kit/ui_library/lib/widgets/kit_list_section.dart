...
import 'kit_glass_card.dart';

/// A grouped inset section: an optional header label over a glass-card-backed
/// group container holding rows (usually `KitListTile`s), with hairline
...
/// widget is named `KitListSection` (a content widget), **not**
/// `KitNativeListSection`. See `core/NATIVE_COMPONENTS.md` ("Content
/// widgets"). The group container is `KitGlassCard`, per the kit's
...
/// plugin rows: stack several `KitListSection`s in a scroll view, one per
...
/// the card.
class KitListSection extends StatelessWidget {
  const KitListSection({
    super.key,
    required this.children,
    this.header,
    this.showDividers = true,
    this.margin = const EdgeInsets.symmetric(horizontal: kPad16),
    this.borderRadius,
  });

  /// The rows, typically `KitListTile`s. Rendered full-bleed inside the
  ...
  /// Corner radius forwarded to the backing `KitGlassCard`. `null` = the
    ...
    final rows = <Widget>[
      for (var i = 0; i < children.length; i++) ...[
        ...
        if (showDividers && i < children.length - 1)
          const Divider(height: 1, indent: kPad16),
      ],
    ];
        ...
        if (header != null)
          Padding(
            padding: EdgeInsets.only(
              left: kPad16 + margin.horizontal / 2,
              right: kPad16 + margin.horizontal / 2,
              bottom: kPad8,
            ),
            child: Text(
              header!,
