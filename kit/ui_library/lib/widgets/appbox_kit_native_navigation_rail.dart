...
class AppBoxKitRailDestination {
  const AppBoxKitRailDestination({
    this.glyph,
    IconData? icon,
    required this.label,
    String? sfSymbol,
  })  : assert(glyph != null || icon != null,
            'AppBoxKitRailDestination needs a glyph or an icon'),
        _icon = icon,
        _sfSymbol = sfSymbol;
...
/// Adaptive navigation rail — three-tier structural gate (mirrors [AppBoxKitNativeTabBar]):
/// - **Android M3 Expressive** — [NavigationRailM3E]. The kit's flat
///   [AppBoxKitRailDestination] list is wrapped in a single
...
class AppBoxKitNativeNavigationRail extends StatelessWidget {
  const AppBoxKitNativeNavigationRail({
    super.key,
  ...
  /// [AppBoxKitNativeTabBar.currentIndex]).
  ...
  final List<AppBoxKitRailDestination> destinations;
...
// yet; swap this tier for the native one when it lands ([AppBoxKitRailDestination.
