...
const Object abxInputTapGroupId = 'arxa.kit.inputs';
...
/// every scaffolded arxa app, which ship no keyboard wiring of their own —
/// has no tap-outside dismissal at all. Arxa wants one by default, on every
...
class ArxaKitInputTapBehavior extends StatelessWidget {
  const ArxaKitInputTapBehavior({
    super.key,
    required this.child,
  ...
  Widget build(BuildContext context) {
    ...
    return TextFieldTapRegion(
      ...
      onTapOutside: (_) => arxaKitDismissKeyboard(),
      child: child,
    );
  }
}
...
/// ([ArxaKitKeyboardX.dismissKeyboard]) at call sites that have a context;
/// this bare function exists for the ones that do not.
void arxaKitDismissKeyboard() {
...
extension ArxaKitKeyboardX on BuildContext {
  ...
  /// }
  /// ```
  void dismissKeyboard() => arxaKitDismissKeyboard();
}
