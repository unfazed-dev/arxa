...
/// [onSelect]) so hosts never import either underlying dep. [ArxaKitMenuItem] is
...
class ArxaKitNativeFabMenu extends StatefulWidget {
  const ArxaKitNativeFabMenu({
    super.key,
    this.glyph,
    IconData? icon,
    String? sfSymbol,
  ...
  /// Paired Material icon + SF Symbol for the primary FAB (see [ArxaKitGlyphs]).
  ...
  @override
  State<ArxaKitNativeFabMenu> createState() => _KitNativeFabMenuState();
}

class _KitNativeFabMenuState extends State<ArxaKitNativeFabMenu> {
  ...
  Widget _m3e(BuildContext context) {
    return FabMenuM3E(
      ...
      overlay: false,
      // ArxaKitFabMorph (not FabM3E): the primary FAB shape-morphs on press like the
      ...
      primaryFab: AnimatedBuilder(
        ...
        builder: (context, _) => ArxaKitFabMorph(
  ...
  Widget _glass(BuildContext context) {
    ...
    // circular buttons. Mirrors ArxaKitNativeFab's glyph sizing.
    final symbol = widget.sfSymbol == null
        ? null
        // Wire the FAB-menu glyph to the kit accent (mirrors ArxaKitNativeFab) so
