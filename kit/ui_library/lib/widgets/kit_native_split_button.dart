...
/// [KitMenuItem] — so hosts never import either underlying dep.
///
...
/// keeps a 1:1 mapping across all three tiers — see [KitNativePopupMenu] for
/// the same rationale.
class KitNativeSplitButton extends StatelessWidget {
  const KitNativeSplitButton({
    super.key,
    this.label,
    this.glyph,
    IconData? icon,
  ...
  /// Paired Material icon + SF Symbol for the action face (see [KitGlyphs]).
  ...
  /// Menu items revealed by the chevron. Each [KitMenuItem] maps to the tier's
  /// native menu type.
  final List<KitMenuItem> menuItems;

  /// Notified with the selected menu item.
  final ValueChanged<KitMenuItem>? onMenuSelected;
  ...
  Widget build(BuildContext context) {
    ...
    if (KitPlatform.isIOS) return _glass(context);
    return _material(context);
  }

  Widget _m3e(BuildContext context) => SplitButtonM3E<KitMenuItem>(
        ...
        items: [
          for (final item in menuItems)
            SplitButtonM3EItem<KitMenuItem>(
              value: item,
              child: item.label,
            ),
        ],
  ...
  Widget _glass(BuildContext context) {
    ...
    return CNSplitButton(
      ...
      items: [
        for (final item in menuItems)
          // Same mapping as KitNativeFabMenu: SF Symbol wins on Apple, the
