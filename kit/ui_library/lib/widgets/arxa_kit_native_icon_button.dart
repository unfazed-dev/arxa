...
/// Mirrors [ArxaKitNativeButton]'s icon convention so the host passes primitives and
...
class ArxaKitNativeIconButton extends StatelessWidget {
  const ArxaKitNativeIconButton({
    super.key,
    this.glyph,
    IconData? icon,
    String? sfSymbol,
  ...
  /// surface — declared by an [ArxaKitGlassLuminance] scope or, absent one,
  ...
  /// [ArxaKitNativeToolbar]'s 18pt symbols) and the M3E Icon default (24) on
  /// Android.
  final double? size;
...
/// Shared implied leading for the kit app bars ([ArxaKitNativeAppBar] +
/// [ArxaKitNativeSliverAppBar]). When the host passed no [leading] and the route
...
/// [ArxaKitNativeIconButton] with [ArxaKitGlyphs.back] — the SAME widget the trailing
...
Widget? arxaKitImpliedAppBarLeading(
