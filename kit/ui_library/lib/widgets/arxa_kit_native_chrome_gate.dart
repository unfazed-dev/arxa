...
/// How [ArxaKitNativeChromeGate] takes the child's pixels off screen.
enum ArxaKitChromeHideMode {
...
///   sheet/dialog routes AND by `arxaKitWithNativeChromeHidden` (a snackbar's full
///   on-screen lifetime).
...
/// - *No layout shift, ever*: in [ArxaKitChromeHideMode.keepAlive] the child
...
///   [ArxaKitChromeHideMode.unmount] a measured same-size placeholder stands in.
///
...
/// `arxa_kit_chrome_gate_transition_scope_test.dart` now builds it. The bar
...
class ArxaKitNativeChromeGate extends StatefulWidget {
  const ArxaKitNativeChromeGate({
    super.key,
    required this.child,
  ...
  @override
  State<ArxaKitNativeChromeGate> createState() => _KitNativeChromeGateState();
}

class _KitNativeChromeGateState extends State<ArxaKitNativeChromeGate> {
  bool _hidden = false;
...
/// Fluent sugar for wrapping a native-glass tier in [ArxaKitNativeChromeGate] — the
/// kit-wide "autoHideOnPageTransition" every `ArxaKitNative*` Liquid Glass widget
...
extension ArxaKitNativeChromeGateX on Widget {
  Widget chromeGated({
