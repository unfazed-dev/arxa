...
/// [AppBoxKitNativeButton] for the same convention.
///
/// [onPressed] is nullable: `null` disables the button on every tier.
class AppBoxKitNativeFab extends StatelessWidget {
  const AppBoxKitNativeFab({
    super.key,
    this.icon,
    this.sfSymbol,
    this.label,
  ...
  Widget _m3e(BuildContext context) {
    ...
    // AppBoxKitFabMorph (not FabM3E): a filled/elevated M3E FAB that shape-morphs on
    // press like the kit's icon buttons. FabM3E's shape is static — no morph.
    return AppBoxKitFabMorph(
