...
import 'package:appbox_kit_core/common/kit_app_constants.dart' show kGap8;
...
/// [KitNativeAppBar]. Drop it in as the FIRST sliver of a [CustomScrollView] so
...
///   [KitNativeAppBar]'s `automaticallyImplyLeading` on M3E.
...
///
/// Prefer this over [KitNativeAppBar] in `Scaffold.appBar` when the body is a
/// [CustomScrollView] and you want the bar to scroll with the content. For a
/// fixed bar over any body, use [KitNativeAppBar] in `Scaffold.appBar`.
class KitNativeSliverAppBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (wantNative && KitPlatform.supportsComposeM3E) {
      // M3E owns its collapsing scroll motion; floating/pinned/snap are ignored
      // here (documented gap — same shape as KitNativeAppBar's
    ...
    // from the KitNativeIconButton trailing actions, so the kit implies its
    ...
    return SliverAppBar(
      ...
      // trailing menu button sits tighter than the iOS fixed bar (KitNativeAppBar
      ...
      // the fixed CN bar spaces its trailing Row by kGap8. Wrap the actions in
      // the same Row so two bar buttons sit the same distance apart in either
      // appbar — shared kGap8 (not a literal) so the two bars can't drift.
