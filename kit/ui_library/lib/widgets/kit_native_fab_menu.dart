...
/// [onSelect]) so hosts never import either underlying dep. [KitMenuItem] is
...
class KitNativeFabMenu extends StatefulWidget {
  const KitNativeFabMenu({
    super.key,
    this.glyph,
    IconData? icon,
    String? sfSymbol,
  ...
  /// Paired Material icon + SF Symbol for the primary FAB (see [KitGlyphs]).
  ...
  @override
  State<KitNativeFabMenu> createState() => _KitNativeFabMenuState();
}

class _KitNativeFabMenuState extends State<KitNativeFabMenu> {
  ...
  Widget _m3e(BuildContext context) {
    return FabMenuM3E(
      ...
      overlay: false,
      // KitFabMorph (not FabM3E): the primary FAB shape-morphs on press like the
  ...
  Widget _glass(BuildContext context) {
    ...
    // circular buttons. Mirrors KitNativeFab's glyph sizing.
    final symbol = widget.sfSymbol == null
        ? null
        // Wire the FAB-menu glyph to the kit accent (mirrors KitNativeFab) so
