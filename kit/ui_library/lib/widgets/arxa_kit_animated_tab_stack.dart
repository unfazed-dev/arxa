...
/// already-swapped tabs stack ([ArxaKitTabSwitchTransition]) can only animate the
...
/// state intact) — the pattern [ArxaKitLazyIndexedStack] established. Unvisited
...
/// `ArxaKitAnimatedTabStack(activeIndex: viewModel.currentIndex, children: MyView.tabViews)`.
...
/// that mount platform views (`ArxaKitNative*` / UiKitView chrome). Opacity-
...
class ArxaKitAnimatedTabStack extends StatefulWidget {
  const ArxaKitAnimatedTabStack({
  ...
  /// carry `ArxaKitNative*` chrome), or `false` to force instant everywhere.
  ///
  ...
  @override
  State<ArxaKitAnimatedTabStack> createState() => _KitAnimatedTabStackState();
}

class _KitAnimatedTabStackState extends State<ArxaKitAnimatedTabStack>
  ...
  bool get _animates => widget.animated ?? !ArxaKitPlatform.isIOS;
  ...
  @override
  void didUpdateWidget(ArxaKitAnimatedTabStack oldWidget) {
