...
///
/// **Do NOT wrap tab stacks that mount platform views (any `ArxaKitNative*` /
...
///     body: ArxaKitDirectionalTabTransition(
...
/// )
/// ```
class ArxaKitDirectionalTabTransition extends StatefulWidget {
  const ArxaKitDirectionalTabTransition({
  ...
  /// [ArxaKitMotionScope]).
  ///
  /// Pass this as a `ArxaKitMotionScope.driver` inside a tab body to replay its
  /// wake choreography on every tab switch:
  ///
  /// ```dart
  /// ArxaKitMotionScope(
  ///   driver: ArxaKitDirectionalTabTransition.timelineOf(context),
  ///   child: ...,
  /// )
  /// ```
  ...
  @override
  State<ArxaKitDirectionalTabTransition> createState() =>
...
class _KitDirectionalTabTransitionState
    extends State<ArxaKitDirectionalTabTransition> {
  /// 0 until the first switch, then +1 (moved to a higher index) or -1.
  int _direction = 0;

  @override
  void didUpdateWidget(ArxaKitDirectionalTabTransition oldWidget) {
