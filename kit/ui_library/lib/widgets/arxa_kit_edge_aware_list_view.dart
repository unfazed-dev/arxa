...
/// A [ListView] that applies [ArxaKitScrollEdgeEffect] to every child, so the
...
/// bare `ArxaKitNativeToolbar` sat in the profile list between two treated
...
/// )
/// ```
class ArxaKitEdgeAwareListView extends StatelessWidget {
  const ArxaKitEdgeAwareListView({
    super.key,
    required this.children,
    this.padding,
    this.controller,
    this.physics,
    this.edges = ArxaKitScrollEdges.both,
  ...
  final ArxaKitScrollEdges edges;
...
Widget _treat(
  ...
  required ArxaKitScrollEdgeEffectStyle style,
}) {
  var treated = child;
  if (topEdge) {
...
/// The sliver counterpart of [ArxaKitEdgeAwareListView], for the
...
/// )
/// ```
class ArxaKitEdgeAwareSliverList extends StatelessWidget {
  const ArxaKitEdgeAwareSliverList({
    super.key,
