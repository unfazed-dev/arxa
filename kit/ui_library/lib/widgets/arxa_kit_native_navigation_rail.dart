...
/// [ArxaKitTab]'s shape).
///
/// Prefer [glyph] (a paired icon + SF Symbol from [ArxaKitGlyphs]) over the raw
/// [icon] / [sfSymbol] escape hatches — one token, both tiers, no drift.
class ArxaKitRailDestination {
  const ArxaKitRailDestination({
    this.glyph,
    IconData? icon,
    required this.label,
    String? sfSymbol,
  })  : assert(glyph != null || icon != null,
            'ArxaKitRailDestination needs a glyph or an icon'),
        _icon = icon,
        _sfSymbol = sfSymbol;

  /// Paired Material icon + SF Symbol (see [ArxaKitGlyphs]).
...
/// Adaptive navigation rail — three-tier structural gate (mirrors [ArxaKitNativeTabBar]):
/// - **Android M3 Expressive** — [NavigationRailM3E]. The kit's flat
///   [ArxaKitRailDestination] list is wrapped in a single
...
class ArxaKitNativeNavigationRail extends StatelessWidget {
  const ArxaKitNativeNavigationRail({
    super.key,
  ...
  /// [ArxaKitNativeTabBar.currentIndex]).
  ...
  final List<ArxaKitRailDestination> destinations;
  ...
  // floating surface. Kit-side clip rather than a fork param so arxa_kit
...
// yet; swap this tier for the native one when it lands ([ArxaKitRailDestination.
