...
import '../../widgets/arxa_kit_native_sheet.dart';

/// Drop-in [BottomSheetService] that presents through [arxaKitShowSheet]
/// instead of `Get.bottomSheet`, so every stacked sheet call site
/// (`showBottomSheet`, `showCustomSheet`, ArxaKitAction's ArxaKitNotificationManager)
...
/// `LazySingleton(classType: ArxaKitBottomSheetService, asType: BottomSheetService)`.
///
...
/// useRootNavigator, elevation) are accepted but ignored — arxaKitShowSheet
...
class ArxaKitBottomSheetService extends BottomSheetService {
  ...
  Future<SheetResponse?> showBottomSheet({
    ...
    if (context == null) {
      debugPrint(
          'ArxaKitBottomSheetService: no navigator context; sheet skipped');
      return Future.value(null);
    }
    return arxaKitShowSheet<SheetResponse>(
      context: context,
  ...
  Future<SheetResponse<T>?> showCustomSheet<T, R>({
    ...
    if (sheetBuilder == null || context == null) {
      debugPrint(
          'ArxaKitBottomSheetService: no builder/context; sheet skipped');
      return Future.value(null);
    }
    return arxaKitShowSheet<SheetResponse<T>>(
      context: context,
      isDismissible: barrierDismissible,
