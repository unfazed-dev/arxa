...
import 'package:arxa_kit_core/common/arxa_kit_colors.dart'
    show ArxaKitColors;
import 'package:arxa_kit_core/arxa_kit_locator.dart' show arxaKitLocator;
import 'package:arxa_kit_ui_library/utils/kit_action/arxa_kit_snackbar_type.dart'
    show ArxaKitSnackbarType;
import '../../utils/arxa_kit_native_overlay.dart'
    show arxaKitWithNativeChromeHidden;
import 'package:arxa_kit_core/platform/arxa_kit_platform.dart'
    show ArxaKitPlatform;

import '../../widgets/arxa_kit_native_dialog.dart';
import '../../widgets/arxa_kit_native_sheet.dart' show arxaKitShowSheet;
import 'arxa_kit_ask_surfaces.dart';

/// Severity for [ArxaKitNotificationService.show]. Drives the M3E-tier snackbar
/// variant (Android / iOS action fallback) and the CNToast style preset (the
/// iOS default tier).
enum ArxaKitNotificationKind { info, success, error, warning }

/// Screen position for [ArxaKitNotificationService.show]'s `position` param.
...
enum ArxaKitToastPosition { top, center, bottom }

/// The kit's transient-feedback service. One entry point that picks the right
/// native surface per platform — supersedes `arxaKitShowNativeToast` and the
/// Material [SnackbarService] coexisting on iOS.
///
/// Routing:
/// - **Android** ([ArxaKitPlatform.supportsComposeM3E]) → host [SnackbarService]
///   (native Material `ScaffoldMessenger`), wrapped in [arxaKitWithNativeChromeHidden]
...
/// - **Snackbar tier + `position: ArxaKitToastPosition.center`** → a kit-owned
...
/// `setupArxaKitSnackbars()` (registers the SnackbarConfig variants the Android +
/// fallback tiers render).
class ArxaKitNotificationService {
  ...
  /// context ([StackedService.navigatorKey]) so context-less callers (ArxaKitAction
  ...
  /// the snackbar tiers honor [ArxaKitToastPosition.center] with a kit-styled
  /// floating pill at screen center, and render top/bottom bottom-anchored
  /// (the registered SnackbarConfig's anchor). Defaults to
  /// [ArxaKitToastPosition.top] to clear the status bar / Dynamic Island.
  ///
  ...
  /// be a [ArxaKitSnackbarType] or a host enum variant with a registered config —
  ...
  Future<void> show(
    ...
    if (ArxaKitPlatform.supportsComposeM3E || actionLabel != null) {
      if (position == ArxaKitToastPosition.center) {
        return _centerPill(
      ...
      final snackbar = arxaKitLocator<SnackbarService>();
      ...
      if (ArxaKitPlatform.supportsLiquidGlass) {
        final ctx =
      ...
      return arxaKitWithNativeChromeHidden(
  ...
  /// ([arxaKitShowNativeDialog]) with a single action, so apps never
  /// register a stacked alert variant. Fire-and-forget. This is also the
  /// surface ArxaKitAction's `dialog` notification type routes through.
  /// Pre-boot no-ops.
  Future<void> alert({
    ...
    if (ctx == null) return;
    await arxaKitShowNativeDialog<void>(
      context: ctx,
      title: title,
      message: message,
      barrierDismissible: barrierDismissible,
      actions: [
        ArxaKitNativeDialogAction(
          label: actionLabel,
  ...
  /// ([arxaKitShowNativeDialog]), so apps never register a stacked confirm
  ...
  Future<bool> confirm({
    ...
    final result = await arxaKitShowNativeDialog<bool>(
      context: ctx,
      title: title,
      message: message,
      actions: [
        ArxaKitNativeDialogAction(label: cancelLabel, value: false),
        ArxaKitNativeDialogAction(
          label: actionLabel,
          value: true,
          role: destructive
              ? ArxaKitDialogActionRole.destructive
  ...
  /// Asks the user for a short text entry — kit-rendered [ArxaKitPromptDialog]
  ...
  /// [ArxaKitNoticeSheetBody] presented through [arxaKitShowSheet]
  ...
  Future<void> notice({
    ...
    await arxaKitShowSheet<void>(
      context: ctx,
      builder: (_) => ArxaKitNoticeSheetBody(title: title, message: message),
    );
  }
  ...
  BuildContext? _overlayContext(BuildContext? context, String dropMessage) {
    ...
    debugPrint('ArxaKitNotificationService: no BuildContext for dialog/sheet '
        'tier; $dropMessage');
    return null;
  }
  ...
  Future<void> _centerPill(
    ...
    if (overlay == null) {
      ...
      debugPrint(
          'ArxaKitNotificationService: no Overlay for center pill tier; '
          'message dropped: "$message"');
      return;
    }
    // Not wrapped in arxaKitWithNativeChromeHidden: the pill paints no blur scrim,
  ...
  Future<void> _cnToast(
    ...
    if (ctx == null || !ctx.mounted) {
      ...
      debugPrint(
          'ArxaKitNotificationService: no BuildContext for iOS toast tier; '
          'message dropped: "$message"');
      return;
    }
    ...
    // arxa_kit_native_chrome_gate.dart). Worse, it is mounted through a bare
    ...
    switch (kind) {
      case ArxaKitNotificationKind.info:
        CNToast.info(
            context: ctx,
            message: message,
            duration: preset,
      ...
      case ArxaKitNotificationKind.warning:
        CNToast.warning(
            context: ctx,
            message: message,
            duration: preset,
            position: pos,
            useGlassEffect: glass);
      case ArxaKitNotificationKind.success:
        CNToast.success(
            context: ctx,
            message: message,
            duration: preset,
            position: pos,
            useGlassEffect: glass);
      case ArxaKitNotificationKind.error:
        CNToast.error(
            context: ctx,
            message: message,
            duration: preset,
  ...
  static ArxaKitSnackbarType _snackbarVariant(
          ArxaKitNotificationKind kind) =>
      switch (kind) {
        ArxaKitNotificationKind.info =>
          ArxaKitSnackbarType.arxaKitAutoProcessInfo,
        ArxaKitNotificationKind.success =>
          ArxaKitSnackbarType.arxaKitAutoProcessSuccess,
        ArxaKitNotificationKind.error =>
          ArxaKitSnackbarType.arxaKitAutoProcessError,
        ArxaKitNotificationKind.warning =>
          ArxaKitSnackbarType.arxaKitAutoProcessWarning,
      };

  static CNToastPosition _cnPosition(ArxaKitToastPosition p) => switch (p) {
        ArxaKitToastPosition.top => CNToastPosition.top,
        ArxaKitToastPosition.center => CNToastPosition.center,
        ArxaKitToastPosition.bottom => CNToastPosition.bottom,
      };
...
/// [ArxaKitNotificationService.show] gets `position: ArxaKitToastPosition.center` on
...
class _KitCenterToastPillState extends State<_KitCenterToastPill>
  ...
  // Matches setupArxaKitSnackbars' animationDuration / curves.
  ...
  Widget build(BuildContext context) {
    // Per-kind palette mirrors setupArxaKitSnackbars (kit_snackbar_setup.dart) —
    // keep the two in sync.
    final (bg, fg, icon) = switch (widget.kind) {
      ArxaKitNotificationKind.info => (
          ArxaKitColors.surface2,
          ArxaKitColors.muted,
          Icons.info_outline
        ),
      ArxaKitNotificationKind.success => (
          ArxaKitColors.good,
          ArxaKitColors.onAccent,
    ...
    return Positioned.fill(
      child: Material(
        ...
        child: Align(
          ...
          // ArxaKitGlassWarmup's off-screen translate exists to dodge), so
            ...
            child: ScaleTransition(
              ...
              child: Padding(
                ...
                child: LiquidGlassContainer(
                  ...
                  child: Container(
                    key: const Key('arxaKitCenterToastPill'),
