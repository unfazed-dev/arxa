import 'dart:async';
import 'appbox_kit_action_builder.dart';
import 'managers/appbox_kit_stream_manager.dart';
...
// ═══════════════════════════════════════════════════════════════════════════════
// AppBoxKitAction v2.0 - Main Facade
// ═══════════════════════════════════════════════════════════════════════════════
//
...
//
// This class provides the main entry point for AppBoxKitAction operations.
...
/// cancellable.cancel();
/// ```
class AppBoxKitAction {
  // Private constructor to prevent instantiation
  AppBoxKitAction._(); // coverage:ignore-line
  ...
  static final Expando<List<String>> _ownerKeys =
      Expando<List<String>>('AppBoxKitAction.ownerKeys');
  ...
  ///
  /// Returns a [AppBoxKitActionBuilder] that can be configured with chainable methods
  ...
  /// [disposeOwner], which `AppBoxKitViewModel.dispose` calls for you. The bare
  /// [widgetId] form remains for ownerless contexts (e.g. `main()` boot).
  /// Inside a `AppBoxKitViewModel`/`AppBoxKitActionOwner` use the `action(name, operation)`
  ...
  /// ```
  static AppBoxKitActionBuilder<T> run<T>(
    ...
    assert(
      ...
      'AppBoxKitAction.run needs an owner (+name) or an explicit widgetId',
    );
    return AppBoxKitActionBuilder<T>(
      operation: operation,
  ...
  /// Live [AppBoxKitActionState] stream (busy + last error) for an operation — the
  /// stream form of `withLoading(setBusy)`, so views bind it with
  /// `AppBoxKitStreamBuilder` instead of plumbing busy flags through the viewmodel.
  /// Seeded [ValueStream]: late subscribers see the current state
  /// immediately. Address it by [owner]+[name] (same pair the `run` used); the
  /// bare [widgetId] form is for ownerless ops. `AppBoxKitViewModel.actionState$`
  /// is the usual call path.
  static ValueStream<AppBoxKitActionState> state$({
    Object? owner,
    String? name,
    String? widgetId,
  }) {
  ...
  /// subjects, and tracked streams. `AppBoxKitViewModel.dispose` calls this;
  ...
  ///
  /// This is a compatibility method for migrating from AppBoxKitAutoProcess.
  ...
  /// @override
  /// void dispose() {
  ///   AppBoxKitAction.dispose(widgetId: 'profile_view');
  ///   super.dispose();
  /// }
  /// ```
  static void dispose({required String widgetId}) {
    ...
    // Also dispose any tracked subjects in AppBoxKitStreamManager
    AppBoxKitStreamManager.disposeWidget(widgetId);
    // And the observable action-state subject, if any view bound state$()
    AppBoxKitActionStateManager.disposeWidget(widgetId);
  }
}
