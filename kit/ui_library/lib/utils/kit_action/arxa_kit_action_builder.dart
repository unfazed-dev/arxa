...
import 'executors/arxa_kit_action_executor.dart';

// ═══════════════════════════════════════════════════════════════════════════════
// ArxaKitAction v2.0 - Fluent Builder
// ═══════════════════════════════════════════════════════════════════════════════
//
// 📖 Technical Specification: docs/kit_action_technical_specification.md
//    - Section 4.2: Component Responsibilities (ArxaKitActionBuilder)
...
///
/// **Low-level API — prefer commands in app code.** This builder is LAZY: the
/// operation only runs when the builder is awaited or `.execute()` is called,
/// so a dropped chain silently never runs. `ArxaKitActionOwner.hub`
/// (see `ArxaKitActionHub`) is the app-level API: send is hot and
...
class ArxaKitActionBuilder<T> implements Future<T> {
  final ArxaKitActionConfig<T> _config;

  /// Memoized execution future backing the [Future] interface — awaiting the
  /// builder twice must not run the operation twice.
  Future<T>? _awaited;

  /// Internal constructor - use ArxaKitAction.run() as the entry point
  /// @nodoc
  ArxaKitActionBuilder({
    required dynamic Function() operation,
    required String widgetId,
  }) : _config = ArxaKitActionConfig<T>(
          operation: operation,
          widgetId: widgetId,
        );
  ...
  ///   .withLoading(setBusy);
  /// ```
  ArxaKitActionBuilder<T> withLoading(void Function(bool isBusy) setBusy) {
  ...
  /// ```
  ArxaKitActionBuilder<T> withLoadingFor(
    Object busyObject,
  ...
  ///   });
  /// ```
  ArxaKitActionBuilder<T> withClearLoadingBeforeSuccess() {
  ...
  /// ```
  ArxaKitActionBuilder<T> completeOnError(
    String message, {
    T? withValue,
  }) {
  ...
  /// ```
  ArxaKitActionBuilder<T> handleError(void Function(Object error) handler) {
  ...
  /// ```
  ArxaKitActionBuilder<T> withLoadingSnackbar(
    String message, {
    String? title,
  ...
  /// ```
  ArxaKitActionBuilder<T> withSuccessSnackbar(
    String message, {
    String? title,
  ...
  /// ```
  ArxaKitActionBuilder<T> withErrorSnackbar(
    String message, {
    String? title,
    Duration duration = const Duration(seconds: 3),
    dynamic type,
  ...
  ///   );
  /// ```
  ArxaKitActionBuilder<T> withSnackbars({
    String? loading,
    String? success,
    String? error,
    String? loadingTitle,
  ...
  /// ```dart
  /// await ArxaKitAction.run<String>(...)
  ///   .withNotificationType(ArxaKitNotificationType.dialog)
  ///   .withSnackbars(
  ///     loading: 'Saving...',
  ///     success: 'Saved!',
  ...
  ///   );
  /// ```
  ArxaKitActionBuilder<T> withNotificationType(
      ArxaKitNotificationType type) {
  ...
  ///     loading: ArxaKitNotificationType.none,
  ///     success: ArxaKitNotificationType.snackbar,
  ///     error: ArxaKitNotificationType.dialog,
  ///   )
  ///   .withSnackbars(
  ...
  ///   );
  /// ```
  ArxaKitActionBuilder<T> withNotificationTypes({
    ArxaKitNotificationType? loading,
    ArxaKitNotificationType? success,
    ArxaKitNotificationType? error,
  }) {
    if (loading != null) {
  ...
  ///   );
  /// ```
  ArxaKitActionBuilder<T> withDialogs() {
    return withNotificationType(ArxaKitNotificationType.dialog);
  }
  ...
  ///   );
  /// ```
  ArxaKitActionBuilder<T> withBottomSheets() {
    return withNotificationType(ArxaKitNotificationType.bottomSheet);
  }
  ...
  ///   });
  /// ```
  ArxaKitActionBuilder<T> onSuccess(
  ...
  /// ```
  ArxaKitActionBuilder<T> onComplete(FutureOr<void> Function() callback) {
  ...
  ///   });
  /// ```
  ArxaKitActionBuilder<T> onLoadingState(
  ...
  ///   });
  /// ```
  ArxaKitActionBuilder<T> onSuccessState(
  ...
  ///   });
  /// ```
  ArxaKitActionBuilder<T> onErrorState(
    void Function(String? message) callback,
  ) {
  ...
  ///   );
  /// ```
  ArxaKitActionBuilder<T> withUIStateCallbacks({
  ...
  ///   );
  /// ```
  ArxaKitActionBuilder<T> withRetry({
    int maxAttempts = 3,
  ...
  ///   });
  /// ```
  ArxaKitActionBuilder<T> withProgress(
  ...
  BehaviorSubject<T> toStream({T? initialValue}) {
    ...
    final executor = ArxaKitActionExecutor<T>(_config);
  ...
  ///   .toStream();
  /// ```
  ArxaKitActionBuilder<T> withAutoCleanup(Stream<void> trigger) {
  ...
  /// cancellable.cancel();
  /// ```
  ArxaKitActionCancellable<T> toCancellable() {
  ...
  /// ```
  ArxaKitActionBuilder<T> withTimeout(Duration timeout) {
  ...
  ///
  /// By default ArxaKitAction prevents parallel runs: a second execution while
  ...
  /// [ArxaKitGuardedException]. This is the double-tap protection the Flutter
  ...
  /// ```
  ArxaKitActionBuilder<T> withParallelExecution() {
  ...
  /// Example:
  /// ```dart
  /// await ArxaKitAction.run<List<String>>(...)
  ///   .withDebounce(Duration(milliseconds: 500));
  /// ```
  ArxaKitActionBuilder<T> withDebounce(Duration duration) {
  ...
  /// ```
  ArxaKitActionBuilder<T> withThrottle(Duration duration) {
  ...
  /// - Manager activities (ArxaKitLoadingManager, ArxaKitErrorManager, etc.)
  ///
  /// Logs appear in:
  /// - Console (CLI)
  ...
  /// ```
  ArxaKitActionBuilder<T> withDebugMode() {
  ...
  /// Example:
  /// ```dart
  /// final result = await ArxaKitAction.run<String>(...)
  ///   .withLoading(setBusy)
  ///   .execute();
  /// ```
  Future<T> execute() {
