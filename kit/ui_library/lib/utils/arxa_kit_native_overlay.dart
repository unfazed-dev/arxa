...
Future<void> arxaKitWithNativeChromeHidden(
  ...
  final scrim = scrimOverlay == null
      ? null
      : ArxaKitSnackbarScrimLease.acquire(scrimOverlay, scrimColor);
...
/// [arxaKitWithNativeChromeHidden]. GetX queues concurrent snackbars, but
/// each presentation wraps itself — the shared entry + depth counter keeps
/// overlapping wrappers from stacking two dims.
class ArxaKitSnackbarScrimLease {
  ArxaKitSnackbarScrimLease._();
  ...
  static ArxaKitSnackbarScrimLease acquire(
    ...
    if (_entry == null) {
      ...
      _mountPending = true;
      _entry = OverlayEntry(
        builder: (_) => _ArxaKitSnackbarNativeScrim(
          visible: _visible,
          color: color,
    ...
    return ArxaKitSnackbarScrimLease._();
  }
  ...
  Future<void> release() async {
    ...
    _visible.value = false;
    await Future<void>.delayed(_ArxaKitSnackbarNativeScrim.fadeDuration);
    if (_depth > 0) return;
    _entry?.remove();
    _entry?.dispose();
    _entry = null;
...
class _ArxaKitSnackbarNativeScrim extends StatelessWidget {
  const _ArxaKitSnackbarNativeScrim({
