...
import 'package:appbox_kit_core/enums/appbox_kit_app_common_enum.dart';
...
extension AppBoxKitOverlayExtension on Widget {
  Widget withOverlay({
    required String id,
    AppBoxKitOverlayOptions options = const AppBoxKitOverlayOptions(),
    Widget? overlayChild,
  }) {
...
class AppBoxKitOverlayService with ListenableServiceMixin {
...
class _KitOverlayViewModel extends ReactiveViewModel {
  ...
  final _overlayService = appBoxKitLocator<AppBoxKitOverlayService>();

  String _id = '';
  AppBoxKitOverlayOptions _options = const AppBoxKitOverlayOptions();
  ...
  void init(String id, AppBoxKitOverlayOptions options) {
    _id = id;
    _options = options;
    _setupStreams();
  ...
  @override
  void dispose() {
    // First dispose the auto-managed subscriptions from AppBoxKitAutoProcess.listener
    AppBoxKitAction.dispose(widgetId: widgetId);
...
class _KitOverlayWidget extends StackedView<_KitOverlayViewModel> {
  final Widget child;
  final String id;
  final AppBoxKitOverlayOptions options;
          ...
          child: options.isBlurred
              ? BackdropFilter(
                  filter: ImageFilter.blur(
                    sigmaX: options.blurEffect == AppBoxKitBlurEffect.custom
                        ? options.blurrinessLevelX
                        : options.blurEffect.sigma,
                    sigmaY: options.blurEffect == AppBoxKitBlurEffect.custom
  ...
  double? _getLeftPosition(AppBoxKitOverlayControlPosition position) {
    switch (position) {
  ...
  double? _getRightPosition(AppBoxKitOverlayControlPosition position) {
    switch (position) {
      case AppBoxKitOverlayControlPosition.topRight:
      case AppBoxKitOverlayControlPosition.bottomRight:
        return axPad12;
  ...
  double? _getTopPosition(AppBoxKitOverlayControlPosition position) {
    switch (position) {
      case AppBoxKitOverlayControlPosition.topLeft:
      case AppBoxKitOverlayControlPosition.topCenter:
      case AppBoxKitOverlayControlPosition.topRight:
  ...
  double? _getBottomPosition(AppBoxKitOverlayControlPosition position) {
    switch (position) {
      case AppBoxKitOverlayControlPosition.bottomLeft:
      case AppBoxKitOverlayControlPosition.bottomCenter:
      case AppBoxKitOverlayControlPosition.bottomRight:
...
class AppBoxKitOverlayOptions {
  final bool isBlurred;
  final AppBoxKitBlurEffect blurEffect;
  ...
  final AppBoxKitOverlayControlPosition controlPosition;
  final EdgeInsets padding;
  final AppBoxKitAppCommonDuration animationDuration;
  final VoidCallback? onOpened;
  final VoidCallback? onClosed;
  final BorderRadius? borderRadius;

  const AppBoxKitOverlayOptions({
    this.isBlurred = true,
    this.blurEffect = AppBoxKitBlurEffect.extraLight,
    ...
    this.controlPosition = AppBoxKitOverlayControlPosition.topRight,
    this.padding = EdgeInsets.zero,
    this.animationDuration = AppBoxKitAppCommonDuration.medium,
    this.onOpened,
    this.onClosed,
    this.borderRadius,
  });
...
enum AppBoxKitOverlayControlPosition {
  topLeft,
  topCenter,
  topRight,
  center,
