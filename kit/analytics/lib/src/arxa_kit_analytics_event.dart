...
/// fanning it out, so each [ArxaKitAnalyticsBackend] receives a single stable shape
/// regardless of the call site.
@immutable
class ArxaKitAnalyticsEvent {
  const ArxaKitAnalyticsEvent({
    required this.name,
  ...
  @override
  String toString() => 'ArxaKitAnalyticsEvent($name, $params)';
}

/// A screen/route view. Mirrors the `screen_view` convention most vendors use.
@immutable
class ArxaKitScreenView {
  const ArxaKitScreenView({
  ...
  @override
  String toString() => 'ArxaKitScreenView('
      '$screenName${screenClass == null ? '' : ', $screenClass'})';
}

/// A named user property (dimension). A null [value] clears the property.
@immutable
class ArxaKitUserProperty {
  const ArxaKitUserProperty(this.name, this.value);

  final String name;
  final String? value;

  @override
  String toString() => 'ArxaKitUserProperty($name = $value)';
}

/// A timing/duration measurement, e.g. time-to-interactive.
@immutable
class ArxaKitAnalyticsTiming {
  const ArxaKitAnalyticsTiming({
    required this.name,
  ...
  @override
  String toString() =>
      'ArxaKitAnalyticsTiming($name, ${duration.inMilliseconds}ms)';
}
