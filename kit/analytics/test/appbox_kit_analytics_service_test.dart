...
import 'package:appbox_kit_analytics/appbox_kit_testing.dart';

void main() {
  group('AppBoxKitAnalyticsService', () {
    test('kit.analytics.fan-out — forwards a track event to every registered backend',
        () async {
      // given
      final first = RecordingAppBoxKitAnalyticsBackend(id: 'first');
      final second = RecordingAppBoxKitAnalyticsBackend(id: 'second');
      final service = AppBoxKitAnalyticsService(backends: [first, second]);
      // when
    ...
    test('kit.analytics.fan-out — every backend shares the timestamp from the injected clock',
      ...
      final service = AppBoxKitAnalyticsService(
    ...
    test('kit.analytics.fan-out — an event without params arrives with empty params',
      ...
      final service = AppBoxKitAnalyticsService(backends: [recorder]);
      // when
      await service.logEvent('app_opened');
      // then
    ...
    test('kit.analytics.error-isolation — a throwing backend does not break the other backends',
      ...
      final recorder = RecordingAppBoxKitAnalyticsBackend(id: 'healthy');
      final service = AppBoxKitAnalyticsService(
        backends: [ThrowingAppBoxKitAnalyticsBackend(), recorder],
        onError: (_, __, ___) {},
      );
      // when
    ...
    test('kit.analytics.error-isolation — a backend error is routed to onError with the backend id and never rethrown',
      ...
      final service = AppBoxKitAnalyticsService(
        backends: [ThrowingAppBoxKitAnalyticsBackend(id: 'flaky')],
    ...
    test('kit.analytics.registration — re-registering an id replaces the previous backend',
        () async {
      // given
      final stale = RecordingAppBoxKitAnalyticsBackend(id: 'console');
      final live = RecordingAppBoxKitAnalyticsBackend(id: 'console');
      final service = AppBoxKitAnalyticsService(backends: [stale]);
      // when
    ...
    test('kit.analytics.registration — a removed backend no longer receives calls',
        () async {
      // given
      final removed = RecordingAppBoxKitAnalyticsBackend(id: 'removed');
      final kept = RecordingAppBoxKitAnalyticsBackend(id: 'kept');
      final service = AppBoxKitAnalyticsService(backends: [removed, kept]);
      // when
    ...
    test('kit.analytics.registration — removing an unknown id reports false and leaves fan-out untouched',
        () async {
      // given
      final recorder = RecordingAppBoxKitAnalyticsBackend(id: 'kept');
      final service = AppBoxKitAnalyticsService(backends: [recorder]);
      // when
  ...
  group('AppBoxKitDebugConsoleAnalyticsBackend', () {
    test('kit.analytics.debug-console — tolerates every call with no SDK attached',
      ...
      final backend = AppBoxKitDebugConsoleAnalyticsBackend();
      // when / then — every call completes without throwing.
      await backend.logEvent(AppBoxKitAnalyticsEvent(
          name: 'app_opened', timestamp: DateTime.utc(2026, 8, 6)));
      await backend.screenView(AppBoxKitScreenView(
          screenName: 'Home', timestamp: DateTime.utc(2026, 8, 6)));
      await backend.timing(AppBoxKitAnalyticsTiming(
          name: 'tti',
          duration: const Duration(milliseconds: 10),
          timestamp: DateTime.utc(2026, 8, 6)));
      await backend.setUserProperty(const AppBoxKitUserProperty('plan', 'pro'));
      await backend.flush();
    });
  });
}
