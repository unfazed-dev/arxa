...
/// Smoke tests for the [FakeAppBoxKitRepository] test double — proves the fake
/// honors the AppBoxKitRepository contract the same way the seed backend does:
/// canonical-id lookups, AppBoxKitQuery evaluation, stream-backed watchers.
class _Widget {
  final String id;
  final String name;
...
void main() {
  ...
  FakeAppBoxKitRepository<_Widget> makeRepo({Iterable<_Widget>? seed}) =>
      FakeAppBoxKitRepository<_Widget>(
  ...
  test('kit.data.fake-repository — AppBoxKitQuery evaluation: eq, gt, lt, orderBy, limit', () async {
    ...
    final gt = await repo.getAll(
      const AppBoxKitQuery(filters: [AppBoxKitFilter.gt('qty', 2)]),
    );
    expect(gt.map((w) => w.name), containsAll(['Alpha', 'Gamma']));

    final lt = await repo.getAll(
      const AppBoxKitQuery(filters: [AppBoxKitFilter.lt('qty', 5)]),
    );
    expect(lt.map((w) => w.name), containsAll(['Beta', 'Gamma']));

    // Ascending: null qty sorts last.
    final asc = await repo.getAll(const AppBoxKitQuery(orderBy: 'name'));
    expect(asc.map((w) => w.name), ['Alpha', 'Beta', 'Gamma']);

    final limited = await repo.getAll(
      const AppBoxKitQuery(orderBy: 'name', descending: true, limit: 2),
    );
