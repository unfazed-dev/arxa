...
import 'package:arxa_kit_data/seeding/arxa_kit_fixture_loader.dart';

/// Integration test for the seed-backend boot path that the showcase app
/// exercises at runtime: `ArxaKitSnapshotPersistence` (write-through JSON, one
/// file per table) + `ArxaKitFixtureLoader` + `arxaKitResolveBootTables` + `ArxaKitSeedStore`
/// + `ArxaKitSeedRepository`.
///
/// The gap this closes: `arxa_kit_data_initialize_test.dart` boots with
/// `ArxaKitSeedPersistenceMode.none` (so `load()` is always null and
/// `arxaKitResolveBootTables` is a no-op), and `arxa_kit_seed_persistence_test.dart` tests
/// `arxaKitResolveBootTables` as a pure function. Neither wires the resolution
...
/// components (no `ArxaKitData.initialize` — it constructs `ArxaKitSnapshotPersistence`
...
void main() {
  ...
  late ArxaKitIdService idService;
  ...
  final notesSchema = ArxaKitTableSchema(
    table: notesTable,
    columns: [
      ArxaKitColumn.id(),
  ...
  final foldersSchema = ArxaKitTableSchema(
    table: foldersTable,
    columns: [
      ArxaKitColumn.id(),
  ...
  /// Boots the seed backend the way `ArxaKitData._initializeSeed` does, but with
  ...
  /// sequence as `arxa_kit_data.dart`, minus `ArxaKitData.initialize`'s static-state
  /// wiring (which can't accept an override directory).
  Future<ArxaKitSeedStore> boot({
    required Map<String, String> fixtureAssets,
  }) async {
    final persistence = ArxaKitSnapshotPersistence(overrideDirectory: tempDir);
    final fixtures = await ArxaKitFixtureLoader(
      idService: idService,
      assetReader: ArxaKitMemoryAssetReader(fixtureAssets),
    ).load(
  ...
  group('seed boot with snapshot persistence', () {
    test(
      ...
      final repo = ArxaKitSeedRepository<Map<String, dynamic>>(
        store: await store,
      ...
      final ownerCanonical =
          idService.canonicalId(kArxaKitAuthUsersTable, 'user-1');
      final notes = await repo.getAll(
        ArxaKitQuery(filters: [ArxaKitFilter.eq('owner', ownerCanonical)]),
      );
    ...
    test(
      ...
      // ArxaKitSeedAuthService.initialize upserts each fake user, which
      // persists kit_auth_users and nothing else on first boot).
      final store1 = await boot(fixtureAssets: fixturesJson);
      await store1.upsertRow(kArxaKitAuthUsersTable, {
        'id': idService.canonicalId(kArxaKitAuthUsersTable, 'user-1'),
      ...
      final notesRepo = ArxaKitSeedRepository<Map<String, dynamic>>(
        store: store2,
        registration: _mapRegistration(notesSchema),
        idService: idService,
      );
      final foldersRepo = ArxaKitSeedRepository<Map<String, dynamic>>(
        store: store2,
    ...
    test(
      ...
      await store1.upsertRow(notesTable, {
        ...
        'owner': idService.canonicalId(kArxaKitAuthUsersTable, 'user-1'),
        'body': 'written in-app',
      });
      ...
      final notes3 = await ArxaKitSeedRepository<Map<String, dynamic>>(
        store: store3,
        registration: _mapRegistration(notesSchema),
        idService: idService,
      ).getAll();
...
ArxaKitEntityRegistration<Map<String, dynamic>> _mapRegistration(
  ArxaKitTableSchema schema,
) =>
    ArxaKitEntityRegistration<Map<String, dynamic>>(
      schema: schema,
