...
/// exercises at runtime: `KitSnapshotPersistence` (write-through JSON, one
/// file per table) + `KitFixtureLoader` + `resolveBootTables` + `KitSeedStore`
/// + `KitSeedRepository`.
///
/// The gap this closes: `kit_data_initialize_test.dart` boots with
/// `KitSeedPersistenceMode.none` (so `load()` is always null and
/// `resolveBootTables` is a no-op), and `kit_seed_persistence_test.dart` tests
/// `resolveBootTables` as a pure function. Neither wires the resolution
...
/// components (no `KitData.initialize` — it constructs `KitSnapshotPersistence`
...
void main() {
  ...
  final notesSchema = KitTableSchema(
    table: notesTable,
    columns: [
      KitColumn.id(),
      KitColumn('folder_id', KitColumnType.reference,
          references: foldersTable),
      KitColumn('owner', KitColumnType.reference,
          references: kKitAuthUsersTable),
      KitColumn('body', KitColumnType.text),
    ],
  );
  final foldersSchema = KitTableSchema(
    table: foldersTable,
    columns: [
      KitColumn.id(),
  ...
  /// Boots the seed backend the way `KitData._initializeSeed` does, but with
  ...
  /// sequence as `kit_data.dart`, minus `KitData.initialize`'s static-state
  /// wiring (which can't accept an override directory).
  Future<KitSeedStore> boot({
    required Map<String, String> fixtureAssets,
  }) async {
    final persistence = KitSnapshotPersistence(overrideDirectory: tempDir);
    final fixtures = await KitFixtureLoader(
      idService: idService,
      assetReader: KitMemoryAssetReader(fixtureAssets),
    ).load(
  ...
  group('seed boot with snapshot persistence', () {
    test(
      ...
      final repo = KitSeedRepository<Map<String, dynamic>>(
        store: await store,
      ...
      final ownerCanonical =
          idService.canonicalId(kKitAuthUsersTable, 'user-1');
      final notes = await repo.getAll(
        KitQuery(filters: [KitFilter.eq('owner', ownerCanonical)]),
      );
    ...
    test(
      ...
      // KitSeedAuthService.initialize upserts each fake user, which
      // persists kit_auth_users and nothing else on first boot).
      final store1 = await boot(fixtureAssets: fixturesJson);
      await store1.upsertRow(kKitAuthUsersTable, {
        'id': idService.canonicalId(kKitAuthUsersTable, 'user-1'),
      ...
      final notesRepo = KitSeedRepository<Map<String, dynamic>>(
        store: store2,
        registration: _mapRegistration(notesSchema),
        idService: idService,
      );
      final foldersRepo = KitSeedRepository<Map<String, dynamic>>(
        store: store2,
    ...
    test(
      ...
      await store1.upsertRow(notesTable, {
        ...
        'owner': idService.canonicalId(kKitAuthUsersTable, 'user-1'),
        'body': 'written in-app',
      });
      ...
      final notes3 = await KitSeedRepository<Map<String, dynamic>>(
        store: store3,
        registration: _mapRegistration(notesSchema),
        idService: idService,
      ).getAll();
...
KitEntityRegistration<Map<String, dynamic>> _mapRegistration(
  KitTableSchema schema,
) =>
    KitEntityRegistration<Map<String, dynamic>>(
      schema: schema,
