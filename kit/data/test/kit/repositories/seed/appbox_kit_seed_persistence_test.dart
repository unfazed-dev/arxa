...
/// AppBoxKitSeedPersistence tests.
///
/// Branches under test:
/// - [AppBoxKitSnapshotPersistence] with `overrideDirectory` pointing at a temp
...
/// - [AppBoxKitNoPersistence].`load` always returns `null`.
void main() {
  ...
  group('appBoxKitFixtureFingerprint', () {
    ...
    test('kit.data.seed-repos — changes when any row content changes', () {
      ...
      expect(appBoxKitFixtureFingerprint(a), isNot(appBoxKitFixtureFingerprint(b)));
    });
  });

  group('appBoxKitResolveBootTables', () {
    final fixtures = {
      'notes': {
        'n-1': {'id': 'n-1', 'body': 'seeded note'},
      },
    ...
    test(
      ...
      final oldFingerprints = {
        'notes': appBoxKitFixtureFingerprint(snapshot['notes']!..remove('user-note')),
      };
