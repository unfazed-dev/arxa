...
import 'package:appbox_kit_data/auth/seed/appbox_kit_seed_auth_service.dart';
...
/// AppBoxKitSeedAuthService tests, backed by a real [AppBoxKitSeedStore] +
/// [AppBoxKitNoPersistence] and a [AppBoxKitMemoryAssetReader] (same approach as
/// appbox_kit_fixture_loader_test.dart — no mocks).
///
/// Branches under test:
...
/// - OTP: `requestOtp` throws `AppBoxKitAuthException` when given both or neither
...
void main() {
  ...
  AppBoxKitSeedStore makeStore() => AppBoxKitSeedStore(persistence: AppBoxKitNoPersistence());

  AppBoxKitSeedAuthService makeService({
    AppBoxKitSeedStore? store,
    ...
    Map<String, String>? assets,
      ...
      AppBoxKitSeedAuthService(
        ...
        assetReader: AppBoxKitMemoryAssetReader(assets ?? <String, String>{}),
      );
  ...
  group('OTP', () {
    test('requestOtp throws when given both email and phone', () async {
      ...
      await expectLater(
        ...
        throwsA(isA<AppBoxKitAuthException>()),
      );
    });
