...
import 'package:appbox_kit_data/storage/appbox_kit_storage_service.dart';
import 'package:appbox_kit_data/storage/seed/appbox_kit_seed_storage_service.dart';
import 'package:appbox_kit_data/storage/seed/appbox_kit_storage_persistence.dart';

/// AppBoxKitSeedStorageService + AppBoxKitSnapshotStoragePersistence tests.
///
/// Branches under test:
/// - In-memory service (default `AppBoxKitNoStoragePersistence`): progress is
///   reported as `0.0` then `1.0`, `getUrl` returns a `data:` URL with the
///   uploaded MIME, `delete` makes a subsequent `getUrl` throw.
/// - `AppBoxKitSnapshotStoragePersistence` with `overrideDirectory` at a temp dir
...
/// - A fresh `AppBoxKitSeedStorageService` over the snapshot persistence surfaces
...
void main() {
  ...
  group('AppBoxKitSeedStorageService (in-memory)', () {
    ...
    test('kit.data.storage — getUrl on a missing object throws StateError naming the path',
      ...
      await expectLater(
        service.getUrl(const AppBoxKitStorageObjectRef(bucket: 'x', path: 'missing')),
        throwsA(
  ...
  group('write-through across instances', () {
    test('kit.data.storage — a second service reads bytes persisted by the first', () async {
      ...
      final b = AppBoxKitSeedStorageService(persistence: persistence);
