...
import 'package:arxa_kit_data/storage/appwrite/arxa_kit_appwrite_storage_service.dart';

/// Live-credentials contract test for [ArxaKitAppwriteStorageService].
///
/// Env-var skip convention (see arxa_kit_supabase_storage_service_test.dart):
...
void main() {
  ...
  group(
    'ArxaKitAppwriteStorageService contract',
    () {
      ...
      test('kit.data.storage — upload → getUrl → delete round trip', () async {
        final service =
            ArxaKitAppwriteStorageService(storage: storage, client: client);
