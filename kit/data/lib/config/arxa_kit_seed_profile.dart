...
/// every `ArxaKitSeedRepository` operation; the host owns the profile names,
/// the fixture selection (`empty` is an empty fixture list, not a store
/// behavior), and any persona sign-in after boot.
///
/// Only meaningful when `ArxaKitDataConfig.backend` is `ArxaKitDataBackend.seed` —
...
class ArxaKitSeedException implements Exception {
  final String message;
  final String code;

  const ArxaKitSeedException(
    this.message, {
    this.code = 'seed-profile-failure',
  });

  @override
  String toString() => 'ArxaKitSeedException($code): $message';
}
...
///
/// The default (`const ArxaKitSeedProfile()`) is a pass-through: zero latency,
/// no failure — existing boots are behavior-identical without it. Auth is
/// never affected: `ArxaKitSeedAuthService` reads the store directly, not
/// through repositories.
class ArxaKitSeedProfile {
  ...
  const ArxaKitSeedProfile({
    this.latency = Duration.zero,
    this.error,
  });

  /// The `slow` profile — everything succeeds, [latency] late.
  const ArxaKitSeedProfile.slow(this.latency) : error = null;

  /// The `failing` profile — everything fails with [error].
  const ArxaKitSeedProfile.failing(this.error) : latency = Duration.zero;
