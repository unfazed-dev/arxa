/// [AppBoxKitRepository] backed by Supabase (PostgREST reads/writes, Realtime via
...
/// Supabase implementation of [AppBoxKitRepository].
///
...
class AppBoxKitSupabaseRepository<T> implements AppBoxKitRepository<T> {
  AppBoxKitSupabaseRepository({
  ...
  @override
  Stream<List<T>> watchAll([AppBoxKitQuery appBoxKitQuery = const AppBoxKitQuery()]) {
    final query = _idService.canonicalizeQuery(_registration.schema, appBoxKitQuery);
  ...
  AppBoxKitFilter? _pickServerSideFilter(List<AppBoxKitFilter> filters) {
    if (filters.isEmpty) return null;
    for (final f in filters) {
      if (f.op == AppBoxKitFilterOp.eq) return f;
    }
  ...
  // -- Client-side filter/sort/limit (the rest of the AppBoxKitQuery) -------------
  //
  // Operate on raw wire-shape rows (`Map<String, dynamic>`), not decoded
  // entities — AppBoxKitQuery columns are wire-shape column names, and this avoids
