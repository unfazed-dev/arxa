/// [ArxaKitRepository] backed by Supabase (PostgREST reads/writes, Realtime via
...
import '../arxa_kit_repository.dart';

/// Supabase implementation of [ArxaKitRepository].
///
/// Constraints (docs/plans/arxa-kit-data-layer.md, "swap rules"):
...
class ArxaKitSupabaseRepository<T> implements ArxaKitRepository<T> {
  ArxaKitSupabaseRepository({
    required SupabaseClient client,
    required ArxaKitEntityRegistration<T> registration,
  ...
  final ArxaKitEntityRegistration<T> _registration;
  ...
  @override
  Future<List<T>> getAll([ArxaKitQuery arxaKitQuery = const ArxaKitQuery()]) async {
    final query = _idService.canonicalizeQuery(_registration.schema, arxaKitQuery);
  ...
  @override
  Stream<List<T>> watchAll([ArxaKitQuery arxaKitQuery = const ArxaKitQuery()]) {
    final query = _idService.canonicalizeQuery(_registration.schema, arxaKitQuery);
  ...
  ArxaKitFilter? _pickServerSideFilter(List<ArxaKitFilter> filters) {
    if (filters.isEmpty) return null;
    for (final f in filters) {
      if (f.op == ArxaKitFilterOp.eq) return f;
    }
  ...
  // -- Client-side filter/sort/limit (the rest of the ArxaKitQuery) -------------
  //
  // Operate on raw wire-shape rows (`Map<String, dynamic>`), not decoded
  // entities — ArxaKitQuery columns are wire-shape column names, and this avoids
