/// [ArxaKitRepository] backed by Appwrite's TablesDB rows API + Realtime.
library;
...
/// Appwrite implementation of [ArxaKitRepository], built on the 25.x TablesDB
...
///   primitive in the create/update row APIs), so `ArxaKitColumnType.jsonb`
///   columns are stored as JSON-encoded strings on this backend and decoded
///   back on read.
class ArxaKitAppwriteRepository<T> implements ArxaKitRepository<T> {
  ArxaKitAppwriteRepository({
  ...
  // -- ArxaKitQuery -> Appwrite Query strings -------------------------------

  List<String> _buildQueries(ArxaKitQuery query) {
  ...
  Map<String, dynamic> _toWireData(Map<String, dynamic> canonicalRow) {
    ...
    for (final column in _registration.schema.columns) {
      if (column.type != ArxaKitColumnType.jsonb) continue;
