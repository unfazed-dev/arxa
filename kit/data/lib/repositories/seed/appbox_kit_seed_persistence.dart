...
/// Write-through durability contract for [AppBoxKitSeedStore] — the store's
...
/// [AppBoxKitNoPersistence]), which is distinct from "persisted and empty".
abstract interface class AppBoxKitSeedPersistence {
  ...
  /// table -> [appBoxKitFixtureFingerprint] recorded at the last boot. `null` when
  /// never recorded — including snapshots written before fingerprinting
  /// existed, which [appBoxKitResolveBootTables] deliberately treats as "fixtures
...
/// Pure in-memory seed backend (`AppBoxKitSeedPersistenceMode.none`): every boot
/// reloads fixtures fresh, nothing survives a restart.
class AppBoxKitNoPersistence implements AppBoxKitSeedPersistence {
  @override
...
/// Deep key-sorted copy, so [appBoxKitFixtureFingerprint] is insensitive to JSON map
...
String appBoxKitFixtureFingerprint(Map<String, Map<String, dynamic>> rows) {
...
/// actually change.
///
/// Snapshots are written per table ([AppBoxKitSeedPersistence.persistTable]), so a
/// partial snapshot is normal — e.g. `AppBoxKitSeedAuthService.initialize`
/// write-throughs only `kit_auth_users` on first boot; rule 1 keeps that
/// partial snapshot from shadowing unseen fixture tables.
AppBoxKitBootResolution appBoxKitResolveBootTables({
  ...
  final fingerprints = <String, String>{
    for (final entry in fixtures.entries)
      entry.key: appBoxKitFixtureFingerprint(entry.value),
  };
  ...
  return AppBoxKitBootResolution(
    tables: tables,
...
/// `<baseDir>/appbox_kit_data/seed/<table>.json`. Boot resolves snapshot vs
/// fixtures per table (see [appBoxKitResolveBootTables]). A corrupt
...
/// unnoticed.
class AppBoxKitSnapshotPersistence implements AppBoxKitSeedPersistence {
  /// Boot-metadata sidecar (see [AppBoxKitSeedPersistence.loadFixtureFingerprints]).
