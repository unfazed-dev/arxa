...
///
/// Pure Dart — depends only on the [ArxaKitAssetReader] port, never on Flutter's
/// `AssetBundle`/`rootBundle`. The composition root supplies a
/// `ArxaKitRootBundleAssetReader`; tests and `dart run` tooling pass a
/// [ArxaKitMemoryAssetReader].
class ArxaKitFixtureLoader {
  final ArxaKitIdService idService;
  final ArxaKitAssetReader assetReader;

  ArxaKitFixtureLoader({required this.idService, required this.assetReader});
  ...
  Future<Map<String, Map<String, Map<String, dynamic>>>> load({
    ...
    for (final assetPath in assetPaths) {
      ...
      if (schema == null) {
        throw ArgumentError(
          'Fixture "$assetPath" names table "$table", which has no registered ArxaKitTableSchema',
        );
      }
