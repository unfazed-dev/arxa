...
///
/// Pure Dart — depends only on the [AppBoxKitAssetReader] port, never on Flutter's
/// `AssetBundle`/`rootBundle`. The composition root supplies a
/// `AppBoxKitRootBundleAssetReader`; tests and `dart run` tooling pass a
/// [AppBoxKitMemoryAssetReader].
class AppBoxKitFixtureLoader {
  final AppBoxKitIdService idService;
  final AppBoxKitAssetReader assetReader;

  AppBoxKitFixtureLoader({required this.idService, required this.assetReader});
  ...
  Future<Map<String, Map<String, Map<String, dynamic>>>> load({
    ...
    for (final assetPath in assetPaths) {
      ...
      if (schema == null) {
        throw ArgumentError(
          'Fixture "$assetPath" names table "$table", which has no registered AppBoxKitTableSchema',
        );
      }
