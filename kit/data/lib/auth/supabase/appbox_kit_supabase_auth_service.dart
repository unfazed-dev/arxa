/// [AppBoxKitAuthService] backed by Supabase Auth.
library;
...
/// Supabase implementation of [AppBoxKitAuthService].
///
...
class AppBoxKitSupabaseAuthService implements AppBoxKitAuthService {
  AppBoxKitSupabaseAuthService({
    required SupabaseClient client,
    required AppBoxKitAuthConfig config,
  ...
  final AppBoxKitAuthConfig _config;
  ...
  /// Uniform error surface shared by every method: [AppBoxKitAuthException]s pass
  ...
  Future<T> _guard<T>(String fallbackMessage, Future<T> Function() body) async {
    try {
      return await body();
    } on AppBoxKitAuthException {
      rethrow;
    } on AuthException catch (error) {
      throw AppBoxKitAuthException(error.message, code: error.code, cause: error);
    } on GoogleSignInException catch (error) {
      throw AppBoxKitAuthException(
        error.description ?? fallbackMessage,
        code: error.code.name,
        cause: error,
      );
    } on SignInWithAppleException catch (error) {
      throw AppBoxKitAuthException(fallbackMessage, cause: error);
    } catch (error) {
      throw AppBoxKitAuthException(fallbackMessage, cause: error);
    }
  }
  ...
  /// Maps [session], throwing [AppBoxKitAuthException] with [code] when the SDK
  /// handed back none — shared by every path that must end signed-in.
  AppBoxKitAuthSession _requireSession(Session? session, String code) {
    final mapped = _mapSession(session);
    if (mapped == null) {
      throw AppBoxKitAuthException('Sign-in returned no session', code: code);
    }
    return mapped;
  }
  ...
  AppBoxKitAuthSession? _mapSession(Session? session) {
  ...
  AppBoxKitAuthUser _mapUser(User user) {
    ...
    return AppBoxKitAuthUser(
      ...
      // normalize so AppBoxKitAuthUser means the same thing on every backend.
