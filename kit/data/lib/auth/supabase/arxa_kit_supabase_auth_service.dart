/// [ArxaKitAuthService] backed by Supabase Auth.
library;
...
/// Supabase implementation of [ArxaKitAuthService].
///
...
class ArxaKitSupabaseAuthService implements ArxaKitAuthService {
  ArxaKitSupabaseAuthService({
    required SupabaseClient client,
    required ArxaKitAuthConfig config,
  ...
  final ArxaKitAuthConfig _config;
  ...
  /// Uniform error surface shared by every method: [ArxaKitAuthException]s pass
  ...
  Future<T> _guard<T>(String fallbackMessage, Future<T> Function() body) async {
    try {
      return await body();
    } on ArxaKitAuthException {
      rethrow;
    } on AuthException catch (error) {
      throw ArxaKitAuthException(error.message, code: error.code, cause: error);
    } on GoogleSignInException catch (error) {
      throw ArxaKitAuthException(
        error.description ?? fallbackMessage,
        code: error.code.name,
        cause: error,
      );
    } on SignInWithAppleException catch (error) {
      throw ArxaKitAuthException(fallbackMessage, cause: error);
    } catch (error) {
      throw ArxaKitAuthException(fallbackMessage, cause: error);
    }
  }
  ...
  /// Maps [session], throwing [ArxaKitAuthException] with [code] when the SDK
  /// handed back none — shared by every path that must end signed-in.
  ArxaKitAuthSession _requireSession(Session? session, String code) {
    final mapped = _mapSession(session);
    if (mapped == null) {
      throw ArxaKitAuthException('Sign-in returned no session', code: code);
    }
    return mapped;
  }
  ...
  @override
  Future<ArxaKitAuthSession> signInAnonymously() =>
  ...
  ArxaKitAuthSession? _mapSession(Session? session) {
  ...
  ArxaKitAuthUser _mapUser(User user) {
    ...
    return ArxaKitAuthUser(
      ...
      // normalize so ArxaKitAuthUser means the same thing on every backend.
