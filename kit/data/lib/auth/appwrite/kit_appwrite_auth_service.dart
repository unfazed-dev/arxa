/// [KitAuthService] backed by Appwrite's `Account` API.
library;
...
/// Appwrite implementation of [KitAuthService], built on `Account`
...
///   to build a [KitAuthUser] out of whatever partial shape each
...
class KitAppwriteAuthService implements KitAuthService {
  KitAppwriteAuthService({
  ...
  /// `KitSeedAuthService`'s `_pendingOtpEmail`/`_pendingOtpPhone` pattern).
  ...
  Future<KitAuthSession> signUpWithEmailPassword({
    ...
    final userId = _idService.canonicalId(kKitAuthUsersTable, email.toLowerCase());
    try {
  ...
  Future<void> requestOtp({String? email, String? phone}) async {
    ...
    final userId = _idService.canonicalId(kKitAuthUsersTable, identity);
    try {
      if (email != null) {
  ...
  Future<KitAuthSession> confirmOtp({
    ...
    final userId = identity != null
        ? _idService.canonicalId(kKitAuthUsersTable, identity.toLowerCase())
        : _pendingOtpUserId;
    if (userId == null) {
      throw const KitAuthException(
  ...
  Future<KitAuthSession> _signInWithOAuth(enums.OAuthProvider provider) async {
    try {
  ...
  Future<KitAuthSession> _refreshSession() async {
    final user = await _account.get();
  ...
  KitAuthSession _sessionFromUser(models.User user) {
    return KitAuthSession(
      ...
      // SDK, so [KitAuthSession.accessToken] stays null here per its own
  ...
  KitAuthUser _mapUser(models.User user) {
    ...
    return KitAuthUser(
      ...
      // a no-op pass-through for it (`KitIdService.isUuid`). For
  ...
  KitAuthException _wrap(String message, AppwriteException error) {
    final detail = error.message;
    return KitAuthException(
