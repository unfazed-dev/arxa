/// [ArxaKitAuthService] backed by Appwrite's `Account` API.
library;
...
/// Appwrite implementation of [ArxaKitAuthService], built on `Account`
...
///   to build a [ArxaKitAuthUser] out of whatever partial shape each
...
class ArxaKitAppwriteAuthService implements ArxaKitAuthService {
  ArxaKitAppwriteAuthService({
  ...
  final BehaviorSubject<ArxaKitAuthSession?> _session$ =
      BehaviorSubject<ArxaKitAuthSession?>();
  ...
  /// `ArxaKitSeedAuthService`'s `_pendingOtpEmail`/`_pendingOtpPhone` pattern).
  ...
  @override
  ArxaKitAuthSession? get currentSession => _session$.valueOrNull;
  ...
  Future<ArxaKitAuthSession> signUpWithEmailPassword({
    ...
    final userId = _idService.canonicalId(kArxaKitAuthUsersTable, email.toLowerCase());
    try {
  ...
  Future<void> requestOtp({String? email, String? phone}) async {
    ...
    final userId = _idService.canonicalId(kArxaKitAuthUsersTable, identity);
    try {
      if (email != null) {
  ...
  Future<ArxaKitAuthSession> confirmOtp({
    ...
    final userId = identity != null
        ? _idService.canonicalId(kArxaKitAuthUsersTable, identity.toLowerCase())
        : _pendingOtpUserId;
    if (userId == null) {
      throw const ArxaKitAuthException(
  ...
  @override
  Future<ArxaKitAuthSession> signInWithGoogle() =>
      _signInWithOAuth(enums.OAuthProvider.google);

  @override
  Future<ArxaKitAuthSession> signInWithApple() =>
      _signInWithOAuth(enums.OAuthProvider.apple);

  Future<ArxaKitAuthSession> _signInWithOAuth(enums.OAuthProvider provider) async {
    try {
  ...
  Future<ArxaKitAuthSession> _refreshSession() async {
    final user = await _account.get();
  ...
  ArxaKitAuthSession _sessionFromUser(models.User user) {
    return ArxaKitAuthSession(
      user: _mapUser(user),
      ...
      // SDK, so [ArxaKitAuthSession.accessToken] stays null here per its own
  ...
  ArxaKitAuthUser _mapUser(models.User user) {
    ...
    return ArxaKitAuthUser(
      ...
      id: _idService.canonicalId(kArxaKitAuthUsersTable, user.$id),
      email: email,
      phone: phone,
  ...
  ArxaKitAuthException _wrap(String message, AppwriteException error) {
    final detail = error.message;
    return ArxaKitAuthException(
