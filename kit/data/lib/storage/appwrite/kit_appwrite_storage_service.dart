/// [KitStorageService] backed by Appwrite Storage.
library;
...
/// Appwrite implementation of [KitStorageService].
///
/// Mirrors [KitAppwriteRepository]'s pattern of taking the Appwrite service
...
/// `fileId` is allocated per upload. The returned [StorageObjectRef.id]
/// carries that `fileId` so [getUrl] / [delete] address the right object.
class KitAppwriteStorageService implements KitStorageService {
  ...
  Future<StorageObjectRef> upload({
    ...
    return StorageObjectRef(bucket: bucket, path: path, id: file.$id);
  }
