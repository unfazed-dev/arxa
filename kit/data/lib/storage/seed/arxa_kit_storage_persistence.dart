...
/// Write-through durability contract for [ArxaKitSeedStorageService] — the
/// in-memory map stays the source of truth; implementations only mirror
/// writes to durable storage. Mirrors [ArxaKitSeedPersistence] (the table
/// persistence seam), with the same `overrideDirectory` test escape hatch.
abstract interface class ArxaKitStoragePersistence {
...
/// Pure in-memory (`ArxaKitSeedPersistenceMode.none`): nothing survives a
/// restart.
class ArxaKitNoStoragePersistence implements ArxaKitStoragePersistence {
  const ArxaKitNoStoragePersistence();
...
/// [ArxaKitSnapshotPersistence]. Each object lands at
/// `<dir>/arxa_kit_data/storage/<bucket>/<path>`. [overrideDirectory]
...
/// a demo backend.
class ArxaKitSnapshotStoragePersistence implements ArxaKitStoragePersistence {
  final Directory? _overrideDirectory;

  const ArxaKitSnapshotStoragePersistence({Directory? overrideDirectory})
      : _overrideDirectory = overrideDirectory;

  Future<Directory> _root() async {
    final base =
        _overrideDirectory ?? await getApplicationDocumentsDirectory();
    return Directory('${base.path}/arxa_kit_data/storage');
  }
